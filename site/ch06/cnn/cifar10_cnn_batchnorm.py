"""
CIFAR-10 CNN with Batch Normalisation
=======================================

Same LeNet-style architecture as ``cifar10_cnn.py``, but with
**batch normalisation enabled** after each convolutional layer.
Dropout remains disabled so we can isolate the effect of batch-norm.

Batch normalisation normalises activations to zero mean / unit variance
within each mini-batch, which stabilises gradient flow and often allows
...
Run
---
    python cifar10_cnn_batchnorm.py
"""
...
# =============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="CIFAR-10 CNN + BatchNorm")
    ...
    args.model_path = "./model/cnn_batchnorm.pth"
    os.makedirs("./model", exist_ok=True)

    # Regularisation flags — BatchNorm ON, Dropout OFF
...
def main():
    ...
    show_predictions(test_loader, model, args, title="Before Training (BatchNorm)")
    train(model, train_loader, optimizer, criterion, args)
    show_predictions(test_loader, model, args, title="After Training (BatchNorm)")
