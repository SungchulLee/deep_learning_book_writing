"""
CIFAR-10 CNN with Dropout
==========================

Same LeNet-style architecture as ``cifar10_cnn.py``, but with
**dropout enabled** (p = 0.5) after the first two fully-connected
layers.  Batch normalisation remains disabled so we can isolate the
regularisation effect of dropout.
...
# =============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="CIFAR-10 CNN + Dropout")
    ...
    args.model_path = "./model/cnn_dropout.pth"
    os.makedirs("./model", exist_ok=True)

    # Regularisation flags — BatchNorm OFF, Dropout ON
...
def main():
    ...
    show_predictions(test_loader, model, args, title="Before Training (Dropout)")
    train(model, train_loader, optimizer, criterion, args)
    show_predictions(test_loader, model, args, title="After Training (Dropout)")
