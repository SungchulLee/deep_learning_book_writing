...
DeepLabV3은 2017년 논문 "Rethinking Atrous Convolution for Semantic Image Segmentation"에서 나왔다. 구멍 뚫린 자리 피라미드 모으기(ASPP)와 여러 벌림 비율을 쓴다.
...
#!/usr/bin/env python3
'''
DeepLabV3 — 뜻 나누기를 위한 구멍 뚫린 누비기 다시 생각하기
논문: "Rethinking Atrous Convolution for Semantic Image Segmentation" (2017)
핵심: 구멍 뚫린 자리 피라미드 모으기(ASPP), 여러 벌림 비율
'''
import torch
...
여기 짠 것은 함께 어울려 온전한 그림 나누기 얼개를 이루는 클래스 2개(`ASPP`, `DeepLabV3`)를 정한다. 클래스마다 뚜렷한 조각 하나를 감싸므로 코드가 단원별로 나뉘고 넓히기 쉽다. `forward` 메서드가 PyTorch의 자동 미분에 쓰이는 셈 그래프를 정한다.
...
**연습문제 1.**
`ASPP`의 앞먹임을 따라가며 텐서 꼴을 좇아라. 붙박이 매개변수로 들임 표본 4개짜리 묶음에 대해 주요 연산(누비기, 모으기, 선형 층)마다 그 뒤의 꼴을 적어라.
...
**연습문제 4.**
층이나 덩이의 개수를 정할 수 있도록 `ASPP`을 넓혀라. `__init__`에 `num_layers` 매개변수를 더하고 `nn.ModuleList`로 깊이가 바뀌는 얼개를 만들어라. 층 2, 4, 8개로 시험하여라.
...
??? success "연습문제 4 풀이"
    ...
    ```
    `forward` 메서드에서 `for layer in self.layers: x = layer(x)`로 되풀이한다. (그냥 파이썬 목록이 아니라) `nn.ModuleList`를 써야 PyTorch가 가장 좋게 하기에 쓸 매개변수를 모두 등록한다. 시험: `for n in [2, 4, 8]: model = ASPP(num_layers=n); print(f'Layers={n}, params={sum(p.numel() for p in model.parameters()):,}')`.

## 정리하며

**다룬 것** — DeepLabV3

여기 짠 것은 함께 어울려 온전한 그림 나누기 얼개를 이루는 클래스 2개(`ASPP`, `DeepLabV3`)를 정한다.

고갱이 갈래는 `ASPP`, `DeepLabV3`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
