...
*증명 얼개.* $(1 \Rightarrow 2)$: 늘림 경로가 있었다면 흐름을 더 키울 수 있으니 가장 크다는 것과 어긋난다. $(2 \Rightarrow 3)$: $S = \{v \in V : v \text{ is reachable from } s \text{ in } G_f\}$이라 하고 $T = V \setminus S$이라 하자. 늘림 경로가 없으므로 $t \notin S$이고 $(S, T)$은 자름이다. $S$에서 $T$으로 가는 변은 모두 가득 차 있어야 하고 $T$에서 $S$으로 가는 변은 모두 흐름이 0이어야 하므로 $|f| = c(S, T)$이다. $(3 \Rightarrow 1)$: 어떤 자름에 대해서도 $|f| \leq c(S, T)$이므로(여린 짝 테두리), $|f| = c(S, T)$이면 $f$은 가장 크다. $\square$

---

## 5. 복잡도

담이가 정수이면 포드-풀커슨 방법은 많아야 $|f^*|$ 번 늘리고 끝난다. 여기서 $f^*$은 최대 흐름 값이며, 늘릴 때마다 흐름이 적어도 1 늘기 때문이다. 늘릴 때마다 길을 찾고(깊이 우선 돌아보기 따위) 흐름을 고치는 데 $O(E)$ 시간이 든다.
...
**되순환 1.** 늘림 경로 $s \to a \to t$을 찾는다. 병목은 $\min(10, 7) = 7$이다. 흐름 7을 밀어 넣는다.

1번째 바퀴 뒤: $f(s,a) = 7$, $f(a,t) = 7$. 남은 담이: $(s,a): 3$, $(a,t): 0$(꽉 참), $(a,s): 7$, $(t,a): 7$.

**되순환 2.** 길 $s \to b \to t$을 찾는다. 병목은 $\min(8, 10) = 8$이다. 흐름 8을 밀어 넣는다.

2번째 바퀴 뒤: 여기에 더해 $f(s,b) = 8$, $f(b,t) = 8$. 남은 담이: $(s,b): 0$, $(b,t): 2$.

**되순환 3.** 길 $s \to a \to b \to t$을 찾는다. 병목은 $\min(3, 5, 2) = 2$이다. 흐름 2을 밀어 넣는다.
...
알고리즘은 최대 흐름 17을 찾으며, 위에서 손으로 좇은 보기와 맞는다. 여기 쓴 $O(V^2)$ 이웃 행렬은 작고 빽빽한 그래프에 편하다. 성긴 그래프에서는 변 객체를 드러내 놓은 이웃 목록이 자리를 더 아낀다.
...
??? success "연습문제 1 풀이"
    포드-풀커슨은 남은 그래프에서 근원부터 바닥까지 늘림 경로를 거듭 찾아 흐름을 밀어 넣기를 늘림 경로가 없어질 때까지 되풀이한다. 늘림 경로를 어떻게 찾을지 정하지 않기에 "방법"이라 부른다. 어떤 길 찾기 꾀든 쓸 수 있다(깊이 우선 돌아보기, 너비 우선 돌아보기, 마구잡이). 꾀가 다르면 알고리즘도 달라진다(에드먼즈-카프는 너비 우선 돌아보기를 쓴다). 무엇을 고르느냐가 끝남의 보장과 시간 복잡도를 가른다. $\square$
...
??? success "연습문제 2 풀이"
    흐름 $f$에 대한 **남은 그래프** $G_f$은 $G$과 꼭짓점이 같다. 담이가 $c$이고 흐름이 $f(u,v)$인 변 $(u,v)$마다, $f(u,v) < c$이면 남은 담이가 $c - f(u,v)$인 앞으로 가는 변 $(u,v)$을 더한다. $f(u,v) > 0$이면 남은 담이가 $f(u,v)$인 뒤로 가는 변 $(v,u)$을 더한다. 앞으로 가는 변은 흐름을 더 밀어 넣게 하고 뒤로 가는 변은 이미 흘린 것을 물리게 한다. $G_f$의 늘림 경로가 $G$에서 옳게 흐름을 키우는 것과 맞물리므로 남은 그래프가 꼭 있어야 한다. $\square$
...
??? success "연습문제 3 풀이"
    늘릴 때마다 흐름이 적어도 1 단위 들어간다(병목 담이가 양의 정수이기 때문이다). 최대 흐름은 많아야 $f^* \leq \sum_{(s,v)} c(s,v)$이며 마디가 있다. 많아야 $f^*$ 번 늘리면 최대 흐름에 닿는다. 늘릴 때마다 $O(E)$이 든다(깊이 우선 돌아보기나 너비 우선 돌아보기). 모두 $O(Ef^*)$이다. $f^*$이 들임 크기에 대해 지수로 클 수 있으므로($f^* = 2^n$ 따위) 이 값은 커질 수 있고, 그래서 에드먼즈-카프의 $O(VE^2)$ 테두리를 더 좋아한다. $\square$

---

**연습문제 4.**
깊이 우선 돌아보기를 쓰는 포드-풀커슨은 $O(Ef^*)$ 시간이 들지만 에드먼즈-카프는 훨씬 빠른 보기를 지어라.

??? success "연습문제 4 풀이"
    근원 $s$, 바닥 $t$, 가운데 꼭짓점 $a, b$을 보자. 변은 $(s,a,1000), (s,b,1000), (a,t,1000), (b,t,1000), (a,b,1)$이다. 깊이 우선 돌아보기는 길 $s \to a \to b \to t$과 $s \to b \to a \to t$을 번갈아 찾으며 목이 되는 변 $(a,b)$으로 1 단위씩 밀어 넣을 수 있다. 그러면 되순환이 2000 번 든다. 에드먼즈-카프는 너비 우선 돌아보기로 $s \to a \to t$(1000을 밀어 넣음)과 $s \to b \to t$(1000을 밀어 넣음)을 찾아 되순환 2 번 만에 온 흐름 2000을 얻는다. $\square$

## 정리하며

이 마당은 남은 그래프、늘림 경로、그 방법、올바름을 차례로 짚었다.
