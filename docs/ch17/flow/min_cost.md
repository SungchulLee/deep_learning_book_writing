...
??? success "연습문제 1 풀이"
    Standard max-flow maximizes flow without considering edge costs. Min-cost max-flow additionally assigns a cost $w(u,v)$ to each edge and seeks a maximum flow that minimizes $\sum_{(u,v)} w(u,v) \cdot f(u,v)$ (total cost). This combines flow maximization with cost optimization. Applications include transportation (minimize shipping cost while meeting demand) and assignment (minimize total assignment cost). $\square$
...
??? success "연습문제 2 풀이"
    Repeatedly find the shortest (minimum cost) augmenting path from $s$ to $t$ in the residual graph (using Bellman-Ford or SPFA, since costs can be negative on residual edges). Push flow along this path. Repeat until no augmenting path exists. Each augmentation sends flow along the cheapest available route, and the successive shortest paths property guarantees optimality. Time: $O(f^* \cdot VE)$ where $f^*$ is the max flow value. $\square$
...
??? success "연습문제 3 풀이"
    When flow $f(u,v) > 0$ is sent along edge $(u,v)$ with cost $w(u,v)$, the residual graph gets a backward edge $(v,u)$ with cost $-w(u,v)$. This negative cost represents the savings from canceling existing flow. Sending flow along the backward edge effectively undoes previously routed flow, reducing cost by $w(u,v)$ per unit. This mechanism allows the algorithm to correct earlier suboptimal flow decisions. $\square$
...
??? success "연습문제 4 풀이"
    Both solve the assignment problem optimally. The Hungarian algorithm is specialized for bipartite matching with costs, running in $O(n^3)$. Min-cost max-flow is more general: it handles non-bipartite networks, multiple assignments per vertex (with capacity constraints), and transportation problems. For the standard assignment problem, the Hungarian is usually faster ($O(n^3)$ vs. $O(n^3 \log n)$ or worse for flow-based methods). For generalized assignment or transportation problems, min-cost max-flow is the appropriate tool. $\square$
