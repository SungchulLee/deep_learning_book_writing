...
!!! tip "빡빡한 부등호와 느슨한 부등호"
    이음매 점의 조건은 $\text{low}[v] \geq \text{disc}[u]$(같아도 된다)이고 다리의 조건은 $\text{low}[v] > \text{disc}[u]$(반드시 커야 한다)이다. 이 다름은 $v$의 아래나무에서 $u$ 자신으로 되돌아가는 이음이 있으면 $u$이 다리의 끝점이 되는 것은 면하지만($u$을 지나는 순환이 다른 길을 주므로) $u$이 이음매 점이 되는 것은 면하지 못하기 때문이다.
...
??? success "연습문제 1 풀이"
    **다리**는 없애면 그래프가 끊어지는 변이다. 변 $(u, v)$($u$은 깊이 우선 돌아보기 나무에서 $v$의 부모)이 다리인 것은 $\text{low}[v] > \text{disc}[u]$인 것과 같은 뜻이다. 이는 $v$의 아래나무에 있는 어떤 꼭짓점도 $u$이나 $u$의 조상으로 되돌아가는 이음을 지니지 않는다는 뜻이므로, $(u, v)$을 없애면 $v$의 아래나무가 나머지에서 끊긴다. 이 알고리즘은 $O(V + E)$에 돈다. $\square$
...
??? success "연습문제 2 풀이"
    변 $e = (u, v)$이 다리이면서 순환 $C$의 한 조각이라고 하자. 가정에 따라 $e$을 없애면 그래프가 끊긴다. 그런데 순환 $C$에서는 남은 변들이 여전히 $u$에서 $v$으로 가는 길을 준다(순환을 반대쪽으로 돌면 된다). 그러니 $u$과 $v$은 그대로 이어져 있고, 없애기 앞에 다른 꼭짓점은 모두 $u$이나 $v$에 이어져 있었으므로 그래프는 이어진 채로 남는다. 이는 $e$이 다리라는 것과 어긋난다. $\square$
...
??? success "연습문제 3 풀이"
    이어진 그래프에 다리가 없으면 변 이어짐이 적어도 2다. 곧 어떤 변 하나를 없애도 그래프가 이어진 채로 남는다. 이런 그래프를 **2 변 이어짐**이라 한다. 같은 말로, 변마다 적어도 하나의 순환 위에 있다. 이런 그래프에는 변이 적어도 $V$개 있다(변마다 지나는 뻗는 순환을 담는다). $\square$
...
??? success "연습문제 4 풀이"
    다리는 저마다 그 변 하나(와 두 끝점)만 담은 두 겹 변 조각을 이룬다. 다리가 아닌 변은 변이 $\geq 2$개인 두 겹 변 조각으로 묶인다. 두 겹 변 조각의 수는 다리의 수에 가장 큰 2 변 이어짐 아래그래프의 수를 더한 값이다. 덩이-자름 나무에서 다리 덩이는 그래프의 서로 다른 자리를 잇는 잎 같은 얼개다. $\square$

## 정리하며

이 마당은 정의、깊이 우선 돌아보기 바탕 찾기、복잡도、구현을 차례로 짚었다.
