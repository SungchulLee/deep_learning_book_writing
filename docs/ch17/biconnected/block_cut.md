...
**걸음 1.** $G$에서 깊이 먼저 찾기를 돌린다. 꼭짓점 $v$마다 찾아낸 때 $\text{disc}[v]$과 낮은 값 $\text{low}[v]$을 지닌다.

**걸음 2.** 이음 쌓개를 쓴다. 깊이 먼저 찾기가 자식 $u$에서 어버이 $v$으로 되짚어 갈 때 $\text{low}[u] \ge \text{disc}[v]$이면 $(v, u)$이 나올 때까지 쌓개에서 이음을 뺀다. 이 이음들이 두 겹 이음 덩이(블록) 하나를 이룬다.

**걸음 3.** 자르는 꼭짓점을 짚는다. (가) $v$이 깊이 먼저 찾기의 뿌리이면서 자식이 둘 이상이거나, (나) $v$이 뿌리가 아니면서 $\text{low}[u] \ge \text{disc}[v]$인 자식 $u$을 지니면 $v$은 자르는 꼭짓점이다.
...
??? success "연습문제 1 풀이"
    **블록-자름 나무**는 그래프의 두 겹 이음 덩이 얼개를 나무로 나타낸 것이다. 마디는 두 갈래다. **블록 마디**(두 겹 이음 덩이마다 하나)와 **자름 마디**(이음매 점마다 하나)다. 이음매 점이 어떤 두 겹 이음 덩이에 들면 그 자름 마디와 블록 마디를 이음으로 잇는다. 결과는 늘 나무다(끊어진 그래프라면 숲이다). $\square$
...
??? success "연습문제 2 풀이"
    **돌이가 없음**: 블록 $B_1, c_1, B_2, c_2, \ldots, B_k, c_k, B_1$으로 이루어진 돌이가 있다고 하자. 그러면 $c_1$이 $B_1$과 $B_2$을, $c_2$이 $B_2$과 $B_3$을 잇는 식이다. 그런데 그러면 $B_1$과 $B_k$이 자르는 꼭짓점 $c_k$을 함께 지니면서 $c_1, B_2, \ldots$을 거쳐서도 이어지므로, 본디 그래프에 여러 두 겹 이음 덩이에 걸친 돌이가 생긴다. 이는 두 겹 이음 덩이가 가장 크다는 것과 어긋난다. **이어져 있음**: 이음매 점을 함께 지닌 블록 둘 사이에는 그 이음매 점을 지나는 길이 블록-자름 나무에 있다. 본디 이어진 그래프의 어떤 두 꼭짓점에 대해서도 그 사이 길에 놓인 블록들이 이어진 잔나무를 이룬다. $\square$
...
??? success "연습문제 3 풀이"
    블록-자름 나무를 지은 뒤($O(V + E)$에 짓는다), 꼭짓점 $u$이 이음매 점인 것은 그 나무에 자름 마디로 나타나는 것과 같은 뜻이다. 곧 $u$이 두 겹 이음 덩이 여럿에 든다는 뜻이다. 참거짓 깃발을 갈무리해 두거나 블록-자름 나무에서 $u$에 맞는 마디의 자릿수를 보면 된다. 자름 마디는 늘 자릿수가 $\geq 2$이다. 이러면 $O(V + E)$의 미리 다듬기 뒤 물음마다 $O(1)$이 든다. $\square$
...
??? success "연습문제 4 풀이"
    꼭짓점 $v$ 하나를 없앤 뒤 어떤 꼭짓점 짝이 그대로 이어져 있는지 가리려면 이렇게 한다. $v$이 자르는 꼭짓점이 아니면 모든 짝이 그대로 이어져 있다. $v$이 자르는 꼭짓점이면 없앨 때 그래프가 블록-자름 나무에서 $v$에 붙은 두 겹 이음 덩이들로 쪼개진다. 같은 잔나무 안의 꼭짓점은 그대로 이어져 있고 다른 잔나무의 꼭짓점은 끊긴다. 블록-자름 나무를 쓰면 미리 다듬은 뒤 이런 물음에 물음마다 $O(1)$에 답할 수 있어 그물의 무른 곳을 잘 살필 수 있다. $\square$
