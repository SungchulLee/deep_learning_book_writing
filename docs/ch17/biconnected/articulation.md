...
## 2. 깊이 우선 돌아보기 바탕 알고리즘

이 알고리즘은 [타잔의 강한 이음 조각 알고리즘](../scc/tarjan.md)과 비슷하게 돌아보기의 찾은 시각과 낮은 이음 값을 쓰되 방향 없는 그래프에 쓴다.

꼭짓점 $u$마다 다음을 정한다:

- $\text{disc}[u]$: 깊이 우선 돌아보기에서 $u$을 찾아낸 때.
- $\text{low}[u]$: 되돌아가는 이음까지 넣어, $u$의 깊이 우선 돌아보기 아래나무를 거쳐 $u$에서 닿을 수 있는 가장 이른 찾아낸 때.
...
변이 $\{0\text{-}1,\ 1\text{-}2,\ 2\text{-}3,\ 3\text{-}1,\ 3\text{-}4,\ 4\text{-}5,\ 5\text{-}6\}$인 그래프를 보자.
...
??? success "연습문제 1 풀이"
    **이음매 점**(자르는 꼭짓점)은 없애면 그래프가 끊어지는 꼭짓점이다. 그물에서 이음매 점은 홀로 무너지면 끝인 자리다. 그 자리의 길잡이, 서버, 변이 무너지면 그물의 일부에 닿을 수 없게 된다. 이음매 점을 짚어 두면 겹으로 두터운 그물을 꾸미는 데 도움이 된다. 낮은 변 값을 쓰는 깊이 우선 돌아보기로 $O(V + E)$에 찾을 수 있다. $\square$
...
??? success "연습문제 2 풀이"
    깊이 우선 돌아보기를 돌리며 꼭짓점마다 찾아낸 때 $\text{disc}[v]$과 낮은 값 $\text{low}[v]$을 셈한다. 다음이면 꼭짓점 $u$은 이음매 점이다. (1) $u$이 깊이 우선 돌아보기의 뿌리이고 그 나무에서 자식이 $\geq 2$개다. (2) $u$이 뿌리가 아니면서 $\text{low}[v] \geq \text{disc}[u]$인 자식 $v$을 지닌다($v$의 아래나무에 있는 어떤 꼭짓점도 되돌아가는 이음으로 $u$ 위로 갈 수 없다). 조건 (2)는 $u$을 없애면 $v$의 아래나무가 그래프의 나머지에서 끊긴다는 뜻이다. $\square$
...
??? success "연습문제 3 풀이"
    $(\Rightarrow)$ 뿌리의 자식이 $\geq 2$개이면 뿌리를 없앨 때 그 아래나무들이 끊긴다(깊이 우선 돌아보기 나무에서 아래나무 사이의 변은 모두 뿌리를 지나고, 방향 없는 깊이 우선 돌아보기에는 엇변이 없다). $(\Leftarrow)$ 뿌리의 자식이 꼭 하나면 뿌리를 없애도 이어진 아래나무 하나가 남는다(모든 꼭짓점이 그 아래나무 안에 있고 나무 변과 되돌아가는 이음으로 이어져 있다). 자식이 없으면 뿌리는 홀로 떨어져 있으니 없애도 아무것도 끊기지 않는다. $\square$
...
??? success "연습문제 4 풀이"
    꼭짓점이 $n$개인 2 꼭짓점 변 그래프에는 변이 적어도 $n$개 있어야 한다. 모든 꼭짓점을 지나는 순환이 있어야 하기 때문이다(그렇지 않으면 잎이 그 이웃의 이음매 점이 된다). $n = 8$이면 해밀턴 순환 하나가 변 8개를 주고 2 변이다. 그러므로 가장 적어도 변 8개다. $\square$

## 정리하며

이 마당은 정의、깊이 우선 돌아보기 바탕 알고리즘、복잡도、구현을 차례로 짚었다.
