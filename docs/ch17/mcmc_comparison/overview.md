...
둘 다 기울기를 쓰지만 HMC는 운동량을 더해 (퍼지는 대신) 탄도처럼 움직인다.
...
**HMC는 기울기 값 매기기 한 번마다 $\sqrt{L}$배 더 효율적으로 살펴본다!**
...
**가장 좋게 맞추면** HMC는 실효 표본마다의 값이 $\mathcal{O}(d)$, 나아가 $\mathcal{O}(\sqrt{d})$까지 될 수 있어 다른 방법보다 훨씬 낫다.
...
## 9. 받아들임 비율과 가장 좋게 맞추기

| 방법 | 가장 좋은 받아들임 | 출처 |
|--------|-------------------|--------|
| 무작위 걸음 MH | 23.4%(높은 차원) | Roberts et al. (1997) |
| 깁스 | 100% | 짜임새 덕분 |
| MALA | 57.4% | 로버츠 & 로젠털 (1998) |
| HMC | 약 65% | 겪어 본 값 |
| NUTS | 달라짐 | 스스로 맞춤 |
...
**HMC는 왜 약 65%인가?** 개구리뜀 $L$ 걸음에서 자취 전체가 높은 받아들임을 얻기를 바라기 때문이다. 겪어 보면 여러 문제에서 65%가 잘 듣는다.
...
**맺음**: 되풀이마다의 값은 크지만 $d$이 크면 HMC가 가장 효율적이다.
...
이 마당은 네 방법 훑어보기、큰 견줌 표、정보의 층위、둘씩 자세히 견주기을 차례로 짚었다.
