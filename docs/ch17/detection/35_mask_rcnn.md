...
마스크 R-CNN은 2017년 논문 "Mask R-CNN"에서 나왔다. 마스크 어림 가지를 더해 더 빠른 R-CNN을 넓힌다.
...
#!/usr/bin/env python3
'''
마스크 R-CNN — 낱 물체 나누기 얼거리
논문: "Mask R-CNN" (2017)
핵심: 마스크 어림 가지를 더해 더 빠른 R-CNN을 넓힌다
'''
import torch
...
class MaskRCNN(nn.Module):
    def __init__(self, num_classes=81):
        ...
        self.rpn_cls = nn.Conv2d(512, 2 * 9, 1)  # 닻 9개, 갈래 2개(물체/아님)
        self.rpn_reg = nn.Conv2d(512, 4 * 9, 1)  # 닻 9개, 자리표 4개
...
`MaskRCNN` 클래스는 PyTorch의 `nn.Module` 사이를 써서 모델 얼개를 감싼다. `forward` 메서드가 셈 그래프를 정하므로 익히는 동안 PyTorch의 자동 미분 체계가 기울기 셈을 알아서 다룬다. 이 단원별 꾸밈 덕분에 낱낱의 조각을 고치거나 모델을 더 큰 물길에 끼워 넣기가 쉽다.
...
**연습문제 1.**
`MaskRCNN`의 앞먹임을 따라가며 텐서 꼴을 좇아라. 붙박이 매개변수로 들임 표본 4개짜리 묶음에 대해 주요 연산(누비기, 모으기, 선형 층)마다 그 뒤의 꼴을 적어라.
...
**연습문제 4.**
층이나 덩이의 개수를 정할 수 있도록 `MaskRCNN`을 넓혀라. `__init__`에 `num_layers` 매개변수를 더하고 `nn.ModuleList`로 깊이가 바뀌는 얼개를 만들어라. 층 2, 4, 8개로 시험하여라.
...
??? success "연습문제 4 풀이"
    ...
    ```
    `forward` 메서드에서 `for layer in self.layers: x = layer(x)`로 되풀이한다. (그냥 파이썬 목록이 아니라) `nn.ModuleList`를 써야 PyTorch가 가장 좋게 하기에 쓸 매개변수를 모두 등록한다. 시험: `for n in [2, 4, 8]: model = MaskRCNN(num_layers=n); print(f'Layers={n}, params={sum(p.numel() for p in model.parameters()):,}')`.

## 정리하며

**다룬 것** — 마스크 R-CNN

`MaskRCNN` 클래스는 PyTorch의 `nn.Module` 사이를 써서 모델 얼개를 감싼다.
