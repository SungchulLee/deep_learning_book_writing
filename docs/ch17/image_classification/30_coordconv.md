...
CoordConv는 2018년 논문 "An Intriguing Failing of Convolutional Neural Networks"에서 나왔다. 자리표 채널을 더해 자리에 대한 따짐을 돕는다.
...
여기 짠 것은 함께 어울려 온전한 그림 가르기 얼개를 이루는 클래스 3개(`AddCoords`, `CoordConv`, `CoordConvNet`)를 정한다. 클래스마다 뚜렷한 조각 하나를 감싸므로 코드가 단원별로 나뉘고 넓히기 쉽다. `forward` 메서드가 PyTorch의 자동 미분에 쓰이는 셈 그래프를 정한다.
...
**연습문제 1.**
`AddCoords`의 앞먹임을 따라가며 텐서 꼴을 좇아라. 붙박이 매개변수로 들임 표본 4개짜리 묶음에 대해 주요 연산(누비기, 모으기, 선형 층)마다 그 뒤의 꼴을 적어라.
...
??? success "연습문제 2 풀이"
    첫 엮음 켜의 `in_channels`을 이제 값에서 3으로 바꾼다. 엮음과 모으기 켜마다 $H_{\text{out}} = \lfloor(H_{\text{in}} + 2p - k) / s\rfloor + 1$으로 자리 차수를 다시 셈한다. 첫 선형 켜의 `in_features`을 마지막 엮음/모으기 켜의 편 날임에 맞춘다. `model = AddCoords(...); x = torch.randn(1, 3, 64, 64); print(model(x).shape)`으로 따진다.
...
**연습문제 4.**
층이나 덩이의 개수를 정할 수 있도록 `AddCoords`을 넓혀라. `__init__`에 `num_layers` 매개변수를 더하고 `nn.ModuleList`로 깊이가 바뀌는 얼개를 만들어라. 층 2, 4, 8개로 시험하여라.
...
??? success "연습문제 4 풀이"
    ...
    ```
    `forward` 메서드에서 `for layer in self.layers: x = layer(x)`로 되풀이한다. (그냥 파이썬 목록이 아니라) `nn.ModuleList`를 써야 PyTorch가 가장 좋게 하기에 쓸 매개변수를 모두 등록한다. 시험: `for n in [2, 4, 8]: model = AddCoords(num_layers=n); print(f'Layers={n}, params={sum(p.numel() for p in model.parameters()):,}')`.

## 정리하며

**다룬 것** — CoordConv

여기 짠 것은 함께 어울려 온전한 그림 가르기 얼개를 이루는 클래스 3개(`AddCoords`, `CoordConv`, `CoordConvNet`)를 정한다.

고갱이 갈래는 `AddCoords`, `CoordConv`, `CoordConvNet`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
