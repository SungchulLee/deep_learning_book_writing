...
`TransformerEncoderBlock`은 `attn_mask`가 아니라 `key_padding_mask`를 쓴다.

핵심 클래스는 `TransformerEncoderBlock`, `TransformerEncoder`, `SinusoidalEncoder`이며 앞의 연습문제 3개로 직접 확인할 수 있다.
