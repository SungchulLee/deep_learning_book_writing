...
실행 가능한 코드와 자세한 분석(선형 분류기와의 동치 관계, 클래스별 정확도 표, 표본 수에 따른 곡선)은 [3.1 템플릿 학습](../../ch03/mnist/01_template_learning.md)에 있다. 그 절을 시작으로 2장은 템플릿 학습 → 선형 모델 → 다층 퍼셉트론 → 합성곱 신경망의 네 걸음을 같은 데이터셋 위에서 견준다.
