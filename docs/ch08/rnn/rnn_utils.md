...
이 구현은 클래스 다섯 개(`SimpleRNN`, `LSTMClassifier`, `GRUPredictor`, `BiLSTM`, 그리고 하나 더)를 정의하며, 이들이 어우러져 완전한 순환 신경망 구조를 이룬다.

핵심 클래스는 `SimpleRNN`, `LSTMClassifier`, `GRUPredictor`, `BiLSTM`이며 앞의 연습문제 4개로 직접 확인할 수 있다.
