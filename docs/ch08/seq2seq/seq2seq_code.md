...
- **바다나우 어텐션 (8.5절)**: 덧셈 어텐션 장치가 고정된 문맥 벡터의 정보 병목을 없애, 복호기가 생성 걸음마다 쓸모 있는 부호기 상태에 물어볼 수 있게 한다.
- **부호기-복호기 틀 (8.4절)**: 전체 구조가 어텐션으로 보강한 문맥 벡터와 함께 부호기-복호기 틀을 실현한다.
- **교사 강요 (8.4절)**: 학습은 수렴 속도와 노출 편향의 균형을 잡으려고 확률적인 교사 강요를 쓴다.
