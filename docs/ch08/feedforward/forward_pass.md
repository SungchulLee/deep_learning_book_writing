...
| 활성화 함수 (7.1절) | 각 선형 변환 뒤에 원소별로 적용된다 |
...
def forward_pass_manual(
    ...
    """
    L = len(weights)
    # 입력을 a0으로 두는 것이 표기의 요령이다. 이렇게 하면 "l번째 층은
    # a{l-1}을 받아 a{l}을 낸다"가 첫 층에서도 그대로 성립해,
    # 아래 반복문에 예외 처리가 필요 없어진다
    cache = {'a0': x}
    a = x
    
    for l in range(L):
        # 아핀: z = a @ W^T + b   (PyTorch의 행 우선 관례)
        # 전치가 붙는 까닭은 W를 (출력, 입력) 모양으로 저장하기 때문이다.
        # nn.Linear의 weight도 같은 모양이라, 여기서 만든 W를 그대로
        # nn.Linear에 넣어 볼 수 있다.
        # b는 (n_l,)이라 배치 축으로 방송되어 표본마다 같은 값이 더해진다
        z = a @ weights[l].T + biases[l]
        # z와 a를 둘 다 남긴다. 역전파에서 ReLU의 문지기 노릇에는 활성화
        # 전의 z가, 가중치의 기울기에는 활성화 뒤의 a가 필요하기 때문이다
        cache[f'z{l+1}'] = z
        
        # 활성화.
        # 마지막 층만 갈라 다루는 것이 이 함수의 유일한 조건문이다.
        # 은닉층은 ReLU로 비선형성을 얻고, 출력층은 시그모이드로 확률을
        # 낸다. 은닉층까지 시그모이드로 두면 층을 지날 때마다 기울기가
        # 0.25배 이하로 줄어 깊은 신경망이 학습되지 않는다
        if l < L - 1:
            a = F.relu(z)          # 은닉층
        else:
...
이 마당은 학습 목표, 미리 알아야 할 것, 개요, 수학적 정식화를 차례로 짚었다.
