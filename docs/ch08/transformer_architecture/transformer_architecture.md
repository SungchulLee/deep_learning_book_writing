...
Before Transformers, sequence-to-sequence tasks relied on recurrent architectures (RNNs, LSTMs, GRUs) with attention mechanisms. While effective, these approaches suffered from fundamental limitations that the Transformer was designed to overcome.

### Sequential Processing Bottleneck

RNNs process sequences one token at a time, where each hidden state $h_t$ depends on the previous state $h_{t-1}$:
...
1. **No parallelization**: Each step must wait for the previous step to complete, making training slow on modern parallel hardware (GPUs, TPUs). Training time scales linearly with sequence length regardless of available compute.
2. **Resource inefficiency**: Every time step requires its own set of operations, resulting in high computational cost for long sequences and poor utilization of parallel hardware.

### Long-Range Dependency Problem

RNNs struggle to maintain information across long distances due to:

1. **Vanishing gradients**: Gradients propagated through many time steps shrink exponentially, preventing the model from learning dependencies between distant tokens. Although LSTMs and GRUs mitigate this through gating mechanisms, they still degrade on very long sequences.
2. **Memory constraints**: The fixed-size hidden state must compress all relevant history, creating an information bottleneck. Context from early tokens is progressively overwritten as the sequence progresses.
3. **Inefficient distant context**: Information must be relayed step-by-step through intermediate hidden states, even when the intermediate tokens are irrelevant to the dependency being modeled.

### How the Transformer Addresses These Challenges

The Transformer architecture resolves these limitations through self-attention:

1. **Full parallelization**: Self-attention processes all tokens simultaneously, computing relationships between every pair of tokens in a single operation. This enables efficient utilization of parallel hardware.
2. **Direct long-range access**: Every token directly attends to every other token regardless of distance. The maximum path length between any two tokens is $O(1)$, compared to $O(n)$ for RNNs.
3. **No vanishing gradients from sequential depth**: Since the Transformer does not process tokens sequentially, gradients flow more freely through the network without accumulating over many time steps.
4. **Positional encoding**: Sequential order information, lost by the parallelization, is recovered through explicit positional encodings added to the input embeddings.

### Key Innovations Summary

The Transformer introduced several architectural innovations that collectively represent a paradigm shift:

| Innovation | Replaces | Benefit |
|-----------|----------|---------|
| Self-attention | Recurrent computation | $O(1)$ path length between any two tokens |
| Multi-head attention | Single attention function | Captures diverse relationship types in parallel |
| Positional encoding | Inherent sequential ordering | Injects position without sacrificing parallelism |
| Scaled dot-product | Additive attention | Efficient GPU computation via matrix multiplication |
| Residual connections + LayerNorm | Deep stacking | Stable gradient flow through many layers |
