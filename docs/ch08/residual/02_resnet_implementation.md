...
ResNet-34            21,289,802 parameters
ResNet-50            23,528,522 parameters
ResNet-101           42,520,650 parameters
...
이 구현은 클래스 세 개(`BasicBlock`, `Bottleneck`, `ResNet`)를 정의하며, 이들이 어우러져 완전한 잔차 신경망 구조를 이룬다.

핵심 클래스는 `BasicBlock`, `Bottleneck`, `ResNet`이며 앞의 연습문제 4개로 직접 확인할 수 있다.
