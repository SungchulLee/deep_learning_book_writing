# 떠돌이 장수 문제 - 되돌이
도시에 $1,2,\ldots,N$으로 번호를 매기고 도시 $1$에서 시작한다고 하자. 도시 $i$과 도시 $j$ 사이 거리는 $d_{i,j}$이다

# 첫째 마디

$i$에서 시작해 $j$에서 끝나는 모든 길 $\pi\in\Pi(i,S,j)$을 보자
다음을 채우는

$$\begin{array}{lll}
(1)&&\text{$\pi$은 $i$에서 시작해 다시 들르지 않는다}\\
(2)&&\text{$\pi$은 $S$($i\notin S, j\notin S$)의 모든 도시를 꼭 한 번씩 들른다}\\
(3)&&\text{$\pi$은 $j$에서 끝나며 그 앞에는 들르지 않았다}\\
\end{array}$$
