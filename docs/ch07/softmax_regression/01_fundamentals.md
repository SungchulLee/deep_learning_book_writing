...
!!! note "MNIST에서는 어떤 값이 되는가"
    ...
    이 값으로 실제 학습을 돌려 92.51%를 얻는 과정은 [3.2 구현과 결과](../../ch03/linear_softmax/06_implementation.md)에 있다. 이 절은 그 특수한 경우가 아니라 모델 자체의 정의를 다룬다.
...
까닭은 두 자료의 성격에 있다. 손으로 쓴 숫자는 획이 놓이는 자리가 클래스마다 꽤 일정해서, 화소마다 가중치 하나씩만 두어도 상당히 가려낸다. 반면 셔츠와 외투는 윤곽이 비슷하고 차이가 무늬와 질감에 있는데, 이는 화소 하나하나의 밝기를 따로 저울질하는 방식으로는 잡아내기 어렵다. 이웃한 화소들이 이루는 무늬를 보아야 하며, 그것이 [10장 합성곱 신경망](../../ch10/index.md)이 하는 일이다.

!!! note "3장의 값과 조금 다른 까닭"
    [3.2절](../../ch03/linear_softmax/06_implementation.md)은 같은 모델로 MNIST에서 92.51%를 보고한다. 위에서 잰 값과 0.4%포인트쯤 차이가 나는데, 이는 실행할 때마다 생기는 흔들림이다. 자료를 섞는 차례와 부동소수점 셈의 순서가 실행마다 달라지기 때문이며, 씨앗을 고정해도 기계와 스레드 수가 다르면 완전히 같아지지는 않는다.
