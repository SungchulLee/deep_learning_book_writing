...
The encoder-decoder architecture represents the fundamental paradigm in sequence-to-sequence learning, enabling models to transform variable-length input sequences into variable-length output sequences. Introduced by Sutskever et al. (2014), this architecture revolutionized machine translation and enabled diverse applications from text summarization to conversational AI.

## Conceptual Foundation

The encoder-decoder framework addresses a fundamental challenge in sequence modeling: mapping an input sequence of arbitrary length to an output sequence of potentially different length. Traditional fixed-size neural networks cannot naturally handle this variability, making the encoder-decoder design essential for sequence transduction tasks.
...
$$\mathbf{c} = \mathbf{h}_T^{enc} \quad \text{or} \quad \mathbf{c} = f(\mathbf{h}_1^{enc}, \ldots, \mathbf{h}_T^{enc})$$
...
Using a bidirectional encoder improves context representation, but requires projecting the combined states to match the decoder's expected dimensions:
...
The basic encoder-decoder architecture has since evolved through several key innovations: attention mechanisms (Bahdanau et al., 2015) addressed the information bottleneck, the transformer architecture (Vaswani et al., 2017) replaced recurrence with self-attention, and pre-trained models (BERT, GPT) leveraged large-scale unsupervised pre-training.
...
**Key components**: the encoder compresses the source sequence into a context vector, the decoder generates the target sequence autoregressively from context, bidirectional encoders capture full source context, and teacher forcing accelerates training using ground truth.

**Key techniques**: scheduled sampling mitigates exposure bias, beam search improves generation quality, packed sequences handle variable lengths efficiently, gradient clipping prevents exploding gradients, and proper weight initialization ensures training stability.
