...
전체 평가 보고서는 문턱값에 따라 달라지는 지표(정확도, 정밀도, 재현율, F1), 문턱값과 무관한 지표(ROC-AUC, 평균 정밀도), 상관에 기반한 지표(MCC, 코헨의 카파)를 함께 담는다.

핵심 클래스는 `ClassificationMetrics`이며 앞의 연습문제 3개로 직접 확인할 수 있다.
