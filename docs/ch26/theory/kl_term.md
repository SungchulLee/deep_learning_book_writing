# KL 벌어짐 항
변분 오토인코더에서 KL 벌주기의 수학 성질과 셈하기 세부.
...
- 앞 KL과 뒤 KL을 설명하고 변분 오토인코더가 왜 뒤 KL을 쓰는지 밝히기
- 변분 오토인코더 익히기에서 KL 벌어짐의 앎 이론상 노릇 이해하기
...
더 근본으로는 증거 하한의 벌어짐이 어림 사후 분포에서 참 사후 분포로의 뒤 KL인 $D_{KL}(q_\phi(z|x) \| p_\theta(z|x))$이며, 이는 인코더가 봉우리를 모두 덮으려 퍼지기보다 참 사후 분포의 확률 높은 자리에 몰리기 쉽다는 뜻이다.
...
### 변분 오토인코더의 특별한 경우: q_phi(z|x)과 표준 정규 분포
...
모든 $x$에 대해 $q_\phi(z|x) = \mathcal{N}(0, I)$이면 $D_{KL}(q_\phi(z|x) \| p(z)) = 0$이며 인코더가 들임을 아예 무시한다는 뜻이다. 이것이 **사후 분포 무너짐**이다. 곧 인코더가 들임과 무관하게 같은 분포를 낸다.

### KL이 클 때

KL이 크면 인코더가 숨은 공간을 많이 써서 들임 $x$마다 상당한 앎을 담는다는 뜻이다. 다시 세우기에는 좋지만 KL이 지나치면 숨은 공간이 사전 분포에서 크게 벗어나 만들어 내기 품질을 해칠 수 있다.
...
따라서 KL 항은 자료와 숨은 부호 사이의 서로 앎과, 모은 사후 분포와 사전 분포의 어긋남에 모두 벌을 준다. 이 쪼개기가 베타 변분 오토인코더와 얽힘 풀기에 쓰이는 전체 상관 쪼개기를 이해하는 열쇠이다.

---

## 9. 다음은

다음 절은 증거 하한의 가능도 조각과 여러 디코더 분포 고름을 다루는 다시 세우기 항을 살핀다.
...
??? success "연습문제 1 풀이"
    ...
    이 항이 닫힌 꼴로 떨어지는 것이 변분 오토인코더를 다루기 쉽게 만드는 큰 이유다.
...
??? success "연습문제 2 풀이"
    ...
    $-\log\sigma^2$ 항이 특히 중요하다. 이것이 없으면 $\sigma \to 0$이 되어 인코더가
    결정적 함수로 되돌아가고, 그러면 [오토인코더](../../ch25/index.md)와 같아진다.
...
??? success "연습문제 3 풀이"
    ...
    **둘째, 틈이 정확히 얼마인지 알 수 있다.** 다르게 전개하면
    ...
    $q$가 참된 사후 분포와 같아지면 등호가 성립한다. 인코더를 익히는 일이 곧 그
    틈을 줄이는 일이다.
...
KL 벌어짐은 대칭이 아니다. 변분 오토인코더가 $D_{\mathrm{KL}}(q \| p)$를 쓰고 $D_{\mathrm{KL}}(p \| q)$를 쓰지 않는 까닭은 무엇인가?
...
??? success "연습문제 4 풀이"
    ...
    변분 오토인코더가 앞쪽을 쓰는 까닭은 **셈할 수 있기 때문**이다. 앞의 유도에서
    ...
    여러 봉우리를 가질 때 변분 오토인코더는 그중 **하나만** 잡는 경향이 있다. 이것이
...
??? success "연습문제 5 풀이"
    ...
    1 쪽으로 당기던 힘이 사라져 인코더가 $\sigma \to 0$으로 간다. 잡음이 없는 편이
    되돌리기에 유리하기 때문이다.

    $\sigma = 0$이면 $z = \mu(x)$로 결정적이 되므로, 이 모델은 정확히
    [오토인코더](../../ch25/index.md)다.

    그리고 오토인코더가 못 하는 일을 똑같이 못 하게 된다.
    [24.5절](../../ch25/limits/latent_sampling.md)에서 잰 대로 $\mathcal{N}(0,I)$에서
...
??? success "연습문제 6 풀이"
    ...
    이 관점에서 보면 [26장의 여러 벌점](../../ch25/ae/loss_functions.md)과 한 줄에
...
??? success "연습문제 9 풀이"
    ...
    그러면 디코더는 코드를 무시하고 자료의 평균 같은 것만 내놓게 된다. 손실에서 KL은
...
| **앞 KL** | $D_{KL}(p \| q)$ | 평균을 좇음(변분 오토인코더에서는 쓰지 않는다) |
