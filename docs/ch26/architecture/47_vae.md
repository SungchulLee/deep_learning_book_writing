...
2013년 Kingma와 Welling이 내놓은 변분 오토인코더(VAE)는 오토인코더 얼거리를 확률 숨은 공간으로 넓힌다. 들임을 붙박이 부호에 옮기는 대신 인코더가 정규 분포의 평균과 흩어짐을 내놓고 다시 매개변수화 재주로 표본을 뽑는다. 그래서 새 표본을 만들어 내는 것과 숨은 공간에 대한 원칙 있는 베이즈 추론이 모두 된다.
...
다시 매개변수화 재주가 변분 오토인코더의 고갱이 새것이다. $z \sim \mathcal{N}(\mu, \sigma^2)$을 곧바로 뽑는(미분할 수 없는) 대신 $\epsilon \sim \mathcal{N}(0, I)$에 대해 $z = \mu + \sigma \odot \epsilon$으로 쓴다. 그러면 무작위가 셈 그래프 밖으로 나가 뒤먹임 퍼뜨리기 때 기울기가 $\mu$과 $\sigma$을 지나 흐를 수 있다.

변분 오토인코더의 손실은 다시 세우기 품질(두 값 엇갈린 엔트로피나 평균 제곱 어긋남)과, 숨은 분포를 표준 정규 사전 분포 쪽으로 끄는 KL 벌어짐 항을 아우른다. KL 항 $D_\text{KL}(q(z|x) \| p(z)) = -\frac{1}{2}\sum(1 + \log\sigma^2 - \mu^2 - \sigma^2)$이 인코더가 점 어림으로 무너지는 것을 막고 숨은 공간이 매끄럽고 이어지게 한다. 이 벌주기 덕분에 변분 오토인코더가 만들어 내는 모델이 된다. 곧 $\mathcal{N}(0, I)$에서 뽑아 풀면 그럴듯한 새 그림이 나온다.

다시 세우기와 KL 벌어짐 사이의 팽팽함이 근본 맞바꿈을 만든다. 다시 세우기 손실을 가장 작게 하면 또렷하고 세밀한 내놓기가 나오지만 숨은 공간이 조각날 수 있다. KL을 가장 작게 하면 매끄럽고 고른 숨은 공간이 되지만 다시 세운 것이 흐릿할 수 있다. 이 두 항을 저울질하는 것이 변분 오토인코더 설계의 핵심 과제이다.
...
MNIST에 변분 오토인코더를 익히고 $z \sim \mathcal{N}(0, I)$을 풀어 아무 표본 100개를 만들어라. 10x10 격자로 보여라. 만든 숫자를 알아볼 수 있는가?
...
??? success "연습문제 2 풀이"
    ...
    plt.show()
    ```
    만든 숫자 대부분을 알아볼 수 있으나 실제 MNIST 그림에 견주면 조금 흐릿하게 보일 수 있다. 이 흐릿함은 평균 제곱 어긋남이나 두 값 엇갈린 엔트로피 다시 세우기 손실을 쓴 변분 오토인코더의 특징이다.
...
??? success "연습문제 3 풀이"
    곧바로 뽑으면 셈 그래프에 미분할 수 없는 연산이 생긴다. 곧 분포에서 뽑는 `sample` 연산은 분포 매개변수에 대한 기울기가 없다. 기울기가 $z$을 지나 $\mu$과 $\log\sigma^2$으로 흐르지 않으면 인코더 매개변수를 뒤먹임 퍼뜨리기로 새로 고칠 수 없다. 다시 매개변수화 재주는 뽑기를 $\mu$, $\sigma$, 바깥 잡음 $\epsilon$의 정해진 함수로 다시 세워 물길 전체를 미분할 수 있게 한다.
...
변분 오토인코더가 오토인코더와 다른 점을 세 줄로 적어라.
...
??? success "연습문제 4 풀이"
    | | 오토인코더 | 변분 오토인코더 |
    |---|---|---|
    | 인코더가 내놓는 것 | 코드 하나 | 분포 $(\mu, \log\sigma^2)$ |
    ...
    거꾸로 셋 가운데 하나라도 빼면 오토인코더로 되돌아간다. KL을 빼면 $\sigma \to 0$이
...
변분 오토인코더의 손실을 코드로 적고 각 줄이 무엇인지 밝혀라.
...
??? success "연습문제 6 풀이"
    **$p(x)$를 정의하기 때문이다.**
    ...
    사전 분포 $p(z) = \mathcal{N}(0,I)$와 디코더 $p(x \mid z)$가 함께 자료 공간의 확률
    분포를 정한다. 그래서 뽑을 수 있고, (하한으로) 가능도를 물을 수 있다.

    오토인코더는 이런 식을 쓸 수 없다. 인코더와 디코더가 있을 뿐 어떤 확률 모형도 세우지
    않으므로 "이 모델이 이 그림에 주는 확률"이라는 것이 정의되지 않는다
    ([24.5절 연습문제 9](../../ch25/limits/latent_sampling.md)).

    실용적인 결과가 이것이다. 변분 오토인코더는 뽑기가 **정의상** 가능하다.
    $z \sim \mathcal{N}(0,I)$을 뽑아 디코더에 넣으면 그것이 모델의 표본이다. 오토인코더에서
...
??? success "연습문제 7 풀이"
    ...
    **음의 로그 가능도**다. 손실의 모양이 곧 $p(x \mid z)$에 대한 가정이다.
    ...
    잘 듣는다([26장 손실 함수](../../ch25/ae/loss_functions.md)에서 BCE가 MSE보다
    나았다).
    ...
    **화소마다 독립**이라는 가정이 훨씬 심각한 문제다. 실제 그림에서 이웃 화소는 강하게
    ...
    이 가정을 깨는 것이 뒤 장들의 큰 줄기다. 자기 회귀 디코더는 화소 사이 의존을 직접
    모형하고, [퍼짐 모델](../../ch30/index.md)은 아예 다른 길로 간다.
...
??? success "연습문제 8 풀이"
    ...
    그래서 변분 오토인코더에서는 이 물음이 오토인코더에서보다 **덜 예민하다.**
    KL 항이 필요 없는 차원을 꺼 주므로 넉넉히 잡아도 크게 낭비되지 않는다. 오토인코더는
...
??? success "연습문제 9 풀이"
    ...
    0이 되고, 이것은 인코더가 입력을 볼 필요조차 없는 답이다.
...
??? success "연습문제 10 풀이"
    오토인코더가 하던 일에 뽑기가 더해진다.

    | 하는 일 | 쓰는 부분 |
    |---|---|
    | 다시 세우기 | 인코더 + 디코더 |
    | 차원 줄이기, 특징 뽑기 | 인코더($\mu$) |
    | **새 표본 만들기** | 디코더만. $z \sim \mathcal{N}(0,I)$ |
    | 코드 사이 끼움 | 인코더 두 번 + 디코더 |
    | 이상 탐지 | 증거 하한이 낮은 것을 찾는다 |

    셋째 줄이 이 장의 새로운 것이다. **인코더를 전혀 쓰지 않는다**는 점을 눈여겨볼
    만하다. 익히고 나면 만들어 내기에는 디코더만 필요하다.

    다섯째 줄도 변분 오토인코더라야 되는 일이다. 오토인코더에서도 다시 세우기 오차로
    비슷한 것을 하지만, 여기서는 증거 하한이라는 확률적 근거가 있는 값을 쓸 수 있다.

    `encode`, `decode`, `forward`를 나누어 두는 설계가 이 쓰임의 다양함 때문에 값지다
    ([26장 모듈](../../ch25/architecture/autoencoder.md)).

## 정리하며

**다룬 것** — 변분 오토인코더

다시 매개변수화 재주가 변분 오토인코더의 고갱이 새것이다.
