...
조건부 변분 오토인코더(cVAE)는 인코더와 디코더를 모두 갈래 이름표에 조건 지어 여느 변분 오토인코더를 넓히고, 그래서 만들어 내기를 다스릴 수 있게 한다. 갈래 이름표가 주어지면 그 갈래의 다양한 표본을 만들 수 있고 하나만 뜨거운 이름표 벡터를 섞어 갈래 사이를 사이 끼움할 수도 있다. 특정 숫자 갈래 만들기, 모양새 옮기기, 자료 부풀리기 같은 쓰임새에 쓸모 있다.
...
조건 짓는 장치는 곧바르다. 곧 하나만 뜨거운 이름표를 인코더와 디코더의 들임에 잇는다. 인코더에서는 그물이 갈래마다 다른 부호화 분포를 배우게 돕는다. 디코더에서는 뽑은 숨은 부호로 옳은 숫자 갈래를 만드는 데 필요한 갈래 정체를 준다. 그래서 숨은 공간이 갈래 사이 흔들림(어느 숫자인가)이 아니라 갈래 안의 흔들림(모양새, 기울기, 굵기)을 잡는다.
...
결정적인 설계 세부 하나는 인코더가 익히는 동안 이름표를 받는다는 것이다. 그래서 인코더가 이름표로 자료의 일부를 설명하며 "속임수"를 써서 앎이 덜 담긴 숨은 나타냄을 배울 수 있다. 어떤 얼개는 디코더만 조건 지어 숨은 공간이 모든 흔들림을 잡게 하지만 그러면 익히기가 더 어렵다.
...
MNIST에 조건부 변분 오토인코더를 익히고 숫자마다(0~9) 표본 10개를 만들어라. 가로줄마다 한 숫자 갈래가 되도록 10x10 격자로 보여라. 만든 표본이 목표 갈래와 맞는가?
...
??? success "연습문제 1 풀이"
    ...
    plt.show()
    ```
    만든 표본이 목표 갈래와 또렷이 맞아야 하며, 이는 디코더가 조건 이름표로 내놓는 숫자를 다스리는 법을 배웠음을 보인다.
...
디코더만 조건 짓도록 조건부 변분 오토인코더를 고쳐라(인코더에서 이름표 잇기를 없앤다). 숨은 공간의 배치와 표본 품질을 여느 조건부 변분 오토인코더와 견주어라.

</div>

??? success "연습문제 3 풀이"
    인코더에 조건이 없으면 숨은 공간이 갈래 정체와 모양새 흔들림을 모두 잡아야 한다. 그러면 흔히 갈래마다 숨은 공간의 뚜렷한 자리를 차지하고(조건 없는 변분 오토인코더와 비슷하다) 이름표에 조건 지어진 디코더가 또렷한 갈래 신호로 내놓기를 다듬는다. 디코더가 앎이 덜 담긴 숨은 부호를 받으므로 표본 품질이 조금 낮을 수 있지만 숨은 공간은 더 짜임새 있고 풀이하기 쉬워진다.
...
조건부 변분 오토인코더는 부류 정보를 어디에 넣는가? 왜 두 곳인가?
...
??? success "연습문제 4 풀이"
    ...
    ```python
    mu, logvar = encoder(torch.cat([x, y], dim=1))   # 인코더
    xhat       = decoder(torch.cat([z, y], dim=1))   # 디코더
    ```

    디코더에 넣는 까닭은 분명하다. 뽑을 때 어떤 숫자를 만들지 지정하려면 디코더가 그 값을
    받아야 한다.

    인코더에도 넣는 까닭이 더 재미있다. 인코더가 $y$를 알면 코드에 부류를 담을 **까닭이
...
??? success "연습문제 5 풀이"
    ...
    평균 90.1%다. 조건 없는 변분 오토인코더의 57.4%에 견주면 크게 나아진 것이지만,
...
조건부 변분 오토인코더에서 코드 $z$와 부류 $y$가 정말 분리되었는지 어떻게 확인하겠는가?
...
??? success "연습문제 10 풀이"
    ...
    **표지가 필요하다.** 가장 큰 제약이다. 오토인코더와 변분 오토인코더의 매력 하나가
