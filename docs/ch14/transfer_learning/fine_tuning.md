...
미세 조정은 사전 학습된 모델을 특정 아래쪽 과제에 맞춘다. 핵심 어려움은 값진 사전 학습 지식을 지키면서 새 데이터에 맞추는 균형을 잡는 것이다. 이 문서는 일반적인 미세 조정 방법을 다룬다. LoRA나 어댑터 같은 매개변수를 아끼는 방법은 22.7절 효율적인 대형 언어 모델 미세 조정을 보라.
...
사전 학습 가중치를 얼리는, 매개변수를 아끼는 방법(LoRA, 어댑터)을 쓴다. 22.7절을 보라.
...
매개변수를 아끼는 방법(LoRA, QLoRA, 접두 조정, 어댑터)은 22.7절 효율적인 대형 언어 모델 미세 조정을 보라.
