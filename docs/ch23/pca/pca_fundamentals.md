...
선형 자기 부호기와 주성분 분석의 관계를 한 문장으로 적고, 무엇이 다른지 밝혀라.

</div>

??? success "연습문제 10 풀이"
    은닉층 하나에 활성화가 없는 자기 부호기를 제곱 어긋남으로 익히면 **주성분 분석과
    같은 부분 공간**을 찾는다.

    다른 것은 **그 안의 기저**다. 주성분 분석은 직교하고 흩어짐 순으로 정렬된 기저를
    주는데, 자기 부호기는 같은 공간을 펼치는 아무 기저나 준다.
    ...
    실제로 재어 보면 이 성질이 또렷하게 확인된다. 선형 자기 부호기를 익히면 어긋남이
    주성분 분석의 값에 **위에서** 다가가고, 두 부분 공간 사이의 주각이 14.51도에서
    0.67도로 줄어든다([24장](../../ch24/architecture/04_ae_principal_manifold.md)).

    그래서 선형이면 자기 부호기를 쓸 까닭이 거의 없다. 쪼개기 한 번으로 정확한 답이
    나오는데 반복 최적화로 어림할 이유가 없고, 얻는 기저는 오히려 나쁘다.

    자기 부호기의 값어치는 활성화를 넣는 순간 생긴다. 그때는 주성분 분석이 절대 찾을 수
    없는 굽은 다양체를 따라갈 수 있다.
