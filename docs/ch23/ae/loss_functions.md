...
MNIST 화소가 $[0,1]$ 사이 값일 때 평균 제곱 어긋남과 두 값 엇갈린 엔트로피 가운데 무엇을 써야 하는가? 실제로 둘 다 익혀 시험 MSE를 견주어라.

</div>

??? success "연습문제 1 풀이"
    병목 16, 15 에포크로 익히고 시험 자료의 MSE를 재면 이렇다.

    | 익힐 때 쓴 손실 | BCE | MSE |
    |---|---|---|
    | 시험 MSE | **0.00926** | 0.00958 |
...
??? success "연습문제 2 풀이"
    ...
    비가 곧 $\beta$ 노릇을 하기 때문이다([24장](../../ch24/index.md)).
...
??? success "연습문제 3 풀이"
    ...
    [맞겨루기 만들개](../../ch27/index.md)가 손실 자체를 버리고 판별기로 바꾼 까닭
    가운데 하나다.
...
성김 벌점에 쓰는 KL은 [24장](../../ch24/index.md)의 KL 항과 같은 것인가?
...
??? success "연습문제 10 풀이"
    ...
    [변분 자기 부호기](../../ch24/index.md)의 KL 항도 이 목록에 들어간다. 그것은
