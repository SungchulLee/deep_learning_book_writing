...
변환기 얼개는 붙박이 길이의 맥락 창을 다룬다. 대화가 이 창을 넘어서면 앞선 차례를 잘라 내야 하고 그러면 **맥락을 잃는다**. 창 안에서도 모델에는 중요한 앎과 곁가지 세부를 가리는 드러난 얼개가 없다. 곧 어텐션 얼개가 모든 토막을 똑같이 다룬다.

여러 차례 대화에서는 이것이 다음으로 나타난다:

- **주제 흘러감.** 새 차례가 앞선 맥락을 실효 어텐션 범위 밖으로 밀어내면서 모델의 초점이 본디 주제에서 차츰 벗어날 수 있다.
- **사실 잊기.** 긴 대화의 앞부분에서 말한 구체적인 세부가 "잊힐" 수 있다. 모델의 힘이 모자라서가 아니라 어텐션 무게가 수많은 토막에 걸쳐 옅어지기 때문이다.
- **시킴 흘려듣기.** 대화가 길어질수록 체계 수준의 시킴에 가는 어텐션이 줄어들 수 있다.
...
여기서 $\mathbf{b}_t$은 쓰는 이의 뜻, 언급된 개체, 주고받기가 나아간 정도에 대한 얼개의 이해를 담는다. 이제의 ChatGPT 모델은 DST을 드러내 놓고 쓰지 않는다. 앞뒤 흐름의 이해는 주고받기 전체에 대한 어텐션으로 넌지시 다룬다.
...
- **찾아 붙이기(RAG):** 찾아온 글월에 만들어 내기를 뿌리내리게 하면 헛것 지어내기가 줄지만 없어지지는 않는다. 21.5절 찾아 붙여 만들어 내기 개요를 보라.
...
!!! note "엇갈린 참고"
    공정함 잣대(인구 특성 균형, 고른 승산, 어림 균형, 눈금 맞추기)의 엄밀한 정의와 수학 세우기는 [41장: 치우침과 공정함](../../ch41/index.md)을 보라.
...
이 마당은 아리송함과 맥락 다루기, 헛것 지어내기, 치우침과 공정함, 윤리의 걱정거리를 차례로 짚었다.
