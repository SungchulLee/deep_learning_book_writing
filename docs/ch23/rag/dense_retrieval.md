...
??? success "연습문제 4 풀이"
    처음 찾기(BM25나 빽빽한 찾기)는 빠르지만 어림이며 글월 $k$개의 후보 모음을 돌려준다. 그다음 **다시 매개**(대개 엇갈린 인코더)가 (물음, 글월) 짝마다 함께 점수를 매겨, 두 인코더 찾기가 놓치는 결이 고운 주고받음을 담아낸다. 엇갈린 인코더는 더 정확하지만 느리므로(벡터 한 번 찾기 대신 앞먹임 $O(k)$번) 상위 $k$개 후보에만 쓴다. 다시 매긴 상위 $n$개 글월($n < k$)을 만들개에 넘긴다. 이 두 단계 방식은 처음 찾기의 빠르기와 엇갈린 어텐션 점수 매기기의 정확도를 아우른다.
