...
??? success "연습문제 1 풀이"
    | 모델 | 얼개 | 미리 익히기 목표 | 방향 |
    |-------|-------------|----------------------|----------------|
    | **GPT** | 디코더만의 변환기 | 인과 말 나타내기(다음 토막 어림) | 왼쪽에서 오른쪽만 |
    | **BERT** | 인코더만의 변환기 | 가린 말 나타내기 + 다음 월 어림 | 두 방향 |
    | **T5** | 인코더-디코더 변환기 | 구간 망가뜨리기(잡음 없애기) | 인코더: 두 방향, 디코더: 왼쪽에서 오른쪽 |
