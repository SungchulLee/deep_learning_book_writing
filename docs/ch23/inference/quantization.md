...
양자화는 무게와 활성화를 정밀도가 낮은 자료 갈래로 나타내어 모델 크기를 줄이고 미룸을 빠르게 한다. 큰 말 모델에서는 일반 하드웨어에 펼치고 내놓기 값을 줄이는 데 결정적이다.
...
AWQ는 활성화의 크기로 두드러진 무게를 가려내어 지킨다:

$$
\text{saliency}_j = \|X_j\|_2
$$

딸린 활성화의 크기가 큰 무게는 양자화 앞에서 키우고 미룸 때 되돌린다.
...
def compute_awq_scales(
    ...
    잣수 = 활성화 크기^alpha. 여기서 alpha가 무게 양자화 어긋남과
    활성화 양자화 어긋남을 저울질한다.
    """
    # 활성화의 크기를 셈한다
    ...
    # 가장 좋은 잣수(무게 어긋남과 활성화 어긋남을 저울질한다)
    # 활성화가 클수록 더 지킨다
