...
- **배치 크기:** 익히는 동안 차츰 늘린다(예컨대 GPT-3는 배치마다 토막 3만 2천에서 320만으로)
...
!!! info "셈의 규모"
    GPT-3(매개변수 1750억)는 셈이 약 3,640 페타플롭/초·일 들었다. 지금 하드웨어 값으로 치면 익히기 값이 수백만 달러에 이르며, 이것이 몇몇 조직만 앞선 말 모델을 맨바닥에서 익힐 수 있는 핵심 까닭이다. 셈에 가장 알맞은 익히기 분석은 [21.3절: 잣수 맞추기 법칙](../scaling/scaling_overview.md)을 보라.
...
!!! note "엇갈린 참고"
    공정 잣대(인구 균등, 고른 승산, 눈금 맞음)의 엄밀한 정의와 치우침 없애기 재주를 아우른 기계 배움의 치우침과 공정에 대한 두루 다룸은 [41장: 치우침과 공정](../../ch41/index.md)을 보라.
...
??? success "연습문제 4 풀이"
    헌법 인공지능(Anthropic, 2022)은 사람 되먹임을 많이 쓰지 않고 원칙 배치("헌법")으로 모델의 몸가짐을 이끈다. 과정은 이렇다: (1) 해로운 시킴말에 응답을 만들고, (2) 모델에게 헌법에 비추어 제 응답을 따지고 고치게 하고, (3) 고친 응답으로 곱게 다듬는다(RLAIF, 곧 인공지능 되먹임에서 배우는 북돋움 배움). 이는 안전 익히기에서 사람 붙임말꾼에 기대는 것을 줄이고 규모를 키우기 쉬우며 결 맞추기 잣대를 또렷하고 살펴볼 수 있게 만든다. RLHF가 두루 쓰이는 품질 선호를 다루는 동안 헌법 인공지능은 안전 걱정을 짜임새 있게 다루어 서로 채워 준다.

## 정리하며

이 마당은 1마디: 미리 익히기, 2마디: 결 맞추기, 익히는 동안의 치우침 누그러뜨리기를 차례로 짚었다.
