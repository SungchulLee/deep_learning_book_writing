...
| **배치 잣대 잡기** | 흔히 붙박이. 잣대/옮김은 배울 수 있다 |
| **박아 넣기** | 박아 넣기마다 또는 묶인 흩어짐 |
| **어텐션** | 열쇠-물음-값에 다른 잣대가 있어야 할 수 있다 |
...
이 배치는 베이즈 신경 그물에 쓰는 여러 앞선 분포를 짜 놓았다.
...
def demo_hierarchical_prior():
    ...
    print(f"{'α':>5} {'가장자리 자유도':>12} {'표본 표준편차':>12} {'뾰족함':>12}")
    print("-" * 45)
...
def demo_prior_predictive():
    ...
    print(f"{'앞선 분포':<25} {'날임 평균':>12} {'날임 표준편차':>12} {'가장 큰 |y|':>12}")
    print("-" * 65)
...
앞선 분포 간추림:
  Gaussian (σ=1)           : mean=+0.002, std=1.003, |w|>2: 0.048
  Gaussian (σ=0.5)         : mean=-0.004, std=0.505, |w|>2: 0.000
  Laplace (scale=1)        : mean=+0.013, std=1.402, |w|>2: 0.136
  Student-t (ν=3)          : mean=-0.017, std=1.769, |w|>2: 0.141
  Student-t (ν=10)         : mean=+0.016, std=1.129, |w|>2: 0.077
...
성김 견주기(0 언저리의 몫):
앞선 분포                  |w|<0.01    |w|<0.1      |w|>2
-------------------------------------------------------
Gaussian                  0.008      0.080      0.046
Laplace                   0.010      0.098      0.132
Spike-and-Slab            0.538      0.812      0.010
Horseshoe                 0.031      0.172      0.232
...
흩어짐에 Inv-Gamma(α, β=1)을 얹은 층진 가우스:
    α     가장자리 자유도    표본 표준편차          뾰족함
---------------------------------------------
  1.0          2.0        3.160      262.558
  2.0          4.0        1.002        8.931
  5.0         10.0        0.504        1.339

... (41 lines omitted)

벌 λ = 1/(2σ²)의 L2 정칙화 MLE와 같다

        σ²          σ     λ = 1/(2σ²)                        풀이
-----------------------------------------------------------------
      0.01      0.100         50.0000                  아주 센 정칙화
      0.10      0.316          5.0000                     센 정칙화
      0.50      0.707          1.0000                   가운데 정칙화
      1.00      1.000          0.5000                   가운데 정칙화
     10.00      3.162          0.0500                    여린 정칙화
    100.00     10.000          0.0050                 아주 여린 정칙화
```
...
| 이야기 | 장 | 이어짐 |
|-------|---------|------------|
| 뒷분포 미루어 봄 | 43장: 뒷분포 미루어 봄 | 앞선 분포가 뒷분포의 꼴에 걸린다 |
| 아리송함 | 43장: 아리송함 | 앞선 분포가 앎의 아리송함에 걸린다 |
| MC 드롭아웃 | 43장: MC 드롭아웃 | 앞선 분포를 넌지시 매긴다 |
| 변이 베이즈 신경 그물 | 43장: 변이 베이즈 신경 그물 | KL 갈림 속의 앞선 분포 |
| 정칙화 | 7장: 정칙화 | MAP ↔ 벌을 준 MLE |
