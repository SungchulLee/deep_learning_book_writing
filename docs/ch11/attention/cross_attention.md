...
교차 어텐션은 순차열 대 순차열 구조에서 인코더와 디코더를 잇는 장치이다. 질의와 열쇠와 값이 모두 같은 순차열에서 나오는 자기 어텐션과 달리, 교차 어텐션은 질의를 한 순차열(대개 디코더)에서, 열쇠와 값을 다른 순차열(대개 인코더)에서 얻는다. 덕분에 디코더가 생성 걸음마다 부호화된 입력에서 정보를 골라 꺼낼 수 있어, 기계 번역이나 요약, 이미지 설명 달기 같은 과제에 없어서는 안 된다.
...
인코더-디코더 구조가 통하게 만드는 것이 바로 교차 어텐션이다. 표준 트랜스포머 디코더에서는 층마다 부분층 세 개를 차례로 적용한다. 디코더 자신의 숨은 상태에 대한 가림막 자기 어텐션, 인코더 출력에 대한 교차 어텐션, 자리별 순방향 신경망이다. 자기 어텐션은 디코더가 출력 순차열 안의 의존을 다루게 하고, 교차 어텐션은 출력 토큰마다 입력의 쓸모 있는 부분에 조건을 걸게 한다.
...
`EncoderDecoderAttention` 블록은 이 부품들이 어떻게 완전한 디코더 층을 이루는지 보인다. 부분층마다 두른 잔차 연결과 층 정규화가 학습을 안정시키고 이런 블록을 깊이 쌓아도 기울기가 흐르게 해 준다. 실제로 트랜스포머 모델은 보통 이런 디코더 층을 6개에서 12개 쓰며, 층마다 같은 인코더 출력에 주목하되 학습된 사영은 저마다 다르다. 가림막 장치도 매우 중요하다. 자기 어텐션 가림막은 인과성을 지키고(디코더가 미래 자리에 주목하지 못하게 한다), 교차 어텐션 가림막은 원본의 덧댐을 처리한다.
...
인코더 출력의 모양이 $(B, 10, 256)$이고 디코더 질의의 모양이 $(B, 1, 256)$이며 머리가 8개인 교차 어텐션 층을 생각해 보자. `MultiHeadCrossAttention` 모듈의 학습 가능한 매개변수 총수를 (편향을 넣어) 계산하라.
...
`EncoderDecoderAttention`의 디코더 자기 어텐션이 `decoder_input`을 질의와 key_value에 모두 넘겨 같은 `MultiHeadCrossAttention` 클래스를 쓰는 까닭을 설명하라. 교차 어텐션은 어떤 조건에서 자기 어텐션이 되는가?

</div>

??? success "연습문제 2 풀이"
    질의의 출처와 열쇠·값의 출처가 같은 순차열이면 교차 어텐션이 자기 어텐션이 된다. 코드에서 `self.self_attention(decoder_input, decoder_input, self_attn_mask)`은 디코더의 숨은 상태를 질의와 key_value 인자에 모두 넘긴다. 곧 질의와 열쇠와 값이 모두 같은 텐서에서 저마다의 사영 행렬을 거쳐 나온다. 수식은 표준 다중 머리 자기 어텐션과 똑같다. 디코더의 자리마다 (가림막이 허락하는 한) 다른 모든 자리에 주목한다. 전용 자기 어텐션 모듈과의 차이는 개념뿐이다. 이 클래스는 질의와 key_value가 서로 다른 곳에서 오는 일반적인 경우를 위해 설계되었지만 자기 어텐션도 그 특수한 경우로 자연스럽게 다룬다.
...
모든 머리에 걸쳐 평균한 교차 어텐션 가중치를 돌려주고 그것으로 디코더 자리와 인코더 자리 사이의 "정렬 점수"를 계산하도록 `EncoderDecoderAttention`을 고쳐라. 예제 입력에 대해 이 정렬 행렬을 matplotlib으로 그려 보라.
...
??? success "연습문제 3 풀이"
    ...
    그려 낸 열지도는 디코더 자리마다 인코더 자리에 얼마나 주목하는지 보여 준다. (학습하지 않은) 무작위 가중치에서는 무늬가 거의 고르지만, 번역 과제로 학습한 뒤에는 대각선 비슷한 무늬가 나타나며 원본 언어와 표적 언어 사이의 단조로운 정렬을 드러낸다.

## 정리하며

**다룬 것** — 교차 어텐션

인코더-디코더 구조가 통하게 만드는 것이 바로 교차 어텐션이다.

핵심 클래스는 `CrossAttention`, `MultiHeadCrossAttention`, `EncoderDecoderAttention`이며 앞의 연습문제 3개로 직접 확인할 수 있다.
