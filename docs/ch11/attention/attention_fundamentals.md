...
어텐션이 나오기 전에 인코더-디코더 모델은 입력 순차열 전체를 길이가 고정된 문맥 벡터 하나로 눌러 담았다.
...
Bahdanau 등(2014)은 디코더가 걸음마다 서로 다른 인코더 자리에 "주목"하게 하자고 제안했다.

$$\mathbf{c}_t = \sum_{i=1}^{T} \alpha_{ti} \mathbf{h}_i$$

여기서 $\alpha_{ti}$은 시각 $t$의 출력을 만들 때 디코더가 인코더 상태 $\mathbf{h}_i$에 얼마나 집중해야 하는지를 나타낸다. 어텐션 가중치는 $\sum_i \alpha_{ti} = 1$과 $\alpha_{ti} \geq 0$을 만족한다.
...
|---------|----------|-------------|----------|
| **자기 어텐션** | 같은 순차열 | 같은 순차열 | 내부 문맥 (BERT 인코더) |
| **교차 어텐션** | 디코더 | 인코더 | 인코더와 디코더 잇기 (번역) |
...
질의는 한 순차열(디코더)에서, 열쇠와 값은 다른 순차열(인코더)에서 온다.
...
이것이 seq2seq 모델에서 인코더와 디코더를 잇는다. 교차 어텐션의 무늬는 어텐션의 무늬 절에서 다룬다.
...
2. **자기 어텐션**: 어텐션이 인코더와 디코더를 잇는다면 같은 순차열 안의 자리끼리는 왜 안 되겠는가?
3. **병렬화**: 순환을 없애면 대규모 병렬 처리가 가능해진다
4. **위치 부호화**: 순환이 없으면 위치를 따로 담아 주어야 한다

| 연도 | 발전 | 영향 |
|------|-------------|--------|
| 2014 | LSTM을 쓰는 seq2seq | 인코더-디코더 틀 |
