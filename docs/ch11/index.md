...
길이가 정해지지 않은 입력 순차열을 역시 길이가 정해지지 않은 출력 순차열로 바꾸는 인코더-디코더 구조.

- Seq2Seq 개요 — 순차열 변환 문제와 인코더-디코더 틀
- Seq2Seq 실습 꾸러미 — 여러 인코더 구조와 어텐션 장치를 갖춘 완전한 구현
- 인코더-디코더 틀 — 입력 순차열을 출력 순차열로 보내는 근본적인 틀
- 문맥 벡터 — 인코더와 디코더를 잇는 차원이 고정된 다리, 그리고 그 정보 병목
...
이 마당은 1 낱말 임베딩, 2 순환 신경망, 3 LSTM과 GRU, 4 순차열 대 순차열 모델을 차례로 짚었다.
