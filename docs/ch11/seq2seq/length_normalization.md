# 길이 정규화
길이 정규화는 빔 탐색의 체계적인 치우침을 바로잡는다. 날 로그 확률 점수는 본디 짧은 순차열을 편든다. 이를 바로잡지 않으면 디코더가 잘린 출력을 즐겨 만들어 생성의 품질을 떨어뜨린다. 이 절은 그 문제를 자세히 살피고, 표준적인 정규화 전략을 보이며, 어텐션 기반 모델에서 덮개 벌점과 어떻게 어우러지는지 알아본다.

---

## 1. 짧은 순차열 쪽으로 치우치는 문제
...
그래서 짧은 출력이 체계적으로 유리해진다. 디코더는 쓸모 있는 내용을 계속 만드는 것보다 순차열 끝 토큰을 일찍 내는 편이 순위가 높다는 것을 배우고, 그 결과 잘리고 불완전한 출력을 낸다.
...
덮개 벌점과 길이 정규화는 서로 보완하는 문제를 다룬다. 길이 정규화는 디코더가 너무 일찍 멈추는 것을 막고, 덮개 벌점은 입력의 일부를 무시하는 것을 막는다. 둘이 함께 완전하고 잘 짜인 출력을 이끌어 낸다.
