...
**탄성 가중치 다지기(EWC)**는 Kirkpatrick 외(2017)가 들여온, 이어 배우기의 벌주기 기반 방법 가운데 가장 큰 영향을 끼친 것에 든다. EWC의 핵심 통찰은 앞서 배운 과제에서 신경망의 매개변수가 다 똑같이 중요하지는 않다는 것이다. 어떤 매개변수는 결정적이고 어떤 것은 마음대로 바꾸어도 된다. EWC은 **피셔 정보 행렬**로 중요한 매개변수를 짚어내어 지킨다.
...
EWC은 배움을 보는 베이즈의 눈에서 나온다. 과제 A를 배운 뒤 매개변수 위의 뒤확률을 생각해 보자.
...
EWC은 뒤확률 $p(\theta | \mathcal{D}_A)$을 가우스 분포로 어림한다.
...
매개변수가 $d$개이면 온전한 피셔 행렬은 $d \times d$이라 셈으로 다룰 수 없다. EWC은 대각 어림을 쓴다.
...
과제마다 피셔 행렬을 따로 담아 두는 대신 온라인 EWC은 흐르는 어림값 하나를 지닌다.
...
!!! warning "규모 확장성"
    과제 차례가 아주 길면 기억을 그대로 유지하도록 온라인 EWC을 생각해 보라.
...
??? success "연습문제 2 풀이"
    피셔 행렬의 대각 성분 $F_i = \mathbb{E}[(\frac{\partial \log p(y|x,\theta)}{\partial \theta_i})^2]$은 가중치 $\theta_i$이 바뀔 때 로그 가능도가 얼마나 달라지는지를 잰다. $F_i$이 크면 그 가중치가 지금 과제의 예측에 중요하다는 뜻이다. EWC은 $F_i$이 큰 가중치가 바뀌면 더 세게 벌을 주어 중요한 앎을 지킨다.
