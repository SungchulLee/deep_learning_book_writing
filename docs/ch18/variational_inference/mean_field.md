...
다른 인수를 모두 붙박아 둔 채 $q_j$에 대해 ELBO를 가장 크게 하고자 한다:
...
CAVI는 다른 것을 붙박아 둔 채 변분 인수를 하나씩 되풀이해 새로 고친다:
...
**정리(CAVI의 단조로움)**: $q_j$을 새로 고쳐도 ELBO가 줄어들 수 없다.

*증명 밑그림*: 좌표를 새로 고칠 때마다 다른 것이 주어진 상태에서 그 인수의 전체 최적점을 찾는다. 인수마다 오목한 범함수를 가장 크게 하므로 ELBO는 커지거나 그대로일 수밖에 없다.
...
1. ELBO를 최적화하는 전략으로서 좌표 오르기 이해하기
...
평균장 인수 나눔 $q(\theta) = \prod_{j=1}^K q_j(\theta_j)$에서 CAVI는 다음을 되풀이한다:
...
**정리**: CAVI로 새로 고쳐도 ELBO가 줄어들 수 없다.

**증명**: 다른 것을 모두 붙박아 둔 채 인수 $q_j$을 새로 고친다고 하자. ELBO는 다음처럼 쓸 수 있다:
...
$q_j = \tilde{p}_j$일 때 KL 벌어짐이 가장 작아지므로(0이 되므로) CAVI의 새로 고침은 $\text{ELBO}_j$을 가장 크게 한다. 그러므로 새로 고칠 때마다 ELBO는 커지거나 그대로일 수밖에 없다.

### 그 자리 최적점으로의 모임

**따름정리**: CAVI는 ELBO의 그 자리 최적점으로 모인다.

ELBO는 위로 $\log p(\mathcal{D})$으로 묶인다. 단조로움과 함께 보면 ELBO 값의 늘어놓음은 모여야 한다. 모인 자리에서는 인수 하나를 새로 고쳐도 ELBO를 낫게 할 수 없으며, 이것이 좌표별 최적점의 정의이다.
