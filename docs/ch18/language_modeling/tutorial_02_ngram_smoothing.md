...
Linear interpolation takes a fundamentally different approach by combining models of different orders. Instead of adjusting counts, it computes a weighted average of the bigram and unigram probabilities: $P_{\text{interp}}(w_i \mid w_{i-1}) = \lambda_2 P_{\text{bigram}} + \lambda_1 P_{\text{unigram}}$. When the bigram model has zero probability for a particular word in a given context, the unigram component provides a fallback estimate. The interpolation weights $\lambda_1$ and $\lambda_2$ can be optimized on held-out data using the EM algorithm, making this approach more data-driven than fixed smoothing constants.
...
??? success "연습문제 1 풀이"
    ...
    (a) Seen word (count = 10): $P = (10 + 1) / (50 + 100) = 11/150 \approx 0.0733$
    
    부드럽게 하지 않으면 $P = 10/50 = 0.200$이다. 라플라스 부드럽게 하기가 이를 절반 넘게 줄였다.
    
    (b) Unseen word (count = 0): $P = (0 + 1) / (50 + 100) = 1/150 \approx 0.00667$
...
**연습문제 2.**
Prove that for linear interpolation with $\lambda_2 + \lambda_1 = 1$, the interpolated probability is always positive if $P_{\text{unigram}}(w) > 0$ for all words in the vocabulary, even when $P_{\text{bigram}}(w \mid c) = 0$.

??? success "연습문제 2 풀이"
    Given $P_{\text{interp}}(w \mid c) = \lambda_2 P_{\text{bigram}}(w \mid c) + \lambda_1 P_{\text{unigram}}(w)$ with $\lambda_1 > 0$ and $\lambda_2 > 0$:
    ...
    Since $\lambda_1 > 0$ and $P_{\text{unigram}}(w) > 0$ (given assumption), we have $P_{\text{interp}}(w \mid c) > 0$. $\square$
...
**연습문제 3.**
Implement a grid search function that finds the optimal $k$ value for Add-k smoothing by minimizing perplexity on a validation set, testing $k \in \{0.01, 0.05, 0.1, 0.25, 0.5, 1.0\}$.
