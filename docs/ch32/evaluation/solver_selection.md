# 신경 상미분 방정식의 솔버 고르기 안내

상미분 방정식 솔버를 어떻게 고르느냐가 신경 상미분 방정식의 성능을 여러 갈래에서 결정적으로 정한다. 풀이 정확도, 셈 비용, 기억 요구, 수치 안정성이 그것이다. 실무자는 드러난 방법(룽게-쿠타 변형), 은근한 방법, 맞추어 가는 얼거리에 걸친 여러 고르기 가운데 골라야 하며 저마다 정확도 보장과 셈 성질이 다르다. 솔버를 잘못 고르면 쓸데없는 셈 비용(정밀하지 않은 신경망에 지나치게 정확한 솔버)이나 나쁜 풀이(솔버 허용 오차가 너무 헐거움)로 이어진다.

어긋남 허용을 빡빡하게 다스릴 수 있는 여느 과학 셈과 달리 신경 상미분 방정식에는 남다른 어려움이 있다. 움직임을 본디 흐릿함이 있는 자료에서 배우고, 신경망이 상미분 방정식 얼개 가정을 만족하지 않을 수 있으며, 익히기에 뒷먹임 미분이 필요해 더 복잡해진다. 이 마디는 솔버 고르기에 대한 실제 안내를 주고 맞바꿈을 따지며 쓰임새 마당마다 권하는 바를 내놓는다.
...
- **미분할 수 있음**: 솔버의 기울기를 셈할 수 있는지(익히기에 결정적이다)
...
신경 상미분 방정식 솔버는 다음의 풀이를 어림한다:

$$\frac{dh}{dt} = f(h, t; \theta), \quad h(t_0) = h_0$$

구간 [t₀, t₁]에서. 솔버는 때 {t_n}에서 어림 {ĥ_n}을 내놓는다.
...
여기서 C은 문제의 조건 상태에 매인다. 솔버 차수가 높을수록 온 어긋남이 작다.
...
솔버 허용 오차 TOL이 정확도를 다스린다. 너무 빡빡하면 셈을 낭비하고 너무 헐거우면 결과가 나쁘다.
...
## 11. 솔버 견주기: 금융 때 차례 보기

**일**: 24시간 자취로 1시간 뒤 주가를 헤아린다.

| 솔버 | 평균 제곱 어긋남 | 시간 | 함수 매김 횟수 | 기억 |
|--------|-----|------|-----|--------|
...
!!! note "솔버 고르기 원리"
    기본 고르기: 허용 오차 1e-4이나 1e-5인 DOP853 맞추어 가는 솔버. 정확도, 효율, 튼튼함의 균형이 좋다. 특별한 요구가 있을 때만 벗어난다. 순수한 빠르기가 필요하면 RK4을, 뻣뻣함을 다루어야 하면 CVODE을, 최대 정확도가 필요하면 더 빡빡한 허용 오차나 은근한 방법을 쓴다.
...
$y(0) = 0$인 뻣뻣한 상미분 방정식 $dy/dt = -1000y + 3000 - 2000e^{-t}$에서 오일러, RK4, 도맨드-프린스 솔버를 견주어라. 솔버마다 안정에 필요한 걸음 크기를 구하라.

</div>

??? success "연습문제 1 풀이"
    닫힌 꼴 풀이는 $y(t) = 3 - 0.998e^{-1000t} - 2.002e^{-t}$이다. 오일러는 안정하려면 $h < 0.002$이어야 하고(고윳값 $-1000$이 $|1 + h\lambda| < 1$을 요구한다) 약 500걸음이 든다. RK4은 안정 자리가 더 넓지만 여전히 $h < 0.0028$이 필요해 약 360걸음이 든다. 맞추어 가는 도맨드-프린스는 $t=0$ 언저리의 빠른 과도 현상을 알아채 그곳에서는 아주 작은 걸음을, 지수가 사그라진 뒤에는 큰 걸음을 써서 함수 매김이 모두 50~100번쯤 든다. 뻣뻣한 계에는 은근한 솔버를 즐겨 쓴다. $\square$
...
때 차례 내다보기에 쓰는 신경 상미분 방정식의 가장 좋은 솔버를 고르는 견줌 시험을 짜라. 잣대, 솔버, 절차를 밝혀라.

</div>

??? success "연습문제 4 풀이"
    잣대: 한 바퀴마다 실제 시간, 함수 매김 횟수(NFE), 시험 손실, 익히는 동안 함수 매김 횟수의 자람. 솔버: 오일러, RK4, 도맨드-프린스, 은근한 애덤스. 절차: 같은 얼개를 솔버마다 여러 허용 오차(맞추어 감)나 걸음 크기(붙박이)로 익힌다. 10바퀴마다 잣대를 적고 정확도 대 셈량의 파레토 경계를 그린 뒤 바라는 맞바꿈에 가장 가까운 솔버를 고른다. 튼튼함을 위해 자료 묶음 3개에서 되풀이한다. $\square$

## 정리하며

이 마당은 핵심 개념、수학적 틀、드러난 룽게-쿠타 방법、맞추어 가는 솔버을 차례로 짚었다.
