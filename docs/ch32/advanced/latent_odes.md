...
??? success "연습문제 3 풀이"
    여느 변분 오토인코더는 앞먹임마다 비용이 $O(T \cdot d)$이다. 맞추어 가는 풀개를 쓴 숨은 상미분 방정식은 비용이 $O(N_{\text{eval}} \cdot d)$이며 $N_{\text{eval}}$은 움직임의 뻣뻣함에 매인다(보통 50~500). 풀개 비용이 관측 개수가 아니라 매끄러움에 매이므로 때 범위에 견주어 관측이 성길 때 숨은 상미분 방정식이 더 효율 좋다. 빽빽하게 뽑은 매끄러운 움직임에서는 변분 오토인코더가 더 쌀 수 있다. 딸림 방법은 뒤로 한 번 더 풀어 셈을 대략 두 배로 늘리지만 기억은 $O(d)$으로 지킨다. $\square$
