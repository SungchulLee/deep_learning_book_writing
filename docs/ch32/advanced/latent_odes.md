...
숨은 상미분 방정식(루바노바 외, 2019)은 고르지 않게 뽑힌 때 차례를 다루려 신경 상미분 방정식 틀과 변분 오토인코더를 합친다. 이어져 바뀌는 숨은 자취를 배워 아무 때 점에서나 헤아릴 수 있게 한다.
...
1. **인코더**: (때를 거꾸로 돌린) 되돌이 신경망이 관측을 처음 숨은 상태의 분포 $q(z_0 \mid x_{1:T})$으로 옮긴다
2. **상미분 방정식 움직임**: $dz/dt = f_\theta(z(t))$이 숨은 상태를 이어져 바꾼다
3. **솔버**: 물어본 때에 숨은 상태를 관측 공간으로 옮긴다
...
??? success "연습문제 2 풀이"
    상미분 방정식 솔버는 틈이 얼마이든 어느 두 때 점 사이에서도 $dz/dt = f_\theta(z, t)$을 이어져 적분한다. 빠진 값은 알아보기 신경망이 다루며, 이는 관측된 때에만 게이트 되돌이 단위로 고치고 상미분 방정식 움직임이 그 틈을 잇는다. 되돌이 신경망은 붙박이 때 걸음이 필요하므로 고르지 않은 뽑기에는 사이 메우기나 채우기가 있어야 한다. 숨은 상미분 방정식의 비용은 관측 밀도가 아니라 움직임의 복잡함에 매이므로 성기고 고르지 않은 자료에 더 효율 좋다. $\square$
...
고르게 놓인 관측 $T$개의 때 차례에서 숨은 상미분 방정식과 여느 변분 오토인코더를 익히는 셈 비용을 견주어라.

</div>

??? success "연습문제 3 풀이"
    여느 변분 오토인코더는 앞먹임마다 비용이 $O(T \cdot d)$이다. 맞추어 가는 솔버를 쓴 숨은 상미분 방정식은 비용이 $O(N_{\text{eval}} \cdot d)$이며 $N_{\text{eval}}$은 움직임의 뻣뻣함에 매인다(보통 50~500). 솔버 비용이 관측 개수가 아니라 매끄러움에 매이므로 때 범위에 견주어 관측이 성길 때 숨은 상미분 방정식이 더 효율 좋다. 빽빽하게 뽑은 매끄러운 움직임에서는 변분 오토인코더가 더 쌀 수 있다. 딸림 방법은 뒤로 한 번 더 풀어 셈을 대략 두 배로 늘리지만 기억은 $O(d)$으로 지킨다. $\square$
