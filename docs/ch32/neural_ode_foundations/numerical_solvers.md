...
신경 상미분 방정식은 $dz/dt = f_\theta(z(t), t)$을 수치로 적분해야 한다. 솔버를 어떻게 고르느냐가 정확도, 셈 비용, 기억 쓰임에 곧바로 영향을 준다.
...
가장 단순한 솔버이다. 일차 정확도(그 자리 어긋남 $O(h)$)이다. 빠르지만 $h$이 아주 작지 않으면 부정확하다.
...
### 도맨드-프린스(dopri5)
`torchdiffeq`의 기본 솔버이다. 걸음 크기를 다스리려 4차 어림을 박아 넣은 5차 방법이다:

$$h_{\text{new}} = h \cdot \left(\frac{\text{tol}}{\text{err}}\right)^{1/5}$$

맞추어 가는 솔버는 걸음 크기를 저절로 맞춘다. 움직임이 매끄러우면 걸음을 크게, 빠르게 바뀌면 작게 밟는다.
...
함수 매김 횟수는 신경 상미분 방정식의 핵심 효율 잣대이다. 매김마다 $f_\theta$을 온전히 앞먹임해야 한다. 맞추어 가는 솔버는 표본마다 매김 횟수가 달라 묶기와 GPU 쓰임을 어렵게 한다.

---

## 5. 뻣뻣한 움직임

$f_\theta$의 때 잣수가 크게 다르면(뻣뻣한 상미분 방정식) 드러난 솔버는 아주 작은 걸음이 필요하다. 은근한 솔버(뒤로 오일러, 후향 미분 공식)는 뻣뻣함을 다루지만 걸음마다 비선형 계를 풀어야 해 신경 상미분 방정식에는 비싸다.
...
$dy/dt = -y$, $y(0) = 1$에 오일러, 호인, RK4 솔버를 짜라. 걸음 크기 $h \in \{0.5, 0.1, 0.01\}$에서 $t = 1$의 모임 차수를 확인하라.
...
??? success "연습문제 2 풀이"
    오일러는 $y_{n+1} = (1 + h\lambda)y_n$을 준다. 안정하려면 $|1 + h\lambda| \leq 1$이어야 하며 이는 $h\lambda$ 평면에서 $-1$을 가운데로 하는 반지름 1의 원판이다. 실수 $\lambda < 0$에서는 $h < 2/|\lambda|$이다. 립시츠 상수가 큰 신경 상미분 방정식에서 오일러는 아주 작은 걸음이 필요하다. 맞추어 가는 솔버가 이를 저절로 다루고, 은근한 솔버는 왼쪽 반평면 전체를 덮는 안정 자리를 가진다. $\square$
...
신경 상미분 방정식 익히기에서 붙박이 걸음 솔버와 맞추어 가는 걸음 솔버의 맞바꿈을 따져라.
...
이 마당은 붙박이 걸음 솔버, 맞추어 가는 솔버, 솔버 고르기 지침, 함수 매김 횟수(NFE)을 차례로 짚었다.
