...
!!! warning "신경망에 대한 뜻"
    어긋남의 차수는 깊은 뜻을 지닌다. 걸음 크기 $\Delta t = 1$으로 층 $L$개를 쓴 남은 이음 신경망은 온 어긋남이 $O(1)$이다. 곧 띄엄띄엄하게 만든 어긋남이 깊이와 상관없이 *붙박여* 있다. 맞추어 가는 솔버를 쓴 신경 상미분 방정식은 걸음을 더 밟아 어긋남을 얼마든지 작게 할 수 있다.
...
$\lambda$이 크고 음수이면(뻣뻣한 계) 아주 작은 때 걸음이 필요하다. 그래서 맞추어 가는 솔버가 꼭 필요하다.
...
실제 쓰는 신경 상미분 방정식 짜기는 `dopri5`(도맨드-프린스) 같은 **맞추어 가는 솔버**를 쓰며 이는:
...
|--------|--------|------------|
| **깊이** | 붙박임(미리 고른다) | 맞추어 감(솔버가 정한다) |
| **매개변수** | 무게 배치 $L$개 | 움직임 함수 하나 |
| **기억** | 층 $L$개에 $O(L)$ | 딸림 방법으로 $O(1)$ |
| **셈** | 들임마다 붙박임 | 들임마다 맞추어 감 |
| **되돌릴 수 있음** | 보장되지 않음 | 보장됨(뒤 상미분 방정식을 푼다) |

!!! tip "맞추어 가는 셈"
    이어진 적기는 **맞추어 가는 셈**을 가능하게 한다. 상미분 방정식 솔버는 움직임이 복잡한 곳에서 걸음을 더 밟고 단순한 곳에서 덜 밟는다. 이는 거래자가 예사롭지 않은 시장 상황을 살피는 데 시간을 더 쓰고 늘 있는 값 움직임에는 덜 쓰는 것과 비슷하다.
...
상미분 방정식 솔버를 바닥부터 세워 보았으니 이제 실제로 쓸 신경 상미분 방정식 짜기에는 `torchdiffeq` 꾸러미를 쓴다.
...
상미분 방정식 솔버의 허용 오차가 모델의 움직임에 곧바로 영향을 준다:
...
이는 더 매끄러운 바꿈을 이끌며 이어진 고르게 맞추는 흐름(26.1절)에 쓰이는 대각합 셈하기와 가깝게 이어진다.
...
- **아주 느린 익히기**: 움직임에 규칙을 세우고 허용 오차를 헐겁게 하며 붙박이 걸음 솔버를 생각해 본다
...
```
======================================================================
ODE FUNDAMENTALS DEMONSTRATION
======================================================================

1. Comparing Numerical Methods on Exponential Growth
--------------------------------------------------
dt=0.5: Euler error=2.326556, RK4 error=5.09e-03
dt=0.1: Euler error=0.661556, RK4 error=9.43e-06
dt=0.01: Euler error=0.073038, RK4 error=1.07e-07

2. Phase Portrait: Damped Oscillator
--------------------------------------------------
Fixed point: [0. 0.]
Jacobian eigenvalues: [-0.20000005+1.9899749j -0.20000005-1.9899749j]
Classification: Asymptotically stable (attractor)
...
5. **`torchdiffeq`은 미분할 수 있는 상미분 방정식 솔버를 준다.** 여러 방법과 정할 수 있는 허용 오차를 갖춘다. 얼개 짜기에는 움직임 함수, 적분 시간, 솔버 고르기, 허용 오차 두기가 들어간다.
...
이 마당은 학습 목표, 미리 알아야 할 것, 상미분 방정식이란 무엇인가?, 고전 상미분 방정식 보기를 차례로 짚었다.
