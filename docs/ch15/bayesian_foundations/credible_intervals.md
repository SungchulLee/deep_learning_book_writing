...
봉우리가 여럿인 뒤확률에서는 구간 하나로 알리는 것이 오해를 낳으니(문제 1) 뒤확률 전체를 보여야 한다. 점 어림값이 필요하면 「[최대 뒤확률 어림](map_estimation.md)」을, 뒤확률을 표본으로 얻어야 하면 「[깁스 표집](../../ch17/mcmc/gibbs_sampling.md)」을 보라.
