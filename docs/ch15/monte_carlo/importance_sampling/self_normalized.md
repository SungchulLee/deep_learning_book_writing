...
For a comprehensive treatment of ESS including derivation, diagnostics, and practical guidelines, see the dedicated [Effective Sample Size](ess.md) section.
...
For a systematic treatment of proposal selection strategies, see [Proposal Distribution Design](proposal_design.md).
...
### Bayesian Parameter Estimation for Asset Returns

SNIS is particularly natural for Bayesian estimation problems in finance, where the posterior over model parameters is rarely available in closed form. A common setting is estimating the parameters of a stochastic volatility model or a regime-switching model where the likelihood is tractable but the normalizing constant is not.

```python
def bayesian_return_model_snis(returns, proposal_dist, n_samples=10000):
    """
    SNIS for Bayesian estimation of return distribution parameters.
    
    Model: r_t ~ N(μ, σ²)
    Prior: μ ~ N(0, 0.1²), log σ ~ N(log(0.02), 0.5²)
    
    Parameters
    ----------
    returns : torch.Tensor
        Observed daily log returns
    proposal_dist : callable
        Joint proposal over (μ, log_σ)
    n_samples : int
    ...
    def unnormalized_log_posterior(params):
        """params: (n_samples, 2) with columns [μ, log_σ]"""
        mu = params[:, 0]
        log_sigma = params[:, 1]
        sigma = torch.exp(log_sigma)
        
        # Log-likelihood: Σ log N(r_t; μ, σ²)
    ...
    # Compute SNIS weights
    log_gamma = unnormalized_log_posterior(samples)
    log_q = proposal_dist.log_prob(samples).sum(dim=-1)  # Independent components
...
SNIS naturally provides an estimate of the normalizing constant $Z = p(y)$, which is the marginal likelihood used for Bayesian model comparison. This is valuable for comparing competing factor models or volatility specifications:

$$
\hat{Z} = \frac{1}{n} \sum_{i=1}^n \tilde{w}_i = \frac{1}{n} \sum_{i=1}^n \frac{\gamma(\theta_i)}{q(\theta_i)}
$$

The log Bayes factor between models $\mathcal{M}_1$ and $\mathcal{M}_2$ is then:
...
### Exercise 5: Marginal Likelihood Estimation
Estimate the marginal likelihood for a Bayesian linear regression model using SNIS with the prior as proposal. Compare the IS estimate to the analytical result available for the conjugate Normal-Inverse-Gamma model. How does ESS relate to the accuracy of the $\hat{Z}$ estimate?
