...
ABC-SMC는 어림 베이즈 셈하기와 잇단 몬테카를로 방법을 합쳐, 너그러움을 알아서 고르고 복잡한 뒤확률에서 효율적으로 표집하게 한다. 이 마당은 그 알고리즘과 여러 판, 그리고 실전 구현 길잡이를 보인다.
...
ABC-SMC는 다음을 겨냥하는 알갱이 무리의 늘어놓음을 만든다:
...
1. **구현.** 정규 모형에 ABC-SMC를 구현하여라. 물리치기 및 MCMC와 효율을 견주어라.
