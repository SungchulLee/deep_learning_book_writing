...
??? success "연습문제 4 풀이"
    다음일 때 최대 뒤확률이 낫다. (1) 뒤확률이 거의 가우스일 때(최대 뒤확률에 라플라스 어림을 곁들이면 넉넉하다), (2) 셈 자원이 빠듯할 때(MCMC이 필요 없다), (3) 점 예측만 필요할 때(불확실성을 나타낼 필요가 없다), (4) 모형이 아주 클 때(깊은 망에서는 온전한 베이즈 추론을 다룰 수 없다).
