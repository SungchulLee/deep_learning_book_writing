...
**1. 점근 효율성**: $n \to \infty$이면 AIC은 후보 모형 가운데 평균제곱 예측 오차를 가장 작게 하는 모형을 고른다.

**2. 한결같지 않음**: $n \to \infty$이어도 AIC은 참 모형으로 모이지 않는다(지나치게 복잡한 모형을 고르는 성향이 있다).

**3. 맞먹는 모형**: 가장 좋은 모형에서 AIC이 2 안쪽인 모형은 뚜렷한 뒷받침을 받는다.
...
**1. 한결같음**: $n \to \infty$이면 BIC은 (후보 가운데 있다면) 확률 1로 참 모형을 고른다.

**2. 더 센 벌**: $n \geq 8$이면 BIC이 AIC보다 복잡도에 더 세게 벌을 준다($\log n > 2$).

**3. 앞확률에 기댐**: BIC은 특정한 (단위 정보) 앞확률을 놓는다.
...
**다음일 때 BIC이 더 단순한 모형을 편든다.**

- 표본이 클 때($n > 8$)
- 참 모형이 후보 가운데 있을 때
- 목표가 참 모형을 짚어내는 것일 때

**다음일 때 AIC이 더 복잡한 모형을 편든다.**
...
3. **또렷한 풀이가 없다**: AIC이나 BIC과 달리 DIC을 뒷받침하는 점근 이론이 없다.
...
**2. 특이 모형에도 쓸 수 있다**: BIC이 무너지는 곳(이를테면 섞음 모형, 신경망)에서도 굴러간다.
...
**엇갈리는 점**: $n = e^2 \approx 7.4$일 때 AIC과 BIC의 벌이 같아진다.
...
| 주제 | 장 | 이음 |
|-------|---------|------------|
| 베이즈 인자 | 13장: 베이즈 인자 | BIC이 로그 베이즈 인자를 어림한다 |
| 모형 증거 | 13장: 모형 증거 | 정보 기준은 온전한 적분을 피한다 |
| 앞확률 고르기 | 13장: 바탕 | BIC은 단위 정보 앞확률을 놓는다 |
