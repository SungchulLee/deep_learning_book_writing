...
학습된 SimCLR 부호기가 주어졌을 때 상위 $k$ 검색 정확도를 셈하는 함수를 구현하라. 이 함수는 질의 그림 배치와 그림 데이터베이스를 부호화한 뒤, 질의마다 (부호기 특징 공간의 코사인 비슷함으로) 가장 가까운 이웃 $k$개를 찾아 그 가운데 같은 부류 이름표를 가진 것이 있는지 살펴야 한다.
