...
가린 자동 인코더(MAE)는 자기 지도 시각 학습의 큰 전환으로, 대조 목표에서 가린 이미지 모형화로 옮겨 간다. BERT 같은 가린 언어 모형에서 영감을 얻어 MAE는 그림 조각의 큰 몫(대개 75%)을 무작위로 가리고 빠진 화소를 되살리도록 비전 트랜스포머를 학습시킨다. 이 방식은 놀랍도록 간단하고 잘 커지며 계산이 효율적이다. 인코더가 보이는 조각만 처리하여 그림 전체를 처리할 때보다 계산이 대략 4배 준다.

## 1. 코드

```python
"""
MAE: 가린 자동 인코더는 잘 커지는 시각 학습기이다
그림의 자기 지도 학습을 위한 가린 자동 인코더 구현.
"""
...
class MAE(nn.Module):
    """
    비전 트랜스포머 등뼈를 쓰는 가린 자동 인코더

    인수:
        img_size: 입력 그림 크기
        patch_size: 조각 크기
        in_chans: 입력 통로의 수
        embed_dim: 인코더의 임베딩 차원
        depth: 인코더의 깊이
        num_heads: 어텐션 머리의 수
        decoder_embed_dim: 디코더의 임베딩 차원
        decoder_depth: 디코더의 깊이
        decoder_num_heads: 디코더의 어텐션 머리 수
...
MAE의 구조는 비대칭 인코더-디코더 설계이다. **인코더**는 보이는(가리지 않은) 조각에서만 도는 표준 비전 트랜스포머로, 가림 비율이 75%이면 조각이 $16 \times 16$인 $224 \times 224$ 그림에서 196개 가운데 49개만 처리한다는 뜻이다. 이것이 핵심 계산 통찰이다. 가린 조각을 무거운 인코더에 넣지 않아 MAE는 모든 조각을 처리할 때보다 학습이 약 $3$배 빨라지고 기억도 크게 아낀다. 보이는 조각은 인코더에 들어가기 전에 위치 임베딩을 받으므로 공간 정보가 지켜진다.

**디코더**는 일부러 가볍게 두고(이를테면 인코더의 12개에 견주어 블록 8개), 토큰 전체, 곧 부호화된 보이는 조각에 가린 자리의 자리 지킴이 노릇을 하는 학습되는 가림 토큰을 더한 것에서 돈다. 디코더가 할 일은 가린 조각의 화소 값을 되살리는 것이며, 가림 토큰에 제 공간 위치를 알려 주려고 제 나름의 위치 임베딩 배치를 쓴다. 사전 학습 뒤에 디코더는 버리고 인코더만 아래쪽 과제에 쓴다. 되살릴 목표를 조각마다 정규화하면 모형이 쉽게 맞힐 수 있는 평균 화소 값에 휘둘리지 않고 국소 짜임에 집중하게 된다.
...
`img_size=224`, `patch_size=16`, `embed_dim=768`, `depth=12`, `mask_ratio=0.75`인 MAE 모형에서 (가) 전체 조각 수, (나) 인코더가 처리하는 보이는 조각 수, (다) (자기 어텐션이 지배하며 $O(N^2)$으로 는다고 할 때) 모든 조각을 처리할 때에 견준 FLOP 절약을 셈하라.
...
??? success "연습문제 1 풀이"
    ...
    (다) 자기 어텐션의 FLOP은 수열 길이를 $N$이라 할 때 $O(N^2)$으로 는다. 가리지 않으면 인코더가 토큰 $N = 197$개(조각 196개와 CLS)를 처리한다. 가리면 토큰 $N = 50$개를 처리한다.

    FLOP 비: $(197^2) / (50^2) = 38{,}809 / 2{,}500 \approx 15.5$.

    따라서 75% 가림에서 인코더의 어텐션 계산이 대략 $15.5$배 싸다. (어텐션과 다층 퍼셉트론을 아우른) 트랜스포머 전체로 보면 다층 퍼셉트론 층이 $N$에 대해 제곱이 아니라 일차로 늘므로 전체 속도 향상은 약 $3$~$4$배이다.
...
??? success "연습문제 3 풀이"
    ...
    `encode_full` 메서드는 196 + 1개 토큰을 모두 인코더에 넣어 온전한 공간 표현을 준다. 가림을 쓰는 사전 학습 중에 인코더는 조각의 25%만 보므로 일부 정보에서 뜻있는 표현을 내는 법을 배운다. 추론 때 모든 조각을 처리하면 자기 어텐션이 모든 공간 자리에 주의할 수 있어 더 넉넉한 표현이 나온다. `encode_full`의 CLS 토큰은 대개 분류 과제에 쓰고, 조각 토큰 수열 전체는 분할 같은 빽빽한 예측 과제에 쓴다.

## 정리하며

**다룬 것** — MAE 비전 트랜스포머

MAE의 구조는 비대칭 인코더-디코더 설계이다.

핵심 클래스는 `PatchEmbed`, `Attention`, `MLP`, `TransformerBlock`이며 앞의 연습문제 3개로 직접 확인할 수 있다.
