...
**런 1 내놓음:** $[3, 5, 7, 8]$(길이 4 > $M = 3$).

**런 2:** $\{1, 2, 6\}$의 표시를 지우고 4을 읽는다.
...
**런 2 내놓음:** $[1, 2, 4, 6]$(길이 4).

모두 길이 3짜리 런 3개(손쉬운 만들기)가 아니라 길이 4짜리 런 2개가 된다.
...
이 마당은 왜 필요한가、알고리즘 훑어보기、기대 런 길이、풀이 예제을 차례로 짚었다.
