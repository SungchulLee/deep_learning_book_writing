...
Module 62.3: Carlini & Wagner (C&W) Attack - Intermediate Level This module implements the Carlini & Wagner (C&W) attack, one of the strongest
...
**Exercise 1.**
Explain what happens if you remove the `optimizer.zero_grad()` call from the training loop. Run the modified code and describe the effect on training loss convergence.

??? success "Solution to Exercise 1"
    Without `optimizer.zero_grad()`, gradients accumulate across iterations because PyTorch adds new gradients to existing `.grad` tensors rather than replacing them. This effectively multiplies the learning rate by the number of accumulated steps, causing the optimization to take increasingly large and erratic steps. Training loss will oscillate wildly or diverge instead of converging smoothly. The fix is simple: always zero gradients before calling `loss.backward()`.
...
**Exercise 4.**
Implement early stopping: track the validation loss after each epoch and stop training if it has not improved for 10 consecutive epochs. Save and restore the best model weights.

??? success "Solution to Exercise 4"
    Add a patience counter and best-loss tracker:
    ```python
    best_loss = float('inf')
    patience_counter = 0
    best_state = None
    for epoch in range(num_epochs):
        ...
        if patience_counter >= 10:
            ...
            break
    ```
    This prevents overfitting by stopping when the model stops improving on held-out data.
