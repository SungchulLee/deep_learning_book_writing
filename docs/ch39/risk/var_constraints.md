...
무릅씀 값(VaR)은 주어진 때 어텐션과 믿음 수준에서 겪을 수 있는 가장 큰 어림 손실을 잰다. 보기로 하루 95% 무릅씀 값이 \$100만이라는 것은 하루에 \$100만보다 크게 잃을 낌새가 5%라는 뜻이다. 규제(바젤 III)가 무릅씀 값에 바탕을 둔 밑천 쌓아 두기를 매기므로, 서비스 시스템에서 무릅씀 값 매임은 꼭 있어야 한다.
