...
Instead of assuming fixed decoder variance, we can learn it:
...
def gaussian_nll_loss(x, mu, logvar):
    """
...
- **Adaptive uncertainty:** Different regions can have different confidence
- **Better calibration:** Model knows when it's uncertain
- **Improved generation:** Can capture heteroscedastic data
...
The next section covers the [Reparameterization Trick](reparameterization.md) that enables gradient-based training of VAEs.
