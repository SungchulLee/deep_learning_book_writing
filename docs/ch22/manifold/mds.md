...
유클리드 거리에서 고전 MDS가 주성분 분석과 같음을 수치로 확인하라.

</div>

??? success "연습문제 5 풀이"
    MNIST 1,000개로 두 방법의 2차원 좌표를 구해 견주면 이렇다.

    | 확인한 것 | 값 |
    |---|---|
    | 좌표의 최대 차이 | $3.8 \times 10^{-14}$ |
    | 축마다의 상관 | 1.000000, 1.000000 |
    | 고윳값 대 특잇값$^2$의 최대 차이 | $6.4 \times 10^{-12}$ |

    부동 소수점 오차 수준에서 **완전히 같다.**

    까닭은 이렇다. 가운데 맞춘 그램 행렬 $B = -\tfrac12 J D^2 J$가 실제로
    $X_c X_c^\top$과 같다. 그리고 $X_c = USV^\top$이면

    $$X_c X_c^\top = U S^2 U^\top$$

    이므로 $B$의 고유 벡터가 $U$이고 고윳값이 $s^2$이다. 고전 MDS의 좌표
    $U\sqrt{\Lambda} = US$는 주성분 분석의 점수와 정확히 같다. $\square$

    부호를 맞춰 주어야 견줄 수 있다는 점만 조심하면 된다. 고유 벡터의 부호가 임의롭기
    때문이다([유도 연습문제 4](../pca/pca_derivation.md)).

    그러므로 유클리드 거리에서는 **고전 MDS를 쓸 까닭이 없다.** 주성분 분석이 더
    값싸다. $n \times n$ 거리 행렬을 만드는 것보다 $X$를 바로 쪼개는 것이 낫다.
...
??? success "연습문제 6 풀이"
    **좌표가 없고 거리만 있을 때**다. 주성분 분석은 자료 행렬 $X$를 요구하는데 MDS는
    거리 행렬 $D$만으로 된다.
    ...
    |---|---|
    | 도시 사이의 거리표 | 실제 거리 |
    | 자산 사이의 상관 | $\sqrt{2(1-\rho)}$ 같은 변환 |
    | 유전자 서열 | 편집 거리 |
    | 설문 응답 | 비슷함에 대한 사람의 판단 |
    | 분자 | 구조적 차이 |

    두 번째가 이 장이 보이는 금융 쓰임새다. 자산의 상관 행렬을 거리로 바꾸면 자산들의
    지도를 그릴 수 있고, 함께 움직이는 것들이 가까이 놓인다.

    그리고 거리가 **유클리드가 아닐 때** MDS가 주성분 분석과 갈린다. 편집 거리처럼
    어떤 좌표계에서도 나오지 않는 거리라면, 고전 MDS는 그것을 가장 잘 흉내 내는
    유클리드 좌표를 찾아 준다.

    그때는 $B$가 반양정이 아닐 수 있어 **고윳값이 음수로 나온다.** 그 몫이 크면 거리가
    유클리드로 잘 표현되지 않는다는 신호이며, 계량 MDS 쪽이 나을 수 있다.
...
고윳값이 음수로 나오면 어떻게 해야 하는가?

</div>

??? success "연습문제 7 풀이"
    거리 행렬이 유클리드가 아니라는 뜻이다. 곧 그 거리를 정확히 실현하는 점들의 배치가
    어떤 차원에도 없다.

    실무의 처리는 이렇다.

    - **음수 고윳값을 버린다.** 양수만 써서 좌표를 만든다. 가장 흔하다
    - **음수 몫을 재어 본다.** $\sum_{\lambda_j < 0} |\lambda_j| / \sum_j |\lambda_j|$가
      작으면 걱정하지 않아도 된다
    - **거리를 고친다.** 상수를 더해 유클리드가 되게 만들 수 있다
    - **계량 MDS로 간다.** 스트레스를 직접 줄이므로 유클리드성을 요구하지 않는다

    두 번째를 먼저 하는 것이 순서다. 음수 고윳값이 아주 작으면 수치 오차일 뿐이고,
    실제로 유클리드 자료에서도 $10^{-12}$ 수준의 음수가 나온다.

    크면 방법이 자료에 안 맞는 것이다. 그때 앞의 두 방법으로 밀어붙이면 **거리 관계를
    크게 왜곡한 그림**을 얻으면서 그 사실을 모르게 된다.

    그러므로 이 값을 보고하는 것이 좋은 습관이다. 셰퍼드 그림도 같은 일을 해 준다.
    원래 거리와 묻힌 거리를 흩뿌려 보면 얼마나 어긋났는지 눈에 보인다.
...
계량 MDS의 스트레스는 무엇을 재는가? 값을 어떻게 읽어야 하는가?

</div>

??? success "연습문제 8 풀이"
    원래 거리와 묻은 뒤의 거리가 얼마나 어긋나는지 잰다.

    $$\text{Stress} = \sqrt{\frac{\sum_{i<j} (d_{ij} - \|z_i - z_j\|)^2}{\sum_{i<j} d_{ij}^2}}$$

    분모로 나누어 두므로 **눈금에 딸리지 않는다.** 거리를 두 배로 하면 분자와 분모가
    함께 커져 값이 변하지 않는다.
    ...
    이 기준을 곧이곧대로 믿지는 않는 편이 좋다. 점의 개수와 차원에 따라 우연히 나오는
    값이 달라지기 때문이다. 점이 적으면 낮은 스트레스가 쉽게 나온다.

    그래서 **견줄 대상**을 함께 보는 것이 낫다. 같은 자료를 무작위로 섞어 얻은
    스트레스와 견주거나, 차원을 늘려 가며 스트레스가 꺾이는 자리를 보는 것이다.

    `sklearn`의 `MDS`가 내놓는 `stress_`는 정규화되지 않은 원시 합이라는 점을 조심해야
    한다. `normalized_stress='auto'`로 두어야 위 식의 값이 나온다.
...
??? success "연습문제 9 풀이"
    거리의 **값**이 아니라 **순서**만 지킨다.

    계량 MDS는 $\|z_i - z_j\| \approx d_{ij}$를 원하는데, 비계량 MDS는 $d_{ij}$의 순서와
    $\|z_i - z_j\|$의 순서가 같기를 원한다. 곧 어떤 단조 증가 함수 $f$에 대해
    $\|z_i - z_j\| \approx f(d_{ij})$를 찾는다.
    ...
    설문에서 "매우 비슷함, 비슷함, 다름"으로 받은 응답이 좋은 예다. 그 셋 사이의 간격이
    같다고 볼 근거가 없으므로 값을 그대로 쓰는 것이 오히려 잘못이다. 순서만 쓰면 그
    가정을 피한다.

    대가는 **자유도가 늘어 해석이 약해진다**는 것이다. $f$까지 함께 찾으므로 스트레스가
    계량보다 낮게 나오지만, 그것이 더 좋은 묻힘이라는 뜻은 아니다. 두 방법의 스트레스를
    직접 견주면 안 된다.

    구현은 등장 회귀(isotonic regression)로 $f$를 맞추는 걸음과 좌표를 옮기는 걸음을
    번갈아 하는 것이 보통이다.
...
셰퍼드 그림에서 무엇을 보아야 하는가?

</div>

??? success "연습문제 10 풀이"
    가로축에 원래 거리, 세로축에 묻힌 거리를 흩뿌린 그림이다. 완벽하면 점들이 직선
    위에 놓인다.

    볼 것이 셋이다.

    **흩어진 정도.** 넓게 퍼져 있으면 거리가 잘 안 지켜진 것이다. 스트레스 하나로
    뭉쳐진 정보를 펼쳐 보는 셈이다.

    **굽음.** 점들이 곧지 않고 휘어 있으면 거리가 **체계적으로** 왜곡된 것이다. 아래로
    휘면 먼 거리가 눌렸다는 뜻이고, 위로 휘면 늘어났다는 뜻이다.

    **어느 구간이 나쁜가.** 가까운 거리는 잘 지키면서 먼 거리를 못 지키는 일이 흔하다.
    낮은 차원에 담으려면 어딘가는 희생해야 하고, 대개 먼 쪽이 희생된다.

    셋째가 t-SNE를 이해하는 데 곧바로 이어진다. t-SNE는 먼 거리를 **일부러** 버리고
    가까운 관계에 집중하므로, 셰퍼드 그림을 그리면 크게 흩어진다. 그것이 실패가 아니라
    설계다([t-SNE](02_tsne_deep_dive.md)).

    그래서 셰퍼드 그림은 **거리를 지키려는 방법**에 대한 잣대다. MDS와 Isomap에는 맞고
    t-SNE에는 맞지 않는다. 방법의 목표에 맞는 잣대를 골라야 한다는 이야기가 여기서도
    같다.
