...
??? success "연습문제 8 풀이"
    ...
    [자기 부호기](../../ch23/index.md)가 훨씬 자연스럽다. 자기 부호기는 풀개를 명시적으로
    가지고 있으므로 되돌리기가 정의상 가능하다.

    이것이 굽은 차원 줄이기에서 자기 부호기가 널리 쓰이게 된 까닭 가운데 하나다.
...
??? success "연습문제 9 풀이"
    ...
    그런데 표본이 많고 굽음을 다루고 싶다면 **자기 부호기가 더 나은 답**인 경우가 많다.
    표본 수에 선형이고 묶음으로 익힐 수 있어 자료가 기억 장치에 다 들어가지 않아도 된다.
...
알맹이 주성분 분석과 자기 부호기는 둘 다 굽은 차원 줄이기다. 어떻게 고르는가?
...
??? success "연습문제 10 풀이"
    ...
    **표본이 많으면 자기 부호기**다. 알맹이 행렬이 감당이 안 되고, 되돌리기와 표본
    만들기가 필요하면 더욱 그렇다.

    이 책의 사다리가 그 순서로 놓인 까닭이기도 하다. 주성분 분석 → 알맹이 주성분 분석은
    선형에서 굽음으로 가는 한 걸음이고, 자기 부호기는 같은 걸음을 **커지는 방식으로**
    다시 밟는다.

    한 가지 더 짚어 둘 만하다. 알맹이 주성분 분석은 알맹이를 **우리가 골라야** 하는데,
    자기 부호기는 그 사상을 **자료에서 배운다.** 어느 굽음이 맞는지 모를 때 뒤쪽이 유리한
