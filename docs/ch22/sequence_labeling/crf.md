...
조건부 마구잡이 밭(CRF)은 이음에 이름 붙이기를 위한 가려내는 확률 모델로, 조건부 확률 $P(\mathbf{Y}|\mathbf{X})$을 곧바로 모델로 삼는다. 토막마다 따로 가르는 분류기와 달리 CRF은 이웃한 이름표 사이의 매임을 담으므로, 이름표 넘어감이 정해진 무늬를 따르는 개체명 알아내기에 특히 잘 듣는다(보기로 I-PER은 B-PER이나 I-PER 뒤에만 올 수 있다).
...
- $\mathbf{E} \in \mathbb{R}^{n \times k}$: 신경 인코더에서 나온 내보냄 행렬
...
힘센 신경 인코더(두 방향 LSTM, 변환기)와 CRF 디코더를 아우른 것은, 특히 이름표 사이 얽힘이 중요할 때 이름 알아보기의 센 방식으로 남아 있다.
