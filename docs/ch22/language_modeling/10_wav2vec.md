...
Wav2Vec 2.0은 크게 두 단계로 돌아간다. 곧 날 소리 물결을 숨은 말소리 나타냄으로 다루는 누비기 특징 인코더와, 그 특징에서 맥락이 담긴 나타냄을 세우는 변환기 인코더이다. 특징 인코더는 배치 고르게 맞추기와 GELU 활성화를 곁들인 1차원 누비기를 이어 붙여 16kHz의 날 소리 신호를 다루기 좋은 때 해상도까지 차츰 줄여 뽑는다. 이 꾸밈 덕분에 멜 스펙트로그램 같은 손수 만든 특징 없이 날 소리를 곧바로 다룰 수 있다.
