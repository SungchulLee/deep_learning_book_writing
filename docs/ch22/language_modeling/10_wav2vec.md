...
Wav2Vec 2.0은 2020년 논문 "wav2vec 2.0: A Framework for Self-Supervised Learning of Speech Representations"에서 나온, 말소리 알아듣기를 위한 스스로 살피는 배움 얼거리이다. 이 모델은 맞대어 배우기에 양자화와 변환기 인코더를 곁들여 말소리 나타냄을 배우며, 이름표 붙인 자료가 적어도 좋은 말소리 알아듣기를 해낸다.
...
핵심: 말소리를 위한 맞대어 배우기, 양자화, 변환기 인코더
'''
import torch
...
```
Parameters: 87,275,584
Input: torch.Size([2, 1, 16000])
```

## 2. 논의

Wav2Vec 2.0은 크게 두 단계로 돌아간다. 곧 날 소리 물결을 숨은 말소리 나타냄으로 다루는 누비기 특징 인코더와, 그 특징에서 맥락이 담긴 나타냄을 세우는 변환기 인코더이다. 특징 인코더는 배치 고르게 맞추기와 GELU 깨어남을 곁들인 1차원 누비기를 이어 붙여 16kHz의 날 소리 신호를 다루기 좋은 때 해상도까지 차츰 줄여 뽑는다. 이 꾸밈 덕분에 멜 스펙트로그램 같은 손수 만든 특징 없이 날 소리를 곧바로 다룰 수 있다.
...
변환기 인코더는 숨은 특징의 온 차례를 다루어 모델이 말소리의 멀리 떨어진 얽힘을 담아내게 한다. 여기 보인 얼개는 온전한 Wav2Vec 2.0 모델을 간추린 판이며, 온전한 판은 상대 자리 부호와 여러 부호책 배치에 걸친 곱 양자화라는 더 정교한 양자화를 쓴다. 실전에서 이 얼개는 말소리 알아듣기 잣대에서, 특히 이름표 붙인 자료가 귀한 곳에서 놀라운 성능을 낸다.
...
Wav2Vec 2.0이 말소리의 스스로 살피는 미리 익히기에 (오토인코더 같은) 되살림 바탕 목표 대신 맞대어 배우기를 쓰는 까닭을 밝혀라. 좋은 점과 있을 수 있는 나쁜 점은 무엇인가?
