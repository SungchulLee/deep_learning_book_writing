...
Wav2Vec 2.0은 크게 두 단계로 돌아간다. 곧 날 소리 물결을 숨은 말소리 나타냄으로 다루는 누비기 특징 부호기와, 그 특징에서 맥락이 담긴 나타냄을 세우는 변환기 부호기이다. 특징 부호기는 배치 고르게 맞추기와 GELU 깨어남을 곁들인 1차원 누비기를 이어 붙여 16kHz의 날 소리 신호를 다루기 좋은 때 해상도까지 차츰 줄여 뽑는다. 이 꾸밈 덕분에 멜 스펙트로그램 같은 손수 만든 특징 없이 날 소리를 곧바로 다룰 수 있다.
...
변환기 부호기는 숨은 특징의 온 차례를 다루어 모델이 말소리의 멀리 떨어진 얽힘을 담아내게 한다. 여기 보인 얼개는 온전한 Wav2Vec 2.0 모델을 간추린 판이며, 온전한 판은 상대 자리 부호와 여러 부호책 배치에 걸친 곱 양자화라는 더 정교한 양자화를 쓴다. 실전에서 이 얼개는 말소리 알아듣기 잣대에서, 특히 이름표 붙인 자료가 귀한 곳에서 놀라운 성능을 낸다.
