...
??? success "연습문제 2 풀이"
    ...
    무게 줄이기를 한결같이 쓰면 치우침이 0 쪽으로 밀리고 LayerNorm이 깨어남의 잣수를 제대로 다시 맞추지 못해 모델이 덜 배우게 된다. 표준 버릇은 매개변수를 가장 좋게 하개 배치 둘로 가른다. 곧 무게 줄이기를 쓰는 쪽(빽빽한 무게 행렬)과 쓰지 않는 쪽(치우침과 고르게 맞추기 매개변수)이다.
