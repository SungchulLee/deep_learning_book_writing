...
상위 k 표집은 후보를 가장 그럴듯한 낱말 $k$개로 제한한 뒤 그 잘라 낸 분포에서 뽑는다. 이러면 조리를 무너뜨릴 만큼 그럴듯하지 않은 낱말이 나오는 것을 막으면서 확률 높은 자리 안에서 여러 갈래임은 지킨다. n-그램 생성기의 핵심 한계는 먼 거리의 조리를 지키지 못한다는 것이다. 곧 바이그램 모델은 앞 낱말 하나만 "기억"하므로 만든 글이 금세 주제에서 벗어난다. 이 때문에 더 긴 맥락을 담는 숨은 상태를 지니는 신경 말 모델로 나아가게 된다.
