...
class TransformerBlock(nn.Module):
    ...
    짜임:
        x -> 층 고르게 맞추기 -> 인과 어텐션 -> + -> 층 고르게 맞추기 -> 앞먹임 신경망 -> +
...
def generate_with_cache(
    ...
    맥락에는 저장턱의 K/V을 쓰고 새 토큰의 어텐션만 셈한다.
    """
    model.eval()
...
class MultiQueryAttention(nn.Module):
    """
    여러 물음 어텐션: 모든 머리가 K, V 하나를 나누어 쓴다.
...
여러 머리 어텐션과 여러 물음 어텐션 사이의 절충으로 머리 무리가 K, V을 나누어 쓴다.

```python
class GroupedQueryAttention(nn.Module):
    """
    무리 물음 어텐션: 무리 안에서 K, V을 나누어 쓴다.
...
5. **얼개 떼어 보기**: 여러 어텐션 변형(여러 머리 어텐션, 여러 물음 어텐션, 무리 물음 어텐션)으로 모델 솜씨를 견주어라.
