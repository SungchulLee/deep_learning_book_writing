...
이 단원은 깊은 만들어 내는 모델의 중요한 부품인 만들어 내는 모델의 따지기 바탕을 짠다. 이 짜기를 이해하면 요즘 만들어 내는 모델에 쓰이는 얼개의 결과 익히기 절차를 꿰뚫어 볼 수 있다. 이 코드는 연구와 실제 얼개에서 널리 쓰이는 쓸모 있는 재주를 보인다.
...
이 짜기는 만들어 내는 모델의 따지기 바탕에 대해 자리 잡은 가장 좋은 방식을 따른다. 코드는 모델 뜻매김, 익히기 논리, 도구 함수를 또렷이 갈라 짜였다. 핵심 설계 결정에는 깨움 함수 고르기, 고르게 맞추기 방책, 가장 좋게 하기 웃매개변수가 들며 모두 익히기의 안정과 내놓기 품질에 크게 영향을 준다.
...
만들어 내는 모델을 값매김하기가 왜 어려운가? 분류와 무엇이 다른가?

</div>

??? success "연습문제 4 풀이"
    **정답이 없다.** 분류에는 표지가 있어 맞았는지 틀렸는지가 정해지지만, 만들어 낸 그림에는
    견줄 정답이 없다.

    더 정확히 말하면 우리가 재고 싶은 것이 **분포 사이의 거리**다. 표본 하나가 좋은지가
    아니라 표본들의 분포가 자료의 분포와 얼마나 닮았는지를 알고 싶다. 그런데 두 분포를
    표본으로만 알고 있고, 차원은 784이며, 표본은 몇만 개뿐이다.

    그래서 값매김이 두 가지를 함께 보아야 한다.

    | 봐야 할 것 | 놓치면 |
    |---|---|
    | 품질 | 얼룩을 내놓는데 다양하기만 하다 |
    | 다양성 | 잘 만든 몇 가지만 되풀이한다 |

    둘째가 특히 놓치기 쉽다. 표본 격자를 눈으로 보면 품질은 곧 보이지만 다양성은 잘
    안 보인다. 격자에 비슷한 것이 여럿 있어도 하나하나가 그럴듯하면 넘어가게 된다.

    그리고 어떤 잣대든 대리일 뿐이라는 점을 새겨야 한다. 이 장에서 되풀이해 보게 될
    것이 **잣대를 올리는 방법과 모델을 좋게 하는 방법이 갈리는** 자리다.
...
표본을 눈으로 보는 것으로 충분하지 않은 까닭은 무엇인가?
...
??? success "연습문제 5 풀이"
    ...
    **다양성을 못 본다.** 열여섯 장의 격자를 보고 모델이 몇 가지를 낼 수 있는지 알 수 없다.
    극단적인 예를 만들 수 있다. 서로 다른 그림 열 장을 되풀이해 1,000개를 만들면 격자로는
    완벽해 보인다. 그리고 인셉션 점수도 9.164로 참 자료와 비슷하게 나온다
    ([인셉션 점수 연습문제 2](inception_score.md)).

    **고른 표본을 보게 된다.** 논문이나 글에 싣는 격자가 좋은 것만 고른 것인지 알 수 없다.
    씨앗을 고정하고 처음 $n$개를 그대로 싣는 것이 정직하다.

    **견줄 수 없다.** 두 모델의 격자를 나란히 놓아도 어느 쪽이 나은지 판단이 갈린다.
    수가 있어야 순서를 매기고 발전을 잴 수 있다.

    그렇다고 눈으로 보는 것을 빼면 안 된다. 수가 못 보는 것을 눈이 보기 때문이다. 격자에
    비슷한 것이 여럿 보이면 다양성 문제이고, 수치로는 그 신호가 뭉개져 있다.

    **둘을 함께 하는 것**이 답이다. 어느 하나로 갈음할 수 없다.
...
이 장에서 쓰는 잣대들을 성격에 따라 정리하라.

</div>

??? success "연습문제 6 풀이"
    | 잣대 | 참 자료를 쓰는가 | 무엇을 보는가 | 하나의 수인가 |
    |---|---|---|---|
    | 인셉션 점수 | 아니다 | 품질 + 부류 다양성 | 하나 |
    | FID | 쓴다 | 특징 분포 전체 | 하나 |
    | 정밀도와 재현율 | 쓴다 | 품질과 다양성을 **나눠서** | 둘 |
    | 가능도 | 쓴다 | 모델이 자료에 주는 확률 | 하나 |
    | 사람이 평가 | — | 무엇이든 | — |

    첫 칸이 중요한 구별이다. 참 자료를 쓰지 않는 잣대는 겨냥하기가 훨씬 쉽다. 만들어 낸
    것만 보므로 자료를 닮을 필요가 없다.

    셋째 줄의 "둘"도 눈여겨볼 만하다. 하나의 수는 순서를 매기기 좋고 진단에는 나쁘다.
    FID가 40이라는 것이 품질 탓인지 다양성 탓인지 말해 주지 않는다.

    넷째 줄에는 큰 제약이 있다. **적대적 생성망은 가능도를 정의하지 않으므로** 쓸 수
    없다([가능도 잣대](02_likelihood_metrics.md)). 변분 자기 부호기와 퍼짐 모델에는
    쓸 수 있다. 곧 이 잣대로는 두 갈래의 모델을 견줄 수 없다.
...
이 장의 수치는 인셉션 그물로 잰 것인가?

</div>

??? success "연습문제 7 풀이"
    아니다. **MNIST 분류기**로 쟀다. 시험 정확도 98.50%이고 128차원 은닉층을 특징으로
    쓴다.

    까닭은 인셉션이 손글씨 숫자를 본 적이 없기 때문이다. ImageNet 1,000부류로 익힌
    그물에 MNIST를 넣으면 $p(y \mid x)$가 자료와 무관한 부류들에 퍼지므로 잣대의 뜻이
    사라진다([인셉션 점수 연습문제 4](inception_score.md)).

    대가는 **논문의 수와 직접 견줄 수 없다**는 것이다. 인셉션 기준 FID 20과 이 장의
    FID 20은 다른 잣대의 값이다.

    그래서 이 장의 모든 표에 기준점을 함께 적어 둔다.

    | 기준점 | 값 |
    |---|---|
    | 참 시험 자료의 인셉션 점수 | 9.467 |
    | 참 대 참 FID (5,000개) | 1.8 |
    | 인셉션 점수의 상한 | 10 (부류 수) |

    이 셋을 알고 있으면 모델의 수를 읽을 수 있다. 절대적인 값이 아니라 기준점 사이의
    자리로 읽는 것이 옳다.
...
두 잣대가 어긋날 때 어떻게 읽어야 하는가?

</div>

??? success "연습문제 8 풀이"
    **어긋남 자체가 정보다.** 무엇이 어긋났는지가 무엇이 문제인지 알려 준다.

    가장 또렷한 예가 이것이다.

    | 묶음 | IS | FID |
    |---|---|---|
    | 참 자료 1,000개 | 9.290 | 18.7 |
    | 서로 다른 그림 열 장을 100번 되풀이 | **9.164** | **356.5** |

    IS는 거의 같고 FID는 스무 배 차이다. 이 꼴의 어긋남은 **다양성 부족**을 가리킨다.
    IS가 부류 안의 다양성을 못 보고 FID는 보기 때문이다.

    반대 방향의 어긋남도 뜻이 있다. FID는 괜찮은데 IS가 낮다면 부류 쏠림을 의심할 만하다.
    특징 분포는 대충 맞는데 부류 예측이 애매하다는 뜻이다.

    그러므로 잣대를 여럿 적는 까닭이 "여러 개면 더 믿을 만하다"가 아니다. **서로 다른
    것을 보기 때문**이고, 어긋날 때 진단이 되기 때문이다. 늘 같은 순서를 준다면 하나만
    적어도 될 것이다.
...
값매김 잣대를 모델 고르기에 쓰면 무슨 문제가 생기는가?

</div>

??? success "연습문제 9 풀이"
    잣대가 심판에서 **목표로 바뀐다.** 그러면 더는 공정한 심판이 아니다.

    이 장에서 몇 번 보게 될 일이다. 잣대를 올리는 길과 모델을 좋게 하는 길이 갈리는
    자리가 늘 있다.

    | 잣대 | 겨냥하는 방법 |
    |---|---|
    | 인셉션 점수 | 부류마다 잘 나온 그림 하나씩 |
    | FID | 학습 자료를 외운다 |
    | 판정 확신도 | 더 날카롭게 만든다 |

    셋째 줄이 [25장에서 실제로 부딪힌 것](../../ch25/training/generate_samples.md)이다.
    뽑을 때 $z$를 키우면 확신도가 계속 올라가는데, 사전 분포에서 멀어질수록 오르므로
    품질이 아니라 날카로움을 재고 있었다.

    둘째 줄이 원칙적으로 가장 고약하다. FID는 자료의 분포와 같아지기를 상으로 주므로
    **외우기가 만점**이다. 벌할 항이 식에 없다.

    다루는 법이 이렇다.

    - 고르는 데 쓴 잣대와 **다른 자료**로 마지막 값을 낸다
    - 외우기를 따로 확인한다. 학습 기준 FID와 시험 기준 FID의 차이를 본다
    - 눈으로 본다. 겨냥된 표본은 대개 사람 눈에 이상하다

    마지막 것이 실용적으로 가장 잘 듣는다.
...
??? success "연습문제 10 풀이"
    잣대마다 다르고, FID가 가장 까다롭다.

    | 잣대 | 권하는 표본 수 | 왜 |
    |---|---|---|
    | 인셉션 점수 | 50,000 | 관례. 조각으로 나누므로 넉넉히 |
    | FID | 10,000~50,000 | 공분산 추정. 쏠림이 있다 |
    | 정밀도와 재현율 | 10,000 | 이웃을 찾으므로 |

    FID의 쏠림이 방향이 정해져 있다는 점이 중요하다. 표본이 적으면 **반드시 크게** 나온다.
    이 장의 설정에서 참 자료끼리 재면 100개에서 81.4, 5,000개에서 1.8이다
    ([FID 연습문제 12](fid.md)).

    어림으로 **특징 차원의 열 배 이상**을 권한다. 2,048차원이면 20,000개 이상이고, 그래서
    50,000개가 관례가 되었다.

    그리고 표본 수를 **밝히는 것**이 개수를 고르는 것보다 중요하다. 표본 수가 다른 FID는
    아예 다른 잣대이므로, 적어 두지 않으면 남이 그 수를 쓸 수 없다.

## 정리하며

**다룬 것** — 만들어 내는 모델의 따지기 바탕
