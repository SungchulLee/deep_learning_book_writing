...
이 단원은 깊은 만들어 내는 모델의 중요한 부품인 가능도에 바탕한 잣대을 짠다. 이 짜기를 이해하면 요즘 만들어 내는 모델에 쓰이는 얼개의 결과 익히기 절차를 꿰뚫어 볼 수 있다. 이 코드는 연구와 실제 얼개에서 널리 쓰이는 쓸모 있는 재주를 보인다.
...
??? success "연습문제 4 풀이"
    ...
    변분 자기 부호기는 $p(x) = \int p(x\mid z) p(z)\,dz$라는 확률 모형을 세우므로 가능도가
    정의된다. 적분이 어려워 직접 셈하지는 못하지만 증거 하한으로 아래에서 묶을 수 있다
    ([25장](../../ch25/theory/kl_term.md)).
...
??? success "연습문제 5 풀이"
    ...
    이고 틈이 $D_{\mathrm{KL}}(q(z\mid x) \| p(z\mid x))$이므로, 부호기가 참된 사후 분포를
    잘 흉내 낼수록 빡빡하다.

    그 틈을 재어 볼 수 있다. 모델을 얼려 두고 표본마다 $(\mu, \log\sigma^2)$을 직접
    최적화하면 $-$ELBO가 99.191에서 95.654까지 내려간다. 곧 **3.537**만큼이 부호기가
    보는 손실이다([25장](../../ch25/architecture/encoder.md)).
    ...
    ([25장](../../ch25/architecture/beta_vae.md)).
...
??? success "연습문제 6 풀이"
    ...
    |---|---|---|---|
    | 변분 자기 부호기 | 5.804 | 148.2 | 쓸 수 있다 |
    | 적대적 생성망 | **6.964** | **40.7** | 못 쓴다 |

    적대적 생성망이 표본 잣대에서 크게 이긴다. 그런데 변분 자기 부호기는 가능도(의 하한)를
...
??? success "연습문제 7 풀이"
    ...
    **더하기와 평균.** [25장의 관례](../../ch25/theory/kl_term.md)대로 화소는 더하고
...
??? success "연습문제 8 풀이"
    **가능도가 낮은 것이 곧 이상한 것은 아니다.**
    ...
    - **다시 세우기 오차와 함께 본다.** 자기 부호기 계열이면 둘을 같이 쓴다
...
변분 자기 부호기와 퍼짐 모델은 가능도로 견줄 수 있는가?
