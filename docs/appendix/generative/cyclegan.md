...
CycleGAN was introduced in the 2017 paper "Unpaired Image-to-Image Translation using Cycle-Consistent Adversarial Networks." - Two generators (A→B, B→A)   - Cycle-consistency loss enforces invertibility.

This implementation provides a concise, educational reference for CycleGAN. The code focuses on the core architecture and forward pass, making it straightforward to study the key design patterns and adapt them for experimentation.
...
이 짜보기는 갈래 2개(`Generator`, `CycleGAN`)를 매기고, 이들이 어울려 온전한 만들개 모형 얼개를 이룬다. 갈래마다 남다른 몫을 담아 코드를 묶음으로 나누고 넓히기 쉽게 한다. `forward` 방법이 PyTorch이 절로 미분하는 데 쓰는 셈 그림을 매긴다.
