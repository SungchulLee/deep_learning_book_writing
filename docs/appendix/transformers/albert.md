...
이 짜보기는 갈래 2개(`SharedTransformerBlock`, `ALBERT`)를 매기고, 이들이 어울려 온전한 변환기 얼개를 이룬다.

고갱이 갈래는 `SharedTransformerBlock`, `ALBERT`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
