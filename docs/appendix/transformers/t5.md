...
T5은 2020년 글 "Exploring the Limits of Transfer Learning with a Unified Text-to-Text Transformer"에서 나왔다. - 모든 것을 글월에서 글월로 옮기는 일로 삼는다(들임도 날임도 글월이다) - 인코더-디코더 변환기다 - 미리 익힐 때 *구간 망가뜨리기*(파수 낱말)를 쓴다.
...
class T5Decoder(nn.Module):
    """변환기 디코더: 제 되돌이 스스로 어텐션 + 인코더로 가는 엇결 어텐션."""
    def __init__(self, vocab_size=32128, d_model=512, nhead=8, num_layers=6):
        super().__init__()
        self.embed = nn.Embedding(vocab_size, d_model)
...
이 짜보기는 갈래 3개(`T5Encoder`, `T5Decoder`, `T5`)를 매기고, 이들이 어울려 온전한 변환기 얼개를 이룬다.

고갱이 갈래는 `T5Encoder`, `T5Decoder`, `T5`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
