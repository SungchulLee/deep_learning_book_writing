...
PSPNet은 2017년 글 "Pyramid Scene Parsing Network"에서 나왔다. 여러 잣대의 앞뒤 흐름을 모으는 두겁 모으기 배치를 쓴다.
...
고갱이: 여러 자의 앞뒤 흐름을 모으는 피라미드 모으기 배치
'''
import torch
...
이 짜보기는 갈래 2개(`PyramidPooling`, `PSPNet`)를 매기고, 이들이 어울려 온전한 그림 나누기 얼개를 이룬다. 갈래마다 남다른 몫을 담아 코드를 묶음으로 나누고 넓히기 쉽게 한다. `forward` 방법이 PyTorch가 절로 미분하는 데 쓰는 셈 그림을 매긴다.
...
`PyramidPooling`의 앞으로 걸음을 따라 텐서의 꼴을 좇아라. 기본 매개변수로 들임 보기 4개를 묶어 넣었을 때, 큰 셈(엮음, 모으기, 선형 켜)마다 꼴이 어떻게 되는지 적어라.
...
??? success "연습문제 2 풀이"
    첫 엮음 켜의 `in_channels`을 이제 값에서 3으로 바꾼다. 엮음과 모으기 켜마다 $H_{\text{out}} = \lfloor(H_{\text{in}} + 2p - k) / s\rfloor + 1$으로 자리 차수를 다시 셈한다. 첫 선형 켜의 `in_features`을 마지막 엮음/모으기 켜의 편 날임에 맞춘다. `model = PyramidPooling(...); x = torch.randn(1, 3, 64, 64); print(model(x).shape)`으로 따진다.
...
`PyramidPooling`을 켜나 덩이의 수를 골라 잡을 수 있게 넓혀라. `__init__`에 `num_layers` 매개변수를 더하고 `nn.ModuleList`으로 깊이를 바꿀 수 있는 얼개를 짜라. 켜 2개, 4개, 8개로 시험하여라.
...
??? success "연습문제 4 풀이"
    ...
    ```
    `forward` 방법에서 `for layer in self.layers: x = layer(x)`으로 되돈다. 여느 파이썬 목록이 아니라 `nn.ModuleList`을 써야 PyTorch가 매개변수를 모두 다듬기에 올린다. `for n in [2, 4, 8]: model = PyramidPooling(num_layers=n); print(f'Layers={n}, params={sum(p.numel() for p in model.parameters()):,}')`으로 시험한다.

## 정리하며

**다룬 것** — PSPNet

이 짜보기는 갈래 2개(`PyramidPooling`, `PSPNet`)를 매기고, 이들이 어울려 온전한 그림 나누기 얼개를 이룬다.

고갱이 갈래는 `PyramidPooling`, `PSPNet`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
