...
LSTM was introduced in the 1997 paper "Long Short-Term Memory." Gating mechanisms to capture long-term dependencies in sequences.

This implementation provides a concise, educational reference for LSTM. The code focuses on the core architecture and forward pass, making it straightforward to study the key design patterns and adapt them for experimentation.
...
**Exercise 1.**
Calculate the total number of learnable parameters in `LSTMModel` with the default initialization. Break down the count by layer, including both weights and biases.
...
**Exercise 4.**
Extend `LSTMModel` to support a configurable number of layers or blocks. Add a `num_layers` parameter to `__init__` and use `nn.ModuleList` to create a variable-depth architecture. Test with 2, 4, and 8 layers.
...
??? success "Solution to Exercise 4"
    ...
    ```
    In the `forward` method, iterate: `for layer in self.layers: x = layer(x)`. Using `nn.ModuleList` (not a plain Python list) ensures PyTorch registers all parameters for optimization. Test with: `for n in [2, 4, 8]: model = LSTMModel(num_layers=n); print(f'Layers={n}, params={sum(p.numel() for p in model.parameters()):,}')`.
