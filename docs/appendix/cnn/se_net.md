...
쥐어짜 북돋우는 그물(SE-Net)은 2018년 글 "쥐어짜 북돋우는 그물"에서 나왔으며, 결 되받음의 눈금을 맞추어 가며 다시 잡는 갈래 어텐션 얼개를 내놓았다. SE 덩이마다 먼저 두루 고르게 모으기로 자리 소식을 갈래 밝힘으로 쥐어짜고, 온통 이은 켜 둘로 갈래마다의 종요로움 짐을 배운다. 이 가벼운 배치는 어떤 CNN 얼개에나 끼워 넣어 됨됨이를 올릴 수 있다.
...
Won ILSVRC 2017
고갱이: 채널 어텐션 얼개로 채널마다 특징을 다시 맞춘다
'''
import torch
...
이 짜보기는 갈래 3개(`SEBlock`, `SEResNetBlock`, `SEResNet`)를 매기고, 이들이 어울려 온전한 엮음 신경 그물 얼개를 이룬다.
