...
??? success "연습문제 3 풀이"
    ...
    class MBConvSE(nn.Module):
        ...
        def forward(self, x):
            ...
            return out
    ```
    SE 배치는 깊이별 엮음 뒤, 되비춤 앞에 놓여 넓힌 갈래 차수에서 움직인다.

## 정리하며

**다룬 것** — EfficientNet

MBConv(손전화 뒤집힌 목 엮음) 덩이가 EfficientNet의 밑바탕 벽돌이다.

고갱이 갈래는 `MBConv`, `EfficientNet`, `MBConvSE`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
