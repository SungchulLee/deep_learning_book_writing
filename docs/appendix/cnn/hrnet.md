...
HRNet을 가르는 결은 나란한 여러 결 얼개다. 결을 먼저 줄였다가 되찾는 여느 인코더-디코더나 결 두겁 방식 대신, HRNet은 높은 결의 흐름을 내내 지키며 낮은 결의 나란한 흐름을 차츰 더한다. 여러 잣대 녹이기 배치가 모든 결 켜 사이에서 소식을 거듭 주고받아, 높은 결의 흐름이 낮은 결이 지닌 뜻의 넉넉함을 얻으면서도 잔 자리 결을 지킨다.
...
자리 결을 지키는 HRNet의 길과 건너뛰는 이음을 쓰는 U-Net의 인코더-디코더 길을 견주어라. 저마다의 좋은 점과 나쁜 점은 무엇인가?
...
HRNet을 가르는 결은 나란한 여러 결 얼개다.

고갱이 갈래는 `BasicBlock`, `HRModule`, `HRNet`, `LightBasicBlock`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
