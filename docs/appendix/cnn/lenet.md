...
엮음 켜는 가까운 자리의 결을 배운다. 첫 켜는 흔히 가장자리와 기울기를 알아내는 것을 배우고, 둘째 켜는 이를 아울러 굽이나 모서리 같은 더 위 켜의 무늬를 이룬다. 고르게 모으기는 옮겨도 안 바뀜과 자리 눌러 담기를 주지만, 요즘 얼개는 이를 가장 크게 모으기나 걸음 있는 엮음으로 거의 갈음했다. 온통 이은 켜는 배운 결 드러냄 위에서 움직이는 분류기 노릇을 한다.
...
LeNet-5은 요즘 CNN이 거의 다 따르는 본을 세웠다.

고갱이 갈래는 `LeNet`, `LeNetCIFAR`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
