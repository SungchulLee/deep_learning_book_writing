...
ResNeSt(가른 어텐션 그물)은 2020년에 나왔으며, 나머지 덩이마다 여러 결 그림 무리에 갈래별 어텐션을 거는 가른 어텐션 덩이를 넣어 ResNet을 넓혔다. SE-Net과 SK-Net에서 실마리를 얻어, ResNeSt은 갈래를 넘나드는 주고받음을 더 잘게 담아, 셈 값을 크게 늘리지 않고도 그림 가름, 물체 알아내기, 뜻 나누기 같은 일의 드러냄 배우기를 낫게 한다.
...
고갱이: 특징 무리를 가로질러 채널마다 어텐션을 두는 나눔 어텐션 덩이
'''
import torch
