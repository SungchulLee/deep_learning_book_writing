...
def scaled_dot_product_attention(Q, K, V, mask=None):
    """
    잣대 맞춘 점곱 어텐션(변환기 어텐션의 고갱이).

    들임:
      Q: 물음   (B, H, Tq, Dh)
      K: 열쇠      (B, H, Tk, Dh)
      V: 값    (B, H, Tk, Dh)
      mask: (B, H, Tq, Tk)으로 펴 맞출 수 있는, 골라 쓰는 어텐션 가림
            - True/1이면 "남긴다", False/0이면 "가린다"

    날임:
      out: (B, H, Tq, Dh)
      attn: (B, H, Tq, Tk) 어텐션 짐
    """
    Dh = Q.size(-1)
...
class AdditiveAttention(nn.Module):
    """
    더하기(바다나우) 어텐션. seq2seq RNN 모형에서 흔하다.
...
이 짜보기는 갈래 2개(`MultiHeadAttention`, `AdditiveAttention`)를 매기고, 이들이 어울려 온전한 잔손질 묶음 얼개를 이룬다.

고갱이 갈래는 `MultiHeadAttention`, `AdditiveAttention`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
