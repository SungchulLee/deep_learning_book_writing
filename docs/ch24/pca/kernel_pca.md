...
알맹이 주성분 분석은 선형 주성분 분석과 오토인코더 같은 온전한 비선형 방법 사이를 이으며, 닫힌 꼴 풀이로 비선형 차원 줄이기를 준다.
...
이는 또렷한 디코더를 배우는 오토인코더에 견주었을 때 타고난 한계이다.
...
| **오토인코더** | 비선형 | 정확(디코더) | 한 바퀴마다 $O(ndk)$ | 얼개, 익히기 |
| **t-SNE** | 비선형 | 없음 | $O(n^2)$ 또는 $O(n\log n)$ | 헷갈림도 |
| **UMAP** | 비선형 | 어림 | $O(n^{1.14})$ | $k$, 이웃 수 |

### 언제 알맹이 주성분 분석을 쓰는가

알맹이 주성분 분석은 비선형 짜임을 잡아야 하되 자료 묶음이 작아 $O(n^3)$ 값이 받아들일 만할 때(흔히 $n < 10{,}000$) 가장 쓸모 있다. 더 큰 자료 묶음에는 아무렇게나 하는 어림이나 신경망 방법(오토인코더)이 더 현실적이다.
...
겹동그라미 자료에서 알맹이 주성분 분석이 얼마나 나은지 수치로 보이라.

</div>

??? success "연습문제 5 풀이"
    안쪽과 바깥쪽 동그라미 400점을 1차원으로 줄인 뒤 로지스틱 회귀로 가르고 5겹 교차
    검증한 정확도다.

    | 방법 | 정확도 |
    |---|---|
    | 주성분 분석 1차원 | 45.2% |
    | 알맹이 주성분 분석 (rbf, $\gamma=5$) | **100.0%** |
    | (견줌) 원래 2차원 그대로 | 48.0% |

    눈여겨볼 것은 셋째 줄이다. **원래 2차원을 그대로 줘도 48%**, 곧 찍기와 비슷하다.
    두 동그라미는 선형으로 가를 수 없기 때문이다.

    그런데 알맹이 주성분 분석으로 **1차원**으로 줄이면 100%가 된다. 차원을 줄였는데
    가를 수 있게 된 것이다.

    까닭은 알맹이가 자료를 높은 차원으로 옮긴 뒤 거기서 주성분 분석을 하기 때문이다.
    그 공간에서는 반지름이 하나의 좌표축처럼 나타나므로, 그 축 하나만 남기면 안쪽과
    바깥쪽이 갈린다.

    주성분 분석이 45.2%로 찍기보다 못한 것도 뜻이 있다. 흩어짐이 가장 큰 방향을 골랐지만
    그 방향이 **가르는 방향과 무관**했다. 표지를 보지 않는 방법의 한계다
    ([유도 연습문제 6](pca_derivation.md)).
...
??? success "연습문제 6 풀이"
    ...
    | $\gamma$ | 0.5 | 1.0 | **5.0** | 10.0 | 20.0 |
    |---|---|---|---|---|---|
    | 정확도 | 44.0% | 46.5% | **100.0%** | 76.8% | 74.5% |

    5에서 완벽하고 양쪽으로 급격히 나빠진다. 단조롭지 않으므로 "크게 두면 된다"가
    통하지 않는다.

    양 끝이 실패하는 까닭이 다르다.

    **$\gamma$가 작으면** 알맹이가 거의 상수에 가까워 모든 점이 비슷하게 보인다. 곧
    선형에 가까워지고 주성분 분석과 다를 바 없어진다(44.0%가 주성분 분석의 45.2%와
    비슷한 것이 그 증거다).

    **$\gamma$가 크면** 알맹이가 자기 자신만 보게 되어 모든 점이 서로 멀다고 판단한다.
    짜임이 사라지고 외우는 쪽으로 간다.

    고르는 법은 **아래쪽 일의 성능으로 교차 검증**하는 것이다. 알맹이 주성분 분석은
    비지도 방법이라 스스로의 잣대로는 고를 수 없다. 설명하는 흩어짐을 보아도 $\gamma$가
    다르면 서로 다른 공간의 흩어짐이라 견줄 수 없다.

    출발점으로 쓸 만한 어림이 있다. 점들 사이 거리의 중앙값 $m$에 대해
    $\gamma \approx 1/(2m^2)$ 정도를 잡는 것이다.
...
알맹이 주성분 분석에서 특징 공간의 가운데 맞추기를 왜 따로 해야 하는가?

</div>

??? success "연습문제 7 풀이"
    특징 공간의 점 $\phi(x_i)$를 직접 만지지 않기 때문이다. 알맹이 행렬만 알므로
    평균을 빼는 일을 **알맹이 행렬 위에서** 해야 한다.

    빼야 할 것은 $\phi(x_i) - \bar\phi$이고 $\bar\phi = \frac1n \sum_j \phi(x_j)$다.
    내적을 펼치면

    $$\tilde K = K - \mathbf{1}_n K - K \mathbf{1}_n + \mathbf{1}_n K \mathbf{1}_n$$

    이 되며 $\mathbf{1}_n$은 원소가 모두 $1/n$인 $n \times n$ 행렬이다. 곧 행 평균과 열
    평균을 빼고 전체 평균을 더한다. 이중 가운데 맞추기다.

    빠뜨리면 어떻게 되는가. 보통 주성분 분석에서 가운데를 안 맞춘 것과 같은 일이 벌어져,
    제1성분이 평균 방향을 잡고 설명하는 흩어짐이 부풀려진다
    ([기본 연습문제 5](pca_fundamentals.md)).

    **새 자료를 변환할 때가 특히 함정이다.** 학습 자료의 평균을 써야 하므로 식이 달라진다.

    $$\tilde k_{\text{new}} = k_{\text{new}} - \mathbf{1}_n' K - k_{\text{new}} \mathbf{1}_n
      + \mathbf{1}_n' K \mathbf{1}_n$$

    직접 짜면 틀리기 쉬운 자리이며, `sklearn`의 `KernelCenterer`가 이 일을 해 준다.
    `KernelPCA`를 쓰면 안에서 알아서 처리한다.
...
알맹이 주성분 분석의 앞그림(pre-image) 문제란 무엇인가?

</div>

??? success "연습문제 8 풀이"
    줄인 것을 원래 공간으로 되돌릴 수 없다는 문제다.

    보통 주성분 분석은 $\hat x = \bar x + V_k z$로 되돌린다. 알맹이 주성분 분석에서
    나오는 것은 **특징 공간의 점**이고, 그 점에 대응하는 원래 공간의 $x$가 있다는 보장이
    없다. $\phi$가 위로의 함수가 아니기 때문이다.

    그래서 `KernelPCA`의 `inverse_transform`은 정확한 역이 아니라 **어림**이다. 원래
    공간에서 그 특징 공간 점에 가장 가까워지는 $x$를 최적화로 찾는다. 답이 여럿일 수도
    없을 수도 있다.

    실용적인 결과가 이렇다.

    | 하고 싶은 일 | 알맹이 주성분 분석으로 되는가 |
    |---|---|
    | 낮은 차원 좌표 얻기 | 된다 |
    | 그려 보기 | 된다 |
    | 특징으로 써서 분류 | 된다 |
    ...
    아래 두 칸이 중요하다. 다시 세우기가 목적이면 알맹이 주성분 분석이 어색한 도구이고
    [오토인코더](../../ch25/index.md)가 훨씬 자연스럽다. 오토인코더는 디코더를 명시적으로
    가지고 있으므로 되돌리기가 정의상 가능하다.

    이것이 굽은 차원 줄이기에서 오토인코더가 널리 쓰이게 된 까닭 가운데 하나다.
...
알맹이 주성분 분석은 자료가 커지면 왜 쓰기 어려워지는가?

</div>

??? success "연습문제 9 풀이"
    알맹이 행렬이 $n \times n$이기 때문이다. 표본 수의 제곱으로 커진다.
    ...
    MNIST 전체에는 쓸 수 없다. 그리고 고유 쪼개기가 $O(n^3)$이므로 셈하기도 감당할 수
    없다.

    보통 주성분 분석과 비교해 보면 차이가 또렷하다. 그쪽은 $d \times d$ 공분산이나
    $X$의 특잇값 쪼개기를 쓰므로 **표본 수에 선형**이다. MNIST에서 60,000개를 다 써도
    100초면 된다.

    다룰 방법이 몇 가지다.

    - **표본을 덜어 쓴다.** 대표 표본 몇 천 개로 성분을 구하고 나머지는 변환만 한다
    - **니스트룀 어림.** 알맹이 행렬의 일부만 셈해 나머지를 어림한다
    - **무작위 특징.** 알맹이를 명시적인 특징 사상으로 어림해 보통 주성분 분석을 한다

    그런데 표본이 많고 굽음을 다루고 싶다면 **오토인코더가 더 나은 답**인 경우가 많다.
    표본 수에 선형이고 배치로 익힐 수 있어 자료가 기억 장치에 다 들어가지 않아도 된다.
...
알맹이 주성분 분석과 오토인코더는 둘 다 굽은 차원 줄이기다. 어떻게 고르는가?
...
??? success "연습문제 10 풀이"
    ...
    | | 알맹이 주성분 분석 | 오토인코더 |
    |---|---|---|
    | 답 | 닫힌 꼴. 하나로 정해진다 | 반복 최적화. 초기값에 딸린다 |
    | 표본 수 | $O(n^2)$ 기억, $O(n^3)$ 셈 | 표본 수에 선형 |
    | 되돌리기 | 앞그림 문제 | 디코더가 있다 |
    | 고를 것 | 알맹이와 $\gamma$ | 얼개, 학습률, 에포크... |
    | 표본이 적을 때 | 좋다 | 과적합하기 쉽다 |

    **표본이 적으면 알맹이 쪽**이다. 답이 하나로 정해지고 고를 것이 적으며, 수백 개
    표본으로도 잘 듣는다. 신경망은 그 크기에서 익히기 어렵다.

    **표본이 많으면 오토인코더**다. 알맹이 행렬이 감당이 안 되고, 되돌리기와 표본
    만들기가 필요하면 더욱 그렇다.

    이 책의 사다리가 그 순서로 놓인 까닭이기도 하다. 주성분 분석 → 알맹이 주성분 분석은
    선형에서 굽음으로 가는 한 걸음이고, 오토인코더는 같은 걸음을 **커지는 방식으로**
    다시 밟는다.

    한 가지 더 짚어 둘 만하다. 알맹이 주성분 분석은 알맹이를 **우리가 골라야** 하는데,
    오토인코더는 그 사상을 **자료에서 배운다.** 어느 굽음이 맞는지 모를 때 뒤쪽이 유리한
    까닭이며, 딸린 대가가 익히기의 번거로움이다.
...
| **오토인코더와 견주면** | 더 단순하나(익히기 없음) 규모를 키우기 어렵고 또렷한 디코더가 없다 |
