...
PyTorch는 깊은 배움 연구에서 으뜸가는 얼거리이며 실전 펼치기에도 점점 많이 쓰인다. 이 길잡이는 바탕 벽돌인 텐서 만들기, 셈하기, 꼴 바꾸기, GPU로 빠르게 하기, 저절로 미분하기를 들여오고, 기울기 내려가기로 익히는 온전한 선형 회귀 보기로 맺는다. PyTorch에서 주성분 분석, 오토인코더, 어떤 신경망 얼개든 다루기 앞서 이 밑감을 익혀야 한다.
...
??? success "연습문제 8 풀이"
    ...
    반복 최적화로 풀면 느리고 덜 정확하기만 하다. 실제로 선형 오토인코더를 기울기로
...
??? success "연습문제 10 풀이"
    ...
    쌓이는 것이 기본 동작인 까닭은 그것이 필요한 때가 있기 때문이다. 배치를 여러 조각으로
    나누어 기울기를 모으고 한 번에 갱신하는(gradient accumulation) 방식이 그렇다. 기억
    장치가 모자라 큰 배치를 한 번에 못 넣을 때 쓴다.
