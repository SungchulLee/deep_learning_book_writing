...
- [다음 장](../../ch25/index.md)이 무엇을 고치려는지 미리 알기
...
부호기와 풀개의 얼개, 병목의 크기, 익히기 예산을 모두 똑같이 두고 자기 부호기 하나와 [변분 자기 부호기](../../ch25/index.md) 하나를 익혔다. 병목은 16차원, 20 에포크, 씨앗 42로 고정했다. 두 모델의 **다른 점은 다음 장이 더하는 두 줄뿐**이다.
...
!!! warning "다시 세우기를 잘한다고 만들어 낼 수 있는 것이 아니다"
    ...
    자기 부호기는 앞의 것만 약속한다. 그래서 **압축기이지 만들어 내는 모델이 아니다.** 이 장이 [23장 차원 줄이기](../../ch23/index.md) 다음에 놓인 것도 그 때문이다. 주성분 분석이 하던 일을 비선형으로 넓혔을 뿐, 아직 만들어 내는 쪽으로는 한 걸음도 가지 않았다.
...
[다음 장](../../ch25/index.md)이 그 일을 두 가지 손질로 해낸다.
...
3절의 실험에서 자기 부호기의 숨은 코드에 **가우시안을 맞춘 뒤** 거기서 뽑으면 얼마나 나아지겠는가? 연습문제 4의 답을 수치로 확인해 보라.
...
??? success "연습문제 5 풀이"
    ...
    그러나 변분 자기 부호기에는 미치지 못한다. 까닭은 4절의 그림에 있다. 코드가
    **열 개의 덩이로 갈라져** 있지 가우시안 한 덩이가 아니다. 가우시안을 맞추면
    ...
    변분 자기 부호기와 다른 점은 하나다. 이쪽은 부호기를 익힌 **뒤에** 뒤따라가지만,
    변분 자기 부호기는 익히는 **동안** 코드를 모은다. 그래서 부호기 자체가 달라진다.
...
??? success "연습문제 6 풀이"
    **수로 나오기 때문이다.** 눈으로 보면 "얼룩 같다"는 인상은 얻지만 0.2%와 57.4%
    ...
    [28장](../../ch28/index.md)의 인셉션 점수와 프레셰 인셉션 거리가 그 일을 한다.
...
??? success "연습문제 7 풀이"
    2차원 코드로 잰 값이 4절의 표에 있다. 자기 부호기는 $|z| < 3$ 안에 **46.7%**가
    들어 있고 변분 자기 부호기는 **97.2%**다.

    16차원일 때보다 자기 부호기 쪽 사정이 낫다. 차원이 낮으면 코드가 퍼질 수 있는
...
다시 세우기 오차는 자기 부호기가 낮은데 뽑기는 변분 자기 부호기가 낫다. 이 둘을 하나의 잣대로 묶을 수 있는가?
...
??? success "연습문제 8 풀이"
    ...
    자기 부호기는 애초에 $p(x)$를 정의하지 않는다. 부호기와 풀개가 있을 뿐 어떤 확률
    모형도 세우지 않으므로, 가능도를 물을 수조차 없다. 그래서 다시 세우기 오차 말고는
    잴 것이 없다.

    변분 자기 부호기는 $p(x) = \int p(x \mid z) p(z)\, dz$라는 모형을 세운다. 그 적분이
    ...
    항**이며, 우리가 따로 재던 것들이 사실 같은 목표의 부분이었다. 자기 부호기는 첫
    항만 보고 둘째 항을 버린 꼴이다.

    이 유도가 [다음 장](../../ch25/index.md)의 출발점이다.
...
??? success "연습문제 9 풀이"
    ...
    같은 손실과 최적화기를 쓴 것도, [주성분 분석과 자기 부호기](../architecture/04_ae_principal_manifold.md)를
    같은 병목에서 견준 것도 마찬가지다.
...
자기 부호기로도 그럴듯한 표본을 얻는 방법이 정말 없는가? 숨은 공간을 건드리지 않고 할 수 있는 일을 생각해 보라.
...
??? success "연습문제 10 풀이"
    ...
    **코드에 모형 얹기.** 앞의 연습문제 5에서 본 방법이다. 가우시안 혼합이나 흐름
    ...
    셋 모두 공통된 한계가 있다. **부호기가 코드를 어떻게 흩어 놓았는지에 매여 있다는
    것**이다. 부호기가 덩이를 멀리 떼어 놓았으면 사이 끼움은 빈 곳을 지나고, 코드 분포가
    복잡하면 얹는 모형도 복잡해야 한다.

    변분 자기 부호기가 다른 점은 문제를 **뒤늦게 수습하지 않고 애초에 만들지 않는다**는
...
그러므로 자기 부호기는 **압축기이지 만들어 내는 모델이 아니다.** 23장의 주성분 분석을 비선형으로 넓힌 자리까지가 이 장의 몫이다.
