...
??? success "연습문제 5 풀이"
    ...
    | 새 자료도 변환해야 한다 | 주성분 분석, UMAP, 오토인코더 |
    | 거리만 있고 좌표가 없다 | MDS |
    | 전체 짜임과 거리 관계 | 주성분 분석 또는 MDS |
    | 되돌리기, 표본 만들기 | 오토인코더 계열 |
...
이 장의 방법들과 오토인코더는 무엇이 근본적으로 다른가?
...
??? success "연습문제 10 풀이"
    **사상을 배우는가**가 갈림길이다.
    ...
    - 오토인코더: 사상을 배우면서 **비선형**이다

    곧 오토인코더는 주성분 분석의 좋은 성질(사상을 배운다, 되돌린다, 커진다)을 지키면서
    선형이라는 제약만 푼 것이다. 그리고 실제로 그 제약을 풀어 얻는 것이 크다. 같은
    병목 16에서 어긋남이 0.02686에서 0.00926으로 준다
    ([26장](../../ch25/architecture/01_ae_fully_connected.md)).

    알맹이 주성분 분석도 굽음을 다루지만 앞그림 문제가 있고 표본 수에 $O(n^2)$이다
    ([알맹이 주성분 분석 연습문제 6](../pca/kernel_pca.md)). 오토인코더는 둘 다 낫다.

    이것이 **주성분 분석 → 오토인코더 → 변분 오토인코더 → 적대적 생성망** 사다리의
