...
흉내낸 담금질을 멈추지 않는 MCMC로 이해하면 왜 되는지(그 순간의 자세한 균형), 왜 모임이 느린지($T$이 낮으면 섞임 시간이 커짐), 왜 보장에 로그 식힘이 필요한지(거의 멈춘 듯한 과정), 표준 MCMC 방법과 어떻게 이어지는지가 뚜렷해진다.
