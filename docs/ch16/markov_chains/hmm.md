...
HMM은 마르코프 사슬 이론과 통계 추론을 잇는다. 곧 숨은 사슬이 시간의 짜임을 주고, 관측 모형은 관측 자료에서 숨은 상태를 되찾는 추론 문제를 만든다. 그래서 HMM은 더 일반적인 숨은 변수 모형의 추론을 다루는 MCMC 방법(16.3절)으로 가는 자연스러운 디딤돌이 된다.
...
숨은 공간이 이어져 있거나 차원이 높아지면 HMM의 정확한 동적 계획법 알고리즘을 더는 쓸 수 없고, 16.3절에서 다루는 MCMC 표집으로 돌아서야 한다.
