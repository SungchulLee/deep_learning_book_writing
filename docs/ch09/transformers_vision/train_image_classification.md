...
**연습문제 4.**
이미지 분류 학습 구현을 검증하는 두루 갖춘 시험 함수를 작성하라. 빈 입력, 원소가 하나인 입력, 아주 큰 입력, 극단적인 값(0, 아주 큰 수)을 가진 입력 같은 모서리 경우를 시험하라.

??? success "연습문제 4 풀이"
    경계 조건을 두루 시험하는 함수를 만든다.
    ```python
    def test_train image classification():
        model = Train Image Classification(...)
        # 보통의 입력
