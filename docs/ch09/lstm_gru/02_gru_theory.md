...
def demonstrate_gru_gates():
    ...
    plt.tight_layout()
    plt.savefig('gru_gates.png', dpi=150, bbox_inches='tight')
...
이 구현은 클래스 두 개(`GRUCell`, `GRU`)를 정의하며, 이들이 어우러져 완전한 순환 신경망 구조를 이룬다.

핵심 클래스는 `GRUCell`, `GRU`이며 앞의 연습문제 4개로 직접 확인할 수 있다.
