...
자기 지도 학습에서 불리기 파이프라인의 설계는 시각적 이해에 어떤 불변성이 중요한지에 관한 사전 지식을 담는다.

핵심 클래스는 `GaussianBlur`, `Solarization`, `SimCLRAugmentation`, `MoCoAugmentation`이며 앞의 연습문제 3개로 직접 확인할 수 있다.
