...
Output shape: torch.Size([32, 25, 10000])
Attention shape: torch.Size([32, 25, 20])
Total parameters: 57,565,969
```
...
이 구현은 클래스 두 개(`Seq2Seq`, `Seq2SeqAttention`)를 정의하며, 이들이 어우러져 완전한 순차열 모델 구조를 이룬다.

핵심 클래스는 `Seq2Seq`, `Seq2SeqAttention`이며 앞의 연습문제 4개로 직접 확인할 수 있다.
