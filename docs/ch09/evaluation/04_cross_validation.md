...
교차 검증은 어느 부분을 시험 집합으로 삼을지 체계적으로 돌려 가며 한 번의 학습-시험 분할보다 믿을 만한 성능 추정을 준다. 전략은 표준 K겹부터 층화, 시계열, 배치를 고려하는 변형까지 다양하며, 저마다 특정한 데이터의 성격과 표본 독립성에 대한 가정에 맞추어 설계되었다.
...
class CrossValidationDemo:
    ...
    def group_kfold_cv(X, y, groups, model, n_splits=5, scoring='accuracy'):
        ...
        과정:
        - 같은 배치의 표본이 학습과 시험에 함께 나오지 않게 한다
        - 개별 표본이 아니라 배치를 기준으로 나눈다
        
        쓸 때:
        - 데이터에 자연스러운 배치가 있을 때 (예: 환자, 회사, 실험)
        - 배치 안의 표본이 서로 독립이 아닐 때
        - 새 배치로의 일반화를 시험하고 싶을 때
...
배치 K겹은 같은 배치(예: 같은 환자나 같은 센서)의 표본이 학습 집합과 시험 집합에 함께 나타나지 않게 한다. 배치 안의 표본이 서로 상관되어 있을 때 표준 분할은 새 배치로의 일반화를 지나치게 좋게 보므로 이는 매우 중요하다.
