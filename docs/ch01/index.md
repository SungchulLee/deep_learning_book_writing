...
그리고 그 사실에는 쓸모 있는 뒷면이 있다. 일감이 작아 GPU가 논다면 **여러 개를 한꺼번에 돌리면 된다.** 이 책이 되풀이하는 씨앗 다섯 개짜리 실험이 바로 그런 일감이다.
...
[4.2절](../ch04/02_depth.md)의 실험을 M3에서 재어 본 값이다. 셋을 동시에 돌리면 각각은 1.6배 느려지지만 **셋을 한꺼번에 끝내므로 순 처리량이 두 배**가 된다. 다섯을 넘기면 다투기 시작해 도로 떨어진다.

**어디가 꼭대기인지는 기계마다 다르므로 재어 보아야 한다.** 다만 작은 모델을 여럿 학습시켜야 할 때 GPU 하나를 온전히 쓰는 길이 있다는 것, 그리고 무작정 늘리면 오히려 손해라는 것은 어느 기계에서나 같다.
...
PyTorch의 도구를 체계적으로 파고드는 것은 [6장](../ch06/index.md)이다. 이 장은 그 준비 운동이므로, 여기서 모든 것을 외우려 하지 않아도 된다. 뒤에서 막힐 때 돌아와 찾아보면 된다.
