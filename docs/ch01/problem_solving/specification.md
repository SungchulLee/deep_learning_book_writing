...
??? success "연습문제 2 풀이"
    MSE는 $\mathbb{E}[(y - \hat{y})^2]$을 최소화하며, 그 최적 예측은 조건부 평균 $\mathbb{E}[y|x]$이다. MAE는 $\mathbb{E}[|y - \hat{y}|]$을 측정하며, 그 최적 예측은 조건부 중앙값이다. 목표 분포가 치우쳐 있으면(예: 소득, 주택 가격) 평균과 중앙값이 다르므로, MSE로 학습한 모델은 MAE 기준으로 최적이 아닌 예측을 낸다. 해결: MAE를 직접 최적화하는 L1 손실(안정성을 위해 Huber 손실)으로 학습한다. 또는 중앙값을 목표로 하는 분위수 회귀 손실을 사용한다.
