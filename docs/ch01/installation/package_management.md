...
conda create -n dl python=3.10 numpy pandas pytorch -c pytorch

# Create from specification file
conda env create -f environment.yml

# Clone existing environment
conda create -n dl_backup --clone dl
```
...
```bash
# Full export (platform-specific, includes build strings)
conda env export > environment.yml

# Cross-platform export (package names only)
conda env export --from-history > environment.yml

# Explicit specification (exact reproducibility, same platform only)
