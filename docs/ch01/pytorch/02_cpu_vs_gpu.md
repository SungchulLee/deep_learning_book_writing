...
!!! warning "같은 기계에서 같은 코드를 돌려도 값이 흔들린다"
    [1.1절의 GPU 쪽](../hardware/02_gpu.md)에 같은 실험의 표가 있다. 같은 M3에서 같은 코드를 돌린 것인데 수가 어긋난다.
    ...
    2048에서 1.8배와 3.1배다. GPU 쪽 값이 9.63 ms와 6.27 ms로 절반 넘게 차이 난다. 기계가 그때 무엇을 하고 있었는지, 열이 얼마나 올랐는지, 메모리가 어떻게 잡혀 있었는지에 따라 이만큼 움직인다.

    **그러므로 이 표에서 읽어야 할 것은 배수의 값이 아니라 배수가 커지는 방향이다.** 작은 쪽에서 1보다 작고 큰 쪽에서 1보다 크다는 것은 두 표가 똑같이 말한다. "2048에서 3.1배"는 그날의 값일 뿐이다.

    이 책이 뒤에서 모든 정확도에 씨앗 다섯 개의 퍼짐을 함께 적는 까닭이 여기에 있다. **한 번 잰 수는 그 자체로는 무엇도 뜻하지 않는다.**
...
| 잰 결과 | 실무의 규칙 |
|---|---|
| 작은 일감에서 GPU가 느리다 | 배치를 너무 작게 잡지 마라 |
...
??? success "연습문제 1 풀이"
    ...
    실제 학습에서는 배치 크기와 층의 너비가 이 문턱을 넘도록 잡아야 GPU가 제 몫을 한다. 배치 8로 작은 모델을 학습시키면서 "GPU가 느리다"고 하는 경우가 흔한데, 그것은 GPU 탓이 아니다.
