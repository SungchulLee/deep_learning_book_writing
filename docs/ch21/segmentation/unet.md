...
- 건너뛰는 이음을 갖춘 U-넷의 대칭 인코더-디코더 얼개를 이해한다
...
이어 붙이기 방식의 건너뛰는 이음을 갖춘 대칭 인코더-디코더라는 이 우아한 풀이는 요즘 나누기 그물의 바탕이 되었다.
...
class DecoderBlock(nn.Module):
    """
    디코더 덩이: 키우기 → 이어 붙이기 → 겹 누비기
    
    자리 차원을 2배로 키우고 인코더 특징과 이어 붙인 뒤
    건너뛰는 이음으로 이어 붙인 뒤 겹 누비기를 쓴다.
    
    인수:
        in_channels: 들임 채널의 개수(앞 디코더 켜에서 옴)
        out_channels: 내놓는 채널의 개수
...
class UNet(nn.Module):
    ...
    이 그물은 오그라드는 길(인코더)과 부풀어 오르는
    길(디코더)로 이루어진다. 인코더는 맥락을 담아내고 디코더는 건너뛰는 이음으로
    정밀한 자리 잡기를 가능하게 한다.
    
    인수:
        in_channels: 들임 채널의 개수(잿빛은 1, RGB는 3)
        num_classes: 출력 클래스의 수
        base_features: 첫 인코더 켜의 특징 수(붙박이: 64)
    ...
    def __init__(self, in_channels: int = 3, num_classes: int = 2,
        ...
        # 병목(두 줄 사이 끼움 계수가 마지막 인코더에 영향을 준다)
...
$\mathbf{E}$을 인코더 결, $\mathbf{D}$을 촘촘하게 한 디코더 결이라 하자.
...
U-Net++는 인코더와 디코더 사이에 촘촘한 건너뛰는 이음을 들여온다:
...
class AttentionGate(nn.Module):
    ...
    def forward(self, gate: torch.Tensor, skip: torch.Tensor) -> torch.Tensor:
        """
        인수:
            gate: 디코더에서 온 문 신호(더 거친 잣수)
            skip: 인코더에서 온 건너뛰는 이음(더 고운 잣수)
        """
...
U-넷은 건너뛰는 이음을 위해 인코더 특징을 쌓아 두어야 하므로 기억 공간을 많이 쓸 수 있다:
...
??? success "연습문제 2 풀이"
    U-넷은 오그라드는 인코더 길(누비기와 모으기의 되풀이)과 부풀어 오르는 디코더 길(키우기와 누비기)로 이루어져 U 꼴을 이룬다. **건너뛰는 이음**은 인코더의 특징 지도를 그에 맞는 디코더 켜에 이어 붙인다. 인코더가 *무엇*(뜻 특징)을 담아내면서 *어디*(자리의 세밀함)를 잃기 때문에 이것이 결정적이다. 건너뛰는 이음은 정밀한 화소 수준 어림에 필요한 높은 해상도의 자리 앎을 주어, 디코더가 거친 뜻 앎과 고운 자리 세부를 아우르게 한다.
...
U-넷은 이어 붙이기 방식의 건너뛰는 이음을 갖춘 우아한 대칭 인코더-디코더 꾸밈으로 뜻 나누기, 특히 의료 영상의 판을 뒤집었다. 이 얼개는 높은 해상도의 인코더 특징과 뜻이 담긴 디코더 나타냄을 아울러 자리 잡기와 맥락 사이의 맞바꿈을 잘 다룬다.
