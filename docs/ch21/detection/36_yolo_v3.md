...
`ConvBlock`의 앞먹임을 따라가며 텐서 꼴을 좇아라. 붙박이 매개변수로 들임 표본 4개짜리 배치에 대해 주요 연산(누비기, 모으기, 선형 층)마다 그 뒤의 꼴을 적어라.
...
여기 짠 것은 함께 어울려 온전한 물체 알아내기 얼개를 이루는 클래스 3개(`ConvBlock`, `ResidualBlock`, `YOLOv3`)를 정한다.
