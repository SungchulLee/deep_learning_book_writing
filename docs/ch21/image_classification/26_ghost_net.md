...
`GhostModule`의 앞먹임을 따라가며 텐서 꼴을 좇아라. 붙박이 매개변수로 들임 표본 4개짜리 배치에 대해 주요 연산(누비기, 모으기, 선형 층)마다 그 뒤의 꼴을 적어라.
...
여기 짠 것은 함께 어울려 온전한 그림 가르기 얼개를 이루는 클래스 3개(`GhostModule`, `GhostBottleneck`, `GhostNet`)를 정한다.

고갱이 갈래는 `GhostModule`, `GhostBottleneck`, `GhostNet`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
