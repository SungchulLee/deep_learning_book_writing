...
??? success "연습문제 1 풀이"
    보통 칸은 들임의 자리 차원을 지키고(성큼 1), 줄임 칸은 자리 차원을 반으로 줄인다(성큼 2). 둘 다 얼개 찾기로 찾아낸 같은 속 이음 무늬를 갖지만 성큼 자리매김이 다르다. 흔한 NASNet은 보통 칸 배치와 줄임 칸을 번갈아 놓는다.
...
NASNet은 북돋움 배움으로 익힌 다스림 되돌이 그물로 가장 좋은 칸 얼개를 찾는다.

고갱이 갈래는 `NASNet`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
