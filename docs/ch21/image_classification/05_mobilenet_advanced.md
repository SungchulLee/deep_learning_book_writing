...
??? success "연습문제 2 풀이"
    ReLU는 음수를 0으로 뭉개므로 낮은 차원의 공간에서 앎을 잃게 한다. 내리쬐기가 채널을 좁은 병목으로 줄이므로 여기에 ReLU를 쓰면 되찾을 수 없는 앎이 사라진다. 선형 활성화는 낮은 차원 다양체의 나타내는 힘을 온전히 지키며, 지은이들은 이것이 더 나은 정확도로 이어짐을 보였다.
