...
??? success "연습문제 2 풀이"
    DenseNet은 앞선 층의 특징 지도를 모두 이어 붙이므로 덩이 안에서 가운데 활성화가 깊이에 선형으로 늘어난다. 층이 $L$개이고 자람 비율이 $k$인 덩이에서 마지막 층의 들임은 채널이 $k_0 + (L-1)k$개다. 이어 붙인 이 특징 지도를 뒤먹임을 위해 모두 기억 공간에 담아 두어야 한다. 반면 ResNet은 층마다 지금의 특징 지도(와 건너뛰는 이음의 들임)만 지닌다. 그래서 DenseNet은 무게 매개변수가 더 적어도 활성화 기억 공간을 더 쓴다.
