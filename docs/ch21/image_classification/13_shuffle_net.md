...
2017년 논문 "ShuffleNet: An Extremely Efficient CNN for Mobile Devices"에서 나온 ShuffleNet은 배치 누비기의 채널 사이로 앎이 흐르게 하는 채널 섞기 연산을 들여왔다. 덕분에 채널 배치가 서로 떨어져 생기는 나타냄의 병목 없이 그물 전체에서 배치 누비기를 쓸 수 있다.
...
배치 누비기는 채널을 서로 얽히지 않은 묶음으로 나누어 셈을 줄이지만 배치끼리 앎을 주고받지 못하게 한다. 채널 섞기 연산은 텐서를 (배치, 배치당 채널) 꼴로 바꾸고 뒤바꾼 뒤 다시 펴서 이를 푼다. 이 단순한 자리바꿈으로 뒤따르는 배치 누비기마다 앞 층의 모든 배치에서 들임을 받는다.
...
채널 섞기 연산을 한 단계씩 짜고, 서로 다른 배치의 채널이 올바로 엇갈리는지 확인하여라.
...
들임과 내놓음 채널이 같을 때 배치가 $g$개인 배치 누비기와 보통 누비기의 셈 값을 견주어라.
...
??? success "연습문제 3 풀이"
    ...
    배치 누비기는 매개변수를 $g$분의 1로 줄인다.

## 정리하며

**다룬 것** — ShuffleNet

배치 누비기는 채널을 서로 얽히지 않은 묶음으로 나누어 셈을 줄이지만 배치끼리 앎을 주고받지 못하게 한다.
