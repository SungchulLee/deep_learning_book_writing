...
이 단원은 깊은 만들어 내는 모델의 중요한 부품인 표본 그려 보기와 품질 따지기를 짠다. 이 짜기를 이해하면 요즘 만들어 내는 모델에 쓰이는 얼개의 결과 익히기 절차를 꿰뚫어 볼 수 있다. 이 코드는 연구와 실제 얼개에서 널리 쓰이는 쓸모 있는 재주를 보인다.
...
class SampleGridVisualizer:
    ...
    def create_image_grid(images: torch.Tensor,
        ...
        인수:
            images: 그림 배치 [batch_size, channels, height, width]
            nrow: 가로줄마다 그림 수
            padding: 그림 사이 화소
...
??? success "연습문제 4 풀이"
    ...
    ([27장](../../ch26/training/generate_samples.md)).
...
??? success "연습문제 5 풀이"
    ...
    배치를 격자로 그리면 64칸에 같은 그림이 여섯 번쯤 나오는데, 무심히 보면 알아채지
    못한다.
...
??? success "연습문제 6 풀이"
    ...
    **외우고 있지 않은가.** 사이 끼움이 학습 표본 사이를 매끄럽게 지나면 모델이 자료를
    ...
    [27장에서 재어 본](../../ch26/training/generate_samples.md) 대로 오토인코더와 변분
