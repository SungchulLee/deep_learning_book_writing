...
인셉션 점수(IS)는 만들어 내는 모델, 그 가운데서도 맞겨루기 생성기 그물(GAN)을 따지는 데 가장 널리 쓰이는 자의 하나다. Salimans 외(2016)가 내놓았으며, 만들어 낸 그림의 품질과 다양함을 한꺼번에 담아내는 스칼라 값 하나를 준다.
...
- $x$는 생성기 분포 $p_g$에서 뽑은, 만들어 낸 그림이다
...
이는 인셉션 판별기가 그림의 갈래를 얼마나 자신 있게 보는지를 나타낸다. 뾰족하게 솟은 분포는 판별기가 자신 있다는 뜻이며, 그림에 또렷이 알아볼 수 있는 물체가 있음을 넌지시 알려 준다.
...
가장자리 분포가 고르면 생성기가 여러 갈래를 두루 덮는 다양한 그림을 내놓는다는 뜻이다.
...
이는 생성기가 자신 있는 헤아림으로 내놓을 수 있는 **실질 갈래 수**로 읽을 수 있다.
...
class InceptionScoreCalculator:
    ...
    def get_predictions(self, 
        ...
        """
        그림 배치에 대한 인셉션 헤아림을 얻는다.
        
        인자:
            images: [0, 1] 범위의 만들어 낸 그림 [N, C, H, W]
            batch_size: 다룰 때 쓰는 배치 크기
...
??? success "연습문제 6 풀이"
    **부류 안의 다양성**을 전혀 보지 못한다. 이렇게 확인할 수 있다.
    ...
    부류마다 그림 한 장씩 열 장만 골라 100번 되풀이한 1,000개 배치를 만든다. 서로 다른
    ...
    IS가 9.164로 참 자료의 9.290과 **거의 같다.** 되풀이한 배치가 조건을 완벽히 만족하기
...
??? success "연습문제 7 풀이"
    ...
    2.146으로 나와 되풀이한 배치(9.418)보다 낮았는데, 그 이상한 결과가 버그를 알려 준
...
??? success "연습문제 9 풀이"
    ...
    **부류마다 가장 잘 나온 그림 하나씩만 내놓으면** 점수가 거의 최대가 된다. 앞에서 본
    ...
    [27장에서 겪은 온도 함정](../../ch26/training/generate_samples.md)과 같은 이야기다.
...
??? success "연습문제 10 풀이"
    ...
    [KL 항의 정보 이론적 읽기](../../ch26/theory/kl_term.md)와 같은 도구가 여기서도
    쓰인다는 점을 눈여겨볼 만하다.
