...
FID는 서로 다른 두 가지 잘못됨을 뒤섞는다.

| 잘못됨 | 설명 | FID의 반응 |
|--------------|-------------|--------------|
| **낮은 충실함** | 만든 표본이 가짜처럼 보인다 | FID가 는다 |
| **낮은 다양함** | 모델이 봉우리의 일부만 만든다 | FID가 는다 |
...
- **만든 다양체** $\mathcal{M}_g$: 생성기가 내놓는 분포의 받침
...
def compare_fid_and_pr():
    """
    정밀도와 재현율이 FID가 주지 못하는 통찰을 어떻게 주는지 보인다.
    """
    # FID는 같지만 정밀도와 재현율이 다른 모델 둘
...
만들어 내는 모델에서 정밀도와 재현율은 각각 무엇을 뜻하는가?

</div>

??? success "연습문제 5 풀이"
    가르기에서의 뜻과 다르다. 여기서는 **두 분포의 덮음**을 잰다.

    | | 뜻 | 낮으면 |
    |---|---|---|
    | 정밀도 | 만든 표본이 자료의 영역 안에 있는 비율 | 품질 문제. 얼룩을 낸다 |
    | 재현율 | 자료가 모델의 영역 안에 있는 비율 | 다양성 문제. 일부만 덮는다 |

    재는 방식은 특징 공간에서 각 표본의 $k$-최근접 이웃으로 영역을 어림하는 것이다. 만든
    표본이 참 자료 표본 어느 하나의 이웃 반지름 안에 들어가면 "자료의 영역 안"으로 센다.

    가르기의 정밀도·재현율과 이름만 같고 셈하는 것이 다르다는 점을 짚어 두어야 한다.
    표지가 없으므로 참양성 같은 것을 셀 수 없고, 대신 **집합의 덮음**을 센다.

    값어치는 FID가 못 하는 일에 있다. FID는 하나의 수라 왜 나쁜지 말해 주지 않는데
    ([FID 연습문제 16](fid.md)), 이 둘은 방향을 알려 준다.
...
FID가 같은 두 모델이 정밀도와 재현율에서 크게 다를 수 있는가?

</div>

??? success "연습문제 6 풀이"
    있다. 그것이 이 잣대를 쓰는 까닭이다.

    FID는 평균과 공분산의 어긋남을 하나의 수로 뭉친다. 그러므로 서로 다른 방식으로 어긋난
    두 모델이 같은 값을 낼 수 있다.

    | 모델 | 정밀도 | 재현율 | FID |
    |---|---|---|---|
    | 잘 만들지만 몇 가지만 | 높다 | 낮다 | 크다 |
    | 두루 만들지만 흐릿하다 | 낮다 | 높다 | 크다 |

    두 줄이 같은 FID를 낼 수 있는데 손볼 방향은 정반대다. 첫 줄은 무너짐을 다루어야 하고
    둘째 줄은 품질을 올려야 한다.

    이 두 갈래가 실제로 두 모델 계열의 성격과 맞아떨어진다고 이야기된다. 적대적 생성망은
    첫 줄 쪽으로, 변분 오토인코더는 둘째 줄 쪽으로 기운다. 변분 오토인코더가 흐릿한
    까닭은 가능도가 자료의 모든 점을 덮으라고 요구하기 때문이고
    ([가능도 잣대](02_likelihood_metrics.md)), 적대적 생성망은 그런 항이 없어 일부를
    버리고 날카로움을 얻을 수 있다.

    그러므로 두 계열을 FID 하나로 견주면 성격의 차이가 보이지 않는다.
...
$k$-최근접 이웃으로 영역을 어림할 때 $k$를 어떻게 고르는가?

</div>

??? success "연습문제 7 풀이"
    $k$가 영역의 크기를 정하므로 값에 크게 영향을 준다.

    - $k$가 작으면 영역이 좁아 정밀도와 재현율이 모두 낮게 나온다
    - $k$가 크면 영역이 넓어 둘 다 높게 나온다. 극단에서는 모든 것이 모든 영역 안이다

    관례로 $k = 3$을 쓴다. 원 논문이 그렇게 했고, 그보다 크게 두면 이상값 하나가 넓은
    영역을 만들어 버린다.

    이상값이 특히 고약하다. 참 자료에 외딴 표본이 하나 있으면 그 표본의 세 번째 이웃까지의
    거리가 크고, 그 큰 반지름의 공이 영역에 들어간다. 모델이 그 근처 아무 데나 내놓아도
    정밀도로 세어진다.

    다루는 방법으로 밀도를 함께 보는 변형이 나왔다. 이웃 하나만으로 세지 않고 몇 개의
    영역 안에 드는지 세어 이상값의 영향을 줄인다(density와 coverage).

    어느 쪽이든 **$k$와 표본 수를 밝혀야** 한다. 이 잣대의 값은 그 둘에 딸린 값이다.
...
정밀도가 1인데 쓸모없는 모델을 만들 수 있는가?

</div>

??? success "연습문제 8 풀이"
    만들 수 있다. **학습 표본 하나를 그대로 내놓으면** 정밀도가 1이다.

    그 표본이 자료의 영역 안에 확실히 있으므로 모든 표본이 세어진다. 그런데 재현율은
    거의 0이다. 자료의 나머지를 하나도 덮지 못한다.

    반대쪽도 만들 수 있다. 자료 전체를 덮도록 아주 넓게 퍼뜨리면 재현율이 높아지고
    정밀도가 떨어진다.

    그래서 **둘을 함께 보아야** 한다. 하나만 보면 어느 쪽이든 겨냥할 수 있다.
    ...
    셋째 줄이 근본적인 문제다. 외우기가 두 잣대 모두에서 만점이다. FID와 똑같은 약점이며
    ([FID 연습문제 19](fid.md)), 표본 기반 잣대가 공통으로 갖는 한계다.

    **외우기는 따로 확인해야 한다.** 만든 표본과 가장 가까운 학습 표본의 거리를 보는
    것이 기본이다([표본 그려 보기 연습문제 5](03_sample_visualization.md)).
...
??? success "연습문제 9 풀이"
    묶을 수 있고, 묶으면 이 잣대를 쓰는 까닭을 잃는다.

    조화 평균을 취하면 $F_1$ 꼴의 수가 나온다. 순서를 매기기에는 편하지만, 정밀도와
    재현율을 나눈 이유가 **진단**이었으므로 다시 뭉치면 FID와 같은 처지가 된다.

    실무에서 권할 만한 방식이 이렇다.

    - **순서를 매길 때**는 FID를 쓴다. 이미 관례이고 하나의 수다
    - **진단할 때**는 정밀도와 재현율을 따로 본다
    - 둘을 평면에 찍어 모델들을 놓아 보면 성격의 차이가 한눈에 보인다

    셋째가 특히 쓸 만하다. 가로축을 재현율, 세로축을 정밀도로 두고 모델마다 점을 찍으면
    적대적 생성망 계열과 변분 오토인코더 계열이 서로 다른 자리에 모이는 것이 보인다.

    곧 하나로 묶는 것이 틀린 것은 아니고 **무엇을 하려는지에 따라** 고를 일이다. 다만
    묶은 수만 적고 두 값을 버리지는 말아야 한다.
...
이 잣대를 재려면 표본이 몇 개나 필요한가?

</div>

??? success "연습문제 10 풀이"
    이웃을 찾는 방법이므로 표본 수에 딸리고, 보통 10,000개쯤을 쓴다.

    적으면 영역의 어림이 거칠어진다. 특징 공간이 128차원이나 2,048차원이므로 표본이
    적으면 서로 멀어 이웃 반지름이 크고, 영역이 실제보다 넓게 어림된다. 그러면 정밀도가
    부풀려진다.

    FID와 달리 쏠림의 방향이 한쪽으로 정해져 있다고 말하기는 어렵다. 정밀도는 부풀려지는
    쪽이고 재현율도 비슷하게 부풀려질 수 있어, 둘이 함께 높게 나오는 경향이 있다.

    셈하기 값도 문제가 된다. 모든 쌍의 거리를 재므로 $O(n^2)$이다. 10,000개씩이면 1억
    번의 거리 셈하기이고 128차원이면 감당할 만하지만, 2,048차원에서 50,000개면 무겁다.

    그래서 근사 최근접 이웃을 쓰거나 표본을 줄인다. 어느 쪽이든 **표본 수와 $k$를
    밝혀야** 한다는 것이 이 장의 다른 잣대들과 같다.
