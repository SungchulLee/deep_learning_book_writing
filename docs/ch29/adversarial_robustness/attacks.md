...
- 화소 하나의 최대 바뀜을 가둔다
- 흔한 배치: $[0, 1]$ 안의 그림에서 $\epsilon = 8/255$
...
부호 함수를 쓰면 흔들림이 $L_\infty$ 배치를 채우게 된다.

$$\|\delta\|_\infty = \epsilon$$

화소마다 꼭 $\pm\epsilon$만큼 흔들려 배치 안에서 흔들림을 가장 크게 한다.
...
대립 표본 공격과 적대적 생성망이 쓰는 도구가 왜 같은가?

</div>

??? success "연습문제 5 풀이"
    둘 다 **모델의 기울기를 타고 입력 쪽을 바꾼다.**

    | | 무엇을 최적화하는가 | 무엇을 고정하는가 |
    |---|---|---|
    | 대립 표본 공격 | 입력 $x + \delta$ | 모델 |
    | 생성기 갱신 | 생성기 매개변수 | 판별기 |

    둘 다 "이 모델을 속이는 방향"을 기울기로 찾는다. 생성기 갱신은 매 걸음 판별기에
    대한 공격이라고 볼 수 있다.

    그리고 방어도 같은 꼴이다. 대립 표본 익히기는 이 최소최대 문제를 푼다.

    $$\min_\theta \; \mathbb{E}\Big[ \max_{\|\delta\| \le \epsilon} \ell(f_\theta(x+\delta), y) \Big]$$

    안쪽 최대화가 공격, 바깥쪽 최소화가 방어다. 적대적 생성망의
    $\min_G \max_D$와 같은 구조이므로, 순환하고 불안정한 성질도 함께 나눈다
    ([내시 균형](../gan_foundations/nash.md)).

    그래서 두 주제가 한 장에 있다. 이름의 "적대적"이 우연이 아니다.
...
FGSM 공격을 식으로 적고 무엇을 가정하는지 밝혀라.

</div>

??? success "연습문제 6 풀이"
    한 걸음으로 손실을 키우는 방향으로 입력을 옮긴다.

    $$x_{\text{adv}} = x + \epsilon \cdot \operatorname{sign}\big(\nabla_x \ell(f(x), y)\big)$$

    부호만 쓰는 까닭은 $\ell_\infty$ 노름 제약 아래에서 그것이 최적이기 때문이다. 각
    화소를 허용된 최대만큼 옳은 방향으로 옮기는 것이다.

    가정이 둘 들어 있다.

    **기울기를 알 수 있다.** 곧 모델의 매개변수에 접근할 수 있는 흰 상자 상황이다. 검은
    상자라면 기울기를 어림해야 한다.

    **손실이 국소적으로 선형이다.** 한 걸음이므로 그 방향이 계속 좋다고 가정한다. 실제로는
    아니어서, 여러 걸음 나누어 가는 PGD가 더 세다.

    $\epsilon$이 제약의 크기이며, $[0,1]$ 화소에서 $8/255$ 같은 값을 흔히 쓴다. 사람
    눈에 거의 안 보이는 정도다.

    그리고 결과를 유효한 그림 범위로 자르는 것을 잊지 말아야 한다.
...
공격의 성공률을 보고할 때 무엇을 함께 적어야 하는가?

</div>

??? success "연습문제 7 풀이"
    공격의 세기를 정하는 것들을 모두 적어야 한다. 빠뜨리면 수를 견줄 수 없다.

    | 적을 것 | 왜 |
    |---|---|
    | 노름과 $\epsilon$ | 제약의 종류와 크기 |
    | 걸음 수와 걸음 크기 | PGD는 이것이 세기를 정한다 |
    | 겨냥한 공격인가 | 특정 부류로 보내는 것이 더 어렵다 |
    | 흰 상자인가 | 기울기를 아는지 |
    | 원래 정확도 | 기준점 |
    | 무작위 출발인지 | PGD의 관례 |

    다섯째가 흔히 빠진다. 공격 뒤 정확도가 10%라는 것이 원래 정확도가 99%였는지 40%였는지
    모르면 뜻이 약하다.

    그리고 **방어를 평가할 때** 특히 조심해야 한다. 약한 공격으로 재면 방어가 실제보다
    강해 보인다. 이 분야에서 여러 방어 방법이 나중에 더 센 공격에 뚫린 역사가 있다.

    그래서 방어를 주장하려면 여러 공격으로, 여러 $\epsilon$으로, 적응적 공격(그 방어를
    알고 설계한 공격)으로 재야 한다는 것이 요즘 관례다.

    값매김의 태도가 [이 장의 잣대들](../gan_evaluation/complete_evaluation_example.md)과
    같다. **잣대가 좋게 나왔다는 것이 좋다는 증명이 아니다.**
...
대립 표본이 존재하는 까닭을 어떻게 설명하는가?

</div>

??? success "연습문제 8 풀이"
    여러 설명이 제시되어 있고 하나로 정해지지 않았다.

    **높은 차원의 선형성.** 모델이 국소적으로 거의 선형이면, 차원이 높을 때 각 화소를
    조금씩 옮긴 것이 합쳐져 큰 변화를 만든다. 784차원에서 화소마다 $8/255$면 $\ell_2$
    노름으로는 작지 않다.

    **결정 경계가 자료 근처에 있다.** 자료 다양체가 얇고 경계가 그 근처를 지나면, 다양체를
    조금 벗어나기만 해도 경계를 넘는다.

    **쓸모 있지만 튼튼하지 않은 특징.** 모델이 실제로 예측에 도움이 되는 특징을 쓰는데,
    그 특징이 사람에게는 안 보이고 작은 변화에 민감하다는 설명이다. 그러면 대립 표본이
    버그가 아니라 **자료의 성질**이다.

    셋째가 특히 눈여겨볼 만하다. 이 설명이 맞으면 튼튼함과 정확도 사이에 맞바꿈이
    있어야 하고, 실제로 대립 표본 익히기가 깨끗한 자료의 정확도를 떨어뜨리는 것이
    관찰된다.

    이 장의 다른 주제와 이어지는 점이 있다. **모델이 쓰는 것과 우리가 뜻한 것이 다를 수
    있다**는 이야기이며, 잣대가 재는 것과 우리가 알고 싶은 것이 갈리는 문제와 같은
    꼴이다.
...
??? success "연습문제 9 풀이"
    ...
    **생성기가 섭동을 만든다.** 입력을 받아 $\delta$를 내놓는 그물을 익힌다. 판별기 자리에
    공격할 분류기를 두고, 분류기가 틀리도록 생성기를 익힌다. 한 번의 앞먹임으로 공격을
    만들 수 있어 빠르다.

    **다양체 위의 대립 표본.** 익힌 생성기의 숨은 공간에서 $z$를 옮겨 분류기를 속이는
    그림을 찾는다. 그러면 섭동이 화소 공간에서 작지 않아도 **그럴듯한 그림**이 나온다.
    $\ell_\infty$ 제약과 다른 종류의 위협이다.

    둘째가 개념적으로 더 재미있다. 보통의 대립 표본은 사람 눈에 원본과 같아 보이는데,
    이쪽은 원본과 달라 보이지만 **자연스러운** 그림이다. 곧 "작은 섭동"이라는 위협 모형
    자체가 좁다는 것을 보여 준다.

    방어 쪽으로도 쓰인다. 생성기로 자료를 늘려 익히거나, 입력을 생성기의 다양체 위로
    되사영해 섭동을 씻어 내는 방법이 있다.

    마지막 것은 조심해야 한다. 그런 방어들이 **기울기를 가려서** 강해 보이는 경우가
    많았고, 적응적 공격에 뚫렸다. 기울기를 셈하기 어렵게 만든 것과 실제로 튼튼한 것은
    다르다.
...
대립 표본 익히기의 값은 얼마나 드는가?

</div>

??? success "연습문제 10 풀이"
    안쪽 최대화를 풀어야 하므로 훨씬 비싸다.

    PGD를 $k$걸음으로 쓰면 배치마다 앞먹임과 뒤먹임이 $k$번 더 든다. $k=10$이면 익히기가
    **열 배쯤** 느려진다.

    | 방법 | 값 | 튼튼함 |
    |---|---|---|
    | 보통 익히기 | 1배 | 없다 |
    | FGSM 익히기 | 2배쯤 | 약하다. 기울기 가림이 생길 수 있다 |
    | PGD 익히기 ($k$걸음) | $k+1$배 | 낫다 |

    둘째 줄에 알려진 함정이 있다. 한 걸음 공격으로만 익히면 모델이 그 공격에만 튼튼해지고
    여러 걸음 공격에는 뚫린다. 곧 **약한 공격으로 익히면 약한 방어를 얻는다.**

    값을 줄이는 방법이 연구되어 있다. 걸음 수를 줄이면서 무작위 출발을 쓰는 방법,
    섭동을 배치 사이에 이어 쓰는 방법 같은 것들이다.

    그리고 값 말고 다른 대가가 있다. 깨끗한 자료의 정확도가 떨어진다. 튼튼함과 정확도의
    맞바꿈이며, 무엇이 중한지에 따라 고를 일이다.
