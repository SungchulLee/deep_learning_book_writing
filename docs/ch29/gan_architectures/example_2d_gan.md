...
2차원 장난감 자료 묶음으로 맞겨루기 생성기를 익히면 맞겨루기 익히기의 움직임에 대한 값진 직관을 얻는다. 두 차원에서 다루면 생성기가 자료 분포를 어떻게 어림해 가는지, 익히는 동안 판별기의 결정 가장자리가 어떻게 바뀌는지 곧바로 그려 볼 수 있다. 이 방식은 봉우리 무너짐, 익히기의 불안정, 두 신경망의 주고받음을 이해하는 데 더없이 값지다.
...
2차원 장난감 자료에서 맞겨루기 생성기 익히기를 그려 본다.
...
SimpleGenerator과 SimpleDiscriminator은 2차원 자료를 위해 만든 작은 여러 층 신경망이다. 생성기는 2차원 숨은 벡터를 ReLU 깨움을 갖춘 숨은 층으로 옮겨 2차원 내놓기 점을 만든다. 판별기는 2차원 점을 받아 LeakyReLU 깨움과 마지막 시그모이드로 확률을 내놓는다. 2차원 숨은 공간은 자료 차원과 맞아 옮김을 더 풀이하기 쉽게 한다.

익히기 되풀이는 여느 맞겨루기 생성기 익히기 절차를 짠다. 곧 먼저 두값 어긋 엔트로피 손실로 실제 자료와 가짜 자료 모두에서 판별기를 익히고, 이어 가짜 자료를 실제 이름표와 함께 판별기에 넣어 생성기를 익힌다. $\beta_1 = 0.5$인 Adam 가장 좋게 하개는 맞겨루기 생성기 익히기에 대한 DCGAN의 권고를 따른다. 그려 보기 함수는 실제 자료, 만든 자료, 판별기의 결정 가장자리를 보이는 세 칸을 만든다.

`generate_data` 함수는 반달, 겹동그라미, 두 정규 분포 섞기 같은 여러 장난감 자료 묶음을 받쳐 준다. 이 자료 묶음은 생성기 배움의 여러 면을 시험한다. 곧 달 모양은 비선형 다양체 배우기를, 동그라미는 돌림 대칭을, 정규 분포는 봉우리 덮기를 시험한다. 결정 가장자리 그림은 특히 알려 주는 바가 많아, 익히기가 나아가면서 판별기가 실제 표본과 만든 표본을 어떻게 갈라내는지 보여 준다.
...
세 자료 묶음(달 모양, 동그라미, 정규 분포)마다 2차원 맞겨루기 생성기를 1000바퀴 돌려라. 마지막 생성기 분포와 판별기 결정 가장자리를 견주어라. 어느 자료 묶음이 배우기 가장 어려운가? 왜인가?

</div>

??? success "연습문제 1 풀이"
    정규 분포 섞기는 떨어진 대칭 봉우리 둘로 이루어져 흔히 가장 쉽다. 달 모양 자료 묶음은 굽고 서로 끼워진 짜임 때문에 웬만큼 어렵다. 동그라미는 안쪽 동그라미가 바깥 동그라미에 온전히 둘러싸여 생성기가 서로 다른 반지름 둘에서 표본을 내야 하므로 가장 어렵기 쉽다. 동그라미의 판별기 결정 가장자리는 닫힌 고리 모양 자리를 이루어야 하는데, 이는 다른 자료 묶음에 필요한 선형이나 부드럽게 굽은 가장자리보다 복잡하다.
...
생성기를 한 번 고칠 때마다 판별기를 $k$번 고치도록 익히기 되풀이를 고쳐라($k = 1, 3, 5$을 시험하라). 이 비율이 만든 표본의 품질과 판별기 결정 가장자리의 매끄러움에 어떤 영향을 주는가?

</div>

??? success "연습문제 2 풀이"
    $k = 1$(기본)이면 판별기와 생성기가 균형을 이루지만 판별기가 좋은 기울기를 줄 만큼 세지 않을 수 있다. $k = 3$이면 판별기가 더 정확해져 생성기에 더 나은 기울기 신호를 주며 흔히 표본 품질이 나아진다. $k = 5$이면 판별기가 너무 세져 생성기의 기울기가 사라질 수 있다. 가장 좋은 비율은 자료 묶음의 복잡함에 달렸다. 단순한 2차원 자료 묶음에서는 흔히 $k = 1$이나 $k = 2$이 잘 듣고, $k$이 크면 결정 가장자리가 매끄러워지지만 생성기가 느리게 모일 수 있다.
...
짝마다 평균 거리로 만든 표본의 다양함을 재는 봉우리 무너짐 알아내개를 짜라. 이 잣대를 익히기 되풀이에 더하고 생성기와 판별기 손실과 함께 그려라.
...
??? success "연습문제 3 풀이"
    ...
    ```
    익히기 바퀴에 따라 이 잣대를 그려라. 짝마다 평균 거리가 갑자기 떨어지면 봉우리 무너짐이며, 생성기가 들임과 상관없이 거의 같은 것을 내놓는다는 뜻이다. 건강한 익히기에서는 이 잣대가 실제 자료의 짝 거리와 비슷한 값에서 안정된다.
...
??? success "연습문제 9 풀이"
    ...
    [27장의 변분 오토인코더](../../ch26/architecture/prior.md)에서 차원이 죽는 것과
