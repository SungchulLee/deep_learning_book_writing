...
??? success "연습문제 7 풀이"
    ...
    **다양성 부족.** 격자에 보이는 몇 장은 그럴듯한데 전체가 몇 가지뿐일 수 있다. 눈으로는
    ...
    셋째가 가장 실질적이다. 서로 다른 그림 열 장을 되풀이한 묶음이 인셉션 점수 9.164에
...
??? success "연습문제 8 풀이"
    부류 표지를 두 그물 모두에 넣는다. [26장의 조건부 변분 자기 부호기](../../ch26/architecture/conditional_vae.md)와
    같은 생각이다.
    ...
    자기 부호기에서 90.1%였던 그 잣대다. 조건부 적대적 생성망에서 같은 것을 재면 두
    모델을 그 축에서 견줄 수 있다.
...
이 예의 결과를 26장의 변분 자기 부호기와 나란히 놓을 때 조심할 점은 무엇인가?
...
??? success "연습문제 9 풀이"
    **같은 조건으로 재었는지**를 먼저 확인해야 한다.
    ...
    셋째와 넷째는 이 장에서 완전히 맞추지 못했다. 변분 자기 부호기는 20 에포크, 적대적
    ...
    재면 변분 자기 부호기가 이긴다. 잣대를 고르는 것이 곧 답을 고르는 일이 될 수 있다.
