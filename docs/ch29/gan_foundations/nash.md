# 내시 균형
맞겨루기 생성기 익히기는 놀이 이론의 눈으로 이해할 수 있다. **내시 균형** 개념은 맞겨루기 생성기가 언제 어떻게 모이는지 이해하는 이론의 바탕을 준다.
...
맞겨루기 생성기는 다음과 같은 두 사람 놀이를 뜻매김한다.

- **첫째 사람(생성기)**: 방책 $G$(매개변수 $\theta_G$)을 고른다
- **둘째 사람(판별기)**: 방책 $D$(매개변수 $\theta_D$)을 고른다
- **셈속 함수**: $V(D, G)$
...
**걸음 1**: 어떤 생성기 G에 대해서든 가장 좋은 판별기는 다음과 같다.
...
| 양 | 값 |
|----------|-------|
| 생성기 분포 | $p_g = p_{\text{data}}$ |
| 판별기 내놓기 | 모든 $x$에 대해 $D(x) = 0.5$ |
...
- 생성기가 흠 없는 표본을 낸다
- 판별기가 실제와 가짜를 가려내지 못한다
- 판별기의 가장 좋은 방책은 아무렇게나 찍기(50/50)이다
...
def ideal_gan_training(G, D, data_loader, n_iterations):
    ...
    for i in range(n_iterations):
        ...
        for _ in range(k_d):  # 판별기 걸음 k_d번
...
**명제**(Goodfellow 외, 2014): G과 D의 담이가 넉넉하고 걸음마다 판별기가 주어진 G에서 가장 좋은 자리에 이를 수 있으며 p_g이 다음 잣대를 높이도록 고쳐진다면,
...
def visualize_loss_landscape():
    """맞겨루기 생성기의 손실 지형은 복잡하며 볼록-오목이 아니다."""
    ...
    # 단순하게 만든 손실 지형(그림 삼아)
    # 실제 맞겨루기 생성기의 지형은 차원이 높고 더 복잡하다
...
안정되려면 J의 고윳값이 음의 실수부를 가져야 한다. 그러나 맞겨루기 생성기에서는:
...
볼록하지 않으므로 맞겨루기 생성기는 흔히 **가까운** 내시 균형을 찾는다.
...
맞겨루기 생성기 익히기 목표를 최소최대 놀이로 설명하라. 각 사람은 무엇을 가장 좋게 하는가?

</div>

??? success "연습문제 1 풀이"
    맞겨루기 생성기의 목표는 다음과 같다.

    $$\min_G \max_D \; \mathbb{E}_{x \sim p_{\text{data}}}[\log D(x)] + \mathbb{E}_{z \sim p_z}[\log(1 - D(G(z)))]$$

    **판별기** $D$은 이를 가장 크게 한다. 곧 실제 자료에는 $D(x) \to 1$, 가짜 자료에는 $D(G(z)) \to 0$을 바란다. **생성기** $G$은 가장 작게 한다. 곧 $D(G(z)) \to 1$(판별기 속이기)을 바란다. 내시 균형에서 $G$은 참 자료 분포에서 표본을 만들고 어디서나 $D(x) = 1/2$이다.
...
생성기가 붙박였을 때 가장 좋은 판별기가 $D^*(x) = \frac{p_{\text{data}}(x)}{p_{\text{data}}(x) + p_g(x)}$임을 밝혀라.

</div>

??? success "연습문제 2 풀이"
    만든 분포가 $p_g$인 붙박인 $G$에서 판별기는 다음을 가장 크게 한다.
...
맞겨루기 생성기 익히기에서 봉우리 무너짐이란 무엇인가? 이를 누그러뜨리는 방식 둘을 설명하라.

</div>

??? success "연습문제 3 풀이"
    봉우리 무너짐은 생성기가 자료 분포의 봉우리 가운데 일부에서만 표본을 내고 나머지를 무시할 때 일어난다. 예컨대 숫자로 익힌 맞겨루기 생성기가 "1"과 "7"만 만들 수 있다. **누그러뜨리는 방식**: (1) **작은 배치 가름**: 판별기가 배치 전체의 통계를 받아 다양함이 모자라면 벌을 준다. (2) **바서슈타인 맞겨루기 생성기(WGAN)**: 젠슨-섀넌 벌어짐 대신 바서슈타인 거리를 써서 분포가 겹치지 않아도 0이 아닌 기울기를 주어 익히기를 안정시키고 봉우리 무너짐을 줄인다. 그 밖에 펼친 맞겨루기 생성기, 스펙트럼 고르게 맞추기, 차츰 키우기가 있다.
...
판별기가 너무 셀 때 본디 맞겨루기 생성기 손실의 기울기가 사라질 수 있는 까닭을 설명하라.

</div>

??? success "연습문제 4 풀이"
    판별기가 생성기보다 훨씬 나으면 만든 표본에서 $D(G(z)) \approx 0$이다. $\log(1 - D(G(z)))$에서 오는 생성기의 기울기는 $\frac{-D'(G(z))}{1 - D(G(z))}$이며 $D(G(z)) \approx 0$일 때 $-D'(G(z))$(작다)에 가까워진다. 더 결정적으로 손실 $\log(1 - D(G(z)))$이 $\log(1) = 0$ 가까이에서 포화해 기울기가 거의 0이 된다. 실제의 손질은 **포화하지 않는 손실**이다. 곧 $\log(1 - D(G(z)))$을 가장 작게 하는 대신 생성기가 $\log D(G(z))$을 가장 크게 하며, 이는 $D(G(z))$이 작을 때 센 기울기를 지닌다.
...
??? success "연습문제 7 풀이"
    ...
    | 방법 | 생각 |
    |---|---|
    | 배치 안의 다양성을 판별기가 보게 | 한 점으로 모이면 들킨다 |
    | 표지 매끄럽게 | 판별기가 지나치게 확신하지 못하게 |
    | 판별기에 기억 | 지나간 거짓 표본도 기억한다 |
    | 바서슈타인 거리 | 퍼짐을 요구하는 기울기를 준다 |

    첫째가 직접적이다. 판별기가 표본 하나가 아니라 배치를 보면 다양성 부족 자체를 벌할
    수 있다.
...
| **맞겨루기 생성기의 균형** | $p_g = p_{\text{data}}$, $D^* = 0.5$ |
| **모임** | 가장 좋은 조건에서는 이론으로 보장된다 |
| **실제의 문제** | 볼록하지 않음, 흔들림, 유한한 담이 |
| **안정시키기** | TTUR, 스펙트럼 잣대, 기울기 벌점 |
| **지켜보기** | D(실제)와 D(가짜)가 0.5으로 가는지 좇는다 |

내시 균형을 아는 것은 맞겨루기 생성기 익히기의 이론 바탕이 되고 실제의 개선을 이끌어 낸다. 완벽히 모이기는 어렵지만 이 통찰이 더 안정된 익히기 알고리즘을 만드는 길잡이가 된다.
