...
A backtesting framework is essential for evaluating trading strategies before deployment. It simulates strategy execution on historical data while accounting for realistic market conditions including transaction costs, slippage, and position drift. This systematic approach separates genuine strategy alpha from artifacts of overfitting or unrealistic assumptions.
...
The backtesting engine at the core of this framework processes historical price data sequentially, maintaining a portfolio tracker that records positions, weights, and cumulative returns. At each time step, the engine queries a strategy function for new target weights, then simulates execution including transaction costs proportional to turnover and random slippage. This realistic execution model is crucial: strategies that appear profitable without costs often become unprofitable when realistic frictions are included.

The portfolio tracker handles weight drift between rebalancing periods. After returns are realized, portfolio weights shift away from their target values as different assets appreciate or depreciate at different rates. The tracker computes drifted weights and records the full history of values, returns, weights, and costs, providing comprehensive data for post-hoc analysis.

Three benchmark strategies illustrate the framework: equal-weight rebalancing, buy-and-hold, and momentum. Equal-weight maintains constant allocation across assets, buy-and-hold avoids rebalancing after initial allocation, and momentum tilts toward recent winners. Comparing RL-trained strategies against these benchmarks is essential for establishing whether learned policies add genuine value beyond simple heuristics.

## Exercises

**Exercise 1.**
Implement a mean-variance optimization strategy within the backtesting framework. At each rebalancing step, compute the optimal weights using a rolling window of 60 days of returns and a risk aversion parameter $\gamma = 1$.

??? success "Solution to Exercise 1"
    The mean-variance strategy computes weights $w^* = \frac{1}{\gamma} \Sigma^{-1} \mu$, where $\mu$ is the rolling mean return vector and $\Sigma$ is the rolling covariance matrix. Regularize $\Sigma$ by adding $\lambda I$ (e.g., $\lambda = 0.01$) to handle near-singular cases:
...
**Exercise 2.**
Explain why slippage modeling matters for high-frequency rebalancing strategies. How does the square root of turnover in the slippage model relate to market microstructure?

??? success "Solution to Exercise 2"
    Slippage arises because executing trades moves the market price against the trader. The square root relationship $\text{slippage} \propto \sigma \sqrt{\text{turnover}}$ reflects the empirical finding in market microstructure that market impact grows sublinearly with trade size. This is because larger orders must be broken into smaller pieces, each of which moves the price incrementally. For high-frequency strategies with large turnover, slippage can dominate transaction costs and erode profits. A strategy with 200% daily turnover pays roughly $\sqrt{2} \approx 1.41$ times the slippage of a strategy with 100% turnover per unit traded.

---


**Exercise 3.**
Add a maximum drawdown constraint to the backtesting engine that automatically reduces position sizes by 50% when the portfolio drawdown exceeds 5%, and flattens all positions when drawdown exceeds 10%.
    ...
    class DrawdownAwareEngine(BacktestEngine):
        def run(self, strategy):
            ...
            for t in range(self.num_steps):
                ...
                if dd > 0.10:
                    new_weights = np.zeros(self.num_assets)  # Flatten
                elif dd > 0.05:
                    new_weights = new_weights * 0.5  # Reduce by 50%
                
                tracker.update(new_weights, self.prices[t], self.prices[t + 1],
                             self.config.transaction_cost, self.config.slippage_std)
            return tracker
    ```
    This implements a simple risk management overlay that reduces exposure as losses accumulate, preventing catastrophic drawdowns while allowing recovery once drawdown diminishes.
