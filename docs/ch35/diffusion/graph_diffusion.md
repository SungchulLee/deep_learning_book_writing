...
그래프의 확산 모델은 잡소리 없애기 확산 틀을 넓혀 그래프 얼개 자료를 만든다. 핵심 생각은 그래프를 차츰 아무 잡소리로 흐트러뜨리는 앞으로 가는 과정을 뜻매김하고 이를 걸음마다 되돌리도록 신경망을 익히는 것이다. 자기 되돌이나 한 번에 만들기와 달리 확산 모델은 되풀이 다듬기로 그래프를 만든다. 순수한 잡소리에서 시작해 차츰 잡소리를 없애 올바른 그래프 얼개로 간다. 이 틀은 자리바꿈에 안 바뀜을 자연스럽게 다루고 품질 좋은 표본을 내며 분자와 일반 그래프 만들기 잣대에서 가장 앞선 결과를 세웠다.

---

## 1. 그래프 확산의 어려움

확산을 그래프에 맞추면 이어진 마당(그림, 소리)에서 겪는 것 말고도 남다른 어려움이 생긴다:

**띄엄띄엄한 얼개.** 이웃 행렬은 두 값이다. 변이 있거나 없다. 여느 가우스 확산은 이어진 자료를 여기므로 띄엄띄엄 확산으로 적거나 그래프를 이어진 값으로 느슨히 해야 한다.

**함께 만들기.** 그래프는 마디 갈래 $\mathbf{X}$, 변 갈래 $\mathbf{E}$, 어쩌면 변 무게처럼 서로 얽힌 조각 여럿으로 이루어지며 이들을 아귀가 맞게 만들어야 한다. 확산 과정은 이 조각들을 함께 흐트러뜨리고 함께 잡소리를 없애야 한다.
...
한 셈속은 띄엄띄엄한 이웃 행렬을 이어진 값으로 느슨히 하고 여느 가우스 확산을 쓴다.
...
다른 길은 갈래 확산을 써서 띄엄띄엄한 그래프 얼개에서 곧바로 돈다.
...
- **마디 특징**: 잡소리 섞인 마디 갈래를 사인 꼴 때 임베딩과 이어 붙인다
- **변 특징**: 잡소리 섞인 변 갈래를 어텐션에서 변 속성으로 다룬다
- **온 자리 조건 주기**: FiLM 조건 주기나 더하기 치우침으로 때 임베딩을 모든 층에 더한다
...
띄엄띄엄 확산에서 일정은 갈래 변수가 얼마나 빨리 고른 분포에 가까워지는지 다스린다. 빠른 일정은 익히기 효율을 높이지만 잡소리 없애기를 너무 어렵게 할 수 있다.

---

## 6. 금융 쓰임새: 확산 바탕 그물 상황

확산 모델은 다음으로 다채롭고 품질 좋은 금융 그물 상황을 만들 수 있다. (1) 지난 그물 스냅숏으로 익히기, (2) 배운 분포에서 뽑아 그럴듯한 다른 짜임 내놓기, (3) 확산의 숨은 자리를 거쳐 기존 그물 사이를 사이 값으로 잇기. 되풀이 다듬기 과정은 자연스럽게 온 자리에서 아귀가 맞는 얼개를 내어, 자기 되돌이와 한 번에 만들기에 흔한 그 자리의 어긋남을 피한다.
...
```python
"""
그래프 확산 모델: 이어진 판과 띄엄띄엄한 판의 앞으로/거꾸로 가는 과정.
"""
import torch
...
이 마당은 그래프 확산의 어려움, 이어진 값으로 느슨히 하는 길, 띄엄띄엄 확산의 길, 잡소리 없애는 그물의 얼개를 차례로 짚었다.
