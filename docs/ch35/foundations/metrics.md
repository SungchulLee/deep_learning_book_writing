...
그래프 만들기 모델을 따지려면 만들어 낸 그래프의 분포 $\{G_i^{\text{gen}}\}$을 잣대 분포 $\{G_i^{\text{ref}}\}$과 견주어야 한다. FID가 표준 잣대를 주는 그림 만들기와 달리, 그래프 따지기는 얼개, 통계, 마당 특유의 성질을 담는 서로 보완하는 잣대 여럿을 요구한다. 잣대 하나로는 모자란다. 생성기가 차수 분포는 맞지만 뭉침 무늬가 틀린 그래프를 낼 수도, 그 반대일 수도 있다.
...
이 마당은 그래프 통계, 분포 견주기 잣대, 올바름과 하나뿐임, 짜기: 따지기 꾸러미를 차례로 짚었다.
