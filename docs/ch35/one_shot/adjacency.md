...
**자리바꿈에 같이 바뀌는 얼개.** $\mathbf{z}$(또는 짜임 있는 판)의 자리를 바꾸면 내놓기도 그에 맞게 자리가 바뀌도록 디코더를 설계한다. 이로써 모델이 같은 그래프의 모든 나타냄에 같은 확률을 준다.
...
**크기에 조건 걸기.** 겪어 얻은 크기 분포에서 그래프 크기 $n \sim p(n)$을 뽑고 $n$에 매어 만든다. 디코더에 $n$을 들임으로 더 주어 짤 수 있다.
...
## 7. 짜기: 한 번에 만드는 이웃 행렬 생성기

```python
"""
여러 디코더 얼개로 하는 한 번에 이웃 행렬 만들기.
"""
import torch
...
이 마당은 적기, 자리바꿈에 안 바뀜의 어려움, 디코더 얼개, 크기 다루기를 차례로 짚었다.
