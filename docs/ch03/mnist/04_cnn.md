...
!!! note "이 얼개는 고양이의 뇌에서 왔다"
    ...
    다만 **이 이야기를 이 장의 결과로 받아들이면 안 된다.** "첫 층의 필터는 무엇을 보는가" 절에서 학습이 끝난 필터를 실제로 재어 보면, MNIST에서는 무작위로 깔아 둔 필터와 거의 구별되지 않는다. 모서리 검출기가 뚜렷이 나타나는 것은 자연 사진을 학습할 때이며, 그 비교는 [4.3절](../../ch04/03_vgg16_transfer.md)에 있다.
...
3단계의 97.42%에서 **99.22%**로 올랐다. 남은 오차의 **70%**가 사라진 셈이다(2.58%p → 0.78%p).

### 매개변수는 어디에 있는가

네 층이 저마다 몇 개를 배우는지 세어 보면 뜻밖의 것이 나온다.

| 층 | 배우는 것 | 셈 | 개수 |
|---|---|---|---|
| `conv1` | 3×3 필터 32장 | $3 \times 3 \times 1 \times 32 + 32$ | 320 |
| `conv2` | 3×3 필터 64장 (입력 채널 32) | $3 \times 3 \times 32 \times 64 + 64$ | 18,496 |
| `fc1` | 3136 → 128 | $3136 \times 128 + 128$ | **401,536** |
| `fc2` | 128 → 10 | $128 \times 10 + 10$ | 1,290 |
| | | | **421,642** |

**합성곱 두 층이 전체의 4.5%뿐이다.** 나머지 95.5%가 뒤의 완전 연결층에 있고, 그중에서도 `fc1` 하나가 401,536개로 전체의 95%다.

이 절이 더한 생각은 **이웃 관계**이고 그것을 하는 것은 합성곱인데, 매개변수로 보면 합성곱은 스무 분의 일도 안 된다. **무엇을 배우느냐와 얼마나 많이 배우느냐가 따로 논다.** 18,816개짜리 합성곱이 [3걸음 MLP](03_mlp.md)의 약 십만 개보다 1.8%포인트를 더 번다.

까닭은 합성곱이 **같은 필터를 온 그림에 되풀이해 쓰기** 때문이다. 3×3 필터 한 장은 매개변수 아홉 개인데 28×28 자리 모두에 걸린다. 완전 연결층이라면 자리마다 다른 가중치를 두어야 하므로 784배가 든다.

!!! note "필터를 5×5로 키우면 매개변수가 얼마나 느는가"
    합성곱 쪽만 보면 크게 는다.

    | | conv1 + conv2 | 전체 |
    |---|---|---|
    | 3×3 | 18,816 | 421,642 |
    | 5×5 | **52,096** (2.77배) | 454,922 (**1.08배**) |

    **합성곱은 2.77배가 되는데 모델 전체로는 8%밖에 늘지 않는다.** `fc1`의 401,536개가 그대로 남아 분모를 누르기 때문이다.

    다음 절이 필터 크기를 바꾸어도 정확도가 움직이지 않는 것을 보이는데, 까닭이 하나 더 있는 셈이다. 남은 오차가 없기도 하거니와, **바꾸는 것이 모델의 8%뿐**이다.
...
3단계와 4단계 사이에서는 매개변수가 네 배로 늘지만 정확도는 1.8%포인트만 오른다. 하지만 그 1.8%포인트가 남은 오차의 **70%**라는 점이 중요하다. 정확도가 높아질수록 한 걸음의 값어치는 남은 오차로 재야 한다.
...
!!! warning "`model.train()`과 `model.eval()`을 빠뜨리면"
    드롭아웃은 **학습할 때와 시험할 때 다르게 움직인다.** 학습할 때는 뉴런을 꺼뜨리고, 시험할 때는 하나도 끄지 않는다. 그물이 지금 어느 쪽인지를 아는 방법은 하나뿐이다. 우리가 말해 주는 것이다.

    ```python
    model.train()   # 학습 단계 — 드롭아웃이 켜진다
    model.eval()    # 시험 단계 — 드롭아웃이 꺼진다
    ```

    위 코드가 에포크마다 이 둘을 번갈아 부르는 까닭이 그것이다.

    **빠뜨려도 오류가 나지 않는다.** 그래서 잡기 어렵다. `model.eval()`을 잊고 시험하면 뉴런 4분의 1이 꺼진 채로 재게 되어 정확도가 까닭 없이 낮게 나오고, 게다가 어느 뉴런이 꺼지는지가 무작위라 **돌릴 때마다 값이 달라진다.** 시험 정확도가 재현되지 않으면 이것부터 의심할 일이다.

    배치 정규화도 같은 성질을 지닌다([9장](../../ch08/index.md)). 학습할 때는 배치의 평균과 분산을 쓰고 시험할 때는 쌓아 둔 값을 쓴다. 그래서 이 두 줄은 드롭아웃이 없는 그물에서도 습관으로 적어 두는 편이 낫다.
...
!!! note "그런데도 왜 두는가"
    ...
    두는 까닭은 둘이다. 하나는 이것이 실제로 쓰이는 짜임이고, 9장과 10장이 정칙화를 본격적으로 다룰 때 돌아올 자리이기 때문이다. 다른 하나는 이 구조가 **책의 다른 곳에 걸려 있기** 때문이다. [4.1절](../../ch04/01_two_ladders.md)의 사다리와 [5장의 심판 CNN](../../ch05/dimreduction/index.md)이 같은 구조를 같은 규약으로 쓰므로, 여기서 바꾸면 그 수들이 모두 어긋난다.
...
!!! note "팽창(dilation)이란"
    필터의 칸 **사이를 벌리는** 것이다. 팽창 2를 준 3×3 필터는 한 칸씩 건너뛰며 보므로, 매개변수는 아홉 개 그대로인데 **보는 넓이는 5×5**가 된다.
    ...
    곧 **넓이를 매개변수 없이 사는** 수법이다. 다만 사이를 건너뛰므로 그 안의 화소는 보지 못한다. 28×28짜리 숫자처럼 작고 촘촘한 그림에서는 건너뛴 자리가 아까운 쪽이고, 아래 표에서 팽창이 벌지 못하는 까닭도 거기에 있을 것이다.
...
!!! warning "첫 줄의 99.07%와 이 절이 내내 말한 99.22%는 같은 모델이다"
    표의 첫 줄이 **이 절의 모델**인데 99.07%로 적혀 있다. 2절의 출력은 99.22%였다.

    2절의 값은 씨를 묶지 않고 **한 번** 돌린 것이고, 여기 값은 씨 0~4로 **다섯 번** 돌려 평균한 것이다. 그리고 다섯 번의 범위가 98.97\~99.17이므로 **99.22%는 그 범위 위로 벗어나 있다.**

    곧 이 절이 표지로 내건 수가 같은 모델을 다섯 번 재어 얻은 띠 밖에 있다. 잘못 잰 것이 아니라 **한 번 잰 수가 하는 일이 원래 그렇다.** 3장이 걸음마다 한 번씩만 재어 사다리를 세웠으므로, 그 사다리의 수들은 모두 이만큼의 여유를 두고 읽어야 한다.

    [4.1절](../../ch04/01_two_ladders.md)이 같은 모델을 또 한 번 다섯 씨로 재는데, 거기서는 99.02%에 98.97\~99.30이 나온다. 이 절의 98.97\~99.17과 **띠는 겹치되 똑같지는 않다.** 다섯 번 재는 일조차 다시 하면 조금 달라진다.
...
씨가 정하는 것이 둘이다. **처음 가중치**와 **자료를 섞는 차례**다. 둘 다 학습이 시작되는 자리와 밟아 가는 길을 바꾼다.

손실면은 골짜기가 하나뿐인 그릇이 아니다. 울퉁불퉁한 산줄기이고 골짜기가 여럿이다. 경사 하강법은 서 있는 자리에서 아래로만 가므로, **어느 골짜기에 떨어지는지는 어디서 출발했는지에 딸려 있다.**

그러므로 씨가 다른 다섯 벌은 같은 모델을 다섯 번 잰 것이 아니라 **서로 다른 골짜기에 내려앉은 다섯 개의 모델**이다. 99.17%와 98.97%는 재기가 흔들린 것이 아니라 정말로 다른 그물이다.

!!! warning "씨 맞추기 — 해서는 안 되는 일"
    여기서 솔깃한 생각이 하나 든다. 씨를 여럿 돌려 보고 **가장 좋은 것을 골라 적으면** 어떨까? 위 표에서 3×3의 씨 0을 고르면 99.17%다.

    이것을 **씨 맞추기**라 부르고, 하면 안 된다. 고른 씨가 시험 자료에서 가장 좋았다는 것은 **그 시험 자료에 맞춘 것**이지 모델이 좋다는 뜻이 아니다. 새 자료에서는 그 씨가 특별할 까닭이 없다.

    학습률이나 층 수를 고르는 것과 다르다. 그런 손잡이는 **모델을 바꾸지만** 씨는 바꾸지 않는다. 씨를 고르는 것은 같은 모델을 여러 번 돌려 운 좋은 한 번을 집는 일이며, 자료를 늘리지도 모델을 낫게 하지도 않는다.

    **씨를 고정하는 것과 고르는 것은 다르다.** 고정은 남이 같은 수를 얻게 하려는 것이고(되풀이됨), 고르는 것은 수를 좋아 보이게 하려는 것이다. 이 책은 고정하되 **퍼짐을 함께 적는다.** 고정만 하면 그 값이 분포의 어디쯤인지 알 수 없기 때문이다.

이 절의 표가 평균과 퍼짐을 함께 싣는 까닭이 그것이다. 평균은 골짜기들의 가운데를 말하고, 퍼짐은 **골짜기들이 얼마나 흩어져 있는지**를 말한다. 둘 가운데 하나만으로는 다음 걸음을 정할 수 없다.
...
작은 필터를 모든 자리에서 공유하며 미끄러뜨리는 것만으로 97.42%에서 99.22%로 오른다. 남은 오차의 **70%**가 사라진 셈이다.
