...
- **출력이 작아졌다.** $5 \times 5$에서 $3 \times 3$이 되었다. $3 \times 3$ 창을 온전히 얹을 수 있는 자리가 세 곳씩뿐이기 때문이며, 일반적으로 $H \to H - k + 1$이다. 코드에서 `padding=1`을 주는 까닭이 이것이다. 테두리를 0으로 한 줄 둘러 주면 크기가 그대로 유지된다.
...
| | 입력 | 필터 | 여백 | 출력 |
|---|---|---|---|---|
| 한 장 | $(1, 5, 5)$ | $(1, 3, 3)$ 한 장 | 없음 | $(\mathbf{1}, 3, 3)$ |
| 네 장 | $(1, 5, 5)$ | $(1, 3, 3)$ 네 장 | 없음 | $(\mathbf{4}, 3, 3)$ |
| 코드의 `conv1` | $(1, 28, 28)$ | $(1, 3, 3)$ 32장 | `padding=1` | $(\mathbf{32}, 28, 28)$ |

곧 **출력 채널 수는 필터의 개수와 같다.** `nn.Conv2d(1, 32, 3)`의 32가 바로 이 수다.

세 줄에서 가로세로가 서로 다르게 움직이는 것은 여백 때문이다. 위의 두 줄은 여백을 주지 않아 $5 \to 5 - 3 + 1 = 3$으로 줄었고, `conv1`은 `padding=1`로 테두리를 한 줄 둘러 $28 \to 28 + 2 - 3 + 1 = 28$로 그대로 남았다. 일반적으로 여백을 $p$라 하면

$$
H' = H + 2p - k + 1
$$

이며, 홀수 $k$에 $p = (k-1)/2$를 주면 언제나 $H' = H$가 된다. 곧 **채널 수는 필터의 개수가 정하고, 가로세로는 여백이 정한다.** 둘은 서로 아무 상관이 없다.
