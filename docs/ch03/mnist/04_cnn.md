...
!!! note "이 얼개는 고양이의 뇌에서 왔다"
    ...
    다만 **이 이야기를 이 장의 결과로 받아들이면 안 된다.** 4절에서 학습이 끝난 필터를 실제로 재어 보면, MNIST에서는 무작위로 깔아 둔 필터와 거의 구별되지 않는다. 모서리 검출기가 뚜렷이 나타나는 것은 자연 사진을 학습할 때이며, 그 비교는 [CIFAR-10으로 다시 보는 장](../../cifar10/02_vgg16_transfer.md)에 있다.
...
그렇다면 실제로는 무엇을 배우는가? 손으로 고른 것과 닮은 모서리 검출기가 나올까? 4절이 학습이 끝난 `conv1`을 열어 재어 본다.
...
완전 연결층 앞에 두는 `nn.Dropout(0.25)`은 학습 동안 뉴런의 4분의 1을 무작위로 0으로 만들어, 어느 한 뉴런에도 기대지 않는 여벌 있는 표현을 배우게 한다. 다만 5 에포크라는 짧은 학습에서는 과적합이 아직 본격적으로 시작되지 않으므로 이 장치가 벌어 주는 양도 작다. 실제로 빼고 재어 보면 0.59%포인트 차이뿐이다(연습문제 7). 정칙화의 값어치는 언제나 "얼마나 과적합할 상황인가"에 상대적이라는 점을 기억해 두면 좋다.
...
!!! note "아래 풀이의 수치에 대하여"
    풀이에 적힌 값은 모두 이 쪽의 설정(정규화 적용, 묶음 128, Adam $10^{-3}$, 5 에포크)에 **씨앗 42를 고정해** 재어 얻은 것이다. 모델을 만들기 직전과 자료를 섞기 직전에 씨앗을 고정했으므로, 같은 기계에서 다시 돌리면 같은 값이 나오고 모든 표가 99.18%라는 하나의 바탕값을 함께 쓴다.
