...
창을 한 칸 옮기면 조각의 왼쪽 열만 0이고 나머지가 1이므로 줄마다 $0 \cdot (-1) + 1 \cdot 0 + 1 \cdot 1 = 1$, 합이 3이다. 한 칸 더 옮기면 조각이 전부 1이라 $-1 + 0 + 1 = 0$이 세 번, 곧 합이 0이다. 이렇게 아홉 자리를 모두 훑으면 출력이 나온다.
...
가로세로는 앞 절의 $H' = \lfloor (H + 2p - k)/s \rfloor + 1$이 정하며, 표의 세 줄 모두 $s = 1$이다. 여백 없는 두 줄은 $5 \to 3$으로 줄고, `conv1`은 `padding=1` 덕분에 $28 \to 28$로 남는다.
