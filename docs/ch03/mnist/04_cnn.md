...
![필터 한 장이 모든 자리를 훑으며 자리마다 수 하나를 내놓는다](figures/conv_one_filter.svg)
...
창을 오른쪽으로 두 칸 옮기면 조각이 전부 1이 되어 $-1 + 0 + 1 = 0$이 세 번, 곧 합이 0이다. 이렇게 아홉 자리를 모두 훑으면 출력이 나온다.
...
- **출력이 작아졌다.** $5 \times 5$에서 $3 \times 3$이 되었다. $3 \times 3$ 창을 온전히 얹을 수 있는 자리가 세 곳씩뿐이기 때문이며, 테두리를 채우지 않고 한 칸씩 옮기면 $H \to H - k + 1$이 된다(더 일반적인 식은 아래에 있다). 코드에서 `padding=1`을 주는 까닭이 이것이다. 테두리를 0으로 한 줄 둘러 주면 크기가 그대로 유지된다.
...
가로세로는 따로 정해진다. 지금까지는 창을 **한 칸씩** 옮긴다고 말없이 가정했는데, 몇 칸씩 옮길지도 고를 수 있다. 이 걸음 폭을 **보폭**(stride) $s$라 한다. 여백 $p$까지 넣어 일반적으로 적으면 다음과 같다.

$$
H' = \left\lfloor \frac{H + 2p - k}{s} \right\rfloor + 1
$$

`nn.Conv2d`의 기본값이 $s = 1$이라 코드에 적혀 있지 않을 뿐이다. 표의 세 줄을 넣어 보면 여백 없는 두 줄은 $\lfloor (5 + 0 - 3)/1 \rfloor + 1 = 3$으로 줄고, `conv1`은 $\lfloor (28 + 2 - 3)/1 \rfloor + 1 = 28$로 그대로 남는다. 홀수 $k$에 $p = (k-1)/2$, $s = 1$을 주면 언제나 $H' = H$다.

이 식이 값진 까닭은 **풀링도 같은 식으로 설명되기** 때문이다. `MaxPool2d(2, 2)`는 $k = 2$, $s = 2$, $p = 0$이므로

$$
\left\lfloor \frac{28 + 0 - 2}{2} \right\rfloor + 1 = 14,
\qquad
\left\lfloor \frac{14 + 0 - 2}{2} \right\rfloor + 1 = 7
$$

이 되어 코드의 $28 \to 14 \to 7$이 나온다. 창을 얹고 옮긴다는 점에서 합성곱과 풀링은 같은 연산이며, 창 안에서 **가중합을 하느냐 최댓값을 고르느냐**만 다르다.

보폭을 키우면 크기를 줄일 수 있으므로 풀링 대신 쓸 수도 있다. $k=3$, $s=2$, $p=1$로 두면 $\lfloor (28+2-3)/2 \rfloor + 1 = 14$로 풀링과 같은 결과가 나오며, 요즘 구조가 풀링을 빼고 보폭 2 합성곱을 쓰는 까닭이 이것이다.
...
class CNN(nn.Module):
    ...
    def forward(self, x):
        # 오른쪽 주석이 그 줄을 지난 뒤의 모양이다. B는 묶음 크기.
        ...
        # 뒤이므로 잃을 것이 없다.
        # flatten(1)은 0번 축(묶음)만 남기고 나머지를 한 줄로 잇는다
...
`conv1`이 배운 것을 직접 들여다볼 수 있다. 필터가 32장이고 저마다 $3 \times 3$이므로 볼 것이 288개의 수뿐이다. 아래는 5 에포크 학습 뒤(시험 정확도 99.18%)의 모습이다.
