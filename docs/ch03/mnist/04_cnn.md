...
이 구조에는 손댈 곳이 여럿이다. 필터를 3×3이 아니라 5×5로 키우면? 팽창을 주어 더 넓게 보게 하면? 드롭아웃을 빼면? 합성곱 뒤의 ReLU를 없애면?

**직접 재어 보았다.** 다만 한 번씩만 돌려서는 안 된다. 씨를 바꾸면 값이 흔들리므로, 갈래마다 **씨 다섯 개**로 돌려 평균과 퍼짐을 함께 적는다.
...
## 5. 앞의 다섯은 서로 구별되지 않는다

표의 위 다섯 줄을 보라. 평균이 98.97%에서 99.14%까지 흩어져 있는데, **갈래 하나 안의 퍼짐이 0.16에서 0.27이다.** 평균끼리의 차이(0.17%포인트)가 한 갈래를 다시 돌릴 때 생기는 흔들림보다 작다.

곧 **필터를 키우든, 팽창을 주든, 드롭아웃을 빼든 이 조건에서는 아무 일도 일어나지 않는다.**
