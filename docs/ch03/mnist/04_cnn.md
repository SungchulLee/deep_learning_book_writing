...
!!! note "이 얼개는 고양이의 뇌에서 왔다"
    ...
    다만 **이 이야기를 이 장의 결과로 받아들이면 안 된다.** 4절에서 학습이 끝난 필터를 실제로 재어 보면, MNIST에서는 무작위로 깔아 둔 필터와 거의 구별되지 않는다. 모서리 검출기가 뚜렷이 나타나는 것은 자연 사진을 학습할 때이며, 그 비교는 [4.2절](../../ch04/02_vgg16_transfer.md)에 있다.
