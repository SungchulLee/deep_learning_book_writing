...
여기까지가 종이 위에서 손으로 따라갈 수 있는 이야기다. 두 가지가 곧바로 따라 나온다.
...
784 → 128 층에 묶음 100장을 넣는다고 하자.

| | 실수 개수 |
|---|---|
| 입력 $x$ | $100 \times 784 = 78{,}400$ |
| 간선의 국소 기울기 $\partial y / \partial A_0$ | $100 \times 128 \times 784 = 10{,}035{,}200$ |

**128배**다. 층 하나에서 이렇고, 층마다 쌓인다. 국소 기울기를 통째로 적어 두는 구현은 어떤 크기의 모델에서도 버티지 못한다.
