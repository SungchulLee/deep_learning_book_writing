...
![28×28 이미지를 784차원으로 편 뒤 첫 선형 변환으로 128차원에 보내고, ReLU를 거쳐 둘째 선형 변환으로 10차원 로짓을 만들고, 소프트맥스로 확률을 얻어 argmax로 숫자를 고르는 흐름. 학습이 찾는 것은 두 선형 변환의 가중치와 편향 101,770개이다.](figures/mlp_architecture.svg)

바뀐 곳은 가운데뿐이다. [3.2절](../linear_softmax/06_implementation.md)에서는 784차원에서 곧바로 10차원으로 갔다. 여기서는 128차원을 한 번 거치고, 그 사이에 ReLU가 놓인다. 그 빨간 화살표 하나가 이 절이 더하는 전부이며, 그것이 없으면 1절에서 본 대로 두 선형 변환이 하나로 합쳐져 3.2절과 똑같은 모델이 된다.

소프트맥스와 argmax는 모델 바깥이 아니라 안쪽의 마지막 두 걸음이다. 다만 코드에서는 소프트맥스가 따로 보이지 않는다. `nn.CrossEntropyLoss`가 안에 품고 있기 때문이다.
