...
ReLU, 시그모이드, tanh 말고도 GELU 등 여러 활성화 함수가 있고 저마다 성질이 다르다. 그 비교와 선택 기준은 [7장의 활성화 함수](../../ch07/index.md)에서 다룬다. 여기서 중요한 것은 **무엇을 쓰느냐가 아니라 반드시 있어야 한다는 것**이다.
...
사슬 하나뿐일 때는 곱셈만 있으면 되지만, 가중치를 여러 번 쓰거나 지름길 연결을 놓으면 이 덧셈이 살아난다. `optimizer.zero_grad()`가 필요한 까닭도 같은 규칙이다. `.grad`는 원래 더해 쌓게 되어 있어서, 지난 묶음의 값을 지우지 않으면 그것까지 함께 더해진다.
...
784 → 128 층에 묶음 100장을 넣는다고 하자. 매개변수 $A_0$에 대한 국소 기울기가 실제로 무엇인지 한 칸씩 따져 보면 이렇다.
...
다만 여기서 "가장 좋은" 모델을 고르는 기준이 **학습** 정확도라는 점은 짚어 두어야 한다. 학습 정확도는 에포크마다 거의 어김없이 오르므로, 이 코드가 되살리는 것은 사실상 마지막 에포크다. 체크포인트가 제값을 하는 것은 기준이 **검증** 정확도일 때다. 그때는 곡선이 한 번 꺾여 내려가고, 꺾이기 전의 가중치를 붙들어 두는 일이 곧 조기 종료가 된다. 여기서는 그 장치를 미리 갖추어 둔 셈이다. 검증 집합을 기준으로 삼는 체크포인트는 [6장의 모델 체크포인트](../../ch06/logistic_regression/03_model_checkpointing.md)에서 자세히 다룬다.
...
??? success "연습문제 20 풀이"
    ...
    784 → 128 층에 시그모이드를 붙이고 묶음 32장으로 재었다.
