...
여기서 한 가지는 정확히 해 두는 것이 좋다. 순전파는 $f'(x_0)$을 **계산해 두지 않는다.** 나중에 그것을 만들 때 필요한 값을 남겨 둘 뿐이다. 시그모이드 마디가 남기는 것은 $\sigma'(z)$가 아니라 출력 $y$ 자체이고, 역전파가 그 자리에서 $y(1-y)$를 만든다. 선형 마디는 입력과 가중치를 남긴다.
...
그래서 `no_grad`가 아끼는 것은 곱셈이 아니라 **메모리**다. 순전파의 계산량 자체는 거의 그대로이며(연습문제 19), 사라지는 것은 마디를 만들고 그 재료를 붙들어 두는 일이다.
