...
ReLU, 시그모이드, tanh 말고도 GELU 등 여러 활성화 함수가 있고 저마다 성질이 다르다. 그 비교와 선택 기준은 [9장의 활성화 함수](../../ch08/index.md)에서 다룬다. 여기서 중요한 것은 **무엇을 쓰느냐가 아니라 반드시 있어야 한다는 것**이다.
...
어떻게 그것이 되는지를 **두 걸음으로 나누어** 본다. 먼저 종이 위에서 통하는 생각을 세우고, 그다음에 PyTorch가 그 생각을 실제로 어떻게 구현하는지를 본다. 둘은 같지 않은데, 다른 까닭이 그 자체로 배울 값이 있다.
...
계산을 그래프로 그리고, 마디에는 값을, **간선에는 국소 기울기**(local gradient)를 적는다. 이름 그대로 그래프의 간선 하나에 붙는 기울기이며, 그 간선이 잇는 두 마디 사이의 도함수다. $x_0 \to x_1 = f(x_0) \to x_2 = g(x_1)$이라면 첫 간선에 $f'(x_0)$이, 둘째 간선에 $g'(x_1)$이 붙는다.

그러면 역전파는 한 문장이다. **출력 쪽에서 1로 시작해, 간선을 거슬러 내려오며 거기 적힌 국소 기울기를 곱한다.** 층이 $L$개이면 국소 기울기 $L$개의 곱이 된다. 이름이 역전파(backpropagation)인 까닭이다.

하필 1로 시작하는 데에는 까닭이 있다. 우리가 구하려는 것이 $\partial \text{loss} / \partial \theta$이고, 그 사슬의 맨 앞 칸은 손실을 손실 자신으로 미분한 $\partial \text{loss} / \partial \text{loss} = 1$이기 때문이다. 코드에서 `loss.backward()`는 `loss.backward(torch.tensor(1.0))`과 같은 말이다.

![위쪽 순전파에서 값이 왼쪽에서 오른쪽으로 흐르고, 아래쪽 역전파에서 오른쪽 끝의 1부터 시작해 국소 기울기를 하나씩 곱하며 왼쪽으로 내려온다. 각 마디 아래에는 그 국소 기울기를 만드는 데 필요한 것을 남겨 둔다는 표시가 있다.](figures/backprop_flow.svg)

위아래 두 줄의 방향이 반대라는 것이 이 그림의 요점이다. (마디 아래 노란 상자가 무엇을 뜻하는지는 둘째 걸음에서 본다.)

**수 하나가 아니라 행렬이다.** 지금까지는 마디마다 수가 하나씩 놓인 것처럼 적었다. 실제 신경망에서는 마디에 놓이는 것이 **벡터**다. 784개를 받아 128개를 내놓는 층이라면 들어가는 것이 784개짜리, 나오는 것이 128개짜리다.

그러면 간선에 붙는 국소 기울기도 수가 아니다. 나가는 값 128개 각각을 들어오는 값 784개 각각으로 미분해야 하므로 **$128 \times 784$ 행렬**이 되며, 이것을 야코비 행렬(Jacobian)이라 부른다. "곱한다"는 말도 수의 곱셈이 아니라 행렬 곱셈이다. 사슬 규칙의 모양은 그대로다.

$$
\frac{\partial \,\text{loss}}{\partial x_0}
= \frac{\partial \,\text{loss}}{\partial x_2} \,
  \frac{\partial x_2}{\partial x_1} \,
  \frac{\partial x_1}{\partial x_0}
$$

이 점을 붙들어 두어야 둘째 걸음이 읽힌다. 간선 하나에 붙는 것이 수 하나가 아니라 수십만 개짜리 행렬이라는 사실이 거기서 문제가 된다.

**사슬은 활성값을 지나가고, 매개변수 기울기는 옆으로 빠진다.** 한 가지가 더 있다. 지금까지 따라온 사슬은 **활성값**을 지나간다. 그런데 우리가 정말로 원하는 것은 매개변수 $A_\ell$에 대한 기울기다. 둘은 이렇게 이어진다. 기울기가 한 마디에 닿으면 그 마디는 두 가지 일을 한다.

- 받은 기울기를 **아래 마디로 넘긴다.** 이것이 사슬을 계속 타고 내려가는 몫이다.
- 받은 기울기와 순전파 때의 입력을 엮어 **자기 매개변수의 기울기를 만들어 `.grad`에 넣는다.** 이 몫은 더 내려가지 않고 거기서 끝난다.

모양을 보면 분명하다.

```python
out = torch.sigmoid(lin(xb))       # xb: (100, 784), lin: 784 -> 128
out.sum().backward()

xb.grad.shape          # (100, 784)  ← 활성값 기울기, 계속 내려간다
lin.weight.grad.shape  # (128, 784)  ← 여기서 만들어지고 멈춘다
```

곧 역전파는 활성값을 따라 **한 줄기로 내려가면서**, 지나는 마디마다 매개변수 기울기를 하나씩 **떨어뜨리고** 간다. 101,770개의 편미분은 그렇게 모인다.

여기에 규칙이 하나 더 붙는다. **갈래를 따라서는 곱하고, 갈래가 만나면 더한다.** 한 마디가 여러 곳으로 흘러 들어가면 각 길을 따라 온 몫을 모두 더해야 한다.

```python
a = torch.tensor(3.0, requires_grad=True)
(a * a).backward()
print(a.grad)        # tensor(6.)  = 3 + 3, 길이 둘이라 더해졌다
```

사슬 하나뿐일 때는 곱셈만 있으면 되지만, 가중치를 여러 번 쓰거나 지름길 연결을 놓으면 이 덧셈이 살아난다. `optimizer.zero_grad()`가 필요한 까닭도 같은 규칙이다. `.grad`는 원래 더해 쌓게 되어 있어서, 지난 배치의 값을 지우지 않으면 그것까지 함께 더해진다.

여기까지가 종이 위에서 손으로 따라갈 수 있는 이야기다. 세 가지가 곧바로 따라 나온다.
...
둘째, 1절에서 시그모이드를 걱정한 까닭이 분명해진다. 국소 기울기가 **곱해지므로**, 그 값이 늘 $1/4$ 이하이면 층을 지날 때마다 기울기가 4분의 1 이하로 줄어든다. 앞쪽 층에 닿을 무렵에는 남는 것이 없다(연습문제 16).

셋째, **왜 하필 거꾸로 가는가**에 답할 수 있다. 손실이 스칼라 하나이기 때문이다. 출력 쪽에서 시작하면 사슬을 타고 내려가는 동안 손에 든 것이 줄곧 벡터 하나뿐이다. 반대로 입력 쪽에서 출발하면 매개변수 하나마다 따로 한 번씩 훑어야 하니 101,770번이 든다. 출력이 1개이고 입력이 101,770개인 문제에서는 거꾸로 가는 쪽이 압도적으로 싸다. 출력이 많고 입력이 적은 문제라면 반대 방향이 유리하며, 그 방식도 실제로 쓰인다(전진 모드 자동 미분).

#### 둘째 걸음 — PyTorch가 실제로 하는 일

첫째 걸음을 곧이곧대로 구현하면 어떻게 될까. 순전파가 지나가면서 간선마다 국소 기울기를 계산해 적어 두면 된다. 그런데 **그렇게 하면 안 된다.** 크기를 재어 보면 안다.

784 → 128 층에 배치 100장을 넣는다고 하자. 매개변수 $A_0$에 대한 국소 기울기가 실제로 무엇인지 한 칸씩 따져 보면 이렇다.

$$
\frac{\partial y_{b,m}}{\partial (A_0)_{m',n}}
= \begin{cases} x_{b,n} & m = m' \\ 0 & m \ne m' \end{cases}
$$

곧 **0이 아닌 칸은 모두 입력 $x$를 그대로 베낀 것**이다. 출력 뉴런 $m$마다 한 벌씩, 128번 되풀이된다.

| | 실수 개수 |
|---|---|
| 입력 $x$를 그대로 남기기 | $100 \times 784 = 78{,}400$ |
| 국소 기울기의 0 아닌 칸 | $100 \times 128 \times 784 = 10{,}035{,}200$ |

**128배**이고, 이 128은 다름 아닌 출력 뉴런의 수다. 국소 기울기를 적어 두는 것은 **같은 $x$를 128번 되풀이해 적는 일**인 셈이다. 층 하나에서 이렇고 층마다 쌓이니, 첫째 걸음을 곧이곧대로 옮긴 구현은 어떤 크기의 모델에서도 버티지 못한다.

그래서 PyTorch는 미룬다. 순전파는 국소 기울기를 **계산하지 않고**, 마디를 하나 만들어 **나중에 그것을 만들 재료**만 붙들어 둔다. 시그모이드 마디가 남기는 것은 $\sigma'(z)$가 아니라 출력 $y$ 자체이고, 역전파가 그 자리에서 $y(1-y)$를 만든다. 선형 마디는 입력과 가중치를 남긴다.

```python
y = torch.sigmoid(z)
print(y.grad_fn)                  # <SigmoidBackward0 ...>
print(y.grad_fn._saved_result)    # y 그 자체이지 y(1-y)가 아니다
```

그래프의 생김새도 첫째 걸음과 다르다. 종이 위에서는 마디에 값을 적고 간선에 국소 기울기를 적었지만, PyTorch에서는 **마디가 연산**이고 간선은 그저 다음 마디를 가리키는 연결선이다. 간선에 붙어 있는 것은 기울기가 아니라 "어느 마디의 몇 번째 입력으로 가라"는 표시뿐이다.

```python
print(y.grad_fn.next_functions)   # ((<MulBackward0 ...>, 0),)
#                                     어느 마디          몇 번째 입력
```

곧 국소 기울기를 만드는 방법은 간선이 아니라 **마디**가 쥐고 있다. 첫째 걸음의 그림은 사슬 규칙을 손으로 따라가기 위한 것이고, 여기서부터는 그 그림이 코드로 옮겨진 모습이 조금 다르다고 알아 두면 된다.

역전파도 국소 기울기를 따로 만들어 놓고 곱하지 않는다. 받아 온 기울기와 국소 기울기의 **곱을 한 번에** 구한다. 선형 마디라면 위의 천만 개짜리를 만드는 대신 `grad_out @ A_0` 한 번이다. 야코비 행렬을 실제로 만들어 보고 싶다면 `torch.autograd.functional.jacobian`이라는 **따로 있는** 함수를 불러야 하고, 그쪽이 훨씬 느리다(연습문제 20).

이 차이가 `no_grad`의 뜻을 정해 준다. **마디를 만들지 않으니 재료를 남길 일도 없다.**

여기서 첫째 걸음이 남기기 쉬운 오해 하나를 짚어야 한다. 순전파가 국소 기울기를 계산한다고 생각하면, `no_grad`가 그 계산을 건너뛰어 **시간을 크게 아낄 것** 같다. 그렇지 않다. 애초에 순전파는 그 계산을 하지 않기 때문이다. 재어 보면 차이가 거의 없다.
...
$1.14$배뿐이고, 그 14%는 도함수 셈이 아니라 마디를 만들고 장부를 적는 값이다. 곧 **`no_grad`가 아끼는 것은 메모리이지 곱셈이 아니다.**

(연습문제 19에서는 같은 비교가 $1.3$배로 나온다. 그쪽은 작은 연산을 600번 되풀이하므로 마디를 만드는 값이 전체에서 차지하는 몫이 더 크다. 연산이 커질수록 이 비율은 1에 가까워진다.)
...
`loss`는 값 하나짜리 텐서이고 `.item()`은 거기서 파이썬 수를 꺼낸다. 단순한 형 변환처럼 보이지만 그렇지 않다. `loss`에는 **자기를 만든 계산 그래프 전체가 달려 있다.** `.item()`으로 수만 꺼내면 그 연결이 끊어지고, 배치 하나를 처리한 뒤 그래프가 버려진다.

`.item()` 없이 `epoch_loss += loss`라고 적으면 배치마다의 그래프가 모두 손에 남는다. 한 에포크 600배치를 돌고 나면 그래프가 604마디 길이로 이어지고 메모리가 45MB 늘어난다. `.item()`을 쓴 쪽은 6MB이다. 이 모델에서는 견딜 만하지만, 큰 모델에서 학습이 몇 분 만에 메모리 부족으로 죽는 흔한 까닭이 바로 이것이다(연습문제 18).
...
| 이 모델에서 효과 | 없음 | 메모리를 아낌 (시간은 조금) |
| 빠뜨리면 | 평가값이 흔들림 | 메모리를 더 씀 |
...
다만 여기서 "가장 좋은" 모델을 고르는 기준이 **학습** 정확도라는 점은 짚어 두어야 한다. 학습 정확도는 에포크마다 거의 어김없이 오르므로, 이 코드가 되살리는 것은 사실상 마지막 에포크다. 체크포인트가 제값을 하는 것은 기준이 **검증** 정확도일 때다. 그때는 곡선이 한 번 꺾여 내려가고, 꺾이기 전의 가중치를 붙들어 두는 일이 곧 조기 종료가 된다. 여기서는 그 장치를 미리 갖추어 둔 셈이다. 검증 집합을 기준으로 삼는 체크포인트는 [8장의 모델 체크포인트](../../ch07/logistic_regression/03_model_checkpointing.md)에서 자세히 다룬다.
...
??? success "연습문제 4 풀이"
    ...
    곱하는 차례가 1절과 뒤집힌 것부터 짚어 두자. 1절은 $y = xW$처럼 **행벡터를
    왼쪽에** 두어 $W' = W_1 W_2$이라 적었고, PyTorch는 `weight`를
    $(\text{나가는 수}, \text{들어오는 수})$ 꼴로 저장해 $y = Wx$처럼 쓴다.
    그래서 코드에서는 `W2 @ W1`이 된다. 한쪽을 전치하면 다른 쪽이니 같은 식이며,
    결과가 $(10, 784)$인 것도 이 차례 때문이다.
...
??? success "연습문제 12 풀이"
    ...
    입력 하나마다 은닉 뉴런 128개 가운데 40%쯤이 꺼지는 것은(연습문제 9) 감당할 수 있는 손실이며,
    ...
    보통이다. 오토인코더의 가장 좁은 층이나 어텐션의 사영층이 흔히 선형으로
...
??? success "연습문제 16 풀이"
    ...
    **측정.** 초기화 직후 배치 하나로 `loss.backward()`를 부르고 첫 `Linear`의
    `weight.grad` 크기를 쟀다.
...
??? success "연습문제 18 풀이"
    ...
    `.item()`은 값만 꺼내 그 연결을 끊는다. 빼먹으면 배치마다의 그래프가 모두
    살아남아 서로 이어진다.

    ```python
    running = torch.zeros(())
    for images, labels in train_loader:          # 600배치
    ...
    배치 600개를 돌면 `AddBackward` 마디가 600개 사슬로 이어지고, 그 각각이
    붙들고 있는 중간 활성값이 함께 살아남는다. 에포크가 끝나야 풀린다.

    이 모델에서는 45MB라 견딜 만하다. 그러나 이 수는 **배치 크기 × 활성값 크기 ×
    배치 수**로 커진다. 큰 모델에서 학습이 몇 분 만에 `CUDA out of memory`로
...
??? success "연습문제 19 풀이"
    ...
    **무관**하다는 점이다. 매개변수를 1억 개로 늘려도 역전파는 여전히 순전파의
    ...
    오려면 순전파가 마디마다 남긴 중간값이 있어야 하기 때문이다. 배치 10,000장을
    ...
    `no_grad`를 쓰는 진짜 까닭은 속도가 아니라 **시험할 때 배치를 크게 잡을 수
    있다**는 데 있다.
...
3절 둘째 걸음은 역전파가 국소 기울기를 따로 만들지 않고 곱을 한 번에 구한다고 했다. 야코비 행렬을 실제로 만들어 보고, 보통의 `backward()`와 시간을 견주어라. 왜 그런 함수가 따로 있는가?

</div>

??? success "연습문제 20 풀이"
    `torch.autograd.functional.jacobian`이 야코비 행렬을 실제로 만들어 준다.
    784 → 128 층에 시그모이드를 붙이고 배치 32장으로 재었다.

    ```python
    f = lambda t: torch.sigmoid(lin(t))

    out = f(xb); out.sum().backward()                 # 보통의 역전파
    J = torch.autograd.functional.jacobian(f, xb)     # 야코비를 실제로 만든다
    print(J.shape)      # (32, 128, 32, 784)
    ```

    | | 시간 | 만들어진 수 |
    |---|---|---|
    | 보통의 `backward()` | 4.4밀리초 | — |
    | `jacobian()` | 375.6밀리초 | 102,760,448 |

    **85배** 느리다. 만들어진 야코비에는 1억 개가 넘는 수가 들어 있는데, 학습에
    쓰는 것은 그중 극히 일부를 접어 넣은 결과뿐이다.

    까닭은 역전파가 야코비 $J$를 만든 뒤 $v^\top J$를 계산하는 것이 아니라,
    $v^\top J$를 **바로** 계산하기 때문이다. 선형 마디에서 이것은
    `grad_out @ A_0` 한 번이며, $J$는 어디에도 나타나지 않는다. 이런 계산을
    벡터-야코비 곱(vector-Jacobian product)이라 부른다.

    사용자 정의 연산을 만들어 보면 이 꼴이 눈에 보인다.

    ```python
    class MyLinear(torch.autograd.Function):
        @staticmethod
        def forward(ctx, x, W):
            ctx.save_for_backward(x, W)     # 재료를 남긴다
            return x @ W.T

        @staticmethod
        def backward(ctx, grad_out):        # 받아 온 기울기가 들어오고
            x, W = ctx.saved_tensors
            return grad_out @ W, grad_out.T @ x   # 곱한 결과가 나간다
    ```

    `backward`가 받는 것도 기울기이고 돌려주는 것도 기울기다. 야코비를 돌려주는
    자리가 아예 없다.

    이 설계가 곧 3절의 128배 표에 대한 답이다. 야코비를 만들지 않기로 하면
    메모리가 매개변수 수가 아니라 **활성값 수**에 비례하게 되고, 그래서 1억
    개짜리 모델도 학습할 수 있다. `jacobian()`이 따로 있는 까닭은 야코비 자체가
    필요한 다른 일(민감도 분석 같은)이 가끔 있기 때문이며, 학습에 쓰라고 있는
    것이 아니다.
...
3절에서 뜯어본 여섯 줄은 3.2절에서도 3.4절에서도 그대로 쓰인다. 역전파는 두 걸음으로 보았다. 종이 위에서는 간선마다 국소 기울기를 적고 경로를 따라 곱하면 되지만, 그대로 구현하면 층 하나에 실수 천만 개가 필요하다. PyTorch가 국소 기울기를 미리 만들지 않고 재료만 남기는 까닭이 그것이며, `no_grad`가 아끼는 것이 시간이 아니라 메모리인 까닭도 같다. `model(images)`가 `__call__`을 거쳐 `forward`를 부르고, `loss.backward()`가 사슬 규칙으로 되돌아오며 편미분 101,770개를 채우고, `optimizer.step()`이 그만큼 움직인다. 순전파가 반드시 먼저 와야 하는 까닭도, `.item()`을 빠뜨리면 메모리가 새는 까닭도, `no_grad`가 정확히 무엇을 끄는 것인지도 모두 그 되돌아오는 길에서 나온다.

그래서 다음 걸음은 모델을 더 키우는 쪽이 아니다. 이 절의 모델은 여전히 첫 줄에서 이미지를 펼쳐 화소의 이웃 관계를 버리고 있다. [3.4 합성곱 신경망](04_cnn.md)이 그것을 되찾는다.

앞의 연습문제 20개로 직접 확인할 수 있다.
