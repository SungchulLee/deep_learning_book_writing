...
최대가능도 추정을 신경망에 쓰기 전에, 손으로 끝까지 풀 수 있는 가장 작은 예제로 원리를 먼저 보자. 이 예제가 좋은 이유는 매개변수가 하나뿐인 데다 정수여서, 미분 대신 이웃한 값끼리 견주는 것만으로 최댓값을 찾을 수 있기 때문이다.

호수에 물고기가 $N$마리 있다. $N$을 알고 싶지만 전부 잡아 셀 수는 없다.

1. **1단계(포획)** — 3마리를 잡아 표지를 붙이고 놓아준다.
2. **2단계(재포획)** — 얼마 뒤 5마리를 잡았더니 그중 1마리에 표지가 있었다.

두 번째 표본에서 표지된 물고기가 정확히 1마리일 확률은 초기하분포로 주어진다.

$$
P_N = \frac{\binom{3}{1}\binom{N-3}{4}}{\binom{N}{5}}
$$

여기서 $N$은 데이터가 아니라 **매개변수**이고, $P_N$은 그 매개변수의 함수인 **가능도**이다. 최대가능도 추정은 관찰한 결과(표지 1마리)를 가장 그럴듯하게 만드는 $N$을 고르는 것이다.

### 이웃과 견주어 최댓값 찾기

$N$이 정수이므로 미분할 수 없다. 대신 $P_N$이 커지다 작아지는 지점, 곧 $P_{N-1} < P_N$이면서 $P_{N+1} \le P_N$인 $N$을 찾는다.

먼저 $P_{N-1} < P_N$을 정리한다. 공통 인수 $\binom{3}{1}$은 양변에서 지워진다.

$$
\frac{\binom{N-4}{4}}{\binom{N-1}{5}} < \frac{\binom{N-3}{4}}{\binom{N}{5}}
\;\Longrightarrow\;
N(N-7) < (N-5)(N-3)
$$

$$
N^2 - 7N < N^2 - 8N + 15 \;\Longrightarrow\; N < 15
$$

다음으로 $P_{N+1} \le P_N$을 정리한다.

$$
\frac{\binom{N-2}{4}}{\binom{N+1}{5}} \le \frac{\binom{N-3}{4}}{\binom{N}{5}}
\;\Longrightarrow\;
(N+1)(N-6) \ge (N-4)(N-2)
$$

$$
N^2 - 5N - 6 \ge N^2 - 6N + 8 \;\Longrightarrow\; N \ge 14
$$

두 조건을 함께 만족하는 정수는 $N = 14$뿐이지만, 부등식의 등호 자리를 따져 보면 $N = 15$에서도 가능도가 같은 값을 갖는다. 실제로 계산하면 다음과 같다.
...
$$
\therefore \; \widehat{N}_{\text{MLE}} \in \{14,\ 15\}
$$

곧 최대가능도 추정값이 하나로 정해지지 않고 두 값에서 비긴다. $\square$

이 비김은 이 예제의 결함이 아니라 최대가능도 추정의 성질을 보여 준다. 가능도 함수는 봉우리 근처에서 평평할 수 있고, 그때는 데이터가 두 값을 갈라놓을 만큼의 정보를 담고 있지 않다는 뜻이다. 표본을 더 모으면 봉우리가 뾰족해지면서 비김이 풀린다.

### 신경망과 어떻게 이어지는가

같은 논리가 그대로 신경망으로 넘어간다. 다른 것은 규모와 푸는 방법뿐이다.

| | 표지-재포획 | MNIST 선형 분류기 |
|---|---|---|
| 매개변수 | $N$ 하나 (정수) | $(A, b)$, $784 \times 10 + 10 = 7850$개 (실수) |
| 가능도 | $P_N$ | $L(A, b)$ |
| 푸는 법 | 이웃과 견주기 | 로그를 취해 경사 하강법 |

매개변수가 7850개가 되면 이웃끼리 견주는 방법은 쓸 수 없다. 그래서 가능도에 로그를 취해 로그가능도 $\ell$을 만들고(곱을 합으로 바꾸어 미분하기 쉽게 한다), 부호를 뒤집어 **손실** $= -\ell$로 두고, 그 손실을 [경사 하강법](../../ch02/gradient_descent/learning_rate.md)으로 최소화한다. 로그는 단조 증가 함수라 최댓값의 위치를 바꾸지 않으므로, 가능도를 최대화하는 일과 손실을 최소화하는 일은 정확히 같은 문제이다.
