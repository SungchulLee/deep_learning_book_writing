...
여기까지가 모델이고, 남은 물음은 7850개의 수를 어떻게 정하느냐이다. 답은 [최대가능도 추정](../mle/capture_recapture_mle.md)이다. 관찰한 레이블을 가장 그럴듯하게 만드는 $(A, b)$를 고르되, 매개변수가 7850개나 되므로 손으로 견주어 볼 수는 없다. 그래서 가능도에 로그를 취해 로그가능도 $\ell$로 바꾸고, 부호를 뒤집어 손실 $=-\ell$로 두고, 그 손실을 [경사 하강법](../../ch02/gradient_descent/learning_rate.md)으로 최소화한다.

$$
\theta_{n+1} = \theta_n - \lambda \frac{\partial \ell}{\partial \theta}
$$

이 손실이 바로 교차 엔트로피이며, 소프트맥스와 짝을 이루어 쓰인다.
