...
- Derive the probability mass function for multiclass classification
...
- Apply these concepts in PyTorch for multiclass classification tasks
...
For multiclass classification with $K$ classes, we need the **categorical distribution** (also called the **multinoulli** or **generalized Bernoulli** distribution). Let $Y \in \{1, 2, \ldots, K\}$ denote the class label, and let $\boldsymbol{\pi} = (\pi_1, \pi_2, \ldots, \pi_K)$ be the probability vector where:
...
One-hot vectors satisfy several important properties:
...
Using one-hot encoding, we can write the categorical PMF in an elegant product form:

$$P(\mathbf{y} \mid \boldsymbol{\pi}) = \prod_{k=1}^{K} \pi_k^{y_k}$$

This mirrors the Bernoulli formulation but extends naturally to $K$ classes. Let us verify this works.
...
where $\pi_k^{(i)} = P(Y = k \mid \mathbf{x}^{(i)};\, \boldsymbol{\theta})$ depends on the input through model parameters $\boldsymbol{\theta}$.
...
### The Softmax Regression Pipeline

The full computational graph for softmax regression can be summarized as:
...
- $\mathbf{W} \in \mathbb{R}^{K \times D}$ is the weight matrix
- $\mathbf{b} \in \mathbb{R}^K$ is the bias vector
- $\mathbf{z} \in \mathbb{R}^K$ are the logits (raw scores)
- $\hat{\boldsymbol{\pi}} \in \Delta^{K-1}$ are predicted probabilities on the probability simplex
...
def one_hot_manual(labels: torch.Tensor, num_classes: int) -> torch.Tensor:
    ...
    # scatter_(dim, index, src) places values from src into positions
    # specified by index. Here we scatter 1s along dimension 1 at
    # positions given by labels.
...
| Product form PMF | $P(\mathbf{y}\mid\boldsymbol{\pi}) = \prod_k \pi_k^{y_k}$ |
| Log-likelihood | $\log P(\mathbf{y}\mid\boldsymbol{\pi}) = \sum_k y_k \log \pi_k$ |
...
!!! warning "Critical Mistakes to Avoid"
    ...
    2. **Don't apply softmax before `nn.CrossEntropyLoss`** — it is applied internally
