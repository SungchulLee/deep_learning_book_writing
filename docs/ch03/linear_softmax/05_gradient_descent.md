...
더 자세한 이야기는 [학습률과 이동 폭](../../ch05/gradient_descent/learning_rate.md)에 있다. 다음 절은 이 말을 눈으로 확인한다.
...
$\partial \text{loss} / \partial \theta$을 직접 유도할 필요는 없다. PyTorch의 [자동 미분](../../ch05/autograd/01_basic_scalar_backward.md)이 계산 그래프를 거슬러 올라가며 대신 구해 준다. 코드에서는 `loss.backward()` 한 줄이다.
