...
- 묶음 하나로 기울기를 어림하는 일이 왜 받아들일 만한지 설명하기
...
|---|---|---|
| 너무 크다 | 골짜기를 넘나들며 튄다. 심하면 발산한다 | 손실 곡선이 들쭉날쭉하거나 `nan`이 된다 |
...
연습문제 4에서 네 가지 학습률로 직접 재어 본다. 더 자세한 이야기는 [학습률과 이동 폭](../../ch04/gradient_descent/learning_rate.md)에 있다.

---

## 4. 묶음으로 어림한다

손실은 표본 6만 개에 대한 합이다. 기울기를 정확히 구하려면 매 걸음마다 6만 장을 모두 훑어야 한다. 그렇게 하는 것을 **온묶음 경사 하강법**이라 한다.

실제로는 그렇게 하지 않는다. 데이터를 128장쯤의 **작은 묶음**으로 나누고, 묶음 하나의 기울기로 한 걸음을 내딛는다. 이를 **확률적 경사 하강법**(stochastic gradient descent)이라 한다.

묶음 하나의 기울기는 전체 기울기와 같지 않다. 그런데도 괜찮은 까닭은 그것이 전체 기울기의 **불편 추정량**이기 때문이다. 묶음을 무작위로 고르면

$$
\mathbb{E}\!\left[\nabla \text{loss}_{\text{묶음}}\right] = \nabla \text{loss}_{\text{전체}}
$$

이다. 곧 방향이 틀릴 수는 있어도 **치우쳐 있지는 않다.** 평균적으로는 옳은 방향이며, 걸음을 많이 내딛으면 잡음이 상쇄된다.

그리고 이 맞바꿈이 유리하다. 6만 장을 한 번 훑는 동안, 온묶음이라면 **한 걸음**을 내딛지만 묶음 128이라면 469걸음을 내딛는다. 각 걸음의 방향이 조금 엉성해도 469번 움직이는 편이 훨씬 빨리 내려간다.

!!! note "잡음이 오히려 돕기도 한다"
    묶음의 잡음은 순전한 손해가 아니다. 얕은 극소점이나 안장점에서 빠져나오는 데 도움이 된다. 이 절의 모델에서는 손실면이 볼록해서 그런 일이 없지만(5절), 다음 절부터는 중요해진다.
...
## 6. 기울기는 손으로 구하지 않아도 된다

$\partial \text{loss} / \partial \theta$을 직접 유도할 필요는 없다. PyTorch의 [자동 미분](../../ch04/autograd/01_basic_scalar_backward.md)이 계산 그래프를 거슬러 올라가며 대신 구해 준다. 코드에서는 `loss.backward()` 한 줄이다.
...
??? success "연습문제 2 풀이"
    ...
    기울기를 더하도록 설계된 까닭은 이것이 필요한 경우가 있어서다. 묶음을 여러 번
    나누어 큰 묶음 하나처럼 다루는 기울기 누적(gradient accumulation)이 그렇다.
    메모리가 부족해 묶음 128을 한 번에 못 올릴 때 32씩 네 번 더한 뒤 갱신한다.
...
??? success "연습문제 3 풀이"
    ...
    **너무 작은 쪽이 고치기 쉽다.** 방향은 옳고 걸음만 짧으므로, 기다리면 결국
    ...
    한 가지 헷갈리기 쉬운 점. 묶음마다의 손실은 원래 조금 흔들린다. 묶음이 다르면
...
학습률을 $10^{-1}, 10^{-2}, 10^{-3}, 10^{-4}$로 바꾸어 각각 10 에포크씩 학습시켜 보라. 3절이 말한 두 가지 잘못이 실제로 보이는가?
...
??? success "연습문제 4 풀이"
    ...
    **더 돌리면 나아진다.** 실제로 에포크를 늘리면 $10^{-3}$과 비슷한 자리에 닿는다.
    ...
    Adam을 쓰고 있어 $10^{-3}$이 좋은 자리라는 점도 덧붙일 만하다. Adam은 기울기
    크기로 걸음을 나누므로 $10^{-3}$ 근처가 대체로 잘 통하며, 평범한 SGD에서는
    $10^{-2}$ 근처가 보통이다.
...
묶음 크기를 1, 32, 128, 1024, 60000으로 바꾸어 정확도와 걸린 시간을 재어라. 온묶음(60000)이 유독 나쁜 까닭은 무엇인가?

</div>

??? success "연습문제 5 풀이"
    | 묶음 | 1 (1 에포크) | 32 | 128 | 1024 | 60000 |
    |---|---|---|---|---|---|
    | 정확도 | 90.03% | 92.07% | **92.42%** | 92.25% | **72.88%** |
    | 걸린 시간 | 108초 | 48초 | 15초 | 6초 | 5초 |
    | 갱신 횟수 | 60,000 | 18,750 | 4,690 | 590 | **10** |

    온묶음이 72.88%로 크게 뒤진다. 까닭은 정확도 표만 보면 안 보이고 **갱신 횟수**를
    보아야 드러난다. 10 에포크를 돌렸지만 한 에포크에 갱신이 **한 번**뿐이므로
    걸음을 딱 **10번** 내딛었다. 7,850차원 공간에서 열 걸음으로 갈 수 있는 곳은 멀지
    않다.

    묶음 하나의 기울기는 전체 기울기보다 엉성하다. 그런데도 묶음 128이 이기는 까닭은,
    **엉성한 걸음 4,690번이 정확한 걸음 10번보다 훨씬 멀리 간다**는 것이다. 4절이
    말한 맞바꿈이 이것이다.

    묶음 1은 갱신을 6만 번 하지만 시간이 108초로 가장 오래 걸리고 정확도도 낮다.
    묶음이 너무 작으면 기울기의 잡음이 커지고, 무엇보다 GPU가 한 번에 하나씩만
    처리하게 되어 병렬성을 전혀 못 쓴다. 그래서 걸음마다의 값이 비싸진다.

    128과 1024는 정확도가 비슷한데 시간은 1024가 2.5배 빠르다. 큰 묶음이 GPU를
    잘 채우기 때문이다. 실무에서 묶음 크기는 정확도보다 **메모리와 처리량**으로
...
??? success "연습문제 6 풀이"
    ...
    까닭은 5절에서 말한 **볼록성**이다. 소프트맥스 회귀의 교차 엔트로피 손실은
...
묶음 하나의 기울기가 전체 기울기의 **불편 추정량**임을 증명하라. 치우치지 않았다는 것과 정확하다는 것은 어떻게 다른가?
...
??? success "연습문제 9 풀이"
    ...
    묶음 $B$를 $n$개에서 **복원 없이 균등하게** $m$개 고른 것이라 하면, 묶음 기울기는
    ...
    곧 묶음을 어떻게 고르든(균등하게만 고르면) 평균적으로는 전체 기울기와 같다.
    ...
    **분산**에 대한 진술이다. $g_B$는 치우치지 않았지만 분산이 있다.
    ...
    묶음을 키우면 분산이 $1/m$로 줄어들지만 0이 되지는 않는다($m = n$일 때만 0).
    그러므로 묶음 기울기는 **방향이 평균적으로 옳지만 매번 틀린다.**

    이것이 왜 괜찮은가. 경사 하강법은 한 걸음의 정확함을 필요로 하지 않는다.
    여러 걸음에 걸쳐 잡음이 상쇄되면서 평균적으로 옳은 방향으로 나아가면 된다.
    그리고 분산을 줄이는 데 드는 값(묶음을 키우는 계산)보다 걸음을 더 많이 내딛는
    편이 이득이라는 것이 연습문제 5의 실측이었다.

    잡음이 순전한 손해가 아니라는 점도 덧붙일 만하다. 비볼록한 손실면에서는 이
    잡음이 얕은 극소점과 안장점을 빠져나오는 데 도움이 된다. 온묶음 경사 하강법은
...
기울기는 묶음 하나로 어림한다. 치우치지 않은 어림이므로 방향이 평균적으로 옳고, 한 번 데이터를 훑는 동안 469걸음을 내딛을 수 있어 훨씬 빠르다.
