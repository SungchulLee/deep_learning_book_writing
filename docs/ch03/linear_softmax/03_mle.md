...
$N = 7$부터 21까지 그려 보면 이 논증이 눈에 보인다.
...
봉우리가 뾰족하지 않다는 점을 눈여겨보라. $N = 12$부터 18까지 일곱 값이 모두 0.477과 0.495 사이에 있어, 가장 높은 값과의 차이가 4%도 되지 않는다. $P_{12} = 0.4773$과 $P_{18} = 0.4779$는 사실상 같다. 자료가 이 범위를 갈라놓지 못한다는 뜻이며, 14와 15가 비기는 것도 그 평평함의 한 모습이다.

오른쪽은 우리가 실제로 셈한 비 $P_{N+1}/P_N$이다. 초록 점은 1보다 커서 "다음 값이 더 그럴듯하다"는 뜻이고, 주황 점은 1보다 작아서 반대를 뜻한다. 색이 바뀌는 자리가 곧 봉우리이며, 그 경계인 $N = 14$에서 비가 정확히 1이다. 위에서 푼 이차방정식이 찾아낸 것이 바로 이 점이다.
