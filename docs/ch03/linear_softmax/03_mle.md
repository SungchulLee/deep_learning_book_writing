...
오른쪽은 우리가 실제로 셈한 비 $P_{N+1}/P_N$이며 같은 범위를 덮는다. 초록 점은 1보다 커서 "다음 값이 더 그럴듯하다"는 뜻이고, 주황 점은 1보다 작아서 반대를 뜻한다. 색이 바뀌는 자리가 곧 봉우리다.

$N = 7$에서 비가 1.875로 크게 출발해 빠르게 1로 내려온다는 점이 먼저 눈에 띈다. 작은 $N$에서는 한 마리를 더 놓느냐 마느냐가 가능도를 크게 바꾸지만, $N$이 커지면 한 마리 차이가 거의 뜻이 없어지기 때문이다. 그래서 $N = 14$ 뒤의 점들은 모두 1에 바짝 붙어 있어 전체 눈금에서는 구별되지 않는다. 확대한 그림을 덧붙인 까닭이 이것이며, 거기서 $N = 14$가 정확히 1 위에 놓인 것을 볼 수 있다. 위에서 푼 이차방정식이 찾아낸 것이 바로 이 점이다.

비가 1에 가깝다는 것은 이웃한 두 값이 거의 같다는 뜻이고, 그것이 왼쪽 그림의 긴 꼬리로 나타난다. 한 가지 더 있다. 비는 $N = 25$에서 0.9777로 **가장 낮았다가 다시 1 쪽으로 올라간다.** 곧 가능도가 가장 빠르게 줄어드는 구간이 25 언저리이고, 그보다 멀어지면 줄어드는 속도마저 느려진다. $N \to \infty$에서 비가 1로 수렴하므로 꼬리는 끝내 끊기지 않는다. 이것이 $P_{50}$이 아직도 최댓값의 절반을 넘는 까닭이다.
...
!!! note "여기서 끝, 그다음은 8장"
    ...
    같은 예제를 코드로 시늉해 보는 것과, 최대가능도 추정 자체를 정규분포·회귀·로지스틱 회귀 등 여러 모형에서 일반적으로 다루는 것은 [5.1 최대가능도 추정](../../ch07/mle/mle.md)에 있다. 표지-재포획을 시늉과 함께 더 파헤친 쪽은 [표지-재포획 MLE](../../ch07/mle/capture_recapture_mle.md)다.
...
??? success "연습문제 6 풀이"
    ...
    배치를 뽑을 때도 같은 구별이 있고, [경사 하강법 연습문제 9](05_gradient_descent.md)는
    되돌리지 않고 뽑는 쪽을 가정해 배치 기울기의 불편성을 증명한다.
