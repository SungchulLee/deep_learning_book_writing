...
첫 에포크에 이미 90.86%에 닿는다는 점을 눈여겨보라. 손실도 $\log 10 \approx 2.30$에서 시작해 0.47까지 내려왔다. 볼록한 문제라 초반에 빠르게 내려가고, 남은 아홉 에포크가 1.65%포인트를 더한다.
...
**정확도는 오르기만 하지 않는다.** 92.25 → 91.97 → 92.20 → 92.32처럼 오르내리며, 9에포크에서는 91.97%로 7에포크(92.57%)보다 낮다. 손실은 한 번도 올라가지 않았는데도 그렇다.

마지막 관찰이 특히 중요하다. 손실은 확률을 재는 **매끄러운** 값이지만 정확도는 "가장 큰 로짓이 맞았는가"만 세는 **계단** 값이다. 경계에 걸친 표본 몇 개가 넘나들면 정확도는 0.3~0.6%포인트씩 흔들리지만 손실은 꿈쩍도 하지 않는다. 위 표에서 7에포크와 9에포크의 차이가 0.60%포인트인데, 그 사이 손실은 0.2641에서 0.2595로 **줄어들기만** 했다. 그러므로 **정확도가 0.2%포인트 올랐다고 모델이 나아진 것이 아니다.** 이 흔들림보다 작은 차이를 두고 두 모델의 우열을 말하면 안 되며, 이 장의 걸음들이 10%포인트 단위로 벌어지는 것은 그래서 다행스러운 일이다.
