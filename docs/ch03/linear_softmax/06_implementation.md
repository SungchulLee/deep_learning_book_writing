...
transform = transforms.Compose([
    ...
    # Normalize는 화소마다 (x - mean) / std 를 한다. 채널마다 상수를
    # 하나씩 받으므로 괄호가 붙어 있다. MNIST는 흑백이라 채널이 하나다.
    #
    #     x' = (x - 0.1307) / 0.3081
    #
    # 두 상수는 학습 집합 6만 장의 화소 전체에서 잰 평균과 표준편차다.
    # 그래서 이 변환을 거치면 학습 집합 전체의 평균이 0, 표준편차가 1이
    # 된다. 실제로 재어 보면 -0.0001과 1.0000이다.
    #
    # 값의 범위가 [0, 1]에서 벗어난다는 점을 눈여겨볼 것.
    #     화소 0 (배경)     -> (0 - 0.1307) / 0.3081 = -0.4242
    #     화소 1 (가장 밝음) -> (1 - 0.1307) / 0.3081 =  2.8215
    # 곧 이 모델이 보는 배경은 0이 아니라 -0.4242이며, MNIST 화소의
    # 80.9%가 그 값이다.
    #
    # 왜 하는가. 입력의 크기가 고르면 손실면이 덜 찌그러져 경사 하강법이
    # 빨리 내려간다. 입력이 모두 양수이면 한 뉴런에 들어오는 기울기의
    # 부호가 죄다 같아져 갱신이 지그재그로 가는데, 가운데를 0으로 옮기면
    # 그 문제도 준다.
    #
    # 그림 하나하나의 평균이 0이 되는 것은 아니다. 모든 그림에 같은 상수를
    # 쓰므로 그림마다의 평균은 표준편차 0.1405로 흩어져 있다. 그 흩어짐이
    # 곧 "이 숫자가 얼마나 굵은가" 같은 정보이므로 지우면 안 된다.
    #
    # 네 걸음 모두 같은 상수를 쓴다. 그래야 결과를 나란히 견줄 수 있다.
...
위 학습 반복문의 여섯 줄은 이 장이 끝날 때까지 한 글자도 바뀌지 않는다. 3.3절과 3.4절에서 모델만 갈아 끼울 뿐이다. 그래서 그 여섯 줄이 저마다 무슨 일을 하는지는 [3.3절에서 한 줄씩](../mnist/03_mlp.md) 따로 뜯어본다. `model(images)`가 객체를 함수처럼 부르는 까닭, `loss.backward()`가 사슬 규칙으로 되돌아오며 편미분을 채우는 과정, `.item()`을 빠뜨리면 메모리가 새는 까닭이 거기에 있다.
...
[1단계 템플릿 학습](../mnist/01_template_learning.md)의 82.03%에서 **92.51%**로 올랐다. 1단계는 클래스마다 학습 이미지를 평균 내어 템플릿 열 장을 만들고 새 그림을 가장 가까운 템플릿의 숫자로 부르는 방법이었다. 그리고 3.1의 「사실은 선형 분류기이다」에서 보았듯 그것도 **가중치를 클래스 평균으로 못박은 선형 분류기**다. 그래서 두 걸음을 나란히 놓을 수 있다.

| | 1단계 템플릿 학습 | 2단계 선형 모델 |
|---|---|---|
| 가중치를 정하는 법 | 클래스 평균으로 못박는다 | 자료에 맞추어 학습한다 |
...
??? success "연습문제 9 풀이"
    ...
    | | 정확도 | 오차 | 오차의 비 |
    |---|---|---|---|
    | 1단계 템플릿 학습 | 82.03% | 17.97% | — |
    | 2단계 선형 모델 | 92.51% | 7.49% | **58.3% 줄임** |
    | 3단계 다층 퍼셉트론 | 97.42% | 2.58% | 65.6% 줄임 |
    | 4단계 합성곱 신경망 | 99.22% | 0.78% | 69.8% 줄임 |
