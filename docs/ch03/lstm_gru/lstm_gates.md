...
Gers & Schmidhuber (2000) proposed peephole connections where gates also look at $C_{t-1}$:

$$f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + \mathbf{w_f \odot C_{t-1}} + b_f)$$

This gives gates direct access to the cell state, useful for tasks requiring precise timing.
