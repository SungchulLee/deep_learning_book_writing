...
Standard LSTM gates only see $h_{t-1}$ and $x_t$. Peephole connections allow gates to also see the cell state:

$$f_t = \sigma(W_f \cdot [h_{t-1}, x_t] + w_f \odot c_{t-1} + b_f)$$
$$i_t = \sigma(W_i \cdot [h_{t-1}, x_t] + w_i \odot c_{t-1} + b_i)$$
$$o_t = \sigma(W_o \cdot [h_{t-1}, x_t] + w_o \odot c_t + b_o)$$

Where $w_f, w_i, w_o \in \mathbb{R}^n$ are diagonal peephole weight vectors.

**Use case:** Tasks requiring precise timing (e.g., rhythm learning, precise interval prediction). Less common in modern practice.
...
def diagnose_lstm(model, dataloader, device):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # (Requires modifying forward pass to return gates)
