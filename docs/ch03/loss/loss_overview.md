...
| **Classification** | Discrete categories | Cross-Entropy, BCE, Focal, Hinge | Image classification, NLP |
...
| Distribution | Log-Likelihood Term | Resulting Loss |
|--------------|---------------------|----------------|
| Gaussian $\mathcal{N}(\mu, \sigma^2)$ | $-\frac{1}{2\sigma^2}(y - \mu)^2$ | MSE |
| Laplace $\text{Laplace}(\mu, b)$ | $-\frac{1}{b}\lvert y - \mu \rvert$ | MAE |
| Bernoulli $\text{Bern}(p)$ | $y\log(p) + (1-y)\log(1-p)$ | Binary Cross-Entropy |
| Categorical $\text{Cat}(\mathbf{p})$ | $\sum_k y_k \log(p_k)$ | Cross-Entropy |
