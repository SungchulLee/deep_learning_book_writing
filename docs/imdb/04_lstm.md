...
이 절이 그 한계를 푼다. **3걸음의 박아 넣기는 그대로 두고** 그 위의 평균만 LSTM으로 바꾼다. 왼쪽에서 오른쪽으로 한 낱말씩 읽으며 기억을 이어 가는 그물이다.

| | 박아 넣기 | 그 위에 얹은 것 | 합계 |
|---|---|---|---|
| [3걸음](03_embedding.md) | 1,280,000 | 평균 — 0 | 1,280,130 |
| 4걸음 | 1,280,000 | **LSTM 33,280** | 1,313,410 |

**두 걸음이 같은 박아 넣기를 지닌다.** 늘어난 것은 33,280개, 곧 모델의 2.5%뿐이다. 그러므로 값이 달라진다면 그것은 크기가 아니라 **평균을 LSTM으로 바꾼 몫**이다.
...
"차례가 도움이 되지 않는다"는 이보다 훨씬 큰 주장이며, 이 표만으로는 적을 수 없다. [다음 걸음](05_attention.md)이 같은 물음에 다른 구조로 답하고, 거기서 이 절의 내려감이 **구조 탓인지 예산 탓인지** 갈린다.
...
??? success "연습문제 3 풀이"
    ...
    **평균을 낸다는 뼈대는 같고, 평균내는 것이 달라진다.** 3걸음의 강점(짧은 길)을 지키면서 차례를 더하는 셈이다.
    ...
    이것이 알려 주는 것이 하나 더 있다. 3걸음이 잘한 까닭의 일부는 "낱말의 뜻을 배워서"가 아니라 **평균이라는 구조가 학습을 쉽게 만들어서**일 수 있다는 것이다. [다음 걸음](05_attention.md)의 어텐션도 이 성질을 지닌다. 어텐션은 무게를 준 평균이고, 평균인 한 모든 칸이 출력까지 짧은 길을 갖는다.
