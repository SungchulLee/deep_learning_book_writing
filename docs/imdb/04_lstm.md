...
이 절이 그 한계를 푼다. **3걸음의 임베딩은 그대로 두고** 그 위의 평균만 LSTM으로 바꾼다. 왼쪽에서 오른쪽으로 한 낱말씩 읽으며 기억을 이어 가는 그물이다.
...
**두 걸음이 같은 임베딩을 지닌다.** 늘어난 것은 33,280개, 곧 모델의 2.5%뿐이다. 그러므로 값이 달라진다면 그것은 크기가 아니라 **평균을 LSTM으로 바꾼 몫**이다.
...
| [3 임베딩 + 평균](03_embedding.md) | **87.03%** | 12.97%p | 39% | 1,280,130 |
...
"차례가 도움이 되지 않는다"는 이보다 훨씬 큰 주장이며, 이 표만으로는 적을 수 없다. [다음 걸음](05_attention.md)이 같은 물음에 다른 구조로 답하고, 아래 6절이 **예산을 여섯 배로 늘려** 같은 물음을 다시 묻는다.

---

## 6. 예산을 여섯 배로 늘려 보면

같은 씨 다섯 벌을 **30 에포크**까지 돌리고, 이번에는 에포크마다 시험 정확도를 재어 두었다. 값 하나가 아니라 곡선을 보려는 것이다.

![씨앗 다섯 벌의 학습 곡선. 다섯 줄 모두 5 에포크 언저리까지 가파르게 오른 뒤 85% 근처에서 평평해지며, 3걸음과 5걸음을 나타내는 가로선 아래에 머문다. 곡선마다 한 번씩 아래로 깊게 꺾이는 자리가 있다](figures/lstm_curves.svg)

### 먼저, 5 에포크의 값이 그대로 되풀이된다

새로 돌린 다섯 벌의 **다섯째 에포크** 값은 83.51 / 83.44 / 81.41 / 80.41 / 78.76이다. 위 2절의 출력과 소수점 아래까지 같다.

씨와 스레드 수를 같게 두었으므로 당연한 결과지만, **당연한 것을 확인해 두는 값이 있다.** 아래에서 값이 달라지는 것은 전부 예산 때문이지 다른 무엇 때문이 아니라는 뜻이 된다.

### 예산은 값을 올린다

| | 평균 | 퍼짐 | 범위 |
|---|---|---|---|
| 5 에포크 (2절의 값) | 81.51% | **4.75** | 78.76\~83.51 |
| **가장 좋았던 에포크** | **85.28%** | **0.34** | 85.12\~85.46 |
| 30 에포크 | 84.19% | 1.06 | 83.61\~84.67 |

**3.77%포인트 올랐다.** 5절의 의심이 맞았다. 5 에포크에서 본 81.51%는 이 모델의 실력이 아니라 **아직 오르는 중에 찍은 값**이었다.

그리고 퍼짐이 4.75에서 **0.34**로 줄었다. 3걸음의 0.34, 5걸음의 0.33과 같은 자리다. 곧 **이 모델의 실력은 다른 걸음들만큼 되풀이된다.** 되풀이되지 않던 것은 실력이 아니라 "5 에포크째에 어디까지 와 있는가"였다.

### 그래도 순서는 바뀌지 않는다

| 걸음 | 범위 |
|---|---|
| [3 임베딩 + 평균](03_embedding.md) | 86.92\~87.26 |
| [5 셀프 어텐션](05_attention.md) | 85.97\~86.30 |
| **4 LSTM (가장 좋았던 에포크)** | **85.12\~85.46** |

**서른 에포크 가운데 가장 좋은 한 점씩만 골라 세워도** LSTM은 5걸음에 0.51만큼 못 미친다. 범위가 겹치지 않으므로 이 차이는 씨를 바꾸어 뒤집히지 않는다. 3걸음과는 1.46만큼 벌어진다.

곧 예산은 **얼마나 못한가**를 바꾸었을 뿐 **못한다는 것**을 바꾸지 못했다. 5.52%포인트 뒤지던 것이 1.75%포인트 뒤지는 것으로 좁혀졌다.

### 그리고 다섯 벌 모두 한 번씩 무너진다

곡선에서 아래로 깊게 꺾이는 자리를 보라. **다섯 벌 모두에서 일어난다.**

| 씨 | 가장 깊이 꺾인 자리 | 그때까지의 꼭대기에서 |
|---|---|---|
| 1 | 10에포크 **68.55%** | −16.61 |
| 3 | 28에포크 72.74% | −12.44 |
| 0 | 14에포크 71.41% | −14.03 |
...
씨 1은 10에포크에서 **68.55%까지 떨어졌다.** [2걸음 선형 학습](02_linear.md)의 78.80%보다 10%포인트 낮고, 다음 에포크에 85.15%로 돌아온다. 한 에포크 사이에 일어나는 일이다.

되돌아오므로 학습이 망가진 것은 아니다. 다만 **어느 에포크에 멈추느냐가 값을 크게 바꾼다.**

### 꼭대기는 되풀이되고 끝점은 되풀이되지 않는다

이 절에서 가장 중요한 수가 이 둘이다.
...
**실력은 0.34 안에 모이는데 끝점은 그렇지 않다.** 무너짐이 아무 때나 일어나므로, 마지막 에포크에 무너짐이 걸린 씨와 걸리지 않은 씨의 값이 갈린다.

그러므로 이런 모델에서는 **"30에포크에서의 정확도"라는 수 자체가 뜻이 흐리다.** 같은 모델이 29에포크에서 79.65%, 30에포크에서 84.20%다(씨 4). 둘 다 참인 측정이고 4.55%포인트 다르다.

[4장](../ch04/index.md)이 "점을 늘리는 것과 곡선을 보는 것은 다른 일이다"라고 적었다. 여기서 한 겹이 더 생긴다. **곡선이 오르내리는 모델에서는 곡선 위의 어느 한 점도 그 모델을 대표하지 못한다.** 꼭대기든 평균이든, 무엇을 고르는지를 밝혀 적어야 한다.

### 그래서 이 절의 결론

> **LSTM은 5 에포크에서 덜 배운 것이 맞다. 그러나 다 배우고 나서도 3걸음과 5걸음에 못 미치고, 게다가 멈추는 자리에 따라 값이 크게 흔들린다.**

5절이 미뤄 둔 물음의 답은 **둘 다**였다. 내려감의 일부는 예산 탓이고(3.77%포인트), 나머지는 예산으로 메워지지 않는다(1.75%포인트).
...
??? success "연습문제 2 풀이"
    ...
    **6절이 그 두 가지를 실제로 했고, 예측한 대로 되었다.** 값은 81.51%에서 85.28%로 올랐고 퍼짐은 4.75에서 0.34로 줄었다. 곧 "덜 배운 것"이 맞았다.

    다만 곡선은 예측에 없던 것을 하나 더 보여 주었다. **다섯 벌 모두 중간에 한 번씩 무너진다.** 이것은 위의 두 갈래 어디에도 들어가지 않는다. 덜 배운 것도, 다르게 배운 것도 아니고, **배운 것을 잠깐 잃는 것**이다.

    시험을 설계할 때 답을 두 갈래로만 나누어 두면 셋째 갈래를 보지 못한다. 곡선을 그려 두었기 때문에 눈에 들어왔다.
