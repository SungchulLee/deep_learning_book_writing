...
print(f"\n3걸음 임베딩 + 평균  평균 {sum(accs)/len(accs):.2f}%  "
...
3걸음 임베딩 + 평균  평균 87.03%  퍼짐 0.34 (86.92~87.26)  매개변수 1,280,130
...
| 3 임베딩 + 평균 | **87.03%** | 12.97%p | **39%** | **1,280,130** |

**8.23%포인트를 벌었다.** 씨앗 퍼짐이 0.34이므로 **그 차이가 참이라는 것**은 의심할 여지가 없다. 그리고 두 걸음이 공교롭게도 남은 오차를 똑같이 39%씩 지운다.
...
앞의 두 걸음은 매개변수가 같았으므로 이 물음이 없었다. 여기서는 물어야 한다. 임베딩 차원만 바꾸어 재어 보면 갈린다.
...
이 장은 다섯 걸음을 모두 5 에포크로 잰다. 하나의 규약으로 재야 나란히 놓을 수 있기 때문이다. 그런데 **5 에포크가 두 걸음에게 똑같이 공평한가?**

2걸음과 3걸음을 30 에포크까지 늘려 에포크마다 재어 보았다.

![2걸음과 3걸음의 학습 곡선. 3걸음은 9에포크 언저리에서 꼭대기를 찍고 내려오고, 2걸음은 30에포크까지 계속 오른다. 5에포크 자리에서 둘의 차이가 가장 크다](figures/rung23_curves.svg)
...
- **3걸음은 9에포크 언저리에서 꼭대기(88.08%)를 찍고 내려온다.** 그 뒤로는 과적합이다.
- **2걸음은 30에포크까지 계속 오른다.** 아직 꼭대기에 닿지도 않았다.

그러므로 **5 에포크는 3걸음에게 거의 가장 좋은 자리이고 2걸음에게는 한참 이른 자리다.** 하필 둘의 차이가 가장 크게 벌어지는 곳에서 잰 셈이다.

!!! warning "그렇다고 규약이 틀린 것은 아니다"
    걸음마다 제 나름의 예산을 주면 견줌이 무너진다. "3걸음이 2걸음을 이겼다"가 "3걸음에 준 예산이 2걸음에 준 예산을 이겼다"와 뒤섞이기 때문이다. **하나의 규약으로 재는 것이 옳다.**

    다만 그 규약이 **어느 쪽에 유리한지는 재어 보아야 알 수 있고**, 여기서는 3걸음 쪽이었다. 그것을 적어 두지 않으면 8.23이라는 수가 실제보다 크게 읽힌다.

    이 장의 표는 그대로 5 에포크 값을 싣는다. 다섯 걸음을 같은 자로 재기 위해서다. 다만 **이 걸음이 번 몫은 8.23이 아니라 2에서 4.5 사이**라고 읽는 편이 실제에 가깝다.

### 그래서 이 걸음이 실제로 번 것은 얼마인가
...
가장 정직한 것은 마지막 줄이다. 두 모델을 저마다 가장 좋은 자리에서 견주므로 예산이 어느 쪽에도 유리하지 않다. 그래도 **2걸음의 83.53%는 아직 오르는 중의 값**이라 이마저 3걸음에 조금 유리하다.

[4장](../ch04/index.md)이 "점을 늘리는 것과 곡선을 보는 것은 다른 일이다"라고 적었다. 여기서 그 말이 **걸음 사이의 견줌**에까지 걸린다. 곡선을 보지 않으면 규약이 누구 편인지 알 수 없다.
...
??? success "연습문제 2 풀이"
    **2걸음 쪽의 크기를 키워 맞대어야 한다.** 지금 표는 임베딩 쪽만 크기를 바꾸었으므로, 크기가 같을 때 어느 나타냄이 나은지는 말하지 못한다.

    맞대는 방법이 둘 있다.

    **하나, 2걸음에 은닉층을 넣는다.** 세기 벡터 20,000개를 받아 64칸으로 줄였다가 2로 보내면 매개변수가 약 128만 개로 3걸음과 거의 같아진다. 그러면 "같은 크기에서 세기냐 임베딩냐"를 물을 수 있다.
