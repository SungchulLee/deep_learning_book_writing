...
어텐션 얼개 덕에 신경망 추천 시스템은 추천을 만들 때 쓰는 이의 지난 일, 물건 특징, 맥락 앎 가운데 가장 알맞은 곳에 집중할 수 있다. 지난 주고받음을 모두 같게 다루는 대신, 어텐션 바탕 시스템은 지난 물건 가운데 어느 것이 지금 추천에 가장 큰 영향을 주는지 그때그때 무게를 배운다. 덕분에 차례를 이룬 쓰는 이의 움직임에서 멀리 떨어진 매임을 담고 길이가 제각각인 차례도 효율 좋게 다룬다.

계량 금융에서 어텐션 얼개는 한결같지 않은 쓰는 이의 취향과 시장 국면을 담는 데 특히 값지다. 성장주를 좋아하는 마음이 오름장에서는 짙어지고 내림장에서는 옅어질 수 있다. 지난 보유, 최근 거래, 지금 시장 형편에 어텐션을 두면 추천 시스템이 바뀌는 맥락에 그때그때 맞춰 갈 수 있다.

이 마디는 추천을 위한 어텐션 얼개를 세우고 어텐션과 함께 거르기를 아우른 얼개를 살피며 금융 쓰임새를 보인다.
...
- **어텐션 무게**: 열쇠마다의 알맞음을 나타내는 배운 점수
- **내놓기**: 어텐션이 이끄는 값의 무게 모으기

### 추천의 어텐션 갈래
- **물건 어텐션**: 지난 물건 가운데 어느 것이 지금 취향에 영향을 주는가
- **차례 어텐션**: 차례의 어느 자리가 가장 중요한가
- **특징 어텐션**: 물건의 어느 특징이 가장 알맞은가
- **엇갈린 어텐션**: 쓰는 이와 물건의 주고받음
...
물건끼리의 어텐션을 가능하게 해 지난 물건들이 서로 어떻게 영향을 주는지 나타낸다.

### 쓰는 이와 물건 맞추기를 위한 엇갈린 어텐션
...
삼의 차례에 어텐션을 두어 쓰는 이 취향의 때 흐름을 배운다:

1. **지난 물건 임베딩**: 삼 $i_1, \ldots, i_T$에 대해 $h_t = \text{embed}(i_t)$
2. **어텐션 셈하기**: 묻기(지금 맥락)가 지난 임베딩을 살핀다
3. **내놓기**: 무게 매긴 지난 맥락을 물건 임베딩과 합친다
...
쓰는 이 어텐션과 물건 어텐션을 아우른다:

1. **물건 어텐션**: 쓰는 이가 물건을 살핌(어느 물건이 쓰는 이에게 중요한가)
2. **쓰는 이 어텐션**: 물건이 닮은 쓰는 이를 살핌(어느 쓰는 이가 그 물건에 중요한가)
3. **함께**: 두 어텐션이 헤아림에서 서로 작용한다
...
3. **어텐션**: 가진 종목 가운데 어느 것이 후보 종목과 가장 닮았는지 정한다
4. **헤아림**: 어텐션을 많이 받은 꾸러미 자리와 닮은 종목을 권한다
...
어텐션 바탕 모델은 잡이 많아 익히기 자료가 넉넉해야 한다:
...
여러 머리 어텐션은 L = 차례 길이, d = 임베딩 차원일 때 O(L² × d)으로 커진다.

차례가 길면(L > 1000) 비싸진다. 손질:

1. **잘라내기**: 최근 100~200개만 살핀다
2. **성긴 어텐션**: 가장 가까운 k개만 살핀다
3. **선형 어텐션**: 소프트맥스를 선형 연산으로 어림한다
...
남겨 둔 시험 모임에서 여느 NDCG과 AUC을 쓴다. 어텐션 모델은 대개 되돌이 신경망 밑그림과 같거나 낫다.

### 어텐션 품질 따지기

**떼어 보기**: 어텐션 얼개를 빼고 맞음이 얼마나 떨어지는지 잰다.

크게 떨어지면(>5%) 어텐션이 참으로 도움이 된다는 뜻이다.

**어텐션의 한결같음**: 닮은 쓰는 이가 닮은 어텐션 무늬를 보이는가?

한결같음이 높으면 배운 무늬가 뜻있다는 것을 시사한다.

### 온라인 따지기

A/B 시험과 함께 어텐션 모델을 펼친다:
...
3. **셀프 어텐션**: 어느 논문이 서로 영향을 주는지 배운다
4. **엇갈린 어텐션**: 묻기(살피는 이의 됨됨이)가 논문을 살핀다
5. **헤아림**: 엇갈린 어텐션으로 후보 논문에 점수를 매긴다

### 결과

- NDCG@5: 0.68(밑그림 되돌이 신경망 0.60)
- 눈여겨봄 12% 나아짐(추천 누름)
- 어텐션이 살피는 이가 기술과 의료 업종에 집중함을 보인다(맡은 범위와 맞다)
...
!!! note "추천을 위한 어텐션"
    어텐션 얼개는 쓰는 이 취향의 흐름을 담고 풀이할 수 있는 추천을 주는 데 뛰어나다. 특히 다음일 때 값지다:
...
??? success "연습문제 1 풀이"
    새 쓰는 이는 취향을 배울 주고받음 지난 일이 없어 함께 거르기 신호를 쓸 수 없다. 누그러뜨리기: 내용 바탕 특징(인구 특성, 밝힌 취향)으로 쓰는 이 임베딩의 첫 값을 잡는다. 새 물건은 아직 주고받은 쓰는 이가 없어 함께 거르기로 매길 수 없다. 누그러뜨리기: 물건 내용 특징(밝힘, 갈래)으로 그 물건을 임베딩 자리에서 닮은 물건 곁에 놓는다. 두 셈속 모두 주고받음이 넉넉히 쌓일 때까지 차가운 낱것을 띄워 준다. $\square$
...
이 마당은 핵심 개념, 수학적 틀, 어텐션 바탕 추천 얼개, 어텐션의 금융 쓰임새를 차례로 짚었다.
