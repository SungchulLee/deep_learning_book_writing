...
- [Policy Parameterization](foundations/parameterization.md) -- Mapping observations to action distributions with softmax (discrete) and Gaussian (continuous) parameterizations
- [Policy Gradient Theorem](foundations/pg_theorem.md) -- The foundational result providing an analytical gradient of expected return without differentiating through environment dynamics
- [REINFORCE](foundations/reinforce.md) -- The Monte Carlo policy gradient algorithm using sampled episode returns for gradient estimation
- [Baseline Methods](foundations/baselines.md) -- Variance reduction through state-dependent baselines that preserve gradient unbiasedness
...
- [Advantage Actor-Critic (A2C)](actor_critic/a2c.md) -- Synchronous parallel environments with advantage-based policy updates and shared actor-critic networks
- [Asynchronous Advantage Actor-Critic (A3C)](actor_critic/a3c.md) -- Distributed asynchronous training with multiple worker threads and a shared global network
...
- [Natural Policy Gradient](trust_region/npg.md) -- Fisher information-based gradient steps that are invariant to policy parameterization
- [Proximal Policy Optimization (PPO)](trust_region/ppo.md) -- Clipped surrogate objective achieving TRPO-like stability with simple first-order optimization
...
- [SAC Implementation](off_policy/sac_implementation.md) -- Complete SAC with squashed Gaussian policy, twin Q-networks, and automatic entropy coefficient adjustment
...
- [Hierarchical RL](advanced/hierarchical.md) -- Options framework, feudal networks, and goal-conditioned policies for temporal abstraction and long-horizon tasks
...
- [Reward Shaping](practical/reward_shaping.md) -- Potential-based reward modification for faster learning while preserving optimal policy guarantees
- [Action Spaces](practical/action_spaces.md) -- Design choices for discrete vs continuous actions, bounded parameterizations, and multi-dimensional action structures
- [Observation Normalization](practical/normalization.md) -- Running mean-variance normalization for observations, rewards, and advantages to stabilize training
- [Debugging RL](practical/debugging.md) -- Systematic strategies for diagnosing policy collapse, value divergence, reward hacking, and other common failure modes
