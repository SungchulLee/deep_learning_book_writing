...
행렬 인수 분해는 안쪽 곱이 본 매김을 어림하도록 쓰는 이와 물건의 낮은 차원 박아 넣기를 배운다. (특잇값 분해와 달리) 본 칸에 대해서만 가장 좋게 하고, 기울기 내려가기를 써서 작은 배치 익히기를 할 수 있으며, PyTorch에서 `nn.Embedding`으로 자연스럽게 적힌다. 치우침 항을 더하면 몸에 밴 효과(쓰는 이가 누구인지, 물건이 무엇인지)를 주고받음 효과(이 짝이 얼마나 잘 맞는지)에서 떼어 놓아, 잡을 거의 늘리지 않고도 헤아림 품질을 뜻있게 높인다. 다음 마디에서는 나타냄 힘을 더 키우려 안쪽 곱을 신경망으로 바꾼다.
