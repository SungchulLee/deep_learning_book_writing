...
본 매김 $\Omega = \{(u, i) : R_{ui} \text{를 보았음}\}$이 주어질 때 다음을 가장 작게 하여 쓰는 이 임베딩 $\mathbf{p}_u \in \mathbb{R}^d$과 물건 임베딩 $\mathbf{q}_i \in \mathbb{R}^d$을 배운다:
...
행렬 $P \in \mathbb{R}^{m \times d}$은 **임베딩 표**로 볼 수 있다. 줄 $u$은 쓰는 이 $u$의 $d$차원 나타냄이다. 마찬가지로 $Q \in \mathbb{R}^{n \times d}$은 물건 임베딩 표이다.
...
**왜 `torch.dot` 대신 원소마다 곱한 뒤 더하는가?** 들임이 묶여 있다. `p_u`과 `q_i`의 꼴은 `(batch_size, d)`이다. 원소마다 곱한 뒤 `sum(1)`을 하면 배치의 **표본마다 따로** 안쪽 곱을 셈해 꼴이 `(batch_size,)`인 텐서가 나온다.

!!! warning "임베딩의 첫 값이 중요하다"
    이 짜기는 임베딩의 첫 값을 `uniform_(0, 0.05)`으로 둔다. 작고 양인 첫 값을 보장한다. 흔한 다른 고름으로는 자비에르 첫 값 두기나 $\mathcal{N}(0, 0.01)$에서 뽑기가 있다. 첫 값이 나쁘면(보기로 큰 값) 가장 좋게 하기가 발산할 수 있다.

---

## 5. PyTorch 짜기: 기본 행렬 인수 분해
...
1. **`__init__`**: 꼴이 `(num_users, emb_size)`과 `(num_items, emb_size)`인 임베딩 표 둘을 만든다.
2. **`forward(u, v)`**: 정수 텐서 `u`(쓰는 이 번호)과 `v`(물건 번호)를 받아 임베딩을 찾고 안쪽 곱을 헤아린 매김으로 돌려준다.
...
치우침 기울기는 낱값이고 임베딩 기울기는 $d$차원 벡터임에 유의한다.

---

## 10. PyTorch 짜기: 치우침 있는 행렬 인수 분해
...
**왜 `nn.Parameter`이 아닌가?** `self.user_bias = nn.Parameter(torch.zeros(num_users))`으로 두고 손으로 번호를 짚어도 *된다*. `nn.Embedding` 쪽이 깔끔한 까닭은 배치 찾기를 저절로 다루고 가장 좋게 하개의 무게 스러짐과 잘 어울리기 때문이다.

**잡 개수**: $(m + n) \times d + (m + n)$. 치우침 항은 잡을 $m + n$개만 더한다. 임베딩에 견주면 하찮다.
...
이 짜기는 **온 배치** 기울기 내려가기를 쓴다(걸음마다 익히기 자료 전체). MovieLens-Small(익히기 매김 약 8만 개)에서는 할 만하지만 더 큰 자료 뭉치에서는 작은 배치 익히기로 바꾸어야 한다:
...
4. **임베딩 차원 훑기**: MovieLens-Small에서 $d \in \{10, 50, 100, 200, 500\}$으로 익혀라. $d$에 대한 익히기와 살피기 평균 제곱 어긋남을 그려라. 어느 지점에서 지나친 맞춤이 시작되는가?
...
8. **다잡기 민감도**: (Adam의 `weight_decay`으로) $\lambda \in \{0, 10^{-4}, 10^{-3}, 10^{-2}, 10^{-1}\}$을 두어 치우침 있는 행렬 인수 분해를 익혀라. $\lambda$에 대한 살피기 평균 제곱 어긋남을 그려라. 가장 좋은 $\lambda$이 치우침과 임베딩에서 같은가?

9. **풀이**: 익힌 뒤 배운 치우침 $b_u$이 가장 높은 이와 가장 낮은 이를 찾아라. 그들의 매김 지난 일은 어떠한가? 치우침이 느낌과 맞는가?

## 정리하며

행렬 인수 분해는 안쪽 곱이 본 매김을 어림하도록 쓰는 이와 물건의 낮은 차원 임베딩을 배운다. (특잇값 분해와 달리) 본 칸에 대해서만 가장 좋게 하고, 기울기 내려가기를 써서 작은 배치 익히기를 할 수 있으며, PyTorch에서 `nn.Embedding`으로 자연스럽게 적힌다. 치우침 항을 더하면 몸에 밴 효과(쓰는 이가 누구인지, 물건이 무엇인지)를 주고받음 효과(이 짝이 얼마나 잘 맞는지)에서 떼어 놓아, 잡을 거의 늘리지 않고도 헤아림 품질을 뜻있게 높인다. 다음 마디에서는 나타냄 힘을 더 키우려 안쪽 곱을 신경망으로 바꾼다.
