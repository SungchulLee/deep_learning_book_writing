"""flow_utils — flow_utils 페이지의 코드를 모듈로 쓸 수 있게 옮겨 놓은 것이다.

이 파일은 ch25/flow_architectures/flow_utils.md 에서 자동으로 뽑아낸 것이므로, 고칠 일이 있으면
...
class PlanarFlow(Flow):
    ...
    def inverse(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        일으키는 예외:
            NotImplementedError: 이 방법은 되풀이 디코더가 필요하다
