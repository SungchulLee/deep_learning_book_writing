...
RealNVP는 가림막을 번갈아 쓴 아핀 짝지음 층을 쌓고 익히기 안정을 위해 배치 고르게 맞추기를 더한다. 그림 자료에는 여러 잣수 얼개를 쓴다.
...
만들어 내는 모델에서 고르게 하는 흐름을 변분 오토인코더, 맞겨루는 그물과 견주어라. 흐름만의 이점은 무엇인가?

</div>

??? success "연습문제 4 풀이"
    | 성질 | 흐름 | 변분 오토인코더 | 맞겨루는 그물 |
    |----------|-------|------|------|
    | **정확한 가능도** | 예 | 하한(증거 하한) | 아니오 |
    | **정확한 추론** | 예($z = f(x)$) | 어림($q_\phi$) | 인코더 없음 |
