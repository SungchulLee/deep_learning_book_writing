...
??? success "연습문제 3 풀이"
    ...
    보기: SAT의 여 문제(UNSAT)는 SAT으로 쿡 줄임이 된다(신탁에 물어 답을 뒤집는다). 그러나 UNSAT에서 SAT으로 가는 카프 줄임은 SAT $\in$ 여 NP를 뜻하고 이는 NP $=$ 여 NP를 뜻한다. 이는 알려져 있지 않으므로 카프 줄임도 알려져 있지 않다.
...
이 마당은 직관、여럿 대 하나 줄임(카프 줄임)、튜링 줄임(쿡 줄임)、줄임 갈래 견주기을 차례로 짚었다.
