...
포화되지 않는 손실이 왜 필요한가? 기울기를 셈해 견주어라.

</div>

??? success "연습문제 5 풀이"
    원래 손실은 판별기가 확신할 때 기울기가 사라진다.

    생성기가 $\log(1 - D(G(z)))$를 **최소화**하는 것이 원래 꼴이다. $D(G(z)) = p$로 두고
    판별기 출력의 로짓에 대한 기울기 크기를 보면

    $$\Big|\frac{\partial}{\partial \text{logit}} \log(1-\sigma)\Big| = p,
      \qquad
      \Big|\frac{\partial}{\partial \text{logit}} \big(-\log \sigma\big)\Big| = 1-p$$

    앞이 원래 손실이고 뒤가 포화되지 않는 꼴이다. 값을 넣어 보면 이렇다.

    | $D(G(z))$ | 0.5 | 0.1 | 0.01 | 0.001 |
    |---|---|---|---|---|
    | 원래 손실 | 0.5000 | 0.1000 | 0.0100 | 0.0010 |
    | 포화되지 않는 손실 | 0.5000 | 0.9000 | 0.9900 | 0.9990 |
    | 비 | 1배 | 9배 | 99배 | **999배** |

    방향이 정반대다. 생성기가 **나쁠수록**($p$가 작을수록) 원래 손실은 기울기를 덜 주고
    포화되지 않는 손실은 더 준다.

    원래 손실의 성질이 거꾸로 되어 있다. 도움이 가장 필요한 때에 신호가 가장 약하다.
    익히기 초반이 정확히 그 상황이므로, 원래 꼴로는 출발하지 못한다.

    포화되지 않는 꼴은 "거짓을 참이라고 우기는" 손실이다. $\log(1-D)$를 최소화하는 대신
    $\log D$를 최대화한다. 최적점은 같고 기울기의 모양이 낫다.

    ```python
    lg = bce(d(g(z)), torch.ones(n, 1))     # 포화되지 않는 꼴
    ```
...
??? success "연습문제 6 풀이"
    생성기를 고정한 판별기의 최적점은 같고, 생성기가 줄이는 **양**은 다르다.

    원래 꼴에서 생성기는 옌센–섀넌 벌어짐을 줄인다
    ([GAN 기초 연습문제 5](../gan_architectures/45_gan.md)). 포화되지 않는 꼴에서는
    다른 양이 되는데, 최적 판별기를 대입하면 역방향 KL이 섞인 꼴이 나온다.

    실용적으로 중요한 점은 둘 다 $p_g = p_{\text{data}}$에서 최적이라는 것이다. 그래서
    이론적 목표가 같다.

    다른 점이 기울기의 성질 말고도 있다고 이야기된다. 포화되지 않는 꼴이 **봉우리를
    빠뜨리는 쪽**으로 기울어 있다는 분석이 있다. 역방향 KL이 그런 성질을 갖기 때문이며,
    [24장에서 본](../../ch24/theory/kl_term.md) $q$가 움츠리는 성질과 같은 이야기다.

    그러면 무너짐의 한 원인이 이 손실 선택에 있는 셈이다. 기울기를 얻는 대가로 봉우리를
    빠뜨리기 쉬워진 것이다.

    그래도 실무에서 포화되지 않는 꼴을 쓴다. 원래 꼴로는 아예 출발이 안 되기 때문이다.
...
이 코드에서 포화되지 않는 손실을 어떻게 적는가?

</div>

??? success "연습문제 7 풀이"
    거짓 그림에 **참의 표지**를 주어 보통의 교차 엔트로피를 쓴다.

    ```python
    # 판별기: 참은 1, 거짓은 0
    ld = bce(d(real), ones) + bce(d(fake.detach()), zeros)

    # 생성기: 거짓에 1을 준다  <- 이것이 포화되지 않는 꼴
    lg = bce(d(g(z)), ones)
    ```

    마지막 줄이 요점이다. "거짓을 참이라고 우기는" 손실이며, 부호를 뒤집은 것과 다르다.

    원래 꼴을 적으려면 이렇게 된다.

    ```python
    lg = -bce(d(g(z)), zeros)          # 원래 꼴. 쓰지 않는다
    ```

    두 줄이 비슷하게 생겼는데 기울기가 999배 차이 날 수 있다는 점을 새겨야 한다
    (연습문제 1).

    `BCEWithLogitsLoss`를 쓰는 것도 중요하다. 시그모이드를 따로 걸고 `BCELoss`를 쓰면
    수치적으로 불안정하다. 로짓에서 바로 셈하면 `log-sum-exp` 요령이 안에서 쓰인다.
...
표지를 매끄럽게 하는 것이 손실에 어떻게 들어가는가?

</div>

??? success "연습문제 8 풀이"
    참의 표지를 1 대신 $1-\epsilon$으로 둔다.

    ```python
    real_target = torch.full((n, 1), 0.9)      # 1.0 대신
    ld = bce(d(real), real_target) + bce(d(fake), zeros)
    ```

    이렇게 하면 판별기가 참 자료에 확률 1을 주려 하지 않게 되어 **지나치게 확신하지
    못한다.** 그 결과 $D(G(z))$가 0으로 붙지 않아 기울기가 살아 있다.

    이 장의 측정에서 가장 잘 들었던 손잡이다.
    ...
    에포크를 60으로 늘리면 10.25가 된다. 손잡이를 고르기 전에 **충분히 익혔는지 먼저
    확인할 일**이다.

    **거짓 쪽 표지는 건드리지 않는 것**이 관례다. 0을 $\epsilon$으로 올리면 판별기가
    거짓 자료에도 확률을 주게 되어, 생성기가 판별기의 잘못된 확신을 좇을 수 있다고
    이야기된다. 한쪽만 매끄럽게 하는 것을 one-sided label smoothing이라 한다.

    $\epsilon$은 0.1 근처를 쓴다. 크게 하면 판별기가 너무 약해진다.
...
판별기와 생성기의 손실을 같은 그림에 그려도 되는가?

</div>

??? success "연습문제 9 풀이"
    그릴 수는 있고 읽을 때 조심해야 한다. 두 값이 **서로 맞물려** 움직이므로 하나가
    내려가면 다른 하나가 올라가는 것이 정상이다.

    그래서 이 그림으로 판단할 수 있는 것이 적다.

    | 보이는 것 | 읽어도 되는가 |
    |---|---|
    | 둘이 오르내린다 | 정상이다 |
    | 판별기 손실이 0으로 | 나쁜 신호. 기울기가 사라진다 |
    | 생성기 손실이 폭발 | 나쁜 신호 |
    | 둘 다 평평 | 수렴인지 멈춤인지 **모른다** |

    마지막 줄이 문제다. 손실이 평평한 것이 균형일 수도 있고 무너진 것일 수도 있다.

    손실보다 **$D(x)$와 $D(G(z))$의 평균**을 그리는 것이 낫다
    ([도구 연습문제 3](../gan_architectures/gan_utils.md)). 두 값이 0.5 근처에 모이면
    균형에 가깝고, 1과 0으로 갈라지면 판별기가 이기고 있다.

    그리고 무엇을 그리든 **FID를 함께 그려야** 한다. 그것만이 모델이 좋아지고 있는지를
    말해 준다.
...
손실을 바꾸는 것과 정칙화를 더하는 것 가운데 무엇이 나은가?
...
??? success "연습문제 10 풀이"
    ...
    **손실을 바꾸는 것**은 기울기의 성질과 줄이는 거리를 바꾼다.

    | 손실 | 줄이는 것 | 고치는 문제 |
    |---|---|---|
    | 원래 | 옌센–섀넌 | — |
    | 포화되지 않는 꼴 | 섞인 꼴 | 초반에 기울기가 없음 |
    | 바서슈타인 | 바서슈타인 거리 | 겹치지 않을 때 기울기가 없음 |
    | 최소 제곱 | 피어슨 $\chi^2$ | 포화 |

    **정칙화**는 판별기의 성질을 다스린다. 기울기 벌점, 스펙트럴 정규화, 드롭아웃, 표지
    매끄럽게가 그것이다.

    이 장의 측정에서는 정칙화 쪽 손잡이가 결정적이었다. 배치 정규화와 드롭아웃의 유무가
    FID를 1792.8과 49.85로 갈랐고, 표지 매끄럽게가 40.68로 더 내렸다. 손실은 모두
    포화되지 않는 꼴을 썼다.

    그래서 실무의 순서를 권하면 이렇다. **포화되지 않는 손실을 기본으로 쓰고, 안정성은
    정칙화로 다룬다.** 그래도 안 되면 바서슈타인으로 옮긴다.

    바서슈타인은 손실을 바꾸면서 정칙화(리프시츠 제약)를 **요구**한다는 점이 재미있다.
    둘이 따로가 아니라 함께 와야 하는 경우다
    ([DCGAN 연습문제 5](../gan_architectures/dcgan.md)).
