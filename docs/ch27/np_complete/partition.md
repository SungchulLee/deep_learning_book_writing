...
|----------|-----------|-------------|
| NP 어려움 | 예 | 예 |
| 거짓 다항 알고리즘 | 예($O(n\Sigma)$) | 아니오(P = NP가 아닌 한) |
| FPTAS | 예(배낭 FPTAS으로) | 아니오(P = NP가 아닌 한) |
...
합이 같은 부분 모임 $k$개로 나눈다. $k \geq 3$이면 이는 강한 NP 어려움이며(3 나누기에서 줄여진다) P = NP가 아닌 한 거짓 다항 알고리즘이 없다.
...
이 마당은 문제의 정의、NP 완전성、거짓 다항 알고리즘、약한 NP 어려움과 강한 NP 어려움을 차례로 짚었다.
