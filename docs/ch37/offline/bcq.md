...
배치에 매인 Q 배우기는 오프라인 자료 뭉치가 잘 받쳐 주는 움직임만 고르도록 방침을 매어 오프라인 힘 북돋우는 배움의 바깥으로 늘려 짚는 어긋남 문제를 다룬다. 움직임 본뜨기 모델을 익혀 자료 분포를 어림하고 Q 배우기 동안 분포 밖 움직임을 걸러, Q 함수가 본 적 없는 상태-움직임 짝에 잘못 높은 값을 매기는 것을 막는다. 이 짜기는 CartPole에서 매임 문턱을 손볼 수 있는 띄엄띄엄 BCQ을 보여 준다.

## 1. 코드

```python
"""
35.5.3 배치로 옭아맨 Q 배우기(BCQ)
===========================================
