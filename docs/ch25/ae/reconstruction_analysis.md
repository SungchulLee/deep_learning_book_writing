...
!!! warning "지나치게 맞춰짐에 대해 헤아릴 점"
    자기 부호기는 익히기 자료에 지나치게 맞춰져 다시 세우기는 완벽하나 두루 통하지 못할 수 있다. 늘 남겨 둔 시험 배치로 값매김하라. 익히기와 시험의 벌어짐이 크면 지나치게 맞춰진 것이니 벌주기를 늘려야 한다.
...
??? success "연습문제 8 풀이"
    ...
    실제로 잰 값에서도 어긋남이 보인다. 누비기 자기 부호기의 특징 맵을 그대로 코드로
    쓰면 코드가 1,568차원이 되어 입력 784보다 **커지는데**, 그때 오차가 0.00068로
    가장 낮다([누비기 자기 부호기](../architecture/02_ae_cnn.md)). 가장 낮은 오차를
    가장 쓸모없는 설정이 낸 셈이다.
...
자기 부호기를 이상 탐지에 쓸 때 다시 세우기 오차를 어떻게 쓰는가? 그 방법이 기대는 가정은 무엇인가?

</div>

??? success "연습문제 9 풀이"
    정상 자료만으로 자기 부호기를 익힌 뒤, 새 자료의 다시 세우기 오차가 문턱을 넘으면
    이상으로 본다. 정상은 잘 되돌리도록 익혔으니 이상은 잘 못 되돌릴 것이라는 생각이다.

    기대는 가정이 둘이다.

    **첫째, 이상이 정상의 다양체 밖에 있다.** 이상 자료가 정상 자료와 비슷한 구조를
    가졌다면 부호기가 그것도 잘 되돌려 버린다.

    **둘째, 자기 부호기가 그만큼 일반화하지는 않는다.** 병목이 넓으면 처음 보는
...
다시 세우기 오차 말고 자기 부호기의 품질을 재는 방법에는 무엇이 있는가?
