# 오토인코더의 손실 함수
오토인코더 변형의 다시 세우기 손실, 벌주기 목표, 특화된 익히기 잣대.
...
모든 오토인코더는 들임 $x$과 다시 세운 $\hat{x}$의 차를 가장 작게 한다는 공통 바탕을 나눠 갖는다. 손실 함수를 고르는 것은 자료 분포에 대한 가정을 담는 일이다.
...
성긴 오토인코더는 숨은 활성화 대부분이 0이 되도록 벌을 주어 더 풀이하기 쉽고 넘치게 갖춘 나타냄을 얻는다.

### 여느 오토인코더와 성긴 오토인코더의 손실
...
- $h_j$은 숨은 층에서 신경 세포 $j$의 활성화다
- $\lambda$은 성김 벌주기의 세기이다
- $\sum_j |h_j|$은 많은 활성화가 딱 0이 되도록 이끈다
...
- $\rho$은 목표 성김 수준이다(예컨대 0.05은 평균 활성화 5%를 뜻한다)
- $\hat{\rho}_j = \frac{1}{n}\sum_{i=1}^n h_j(x_i)$은 신경 세포 $j$의 평균 활성화다
- $\text{KL}(\rho \| \hat{\rho}_j) = \rho \log\frac{\rho}{\hat{\rho}_j} + (1-\rho) \log\frac{1-\rho}{1-\hat{\rho}_j}$

KL 벌어짐은 $\hat{\rho}_j = \rho$일 때 가장 작아지므로 목표 활성화 수준을 **정밀히 다스릴** 수 있다.
...
class SparseAutoencoder_KL(nn.Module):
    ...
    신경 세포마다의 평균 활성화가 목표 성김 수준 ρ(예컨대 0.05)에
    가깝도록 옭아맨다.
    """
...
def kl_divergence_loss(latent: torch.Tensor, rho: float = 0.05) -> torch.Tensor:
    ...
    신경 세포 j마다 평균 활성화가 ρ̂ⱼ ≈ ρ이기를 바란다.
    
    KL(ρ || ρ̂ⱼ) = ρ log(ρ/ρ̂ⱼ) + (1-ρ) log((1-ρ)/(1-ρ̂ⱼ))
    
    ρ̂ⱼ = ρ일 때 가장 작다.
    """
    # 배치에 걸친 신경 세포마다의 평균 활성화
...
| **KL** | 목표 $\rho$에서 벌어진 만큼 벌주기 | 성김을 정밀히 다스린다 | 에스자 활성화가 필요하다 |

---

## 3. 3부: 오그림 벌주기

**오그리는 오토인코더(CAE)**는 인코더 야코비 행렬의 프로베니우스 노름에 벌을 주어 인코더가 들임의 흔들림에 무디도록 이끈다.
...
- $J_f(x) = \frac{\partial f(x)}{\partial x} \in \mathbb{R}^{k \times d}$: 인코더의 야코비 행렬
...
1. **평평한 숨은 다양체:** 인코더가 내놓는 것이 들임에 따라 천천히 바뀐다
...
잡음 없애는 오토인코더는 근본이 다른 익히기 목표를 쓴다. 곧 **망가뜨린** 들임에서 **깨끗한** 자료를 다시 세운다.
...
흩어짐이 $\sigma^2$인 작은 정규 잡음에서 잡음 없애는 오토인코더는 넌지시 **점수 함수**를 어림한다:
...
성긴 오토인코더는 빽빽한 오토인코더보다 흔히 더 풀이하기 쉬운 특징을 배운다:
...
??? success "연습문제 1 풀이"
    ...
    **중앙값**이고, MNIST 화소의 대부분이 배경 0이므로 화소마다의 중앙값이 0이다.
    ...
    다만 BCE를 쓰려면 출력이 반드시 $[0,1]$이어야 한다. 그래서 디코더 끝에 시그모이드가
...
??? success "연습문제 2 풀이"
    ...
    변분 오토인코더에서는 이 선택이 더 중요해진다. 다시 세우기 항과 KL 항의 크기
    비가 곧 $\beta$ 노릇을 하기 때문이다([27장](../../ch26/index.md)).
...
??? success "연습문제 3 풀이"
    ...
    그래서 MSE로 익힌 오토인코더의 출력은 특유의 뿌연 느낌을 갖는다. 이 흐릿함은
    오토인코더와 변분 오토인코더가 공통으로 겪는 문제이며,
    [맞겨루기 생성기](../../ch29/index.md)가 손실 자체를 버리고 판별기로 바꾼 까닭
    가운데 하나다.
...
성긴 오토인코더는 코드 대부분이 0이 되도록 벌점을 준다. 병목을 좁히는 것과 무엇이 다른가?
...
??? success "연습문제 5 풀이"
    ...
    **KL**은 단위마다의 평균 활성화 $\hat\rho_j$를 목표 $\rho$(이를테면 0.05)에
    맞춘다.
    ...
    둘째, **KL은 평균을 본다.** 자료 전체에 걸친 평균 활성화를 맞추므로, 한 자료에서
...
성김 벌점에 쓰는 KL은 [27장](../../ch26/index.md)의 KL 항과 같은 것인가?
...
??? success "연습문제 6 풀이"
    **식의 이름만 같고 하는 일이 다르다.**
    ...
    | | 성긴 오토인코더 | 변분 오토인코더 |
    |---|---|---|
    | 무엇의 KL인가 | 단위마다의 **평균 활성화** $\hat\rho_j$와 목표 $\rho$ | 자료마다의 **사후 분포** $q(z \mid x)$와 사전 분포 $p(z)$ |
    | 확률인가 | 활성화를 확률처럼 **본** 것 | 진짜 확률 분포 |
    | 자료마다 다른가 | 아니다, 자료 전체의 평균 | 그렇다, $x$마다 다르다 |
    | 뽑을 수 있게 되는가 | **아니다** | 그렇다 |

    성긴 오토인코더의 KL은 두 베르누이 분포 사이의 거리를 빌려 와 "이 단위가 켜지는
    빈도를 5%로 맞추라"고 말하는 **정칙화 장치**다. 활성화 값은 확률이 아니며 아무도
    그것을 분포로 다루지 않는다.

    변분 오토인코더의 KL은 실제로 인코더가 내놓은 분포와 사전 분포 사이의 거리이고,
    그 항이 있어야 증거 하한이 성립한다. 정칙화로 덧붙인 것이 아니라 **유도에서 나온
    것**이다.

    그래서 성긴 오토인코더는 벌점을 아무리 세게 주어도
...
??? success "연습문제 8 풀이"
    ...
    오그림 항만 있다면 인코더가 상수 함수가 되는 것이 최선이다. 야코비가 0이 되어
    벌점이 완전히 사라진다. 그러나 그러면 다시 세우기가 불가능해져 첫째 항이
    폭발한다.

    두 힘이 맞서면서 인코더는 **자료가 실제로 놓인 방향**은 살리고 **놓이지 않은
    방향**만 누르게 된다. 다양체를 따라가는 방향으로는 민감해야 되돌릴 수 있고,
    다양체에서 벗어나는 방향으로는 둔감해도 잃을 것이 없기 때문이다.

    그래서 오그리는 오토인코더가 배우는 것은 **접평면**이다. 야코비의 특이값을
...
잡음 없애는 오토인코더와 오그리는 오토인코더는 무엇이 같고 무엇이 다른가?

</div>

??? success "연습문제 9 풀이"
    **같은 것**은 목표다. 둘 다 인코더가 입력의 작은 흔들림에 둔감해지기를 바란다.
...
??? success "연습문제 10 풀이"
    ...
    다시 세우기 손실만 두면 오토인코더는 아무것도 버리지 않으려 한다. 극단적으로
    ...
    [변분 오토인코더](../../ch26/index.md)의 KL 항도 이 목록에 들어간다. 그것은
...
| **L1 성김** | $\lambda\sum\|h_j\|$ | 활성화를 0으로 몬다 | 풀이할 수 있는 특징 |
| **KL 성김** | $\beta\sum\text{KL}(\rho\|\hat{\rho}_j)$ | 성김을 정밀히 다스림 | 넘치게 갖춘 오토인코더 |
| **오그림** | $\lambda\|J_f\|_F^2$ | 들임에 무딘 인코더 | 튼튼한 다양체 배우기 |
| **잡음 없애기** | 들임을 망가뜨리고 깨끗한 것을 다시 세움 | 넌지시 벌주기 | 튼튼한 특징 |

**핵심 눈썰미:** 손실 함수와 벌주기를 고르는 것이 오토인코더가 무엇을 배우는지 근본에서 정한다. 다시 세우기 손실은 자료 잡음에 대한 가정을 담고, 벌주기 항은 숨은 공간의 기하와 풀이 가능함을 빚는다.
