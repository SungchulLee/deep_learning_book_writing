...
??? success "연습문제 5 풀이"
    ...
    [오토인코더 모듈](../architecture/autoencoder.md) 쪽에 그대로 실어 둔다.
...
익히기와 값매김에서 `model.train()`과 `model.eval()`을 부르는데, 이 오토인코더에서 실제로 달라지는 것이 있는가?
...
??? success "연습문제 7 풀이"
    ...
    - **어떤 부류를 못 배우는가.** 8과 5가 오래 뭉개져 있다면
      ...
      셈이다.
    - **무너지고 있는가.** 모든 출력이 같은 그림으로 수렴하면 인코더가 죽은 것이다.
...
배치 크기를 바꾸면 무엇이 달라지는가? 오토인코더에서도 [3.2절](../../ch03/linear_softmax/05_gradient_descent.md)과 같은 이야기인가?
...
??? success "연습문제 8 풀이"
    ...
    오토인코더에 특별한 점이 하나 있다. 손실을 화소에 대해 `sum`으로 더하고 표본에
...
오토인코더를 익힐 때 검증 자료가 필요한가? 무엇을 보고 언제 멈출 것인가?
...
??? success "연습문제 9 풀이"
    ...
    지도 학습에서는 검증 손실이 올라가기 시작하면 멈춘다. 오토인코더도 같은 신호를
    쓸 수 있지만, **병목이 좁으면 그 신호가 거의 오지 않는다.** MNIST에 병목 16이면
    외울 그릇이 없어 학습 오차와 검증 오차가 붙어 간다
    ([오토인코더 연습문제 7](../ae/48_autoencoder.md)).
...
익힌 오토인코더를 저장할 때 무엇을 함께 저장해야 다시 쓸 수 있는가?
