...
??? success "연습문제 5 풀이"
    ...
    ([24장 모듈 연습문제 2](../../ch24/architecture/autoencoder.md)). 둘이 붙어 있으면
    이 일을 할 수 없다.

    자기 부호기에도 같은 코드를 돌릴 수 있다. 문법적으로 막는 것이 없다. 다만 결과가
    얼룩이다([23.5절](../../ch24/limits/latent_sampling.md)). 그 차이가 이 장의
    존재 이유다.
...
??? success "연습문제 8 풀이"
    ...
    **변분 자기 부호기가 오히려 조금 나쁘다**(차이가 의미 있을 만큼 크지는 않다).
    ...
    [무작위로 뽑기](../../ch24/limits/latent_sampling.md)이고, 거기서는 0.2% 대 57.4%로
    크게 벌어진다.
...
??? success "연습문제 9 풀이"
    ...
    이것이 [28장](../../ch28/index.md)의 잣대들이 풀려는 문제다. 인셉션 점수는 이
