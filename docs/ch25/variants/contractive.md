# 오그리는 오토인코더
들임의 흔들림에 대한 인코더의 민감함에 대놓고 벌을 주어 튼튼한 나타냄을 배운다.

---

**배울 것:**

- 인코더의 야코비 행렬과 민감함을 재는 데서의 노릇
- 오그림 벌주기: 인코더 야코비 행렬의 프로베니우스 노름
- 다양체 배움으로 보는 기하 풀이
- 잡음 없애는 오토인코더와의 이음
...
**오그리는 오토인코더(CAE)**는 여느 다시 세우기 손실에 인코더 야코비 행렬의 프로베니우스 노름 벌주기를 더한다:

$$\mathcal{L} = \underbrace{\|x - g(f(x))\|^2}_{\text{다시 세우기}} + \underbrace{\lambda \|J_f(x)\|_F^2}_{\text{오그림 벌주기}}$$

여기서 각 기호는 다음과 같다.

- $f$: 들임을 숨은 나타냄에 옮기는 인코더 함수
- $g$: 숨은 것을 들임 공간으로 되돌리는 디코더 함수
...
- **튼튼한 나타냄:** 인코더가 요긴한 짜임은 잡고 잡음은 무시하는 법을 배운다
...
다시 세우기 항은 (정확히 다시 세우려) $f$이 들임의 모든 흔들림에 민감하기를 바라고 오그림 항은 무디기를 바란다. 이 저울질이 인코더를 다시 세우는 데 중요한 방향, 곧 자료 다양체를 따라서만 민감하게 만든다.
...
흩어짐이 $\sigma^2$인 작은 정규 잡음에서 잡음 없애는 오토인코더의 목표는 대략 다음을 가장 작게 한다:
...
오그림 벌주기는 인코더가 다음과 같은 대응을 배우도록 이끈다:

1. **자료 다양체 방향 지킴:** 인코더가 자료가 실제로 놓인 방향을 따라 바뀐다
...
오그리는 오토인코더는 금융에서 **안정된 요인 나타냄**을 배우는 데 값지다:
...
- **벌주기를 갖춘 공분산 어림:** 오그리는 인코더가 배운 공분산 짜임에 넌지시 벌을 주어 차원 높은 상황에서 어림 어긋남을 줄인다
...
$\lambda$를 아주 크게 하면 무슨 일이 일어나겠는가? 그 극한에서 인코더는 무엇이 되는가?
...
??? success "연습문제 2 풀이"
    ...
    곧 인코더가 모든 입력을 같은 코드로 보낸다. 그러면 디코더는 입력이 무엇이든 같은
...
??? success "연습문제 3 풀이"
    ...
    코드가 16이면 16배, 128이면 128배다. 오그리는 오토인코더가 널리 쓰이지 않는
...
오그림 벌점을 인코더가 아니라 **디코더**에 걸면 어떻게 되는가?
...
??? success "연습문제 4 풀이"
    ...
    인코더의 야코비 $\partial z / \partial x$를 누르면 "입력이 흔들려도 코드가 덜
    흔들린다"는 뜻이었다. 디코더의 야코비 $\partial \hat{x} / \partial z$를 누르면
    **"코드가 흔들려도 그림이 덜 흔들린다"**가 된다.
    ...
    디코더를 매끄럽게 해도 코드가 **어디에 놓이는지**는 여전히 아무도 단속하지 않기
    때문이다. 빈 곳에서 뽑으면 매끄럽게 바뀌는 얼룩이 나올 뿐이다.

    그래도 이 생각은 살아 있다. 디코더의 야코비를 제어하는 것은 숨은 공간의 기하를
    다루는 여러 방법의 출발점이고, 특히 흐름 모델에서는 그 야코비의 행렬식이
    **모델의 정의 자체**에 들어간다([28장](../../ch27/index.md)).
...
오그림 벌점을 준 인코더의 야코비 특이값을 구해 보라. 무엇을 기대할 수 있는가?
...
??? success "연습문제 6 풀이"
    ...
    이것이 오그리는 오토인코더가 다른 정칙화와 구별되는 점이다. 벌점을 주면서도
...
??? success "연습문제 7 풀이"
    ...
    코드가 그보다 덜 움직이므로, 인코더가 공간을 **오그린다.**

    다만 이름이 약속하는 것보다는 느슨하다. 벌점은 야코비를 작게 만들 뿐 1 이하로
    묶어 주지는 않으므로, 익힌 인코더가 엄밀한 뜻의 오그림 사상이라는 보장은 없다.
...
오그림 벌점은 인코더에만 걸고 디코더에는 걸지 않는다. 디코더는 정칙화하지 않아도 되는가?

</div>

??? success "연습문제 8 풀이"
    이 방법의 관심이 **나타냄**에 있기 때문이다. 무엇을 코드에 담고 무엇을 버릴지는
    인코더가 정하므로, 버리라는 압력도 인코더에 걸어야 한다.

    디코더를 정칙화하지 않아도 되는 까닭은, 디코더가 마음대로 해도 **다시 세우기 손실이
    이미 단속하고 있기** 때문이다. 디코더가 엉뚱한 함수가 되면 곧바로 오차로 드러난다.
    반면 인코더는 코드를 아무렇게나 흩어 놓아도 디코더가 따라가 주기만 하면 오차가
    늘지 않는다. 단속받지 않는 자유가 인코더 쪽에 있는 것이다.

    이 비대칭이 이 장 전체를 관통한다. [뽑기 문제](../limits/latent_sampling.md)도
    인코더가 코드를 어디에 놓든 벌점이 없다는 같은 사실에서 나온다.
...
오그리는 오토인코더를 실제 일에서 쓸 것인가? 쓴다면 어떤 자리인가?

</div>

??? success "연습문제 10 풀이"
    솔직히 말해 자주 쓰이지 않는다. 야코비를 셈하는 값이 코드 차원에 비례해 드는데,
    [잡음 없애는 오토인코더](../architecture/03_ae_denoising.md)가 공짜로 비슷한
    효과를 내기 때문이다.
...
**핵심 눈썰미:** 오그리는 오토인코더는 인코더의 민감함에 곧바로 벌을 주어 들임의 흔들림에 튼튼한 나타냄을 배우는 원칙 있는 길을 준다. (작은 정규 잡음에서) 잡음 없애는 오토인코더와 이론상 같다는 점이 겉보기에 다른 두 벌주기 전략을 하나의 얼거리로 아우른다.
