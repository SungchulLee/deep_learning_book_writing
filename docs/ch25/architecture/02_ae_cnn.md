...
누비기마다 뒤에 두는 배치 고르게 맞추기가 배치에 걸쳐 활성화를 고르게 해 익히기를 안정시킨다. 없으면 층의 들임 분포가 익히는 동안 바뀌는 속 공변량 옮김이 생겨 모임이 느려질 수 있다. 배치 고르게 맞추기와 정류 선형을 아우른 것은 요즘 누비기 얼개의 표준 벽돌이다.

디코더는 옮겨 놓은 누비기 대신 `nn.Upsample(mode='nearest')` 뒤에 누비기를 둔다. 이 "크기 바꾸고 누비기" 방식은 옮겨 놓은 알맹이가 고르지 않게 겹쳐 생기는 바둑판 무늬 헛것을 피한다. 마지막 에스자 활성화가 내놓는 화소를 $[0, 1]$으로 옭아맨다.
