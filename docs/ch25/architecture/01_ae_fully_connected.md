...
온전히 이어진 오토인코더는 눌러 담은 자료 나타냄을 배우는 가장 단순한 신경망 얼개이다. 주성분 분석의 선형 쏘기와 달리 오토인코더는 비선형 깨어남 함수로 더 풍부하고 차원이 낮은 부호를 찾아낸다. 이 보기는 인코더(784에서 32차원)와 디코더(32에서 784로)를 갖춘 온전한 오토인코더를 세우고 두 값 엇갈린 엔트로피 손실로 MNIST에 익혀 이끌리지 않은 배움의 온전한 물길을 보인다.
...
병목 층(32차원)이 그물로 하여금 잡음과 남아도는 것은 버리고 숫자 그림의 요긴한 특징을 잡는 눌러 담은 나타냄을 배우게 한다. 성분 32개를 쓴 주성분 분석과 비슷하지만 비선형 인코더와 디코더는 선형 방법이 놓치는 굽은 다양체 짜임을 잡을 수 있다. 에스자 내놓기 깨어남이 다시 세운 것을 $[0, 1]$에 두어 고른 화소 범위에 맞춘다.
