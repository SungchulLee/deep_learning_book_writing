...
온전히 이어진 오토인코더는 눌러 담은 자료 나타냄을 배우는 가장 단순한 신경망 얼개이다. 주성분 분석의 선형 쏘기와 달리 오토인코더는 비선형 활성화 함수로 더 풍부하고 차원이 낮은 부호를 찾아낸다. 이 보기는 인코더(784에서 32차원)와 디코더(32에서 784로)를 갖춘 온전한 오토인코더를 세우고 두 값 엇갈린 엔트로피 손실로 MNIST에 익혀 이끌리지 않은 배움의 온전한 물길을 보인다.
...
병목 층(32차원)이 그물로 하여금 잡음과 남아도는 것은 버리고 숫자 그림의 요긴한 특징을 잡는 눌러 담은 나타냄을 배우게 한다. 성분 32개를 쓴 주성분 분석과 비슷하지만 비선형 인코더와 디코더는 선형 방법이 놓치는 굽은 다양체 짜임을 잡을 수 있다. 에스자 내놓기 활성화가 다시 세운 것을 $[0, 1]$에 두어 고른 화소 범위에 맞춘다.

$[0, 1]$ 자료에서는 두 값 엇갈린 엔트로피 손실이 평균 제곱 어긋남보다 낫다. 화소마다 베르누이 확률 변수로 보아 0이나 1에 가까운 화소에 더 센 기울기를 주기 때문이다. 차원을 차츰 줄이는(784에서 128, 64, 32로) 대칭 인코더-디코더 얼개는 갑작스러운 병목보다 가장 좋게 하기 쉬운 매끄러운 눌러 담기 길을 만든다.

익힌 오토인코더의 숨은 공간은 숫자 그림을 눈에 보이는 닮음으로 갈무리한다. 곧 비슷한 숫자가 함께 무리 지고 숨은 부호 사이를 사이 끼움하면 숫자 모양이 매끄럽게 옮아간다. 다만 변분 오토인코더와 달리 여느 오토인코더의 숨은 공간에는 벌주기가 없어 풀었을 때 뜻 없는 것이 나오는 "구멍"이 있을 수 있다.
...
숨은 차원 2, 8, 32, 128로 오토인코더를 익혀라. 숨은 차원에 대한 시험 다시 세우기 평균 제곱 어긋남을 그리고 얻는 것이 줄어드는 지점을 찾아라.
...
??? success "연습문제 1 풀이"
    ...
    ```
    평균 제곱 어긋남이 2에서 32차원까지 가파르게 떨어진 뒤 평평해진다. 64차원을 넘으면 나아짐이 미미하다. 디코더가 특징 32~64개로도 잘 다시 세울 담이를 갖췄기 때문이다.
...
인코더 층에 떨구기(비율 0.3)를 더하고 떨구기가 있을 때와 없을 때의 시험 다시 세우기 어긋남을 견주어라. 이 이끌리지 않은 상황에서 벌주기가 지나치게 맞춰지는 것을 막는 데 도움이 되는가?
...
??? success "연습문제 3 풀이"
    ...
    ```
    떨구기를 쓰면 익히기 손실은 흔히 더 높지만 익히기와 시험 손실의 벌어짐이 줄어든다. MNIST 오토인코더에서는 자료 묶음이 모델에 견주어 커서 지나치게 맞춰짐이 크지 않으므로 떨구기의 덕이 크지 않다. 자료 묶음이 작으면 벌주기 효과가 더 뚜렷할 것이다.
...
완전 연결 오토인코더와 누비기 오토인코더를 **같은 16차원 병목**으로 맞추어 견주어라. 어느 쪽이 이기며 매개변수는 얼마나 드는가?
...
??? success "연습문제 4 풀이"
    ...
    까닭은 3.4절에서 본 것과 같다. 완전 연결 인코더는 첫 줄에서 그림을 펼쳐 화소의
...
??? success "연습문제 5 풀이"
    ...
    그러므로 이 줄은 **다시 세우기 오차가 오토인코더의 품질을 재지 못한다**는 증거로
    읽어야 한다. 가장 낮은 오차를 가장 쓸모없는 설정이 냈다.

    이 함정은 흔하다. 누비기 오토인코더를 쓸 때 특징 맵을 그대로 코드라 부르는 코드를
...
??? success "연습문제 6 풀이"
    ...
    깊이가 하는 일은 인코더가 표현할 수 있는 함수의 모둠을 넓히는 것이다.
...
인코더와 디코더의 가중치를 묶으면($W_{\text{dec}} = W_{\text{enc}}^{\top}$) 무엇이 좋고 무엇을 잃는가?
...
??? success "연습문제 7 풀이"
    ...
    **잃는 것**은 표현력이다. 인코더와 디코더가 따로일 때는 $W_2 W_1$이 임의의 계수
...
??? success "연습문제 8 풀이"
    ...
    예외가 성긴 오토인코더다. 거기서는 코드가 0이 되기를 **바라므로** ReLU가 오히려
...
오토인코더의 코드에 아무 의미도 부여되지 않는데, 코드의 차원 하나를 움직이면 그림이 어떻게 달라지는가? 무엇을 기대할 수 있고 무엇은 기대할 수 없는가?

</div>

??? success "연습문제 9 풀이"
    **기대할 수 있는 것**은 그림이 달라진다는 것뿐이다. 코드가 바뀌면 디코더의 출력이
    바뀌고, 대개 매끄럽게 바뀐다.
    ...
    **기대할 수 없는 것**은 그 변화가 사람이 알아볼 뜻을 갖는 것이다. "이 차원은
    ...
    무게를 키워 그 일을 시도하며([27장](../../ch26/index.md)), 성김 벌점도 부분적으로
    같은 구실을 한다.
...
이 쪽의 오토인코더를 MNIST가 아닌 자료에 그대로 쓸 수 있는가?
...
??? success "연습문제 10 풀이"
    ...
    **둘째, 병목 크기.** 16은 MNIST에 맞춘 값이다. 자료가 더 복잡하면 늘려야 하고,
    적당한 값은 오차가 꺾이는 자리를 보고 고른다([오토인코더 연습문제 6](../ae/48_autoencoder.md)).

    그림 자료라면 애초에 [누비기 쪽](02_ae_cnn.md)을 쓰는 편이 낫다. 완전 연결
    오토인코더는 구조가 없는 표 형태 자료에 더 어울린다.
