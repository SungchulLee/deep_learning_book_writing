...
이 단원은 차원 줄이기와 다시 세우기를 위한 말끔하고 다시 쓸 수 있는 `SimpleAutoencoder` 갈래를 정한다. 얼개는 정류 선형 깨어남과 에스자 내놓기를 갖춘 세 층짜리 온전히 이어진 인코더와 디코더 그물을 쓴다. 잡음 없애는 오토인코더나 변분 오토인코더 같은 더 복잡한 얼개의 바탕 벽돌 노릇을 하며 `encode`, `decode`, `loss_function` 방법을 준다.
...
??? success "연습문제 2 풀이"
    두 값 엇갈린 엔트로피는 어림과 목표가 모두 $[0, 1]$에 있어야 하며 이를 베르누이 확률 변수의 확률로 본다. 에스자 깨어남이 디코더의 내놓기를 $(0, 1)$에 옮겨 이 제약을 채운다. 들임 자료도 $[0, 1]$으로 골라야 한다(MNIST 그림은 `ToTensor()` 뒤 그렇게 된다). 이 범위 밖 자료에 두 값 엇갈린 엔트로피를 쓰면 음의 손실이나 NaN 기울기가 나온다.
