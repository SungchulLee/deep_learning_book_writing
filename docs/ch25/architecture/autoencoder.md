...
이 단원은 차원 줄이기와 다시 세우기를 위한 말끔하고 다시 쓸 수 있는 `SimpleAutoencoder` 갈래를 정한다. 얼개는 정류 선형 활성화와 에스자 내놓기를 갖춘 세 층짜리 온전히 이어진 인코더와 디코더 그물을 쓴다. 잡음 없애는 오토인코더나 변분 오토인코더 같은 더 복잡한 얼개의 바탕 벽돌 노릇을 하며 `encode`, `decode`, `loss_function` 방법을 준다.
...
평균 제곱 어긋남 손실에서 붙박이인 `'mean'` 대신 `reduction='sum'`을 쓰는 것은 기울기의 잣수에 영향을 주는 일부러 한 고름이다. 합으로 줄이면 손실이 배치 크기와 들임 차원에 비례하므로 배움 빠르기가 사실상 그 값에 따라 잣수 맞춰진다. 변분 오토인코더 짜기 여럿이 KL 벌어짐 항(이 또한 흔히 차원에 걸쳐 더한다)과 결을 맞추려 합으로 줄인다.

이 최소 짜기에 배치 고르게 맞추기, 떨구기, 무게 잦아듦이 없다는 것은 다스려진 바탕으로 쓸 수 있다는 뜻이다. 변분 오토인코더나 잡음 없애는 오토인코더로 넓힐 때는 흔히 벌주기와 고르게 맞추기 층을 더하지만, 말끔한 바탕 갈래가 있으면 고침마다의 효과를 따로 떼어 보기 쉽다.
...
평균 제곱 어긋남 손실을 두 값 엇갈린 엔트로피 손실로 갈음하라. 두 값 엇갈린 엔트로피가 옳으려면 들임 자료가 어떤 제약을 채워야 하며, 에스자 내놓기 활성화가 왜 그것을 보장하는가?

</div>

??? success "연습문제 2 풀이"
    두 값 엇갈린 엔트로피는 어림과 목표가 모두 $[0, 1]$에 있어야 하며 이를 베르누이 확률 변수의 확률로 본다. 에스자 활성화가 디코더의 내놓기를 $(0, 1)$에 옮겨 이 제약을 채운다. 들임 자료도 $[0, 1]$으로 골라야 한다(MNIST 그림은 `ToTensor()` 뒤 그렇게 된다). 이 범위 밖 자료에 두 값 엇갈린 엔트로피를 쓰면 음의 손실이나 NaN 기울기가 나온다.
...
`SimpleAutoencoder`의 매개변수를 층별로 세어 보라. 인코더와 디코더 가운데 어느 쪽이 더 무거운가?
...
??? success "연습문제 4 풀이"
    ...
    | | 가중치 | 편향 | 합 |
    |---|---|---|---|
    | 인코더 | $784{\cdot}512 + 512{\cdot}256 + 256{\cdot}16$ | $512{+}256{+}16$ | 537,360 |
    | 디코더 | $16{\cdot}256 + 256{\cdot}512 + 512{\cdot}784$ | $256{+}512{+}784$ | 538,128 |
    | 합 | | | **1,075,488** |

    대칭 설계이므로 거의 같고, 디코더가 768개 더 많다. 편향의 개수가 다르기 때문이다.
    인코더는 512, 256, 16개의 편향을 갖고 디코더는 256, 512, 784개를 갖는다.
...
??? success "연습문제 5 풀이"
    ...
    [뽑기 실험](../limits/latent_sampling.md)이 좋은 예다. 거기서는 인코더를 전혀 쓰지
    않고 $z \sim \mathcal{N}(0,I)$를 `decode`에만 넣는다. 둘이 붙어 있으면 그런 실험을
    할 수 없다.

    변분 오토인코더로 넘어가면 이 구분이 더 중요해진다. `encode`가 값 하나가 아니라
    $(\mu, \log\sigma^2)$ 짝을 내놓게 바뀌므로, 경계가 또렷해야 무엇이 달라졌는지
    보인다([27장](../../ch26/index.md)).
...
??? success "연습문제 6 풀이"
    보통 오토인코더라면 밖에 두어도 된다. 손실이 출력과 표적만 있으면 셈해지기
    때문이다.

    안에 두는 편이 나은 경우는 **손실이 모델 내부를 알아야 할 때**다.

    - 변분 오토인코더: KL 항이 $\mu$와 $\log\sigma^2$를 필요로 한다
    - 성긴 오토인코더: 벌점이 은닉 활성화를 필요로 한다
    - 오그리는 오토인코더: 벌점이 야코비를 필요로 한다
...
`SimpleAutoencoder`의 디코더 끝에 시그모이드가 붙어 있다. 왜인가? 떼면 무슨 일이 생기는가?
...
??? success "연습문제 7 풀이"
    ...
    "선형" 오토인코더를 만든다면서 시그모이드를 남겨 두면 그것은 선형이 아니다.
...
이 모듈을 잡음 없애는 오토인코더로 바꾸려면 무엇을 고쳐야 하는가?
...
??? success "연습문제 8 풀이"
    **모델은 한 줄도 고치지 않는다.** 익히기 반복문에서 입력만 더럽히면 된다.
    ...
    이 비대칭이 잡음 없애는 오토인코더의 전부다
    ([잡음 없애는 오토인코더](03_ae_denoising.md)).

    같은 모듈로 이렇게 여러 변형을 만들 수 있다는 점이 이 설계의 값어치다. 성김은
    손실에 항을 더하면 되고, 오그림은 야코비 벌점을 더하면 된다. 바뀌는 것은 익히기
    절차이지 얼개가 아니다.

    바뀌는 폭이 얼개까지 미치는 것이 변분 오토인코더다. 인코더가 내놓는 것의 개수가
    달라지므로 모듈 자체를 손봐야 한다.
...
이 모듈을 그대로 두고 변분 오토인코더를 만들려면 무엇을 더해야 하는지 코드로 적어라.

</div>

??? success "연습문제 9 풀이"
    인코더의 마지막 층을 $2k$차원으로 늘려 $\mu$와 $\log\sigma^2$을 함께 내놓게 하고,
    다시 뽑기와 KL 항을 더한다.
