...
잡음 없애는 오토인코더(DAE)는 망가뜨린 들임에서 깨끗한 그림을 다시 세우도록 익혀, 그물이 신호와 잡음을 가르는 튼튼한 나타냄을 배우게 한다. 뻔한 항등 대응을 배울 수 있는 여느 오토인코더와 달리 잡음 없애는 오토인코더는 잡음을 없애려 뜻 있는 특징을 뽑아야 한다. 이 보기는 정규, 소금과 후추, 떨구기 잡음의 망가뜨리기 전략을 짜고 최대 신호 대 잡음비로 잡음 없애기 성능을 값매김한다.
...
여느 오토인코더와의 핵심 차이는 들임은 망가뜨리되 목표는 깨끗이 둔다는 것이다. 곧 손실이 그물의 내놓기를 잡음 낀 들임이 아니라 본디 그림과 견준다. 그래서 인코더가 더한 잡음은 무시하고 바탕 숫자 짜임을 잡는 특징을 배우게 된다. 그렇게 얻은 나타냄은 흔히 여느 오토인코더의 것보다 뒤따르는 일에 더 쓸모 있다.

세 잡음 갈래가 실제 세상의 서로 다른 망가짐을 나타낸다. 정규 잡음은 사진기와 과학 기기의 감지기 잡음을 흉내낸다. 소금과 후추 잡음은 디지털 그림의 죽은 화소와 전송 어긋남을 나타낸다. 떨구기 잡음(화소를 아무렇게나 0으로 만들기)은 가려짐과 빠진 자료를 흉내낸다. 잘 익힌 잡음 없애는 오토인코더는 익힌 잡음 수준은 다루지만 다시 익히지 않으면 못 본 잡음 갈래에는 잘 통하지 않는다.

성능은 최대 신호 대 잡음비(PSNR)로 재며 $[0, 1]$ 신호에서 $\text{PSNR} = -10 \log_{10}(\text{MSE})$으로 정의한다. 잡음 낀 들임보다 흔히 5~10 dB 나아지며, 이는 다시 세운 그림이 훨씬 깨끗하다는 뜻이다. 실전 잡음 없애기에는 건너뛰는 이음을 갖춘 U-그물 같은 더 정교한 얼개가 대체로 여기 쓴 단순한 인코더-디코더 짜임보다 낫다.
...
정규, 소금과 후추, 떨구기 잡음마다 따로 잡음 없애는 오토인코더 셋을 익혀라. 모델마다 세 잡음 갈래로 모두 시험해 최대 신호 대 잡음비 나아짐의 3x3 표를 만들어라. 어느 모델이 잡음 갈래에 걸쳐 가장 두루 통하는가?
...
잡음 인수를 0.3에서 0.5, 0.7로 올려라. 어느 잡음 수준에서 잡음 없애는 오토인코더가 알아볼 만한 다시 세우기를 못 내는가? 잡음 수준에 대한 평균 최대 신호 대 잡음비 나아짐을 그려라.
...
??? success "연습문제 2 풀이"
    ...
    for nf in [0.1, 0.3, 0.5, 0.7, 0.9]:
        # noise_factor=nf로 잡음 없애는 오토인코더를 익히고 최대 신호 대 잡음비 나아짐을 셈한다
        print(f"noise_factor={nf}: PSNR improvement = {improvement:.1f} dB")
    ```
    최대 신호 대 잡음비 나아짐은 흔히 잡음 인수 0.3~0.5쯤에서 가장 크다. 0.7 이상이면 들임이 너무 망가져 잡음 없애는 오토인코더가 숫자 짜임을 믿을 만하게 알아내지 못하고 다시 세운 것이 흐릿한 평균이 된다. dB 나아짐은 여전히 양수일 수 있으나 절대 품질은 나쁘다.
...
인코더에서 디코더로 건너뛰는 이음을 더해(U-그물 꼴 얼개를 만들어) 여느 잡음 없애는 오토인코더와 성능을 견주어라. 건너뛰는 이음이 잡음 없애기에 왜 도움이 되는가?

</div>

??? success "연습문제 3 풀이"
    건너뛰는 이음은 디코더가 앎 병목을 우회해 인코더 앞선 층의 해상도 높은 공간 세부에 닿게 한다. 잡음 없애기에 이로운 까닭은 인코더 앞선 층에 또렷한 모서리와 결을 다시 세우는 데 도움이 되는 잔 공간 앎이 있고, 병목은 신호와 잡음을 가르는 데 필요한 뜻을 잡기 때문이다. U-그물 얼개는 흔히 수수한 인코더-디코더 잡음 없애는 오토인코더보다 최대 신호 대 잡음비를 1~3 dB 높인다.
...
잡음 없애는 오토인코더와 보통 오토인코더를 깨끗한 입력과 더럽힌 입력 양쪽에서 견주어라. 어느 쪽이 이기는가?
...
잡음 없애는 오토인코더의 손실이 왜 더 나은 나타냄을 만드는가? 보통 오토인코더와 목표가 어떻게 다른지 식으로 적어라.
...
??? success "연습문제 5 풀이"
    ...
    **갈라낼** 수밖에 없고, 그러려면 자료가 실제로 어떻게 생겼는지 알아야 한다.
    ...
    걷어 내는 절차를 되풀이하면 [퍼짐 모델](../../ch30/index.md)이 된다. 잡음 없애는
    오토인코더는 그 계보의 첫걸음이다.
...
??? success "연습문제 7 풀이"
    너무 작으면 더럽힌 것과 깨끗한 것이 거의 같아 보통 오토인코더로 돌아간다.
...
??? success "연습문제 8 풀이"
    **매번 새로 뽑아야 한다.** 미리 만들어 고정해 두면 같은 $(\tilde{x}, x)$ 짝이
    ...
    두지 않고 배치를 꺼낼 때마다 새로 뽑는다. 그래서 `Dataset`의 `__getitem__` 안에서
    변환을 하는 것이 관례다.
...
잡음 없애는 오토인코더를 실제로 잡음 제거에 쓰려면 무엇을 조심해야 하는가?
...
??? success "연습문제 9 풀이"
    **익힌 잡음과 실제 잡음이 같아야 한다.** 가우시안으로 익힌 모델을 소금-후추 잡음에
    ...
    두 번째 생각이 [퍼짐 모델](../../ch30/index.md)에서 핵심이 된다. 거기서는 잡음
...
잡음 없애는 오토인코더가 배우는 것을 확률로 보면 무엇인가?
...
??? success "연습문제 10 풀이"
    ...
    이 장의 오토인코더가 뽑지 못하는 것과 대비된다. 잡음 없애는 오토인코더는
    저도 모르게 만들어 내는 모델의 재료를 쥐고 있었던 셈이고, 그 재료를 제대로 쓰는
    것이 [점수 바탕 모델과 퍼짐 모델](../../ch30/index.md)이다.

## 정리하며

**다룬 것** — 잡음 없애는 오토인코더

여느 오토인코더와의 핵심 차이는 들임은 망가뜨리되 목표는 깨끗이 둔다는 것이다.
