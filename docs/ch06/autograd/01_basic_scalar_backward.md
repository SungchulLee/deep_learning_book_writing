...
이 스크립트는 스칼라에 대한 기본적인 역전파를 보여준다. 이 개념들을 이해하는 것은 효과적인 PyTorch 프로그래밍과 딥러닝 모델 개발에 필수적이다.
...
"""
==============================================================================
첫걸음 튜토리얼 01: 홑값 역전파 기초
==============================================================================

학습 목표:
-------------------
1. PyTorch의 자동 미분 구조에서 "잎" 텐서가 무엇인지 이해한다
2. requires_grad=True로 기울기 좇기를 켜는 법을 배운다
3. 홑값 손실에 .backward()을 써서 기울기를 셈하는 법을 본다
...
- 셈 그래프: PyTorch가 셈을 좇으려고 절로 세운다
- 기울기 쌓임: 드러내 놓고 0으로 만들지 않으면 기울기가 더해진다
- 벡터-야코비 곱(VJP): PyTorch가 기울기를 잘 들게 셈하는 방식
...
def main():
    """
    홑값 손실 함수에 대한 기본 기울기 셈을 보인다.
    
    다음을 계산한다: loss = sum(x^2)
    ...
    print("="*70)
    print("""
    PyTorch는 벡터-야코비 곱(VJP)으로 사슬 법칙을 쓴다.
    ...
    - 셈마다 들어온 기울기 * 그 자리 야코비를 계산한다
    - 이것이 셈 그래프를 따라 거꾸로 퍼진다
    
    홑값이 아닌 출력에는 위쪽 기울기를 드러내 놓고 주어야 한다!
    (이는 앞선 학습에서 본다)
    """)
