...
이 스크립트는 완전한 선형 회귀를 보여준다. 이 개념들을 이해하는 것은 효과적인 PyTorch 프로그래밍과 딥러닝 모델 개발에 필수적이다.
...
"""
==============================================================================
첫걸음 튜토리얼 05: 온전한 선형 회귀 학습 루프
==============================================================================

학습 목표:
-------------------
1. 온전한 학습 루프를 밑바닥부터 짓는다
2. 여느 학습 돌이를 이해한다: 앞으로 → 손실 → 뒤로 → 고치기
3. 익힐 때 기울기를 제대로 다루는 법을 배운다
4. 학습이 나아가는 모습을 그림으로 본다
5. 자동 미분 없이 매개변수를 고치는 법을 이해한다

핵심 개념:
------------
- 학습 루프: 앞으로, 손실, 뒤로, 고치기(반복)
- 기울기 0으로 만들기: 역전파 앞마다 꼭 해야 한다
- torch.no_grad(): 매개변수를 고칠 때 쓰는 자리
- 손실 좇기: 모여드는지 지켜본다
- 인공 데이터: 깨끗한 시험 데이터를 만든다

참 세상에서의 쓰임:
-----------------------
이것이 모든 깊은 학습 학습 루프의 바탕이다!
...
def main():
    """
    선형 회귀 모델을 밑바닥부터 익히는 온전한 보기.
    x에서 y을 예측하며 매개변수 w(가중치)과 b(편향)을 배운다.
    """
    ...
    1. 순전파
       └─ 예측을 계산한다: y_pred = w*x + b
       └─ 손실을 계산한다: MSE = mean((y_pred - y)^2)
    
    2. 역전파
       └─ 묵은 기울기를 0으로 만든다: w.grad.zero_(), b.grad.zero_()
       └─ 새 기울기를 계산한다: loss.backward()
    
    3. 매개변수 고치기
       └─ 경사 하강법: w = w - lr * w.grad
       └─                   b = b - lr * b.grad
    ...
    • 새 기울기를 계산 앞에 묵은 것을 지워야 한다
    • 이를 잊으면 매개변수를 잘못 고치게 된다!
    
    학습률:
    • 너무 크면 학습이 흔들리고 퍼져 나갈 수 있다
    • 너무 작으면 더디게 모여든다
