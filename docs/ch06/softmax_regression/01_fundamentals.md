...
까닭은 두 자료의 성격에 있다. 손으로 쓴 숫자는 획이 놓이는 자리가 클래스마다 꽤 일정해서, 화소마다 가중치 하나씩만 두어도 상당히 가려낸다. 반면 셔츠와 외투는 윤곽이 비슷하고 차이가 무늬와 질감에 있는데, 이는 화소 하나하나의 밝기를 따로 저울질하는 방식으로는 잡아내기 어렵다. 이웃한 화소들이 이루는 무늬를 보아야 하며, 그것이 [9장 합성곱 신경망](../../ch09/index.md)이 하는 일이다.
