...
Note that batch normalization and activation layers are *not* counted—only convolutional and fully connected layers with learnable weight matrices contribute to the layer count.
...
The factor of 2 accounts for both multiply and add operations in the multiply-accumulate.
...
When $W = C/4$, this is approximately $17C^2HW/4 \approx 4.25C^2HW$, which is much less than a Basic Block despite having more layers. This computational efficiency is precisely why bottleneck blocks are used for deeper variants.
...
For quantitative finance applications where input is sequential rather than spatial, the ResNet architecture adapts naturally to 1D convolutions:

```python
class BasicBlock1D(nn.Module):
    """Basic residual block for 1D (temporal) data."""
...
# Or cosine annealing (often preferred for modern training)
...
He et al. (2019) showed that initializing the last batch normalization in each residual block to zero improves training. This ensures that at initialization, each residual block computes the identity:

```python
# After standard Kaiming init, zero-initialize the last BN in each block
...
def create_fine_tuned_classifier(
    ...
    Args:
        ...
        freeze_layers: Number of layer groups to freeze (0-7)
            0=conv1, 1=bn1, 2=layer1, ..., 5=layer4, 6=fc
    """
    model = resnet50(num_classes=1000)
    weights = torch.hub.load_state_dict_from_url(
        'https://download.pytorch.org/models/resnet50-19c8e357.pth'
...
### Document and Chart Recognition

ResNet backbones power OCR and chart recognition systems that extract data from financial documents, earnings reports, and historical price charts—a critical capability for alternative data pipelines.

### Satellite Imagery for Commodity Trading

ResNet-based models analyze satellite imagery to estimate crop yields, monitor shipping activity, and track construction—providing alpha signals for commodity and equity strategies.

### ResNet as Feature Backbone for Tabular Models

The staged, hierarchical feature extraction pattern of ResNet inspires architectures for tabular financial data, where residual connections help preserve raw feature information through deep networks:

```python
class ResNetTabular(nn.Module):
    """
    ResNet-inspired architecture for tabular financial data.
    
    Uses residual blocks with fully connected layers instead of
    convolutions, maintaining the skip connection principle.
    """
    
    def __init__(self, input_dim: int, hidden_dim: int = 256,
                 num_blocks: int = 4, num_classes: int = 1):
        super().__init__()
    ...
    @staticmethod
    def _make_block(dim: int) -> nn.Module:
...
These design choices enable training networks with hundreds of layers that achieve state-of-the-art performance across many computer vision tasks, and the residual principle has become a foundational building block in virtually all modern deep learning architectures.
...
4. He, T., Zhang, Z., Zhang, H., Zhang, Z., Xie, J., & Li, M. (2019). Bag of Tricks for Image Classification with Convolutional Neural Networks. *CVPR 2019*.
