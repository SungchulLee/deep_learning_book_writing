"""
CIFAR-10 CNN with Batch Normalisation + Dropout
=================================================

Same LeNet-style architecture as ``cifar10_cnn.py``, but with **both**
batch normalisation and dropout enabled.  This is the most regularised
variant in the series and is intended for direct comparison with the
base CNN, BatchNorm-only, and Dropout-only scripts.

Architecture
------------
::

    Conv2d(3→6, 5×5) → BatchNorm2d(6) → ReLU → MaxPool(2)
    Conv2d(6→16, 5×5) → BatchNorm2d(16) → ReLU → MaxPool(2)
    Flatten
    Linear(400→120) → ReLU → Dropout(0.5)
...
-----------------
+----------------------------------+-----------+---------+
| Script                           | BatchNorm | Dropout |
+==================================+===========+=========+
| ``cifar10_cnn.py``               |     ✗     |    ✗    |
+----------------------------------+-----------+---------+
| ``cifar10_cnn_batchnorm.py``     |     ✓     |    ✗    |
+----------------------------------+-----------+---------+
| ``cifar10_cnn_dropout.py``       |     ✗     |    ✓    |
+----------------------------------+-----------+---------+
| ``cifar10_cnn_batchnorm_dropout.py`` |  ✓   |    ✓    |
+----------------------------------+-----------+---------+

Run
---
    python cifar10_cnn_batchnorm_dropout.py
"""
...
# =============================================================================
def parse_args():
    parser = argparse.ArgumentParser(description="CIFAR-10 CNN + BatchNorm + Dropout")
    ...
    args.model_path = "./model/cnn_batchnorm_dropout.pth"
    os.makedirs("./model", exist_ok=True)

    # Regularisation flags — BOTH ON
...
# =============================================================================
# 3. Model  (identical to cifar10_cnn.ConvNet — flags change behaviour)
# =============================================================================
...
def main():
    ...
    print(f"BatchNorm: {args.use_batchnorm}  |  Dropout: {args.use_dropout} (p={args.dropout_prob})\n")
    ...
    show_predictions(test_loader, model, args, title="Before Training (BatchNorm + Dropout)")
    train(model, train_loader, optimizer, criterion, args)
    show_predictions(test_loader, model, args, title="After Training (BatchNorm + Dropout)")
