...
??? success "연습문제 3 풀이"
    인코더: 그래프 신경망이 얼개와 특징의 앎을 담은 마디 박아 넣기 $z_u = \text{GNN}(G, X)_u$을 셈한다. 디코더: 점수 함수가 변이 있는지 헤아린다. $\text{score}(u,v) = \sigma(z_u^T z_v)$(안쪽 곱)이거나 $\text{score}(u,v) = \text{MLP}(z_u \| z_v)$(이어 붙이기 + 여러 층 신경망)이다. 익히기: 헤아린 점수와 참 이름표(양의 변은 1, 음의 표본은 0) 사이의 두 값 교차 엔트로피를 가장 작게 한다. 그래프 신경망 인코더는 어림짐작을 넓힌다. 공통 이웃, 애더믹-애더, 더 복잡한 무늬까지 어림하도록 배울 수 있다.
