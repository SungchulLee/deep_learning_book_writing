...
여기서 $\mathbf{h}_u, \mathbf{h}_v$은 배운 마디 임베딩(보통 그래프 신경망에서 나온다)이고
...
```python
"""
그래프 신경망 임베딩과 안쪽 곱 점수로 하는 이음 헤아리기.
...
그래프 신경망 바탕 이음 헤아리기는 어떻게 도는가? 인코더-디코더 틀을 밝혀라.

</div>

??? success "연습문제 3 풀이"
    인코더: 그래프 신경망이 얼개와 특징의 앎을 담은 마디 임베딩 $z_u = \text{GNN}(G, X)_u$을 셈한다. 디코더: 점수 함수가 변이 있는지 헤아린다. $\text{score}(u,v) = \sigma(z_u^T z_v)$(안쪽 곱)이거나 $\text{score}(u,v) = \text{MLP}(z_u \| z_v)$(이어 붙이기 + 여러 층 신경망)이다. 익히기: 헤아린 점수와 참 이름표(양의 변은 1, 음의 표본은 0) 사이의 두 값 교차 엔트로피를 가장 작게 한다. 그래프 신경망 인코더는 어림짐작을 넓힌다. 공통 이웃, 애더믹-애더, 더 복잡한 무늬까지 어림하도록 배울 수 있다.
...
??? success "연습문제 4 풀이"
    여느 그래프 신경망은 마디 임베딩을 따로 셈한 뒤 합쳐 변 점수를 매긴다. 이러면 우연히 같은 임베딩을 받은, 얼개가 다른 마디 짝을 가려내지 못한다(보기로 그 자리 이웃은 같지만 겹침 무늬가 다른 두 짝). SEAL은 과녁 변 $(u,v)$ 둘레의 그 자리 부분 그래프를 뽑아내고 $u$과 $v$까지의 거리로 마디에 이름표를 매긴 뒤(두 반지름 마디 이름표 매기기) 이 이름표 붙은 부분 그래프에 그래프 신경망을 돌린다. 이는 이음 얼개를 드러나게 담아 모델이 자료에서 어떤 이음 어림짐작이든 배울 수 있게 한다.

## 정리하며

이 마당은 문제 정식화, 점수 함수, 학습, 평가 지표를 차례로 짚었다.
