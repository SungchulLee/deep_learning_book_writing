...
해밀턴 외(2017)의 **GraphSAGE**(뽑고 모으기)는 마디의 그 자리 이웃에서 특징을 뽑아 모아 마디 임베딩을 만드는 **귀납** 그래프 배움 틀을 내놓는다. 그래프 겹말기 신경망과 달리 GraphSAGE은 본 적 없는 마디와 그래프로도 넓혀진다.
...
가까운 마디는 임베딩이 비슷해야 하고 아무 마디는 달라야 한다.

---

## 5. 그래프 겹말기 신경망보다 나은 점

1. **귀납**: 다시 익히지 않고도 새 마디와 그래프로 넓혀진다
2. **키울 수 있음**: 이웃 뽑기가 온 그래프 셈을 피한다
3. **작은 배치 익히기**: 뽑기로 작은 배치를 자연스럽게 받쳐 준다
...
??? success "연습문제 4 풀이"
    스펙트럼 방법(보기로 ChebNet)은 그래프 라플라스의 고유 분해로 겹말기를 뜻매김해 원리 있는 신호 다루기 틀을 주지만 고유 분해($O(n^3)$)가 필요하고 그래프 위상이 붙박여 있어야 한다. 자리 방법(보기로 GraphSAGE, 그래프 어텐션 신경망)은 이웃 모으기로 겹말기를 뜻매김해 달라지는 그래프 얼개를 자연스럽게 다루고 작은 배치로 큰 그래프에도 키울 수 있다. 정확한 진동수 거르기가 중요한 작고 위상이 붙박인 그래프에는 스펙트럼 방법이, 귀납 배움이 필요한 크거나 바뀌거나 뒤섞인 그래프에는 자리 방법이 낫다. $\square$
