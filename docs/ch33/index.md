...
- [고치기 함수](message_passing/update.md) -- 스스로의 앎과 모은 이웃 쪽지를 합쳐 새 마디 임베딩을 만들기
...
- 그래프 어텐션 신경망(GAT) -- 자료에 따라 달라지는 이웃 중요도 무게를 배우는 어텐션 바탕 그래프 신경망
...
- [그래프 트랜스포머](advanced/graph_transformers.md) -- 온 자리 앎을 주고받으려 트랜스포머의 셀프 어텐션을 그래프에 쓰기(Graphormer, GPS)
...
- 그래프 가름 -- 그래프 신경망 층, 읽어내기 함수, 여러 층 신경망 분류기로 그래프 전체에 이름표 매기기
- 그래프 되돌이 맞춤 -- 양자 화학과 분자 녹음성에 쓰이는, 그래프 전체의 이어진 값 헤아리기
- [그래프 모으기](graph_tasks/pooling.md) -- 마디 임베딩을 그래프 나타냄으로 누르는 납작한 모으기 방법(합, 평균, 최대, 어텐션 무게)
- 켜진 모으기 -- 여러 눈금의 나타냄을 얻으려 DiffPool, TopKPool, SAGPool으로 그래프를 차츰 성글게 하기
- Set2Set -- 단순한 모으기보다 나타냄 힘이 센, 차례에 안 바뀌는 그래프 나타냄을 내놓는 어텐션 바탕 장단기 기억망 읽어내기
...
- 마디 임베딩 -- DeepWalk, Node2Vec, LINE, 견줌 그래프 신경망 방법으로 하는 스승 없는 마디 나타냄 배움
...
- 앎 그래프 -- 이음 헤아리기를 위한 R-GCN, CompGCN, 점수 함수로 하는 낱것과 관계 임베딩
