...
실제로 최고 수준의 약-과녁 주고받음 모델은 3차원 분자 기하, 결합 갈래, 원자 성질을 담는 더 정교한 분자 인코더(쪽지 건네기 신경망이나 SchNet 같은 것)를 쓴다. 또한 단백질 인코더에 눈길 얼개를 쓰고, 얼개가 비슷한 분자가 생물 활성에서 크게 다른 활성 절벽을 조심스레 다루며 더 큰 자료 묶음을 쓴다.
