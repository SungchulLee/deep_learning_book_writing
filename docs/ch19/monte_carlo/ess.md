# 실효 표본 크기
실효 표본 크기(ESS)는 무게 준 표본이나 얽힌 표본 배치의 정보량을 잰다. 중요도 표집에서 MCMC까지 몬테카를로 방법 전반에 쓰이는 근본 진단이다. 이 마당은 (중요도 표집처럼) 서로 독립인 무게 준 표본의 ESS을 다룬다. 자기상관에 기댄 MCMC의 ESS은 [진단](../mcmc/diagnostics.md)을 보아라.
