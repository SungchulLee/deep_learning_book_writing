...
```
Emissions shape: torch.Size([1, 5, 10])
BiLSTM-CRF model created successfully!
```
...
**다룬 것** — 두 방향 LSTM-CRF 이름 알아보기

`BiLSTM_CRF` 클래스는 PyTorch의 `nn.Module` 사이를 써서 모델 얼개를 감싼다.

고갱이 갈래는 `BiLSTM_CRF`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
