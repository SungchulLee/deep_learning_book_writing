...
*증명 얼개.* $S$ 위 확률 분포의 배치는 옹골지고($\mathbb{R}^N$에서 닫혀 있고 유계이고) 사상 $\mu \mapsto \mu P$은 이어져 있다. 브라우어 고정점 정리에 따라 고정점이 있다.
...
자세한 균형은 멈춤에 *넉넉하지만* *꼭 있어야 하는 것*은 아니다. 그러나 MCMC 알고리즘을 짜는 으뜸 도구이다. 메트로폴리스-헤이스팅스 알고리즘(17.4절)은 과녁 분포에 대해 자세한 균형을 만족하는 옮김 알맹이를 짓는다.
