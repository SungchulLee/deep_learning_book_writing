...
마르코프 사슬은 모든 마르코프 사슬 몬테카를로(MCMC) 방법의 이론적 등뼈이다. 과녁 분포로 모이는 표집기를 짓기에 앞서, 마르코프 사슬이 어떻게 흘러가는지, 옮김 확률이 여러 걸음에 걸쳐 어떻게 이어지는지, 그리고 그 동역학이 행렬 하나에 어떻게 온전히 담기는지를 이해해야 한다. 이 마당은 20장의 나머지를 떠받치는 핵심 정의와 셈 도구를 세운다.
...
**상태 공간** $S$은 사슬이 가질 수 있는 모든 값의 배치다:
...
16.1-18.2절은 주로 끝이 있는 상태 공간에 초점을 맞추고, 18.4절(MCMC)은 이어진 공간으로 넓힌다.
