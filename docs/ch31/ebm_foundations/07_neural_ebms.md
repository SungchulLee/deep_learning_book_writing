...
실제로 중요한 것은 음의 표본의 품질이다. 짧게 돌린 마르코프 사슬 몬테카를로는 참 모델 분포를 나타내지 못하는 치우친 표본을 만들어 익히기를 흔들리게 한다. 되돌림 곳간(앞의 표본을 담아 두었다가 다시 시작하기), 에너지 신경망의 스펙트럼 고르게 맞추기, 에너지 크기의 규칙 세우기 같은 재주가 익히기를 안정시키는 데 도움이 된다. 이 신경망 에너지 바탕 모델은 더 넓은 은근한 만들어 내는 모델 무리와 이어지며 점수 바탕 확산 모델과 깊이 이어져 있다.
