# 그래프 신경망으로 분자 성질 헤아리기

그래프 신경망으로 분자 성질 헤아리기는 그래프 신경망 쓰임새의 중요한 개념이다. 이 짜기는 얽힌 핵심 알고리즘과 자료 얼개를 손으로 만져 보게 하며 이론 바탕과 실제로 펼칠 때 살필 것을 함께 보여 준다.

## 1. 코드

```python
"""29.7.1: 그래프 신경망으로 하는 분자 성질 헤아리기."""
...
이 짜기는 그래프 신경망 분자 성질 헤아리기의 핵심 논리를 감싼 `MoleculeGNN` 갈래를 한가운데 둔다. 코드는 알고리즘 조각을 보여 주기와 따지기 논리에서 떼어 놓는 조각 짜기를 따른다.
