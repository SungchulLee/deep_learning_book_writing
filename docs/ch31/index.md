...
- 결합 에너지 모델(JEM) -- 분류기의 로짓을 에너지 함수로 다시 읽어 가름과 만들어 내기를 하나로 묶기.
- [함께 배우기](modern_ebm/cooperative.md) -- 서로 보완하는 힘을 살리려 에너지 바탕 모델을 오토인코더, 변분 오토인코더, 맞겨루기 생성기와 합치기.
- 가름과 그 너머의 에너지 바탕 모델 -- 에너지 점수를 쓴 분포 밖 찾기, 얽어 만들어 내기, 그림 잡음 없애기.
- 확산 모델과의 이음 -- 점수 바탕 만들어 내기로 에너지 바탕 모델과 확산 모델을 수학으로 하나로 묶기.
