...
에너지 바탕 모델은 예로부터 밀도 어림과 이상 찾기에 쓰였지만 만들어 내기, 곧 배운 분포에서 새 표본을 만드는 데에도 맞출 수 있다. 잡음에서 자료로 곧바로 옮기는 드러난 만들어 내는 모델(변분 오토인코더, 맞겨루기 생성기)과 달리, 에너지 바탕 모델은 에너지 풍경을 배우고 움직임(랑주뱅, 해밀턴)으로 뽑아 만들어 낸다. 다른 방법보다 셈이 비싸지만 에너지 바탕 모델의 만들어 내기에는 남다른 이점이 있다. 원리 있는 확률 틀, 너그러운 얼개, 봉우리를 더 잘 덮도록 가름 손실과 합칠 수 있는 힘이다.
...
### 에너지 바탕 모델과 변분 오토인코더의 만들어 내기

| 면 | 에너지 바탕 모델 | 변분 오토인코더 |
|--------|-----|-----|
...
에너지 바탕 모델은 제약 있는 만들어 내기에, 변분 오토인코더는 빠르기에 뛰어나다.

### 에너지 바탕 모델과 맞겨루기 생성기의 만들어 내기

| 면 | 에너지 바탕 모델 | 맞겨루기 생성기 |
|--------|-----|-----|
| **목표** | 밀도 맞추기 | 맞겨루기 놀이 |
...
에너지 바탕 모델이 이론으로 더 정당하고 맞겨루기 생성기가 표본을 더 빨리 만든다.
...
3. **점수 바탕 첫자리매김**: 확산 모델로 사슬을 첫자리매김한다
...
- 더 잘 살피려 온도 바꾸기를 쓴다
- 여러 GPU에 나란히 돌린다
- 빠른 만들어 내기 방법과 합친다(변분 오토인코더로 우려내기)
...
- 뒤섞기: 봉우리 찾기에 변분 오토인코더, 다듬기에 에너지 바탕 모델
...
!!! warning "만들어 내기의 실용성"
    에너지 바탕 모델의 만들어 내기는 힘세지만 셈이 비싸다. 대부분의 쓰임새에서는 뒤섞은 방식이 가장 잘 듣는다. 큰 덩어리는 빠른 방법(변분 오토인코더, 맞겨루기 생성기)으로 만들고 다스린 다듬기나 제약 있는 만들어 내기에 에너지 바탕 모델을 쓴다. 순수한 에너지 바탕 만들어 내기는 제약이 꼭 필요하거나 봉우리를 샅샅이 살펴야 할 때 가장 알맞다.
...
이 마당은 핵심 개념, 수학적 틀, 조건 만들어 내기, 다른 만들어 내기 방법과 견주기를 차례로 짚었다.
