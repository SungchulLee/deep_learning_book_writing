...
함께 배우기 틀은 에너지 바탕 모델을 오토인코더, 변분 오토인코더, 맞겨루기 생성기, 가름 모델과 합쳐 서로의 한계를 메우게 한다. 에너지 바탕 모델은 만들어 내는 모델에 규칙을 세우는 원리 있는 밀도 나타내기를 줄 수 있고, 오토인코더는 에너지 바탕 모델을 익히기 전에 효율 좋게 차원을 줄여 줄 수 있으며, 변분 오토인코더는 비싼 에너지 바탕 모델 뽑기를 첫자리매김할 빠른 어림 뽑기를 줄 수 있고, 가름 모델은 에너지 바탕 모델 익히기를 가름 경계에 모이게 할 수 있다.
...
## 3. 에너지 바탕 모델 + 오토인코더의 함께 하기

### 구조

1. **오토인코더**: 차원 높은 관측을 차원 낮은 숨은 공간으로 눌러 담는다
2. **에너지 바탕 모델**: 숨은 나타냄으로 익히며 p(z; θ_e)을 나타낸다
3. **함께 익히기**: 오토인코더 되짓기 손실 + 에너지 바탕 모델 가능도 손실 + 쿨백-라이블러 규칙 세우기
...
- **키울 수 있음**: 오토인코더가 에너지 바탕 모델 익히기 전에 차원을 줄인다(차원의 저주를 피한다)
- **효율**: 오토인코더가 빠른 되짓기를 주고 에너지 바탕 모델이 밀도 나타내기를 준다
- **규칙 세우기**: 에너지 바탕 모델의 밀도가 오토인코더 숨은 공간의 사전 분포 노릇을 한다
- **만들어 내기**: 숨은 공간에서 에너지 바탕 모델로 뽑고 오토인코더로 푼다

### 쓰임새: 금융 자료

차원 높은 시장 미시 얼개 자료에서:

1. **오토인코더 익히기**: 호가창 특징 100개 넘는 것을 숨은 차원 10~20개로 눌러 담는다
...
## 4. 에너지 바탕 모델 + 변분 오토인코더의 함께 하기

### 함께 하는 얼개

1. **변분 오토인코더**: 다룰 만한 p(z)을 가진 확률 숨은 공간을 준다
2. **에너지 바탕 모델**: 숨은 밀도를 다듬어 정규가 아닌 면을 담도록 p(z; θ_e)을 나타낸다
3. **함께 쓰는 디코더**: 둘이 같은 디코더를 쓰며 숨은 나타냄을 두고 겨루거나 함께한다
...
- **너그러운 숨은 밀도**: 변분 오토인코더가 바탕선을 주고 에너지 바탕 모델이 정규가 아님을 다듬는다
- **안정된 익히기**: 변분 오토인코더가 안정된 기울기 흐름을 주고 에너지 바탕 모델이 꼬리 움직임에 집중한다
- **나아진 뽑기**: 빠른 변분 오토인코더 뽑기와 에너지 바탕 모델의 다듬기를 합친다
- **더 나은 가능도**: 합친 모델이 낱낱의 조각보다 높은 가능도를 이룬다

### 정보의 흐름

- **변분 오토인코더 → 에너지 바탕 모델**: 변분 오토인코더가 효율 좋은 숨은 나타냄을 배우고 에너지 바탕 모델이 다듬은 밀도를 나타낸다
- **에너지 바탕 모델 → 변분 오토인코더**: 에너지 바탕 모델의 기울기가 되돌아 흘러 변분 오토인코더의 숨은 공간을 낫게 한다
- **서로 이로움**: 어느 조각도 홀로 앞서지 않고 둘 다 합친 목표에 보탠다

---

## 5. 에너지 바탕 모델 + 맞겨루기 생성기의 함께 하기

### 맞겨루며 함께 하기

여느 얼개: 판별기(D)가 실제와 만들어 낸 것의 확률을 배운다. 함께 하는 얼개:

1. **생성기(G)**: 표본을 만든다
2. **에너지 바탕 모델**: p_data을 나타내고 밀도 어림을 준다
3. **판별기**: 맞겨루기 익히기로 생성기와 에너지 바탕 모델을 모두 돕는다
...
여기서 E은 에너지 바탕 모델의 에너지이고 D은 판별기이다.

### 좋은 점

- **은근한 모델과 드러난 모델**: 맞겨루기 생성기가 은근한 만들어 내기를, 에너지 바탕 모델이 드러난 밀도를 준다
...
## 6. 에너지 바탕 모델 + 가름 모델의 함께 하기

### 가름 경계에 집중하기

보통과 이상을 가르도록 판별기(보기로 신경망)를 익힌다:

1. **가름 모델**: 이름표 붙은 자료로 스승 있는 배움을 하여 p(y|x)을 배운다
2. **에너지 바탕 모델**: 보통 자료로 스승 없이 p(x | y=normal)을 나타낸다
3. **함께 하기**: 에너지 바탕 모델이 가름 경계 가까운 자리에 집중하고 판별기가 이름표를 준다
...
2. **되짓기 어긋남**: 오토인코더 되짓기의 평균 제곱 어긋남
3. **가름 정확도**: 이상 찾기 성능
4. **셈 비용**: 추론에 걸린 실제 시간
...
1. **오토인코더**: 시장 미시 얼개를 숨은 요인으로 눌러 담는다
2. **에너지 바탕 모델**: 보통의 숨은 분포를 나타낸다
3. **장단기 기억망**: 때에 따른 매임을 좇는다

이상 = 에너지 바탕 모델의 에너지가 높거나, 장단기 기억망의 어긋남이 크거나, 오토인코더의 되짓기 어긋남이 큰 것.

### 함께 하는 위험 내다보기

아우르기:

1. **변분 오토인코더**: 수익률 분포를 배운다
...
!!! note "함께 하기 짜기 원리"
    성공한 함께 하기 계는 서로 다른 면에서 뛰어난 모델을 짝짓는다. 하나는 효율 좋지만 어림이고(변분 오토인코더, 맞겨루기 생성기) 하나는 원리 있지만 비싸다(에너지 바탕 모델). 이음 손실이 어울리는 나타냄을 이끈다. 차례대로 익히기로 시작하고 필요하면 함께 익히기로 올린다.
...
함께 쓰는 인코더 매개변수 $\phi$에 대한 합친 손실 $\mathcal{L}_{\text{joint}} = \mathcal{L}_{\text{EBM}} + \mathcal{L}_{\text{AE}} + \lambda \mathcal{L}_{\text{interaction}}$의 기울기를 이끌어 내라. 각 항이 인코더 고침에 어떻게 보태는지 밝혀라.
...
??? success "연습문제 1 풀이"
    ...
    오토인코더 항 $\nabla_\phi \|x - \text{dec}(f_{\text{enc}}(x; \phi))\|^2$은 인코더를 되지을 수 있는 나타냄 쪽으로 민다. 에너지 바탕 모델 항 $\nabla_\phi (-\log p_{\text{EBM}}(z; \theta_e)) = \frac{1}{T}\nabla_z E(z; \theta_e) \cdot \nabla_\phi z$은 $z$을 에너지가 낮은(밀도가 높은) 자리로 민다. 주고받음 항 $\nabla_\phi \text{KL}(p_{\text{EBM}} \| p_{\text{aux}})$은 두 모델이 뜻을 같이하는 나타냄을 만들도록 인코더를 이끈다. 인코더는 되짓기 품질, 밀도 나타내기, 모델 사이 어울림의 균형을 잡아야 한다. $\square$
...
만든 자료 묶음에서 차례대로 익히기(오토인코더를 먼저 익히고 얼린 숨은 공간에서 에너지 바탕 모델을 익히기)와 함께 익히기를 견주어라. 어떤 조건에서 함께 익히기가 크게 이로운가?

</div>

??? success "연습문제 2 풀이"
    차례대로 익히기는 더 단순하고 흔들림을 피하지만, 오토인코더의 숨은 공간이 되짓기만을 위해 가장 좋게 되어 에너지 바탕 모델의 밀도 나타내기에 알맞은 나타냄이 되지 않을 수 있다. 함께 익히기는 오토인코더가 숨은 공간을 "에너지 바탕 모델에 친한" 쪽으로도 맞추게 한다. 함께 익히기의 이로움은 다음일 때 가장 크다. (1) 되짓기 목표만으로는 에너지 바탕 모델이 나타내기 어려운 복잡하고 봉우리 여럿인 기하를 가진 숨은 공간이 생길 때, (2) 숨은 공간에 비쳐야 할 강한 제약이 있을 때, (3) 뒤따르는 일이 되짓기가 아니라 정확한 밀도 어림에 매일 때. 오토인코더가 이미 잘 짜인(보기로 대략 정규인) 숨은 공간을 만들 때는 함께 익히기의 이로움이 작다. $\square$
...
이 마당은 핵심 개념, 수학적 틀, 에너지 바탕 모델 + 오토인코더의 함께 하기, 에너지 바탕 모델 + 변분 오토인코더의 함께 하기를 차례로 짚었다.
