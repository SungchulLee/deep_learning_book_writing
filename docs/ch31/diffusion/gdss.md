...
The forward process corrupts $\mathcal{G}_0 = (\mathbf{X}_0, \mathbf{A}_0)$ via coupled VP-SDEs:
...
At time $T$, the marginal approaches $\mathcal{N}(\mathbf{0}, \mathbf{I})$.

### Reverse SDE

Generation runs the process backward using the score $\nabla \log p_t$:
...
The joint score $\nabla_{\mathbf{X}} \log p_t(\mathbf{X}, \mathbf{A})$ and $\nabla_{\mathbf{A}} \log p_t(\mathbf{X}, \mathbf{A})$ couple nodes and edges: denoising node features depends on adjacency, and vice versa.

## Score Matching Loss

Training minimizes denoising score matching:
...
where $\lambda(t) = g(t)^2$ are importance weights and the target scores have closed-form solutions under VP-SDE.

## Predictor-Corrector Sampling

GDSS uses alternating predictor and corrector steps:

**Predictor** (reverse SDE step via Euler-Maruyama):
$$
\mathbf{X}_{t-\Delta t} = \mathbf{X}_t + [-\mathbf{f}_X + g_X^2 \mathbf{s}_\theta^X]\Delta t + g_X\sqrt{\Delta t}\,\boldsymbol{\epsilon}
$$

**Corrector** (Langevin MCMC refinement):
$$
\mathbf{X}_t \leftarrow \mathbf{X}_t + \tfrac{\eta}{2}\mathbf{s}_\theta^X(\mathbf{X}_t, \mathbf{A}_t, t) + \sqrt{\eta}\,\boldsymbol{\epsilon}
$$

Symmetry is enforced by projecting: $\mathbf{A}_t \leftarrow (\mathbf{A}_t + \mathbf{A}_t^\top)/2$ after each step.

## Score Network Architecture

The score network uses a GNN processing both node features and adjacency to produce score estimates of matching dimensions. The architecture processes the noisy graph at timestep $t$ and outputs separate score estimates for nodes $\mathbf{s}_\theta^X \in \mathbb{R}^{n \times d}$ and edges $\mathbf{s}_\theta^A \in \mathbb{R}^{n \times n}$.

## Advantages over Discrete Diffusion

GDSS operates in continuous space, enabling gradient-based sampling and avoiding the combinatorial challenges of discrete reverse transitions. The SDE framework also allows probability flow ODEs for deterministic sampling and likelihood computation:

$$
d\mathbf{X} = \left[-\mathbf{f}_X + \frac{1}{2}g_X^2 \nabla_{\mathbf{X}} \log p_t\right] dt
$$

## Finance Application: Continuous Network Evolution

The SDE framework naturally models continuous evolution of financial networks — edge weights representing exposures vary continuously in time, and the drift/diffusion decomposition separates systematic trends from stochastic fluctuations in network topology.
