...
??? success "연습문제 5 풀이"
    0-1 손실은 $\mathbf{1}[yf(x) \leq 0]$이다. 힌지: $yf(x) \leq 0$일 때 힌지 $\geq 1$이고 지시함수는 1이므로 $\max(0, 1 - yf(x)) \geq \mathbf{1}[yf(x) \leq 0]$이다. 교차 엔트로피: 여기서는 **밑이 2인 로그**를 써야 상계가 된다. $z = yf(x) \leq 0$이면 $e^{-z} \geq 1$이므로 $\log_2(1+e^{-z}) \geq \log_2 2 = 1$이고, $z > 0$이면 지시함수가 $0$인데 손실은 양수다. 곧 $\log_2(1 + e^{-yf(x)}) \geq \mathbf{1}[yf(x) \leq 0]$이다. 둘 다 볼록이다.

    자연로그로 적으면 상계가 **아니다.** $z=0$에서 $\ln 2 \approx 0.693 < 1$이기 때문이다. 실무에서 쓰는 교차 엔트로피는 자연로그판이므로, 그것은 0-1 손실의 상계가 아니라 **상수배 안에서** 그 구실을 하는 볼록 대리 손실이다. 최소화하는 자리는 밑을 바꾸어도 같으므로 학습에는 아무 차이가 없다. 딥러닝이 교차 엔트로피를 선호하는 이유는 다음과 같다. (1) 어디서나 매끄럽다(미분 가능한 반면 힌지는 미분 불가능한 꺾임이 있다). (2) 시그모이드를 통해 잘 보정된 확률 출력을 준다. (3) 교차 엔트로피의 경사 $p - 1$은 이미 잘 분류된 예제에 대해서도 더 강한 신호를 주어 확신을 높이도록 유도하는 반면, 힌지의 경사는 $yf > 1$에서 0이다.

## 정리하며

**다룬 것** — 마진과 커널

SVM은 두 갈래를 가르되 **가장 넓은 길**을 내는 초평면을 찾는다. 마진은 $2/\|\mathbf{w}\|$이므로 마진을 넓히는 일이 곧 $\|\mathbf{w}\|$를 줄이는 일이 되고, 그래서 목적함수의 첫 항이 $\tfrac12\|\mathbf{w}\|^2$이다. 경계를 정하는 것은 마진 위나 안쪽의 **서포트 벡터뿐**이며 나머지 점은 치워도 답이 같다.

커널은 $\phi$를 실제로 셈하지 않고 높은 차원에서 일하게 해 준다. 쌍대 문제가 입력을 내적으로만 쓰기 때문이며(연습문제 3), RBF 커널은 무한 차원으로 보낸다.

**신경망과의 관계를 한 줄로 적으면 이렇다.** SVM은 $\phi$를 사람이 골라 주고 그 위에서 선을 긋고, 신경망은 $\phi$ 자체를 배운다. 힌지 손실 $\max(0, 1-yf)$가 ReLU와 같은 꼴인 것도 우연이 아니다.

SVM이 큰 자료에서 밀려난 까닭은 정확도가 아니라 **값**이다. 학습이 $O(n^2)$\~$O(n^3)$인 반면 신경망은 에포크당 $O(n)$이라, $n$이 커질수록 격차가 자릿수로 벌어진다(연습문제 4).
