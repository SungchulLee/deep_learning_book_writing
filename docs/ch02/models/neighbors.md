...
KNN에는 **학습 단계가 없다.** 자료를 통째로 들고 있다가 물음이 올 때 거리를 잰다. 그래서 학습 비용이 0이고 추론 비용이 $O(nd)$로, 다른 모델들과 정확히 반대의 셈법을 갖는다.

거리로만 판단하므로 **표준화가 선택이 아니라 필수**다. 자가 큰 특성 하나가 거리를 독차지하기 때문이며, 문턱값으로 자를 뿐인 [결정 트리](trees.md)와 갈리는 자리가 여기다.

$k$가 곧 매끄러움의 손잡이다. 작으면 잡음까지 따라가고(편향 낮음·분산 큼) 크면 경계가 뭉개진다(편향 큼·분산 작음). [지도학습](../paradigms/supervised.md)의 정리 3이 말한 맞바꿈이 손잡이 하나로 드러나는 드문 보기다.
...
그래서 이 방법은 낡은 것이 아니라 **자리가 옮겨간 것**이다. 얼굴 인식과 검색은 신경망이 뽑은 벡터 위에서 최근접 이웃을 찾는 일이고, [3장 1걸음](../../ch03/mnist/01_template_learning.md)의 템플릿 학습도 갈래마다 평균 하나만 두는 KNN의 사촌이다.
