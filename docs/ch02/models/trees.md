...
# 뒤에 나올 k-최근접 이웃과 달리 표준화가 필요 없는 까닭이다
...
트리는 특징 공간을 **축에 평행한** 분할로 재귀적으로 나눈다. 각 노드에서 불순도를 가장 많이 줄이는 분할을 탐욕적으로 고르므로 학습이 빠르고, 만들어진 트리를 사람이 읽을 수 있다.

값어치 있는 성질이 둘이다. **자에 휘둘리지 않는다** — 문턱값으로 자를 뿐이라 특성을 표준화하지 않아도 된다. 그리고 **추론이 싸다** — 뿌리에서 잎까지 한 경로만 밟으므로 $O(\log n)$이고, 매개변수가 $P$개인 신경망의 $O(P)$와는 자릿수가 다르다(연습문제 3).

대가는 **분산**이다. 자료가 조금만 달라져도 전혀 다른 트리가 나온다. 랜덤 포레스트는 여러 트리를 평균내어, 그래디언트 부스팅은 얕은 트리를 순차로 쌓아 이 분산을 다스린다.

이 절의 예제에서 눈여겨볼 것은 마지막 줄이다. **같은 표 자료에서 트리가 88.00%, 신경망이 89.00%다.** 신경망이 늘 이기지 않으며, 표 형태의 자료에서는 트리 계열이 맞먹거나 앞서는 일이 흔하다. [2장 개요](../index.md)가 견줄 대상을 마련한다고 적은 까닭이 여기에 있다.
