...
??? success "연습문제 1 풀이"
    ...
    **참 군집 수가 5인데 k=3이 가장 높다.** 본문의 전체 훑기(k=2\~11)에서도 꼭대기는 k=4였다.
    ...
    까닭은 자료를 만들 때 준 `cluster_std=[1.0, 0.7, 1.2, 0.9, 1.5]`에 있다. 퍼짐이 갈래마다 두 배 넘게 다르므로, 넓게 퍼진 두 덩이가 서로 겹쳐 하나처럼 보인다. 실루엣은 **보이는 대로** 재므로 겹친 둘을 하나로 묶는 쪽에 높은 점수를 준다.
...
**$k$를 고르는 일이 이 알고리즘의 가장 어려운 부분**이며, 이 절이 남기는 교훈도 거기에 있다. 관성은 $k$가 커지면 반드시 줄어들어 쓸 수 없고, 실루엣은 단조롭지 않아 쓸 수 있다. 다만 연습문제 1이 보이듯 **실루엣이 참값을 되찾아 준다는 보장은 없다.** 갈래마다 퍼짐이 다르면 겹친 둘을 하나로 세는 쪽이 더 높은 점수를 받는다.

미니배치 K-평균은 중심을 자료 전체가 아니라 작은 부분집합으로 갱신한다. 위 예제에서 **14.7배 빨라지고 관성은 6% 나빠졌다**. 자료가 수만을 넘어가면 이 맞바꿈이 거의 언제나 남는 장사다.
