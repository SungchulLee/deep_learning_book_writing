...
GMM은 자료가 가우시안 $K$개에서 나왔다고 보고 그 가중합 $p(\mathbf{x}) = \sum_k \pi_k \mathcal{N}(\mathbf{x} \mid \boldsymbol{\mu}_k, \boldsymbol{\Sigma}_k)$을 적합한다. [K-평균](01_kmeans.md)과 가르는 점이 둘이다.

**하나, 배정이 부드럽다.** K-평균은 점마다 군집 하나를 찍지만 GMM은 **책임도**, 곧 성분들에 대한 확률 분포를 준다. 두 군집의 경계에 놓인 점은 0.5 대 0.5로 나오며, 그 모호함 자체가 쓸모 있는 정보다.

**둘, 모양이 자유롭다.** 공분산 $\boldsymbol{\Sigma}_k$를 성분마다 따로 두면 기울어진 타원도 담는다. `spherical`로 묶으면 사실상 K-평균으로 돌아가므로, **K-평균은 GMM의 특수한 경우**로 볼 수 있다.

성분 수를 고르는 길이 둘 있다. **BIC/AIC**는 적합도와 복잡도를 맞바꾼다. BIC의 벌점이 $p\ln n$, AIC가 $2p$이므로 $n \ge 8$부터는 BIC가 더 엄해 단순한 모형을 고른다. **베이즈 GMM**은 더 우아하다. $K$를 넉넉히 주고 디리클레 사전분포가 쓸모없는 성분의 가중치를 0으로 눌러 주면, 위 출력처럼 10개 가운데 3개만 살아남는다.

그리고 GMM은 **밀도를 실제로 추정하므로** 이상 탐지가 덤으로 따라온다. 로그가능도가 낮은 점이 곧 "어느 성분에도 잘 맞지 않는 점"이다. 군집화 알고리즘 가운데 이 성질을 그냥 갖는 것은 드물다.

[5장](../../ch05/index.md)이 이 생각을 훨씬 멀리 끌고 간다. 자료의 분포를 모형화한다는 뼈대는 그대로 두고, 가우시안 몇 개 대신 신경망을 쓰는 것이 VAE와 확산 모델이다.
