...
??? success "연습문제 5 풀이"
    각 항 $(\sigma_i + \sigma_j) / d(c_i, c_j)$는 최악의 쌍에 대해 군집의 퍼짐과 군집 간 분리의 비를 측정한다. 값이 낮다는 것은 군집이 조밀하고($\sigma$가 작고) 잘 분리되어 있다($d$가 크다)는 뜻이다. 두 군집이 매우 가까우면($d(c_i, c_j) \to 0$) 이 비가 무한대로 발산하여 DB 지수가 커지고, 겹치는 군집에 올바르게 벌점을 준다. 이 지수가 0이 되려면 모든 $\sigma_i$ 가 0이어야 한다. 분자가 0이면 분모가 유한하든 말든 항이 0이므로, 분리가 무한대일 필요는 없다. 곧 군집마다 점이 하나뿐이거나 완전히 겹쳐 있는 자리이며, 실무에서는 0에 가까운 값을 추구한다.

## 정리하며

**다룬 것** — 정답 없이 군집을 재는 법

군집 평가가 어려운 까닭은 하나다. **참값이 없다.** 그래서 자가 둘로 갈린다. 레이블 없이 자료와 배정만 보는 **내부 지표**(실루엣, 칼린스키-하라바스, 데이비스-볼딘)와, 참 레이블과 맞대어 보는 **외부 지표**(ARI, NMI)다.

내부 지표의 뼈대는 모두 같다. **군집 안은 촘촘한가, 군집 사이는 떨어져 있는가.** 실루엣은 그 둘을 점마다 견주고, 칼린스키-하라바스는 분산의 비로, 데이비스-볼딘은 가장 닮은 이웃과의 겹침으로 잰다.

**군집 수를 고르는 데 내부 지표가 필요한 까닭**이 [비지도학습](../paradigms/unsupervised.md)의 정리 3에 있다. 군집 내 제곱합은 $k$가 커지면 반드시 줄어들어 $k=n$을 고르게 만든다. 실루엣은 군집 사이 거리도 함께 보므로 단조롭지 않고, 그래서 최댓값을 고르는 일이 뜻을 갖는다. 위 예제에서 참값인 $k=4$가 정확히 꼭대기로 나온다.

외부 지표를 쓸 때 **정확도는 쓸 수 없다.** 군집에 붙는 번호는 뜻이 없어 번호만 바꾸어도 값이 달라지기 때문이다. ARI와 NMI는 번호 바꾸기에 흔들리지 않으며, ARI는 우연 일치까지 빼내어 무작위 군집화에 0을 준다.
