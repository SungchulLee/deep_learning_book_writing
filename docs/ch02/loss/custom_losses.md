...
Standard loss functions encode common assumptions, but real-world problems often require specialized objectives. This section covers the design and implementation of custom loss functions in PyTorch, including practical examples for common scenarios.
...
2. **Data imbalance**: Hard example mining (Focal Loss), class-weighted losses
...
!!! note "Critical Requirements"
    All operations must use PyTorch tensors (not NumPy), the output must be a scalar tensor, and all operations must be differentiable.
...
weights = torch.tensor([0.5, 1.0, 2.0])  # Later samples more important
loss = criterion(pred, target, weights)
```

## Focal Loss: Handling Class Imbalance

Focal Loss addresses the problem of class imbalance by down-weighting easy examples and focusing training on hard negatives.

### The Problem

In heavily imbalanced datasets (e.g., 99% negative, 1% positive), standard cross-entropy allows the model to achieve high accuracy by simply predicting the majority class. Easy negatives dominate the gradient signal, preventing the model from learning to identify the rare positive class.
...
- $\gamma$ (focusing parameter) controls the rate of down-weighting (typically 2)
- $\alpha$ balances positive/negative importance

When $\gamma = 0$, Focal Loss reduces to standard cross-entropy. As $\gamma$ increases, easy examples (high $p_t$) are increasingly down-weighted.
...
class FocalLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        # Calculate focal weight: (1-p)^γ for positives, p^γ for negatives
        ...
        # Apply focal weighting and alpha
...
# Focal loss is lower because it down-weights easy examples
```

## Dice Loss: Segmentation Overlap

Dice Loss optimizes the Dice coefficient (also called F1 score for sets), measuring overlap between predicted and ground truth segmentation masks.
...
class DiceLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        """
        Args:
            predictions: Model outputs after sigmoid (0-1)
...
For multi-task learning, weights can be learned or dynamically adjusted:
...
2. **Return scalar**: Loss must reduce to a single value for backpropagation
...
|---------|---------|----------|
| Using `.item()` inside loss | Breaks gradient tracking | Keep as tensor until final |
...
Custom loss functions extend PyTorch's capabilities to domain-specific objectives. The class-based approach (`nn.Module`) is preferred for its flexibility and integration with PyTorch conventions. Focal Loss addresses class imbalance by focusing on hard examples. Dice Loss optimizes overlap metrics for segmentation. Combined losses enable multi-objective optimization with fixed or learned weights. Always validate custom losses for correct gradient flow and numerical stability before deploying in training.
