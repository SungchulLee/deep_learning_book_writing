...
이 장의 나머지는 텐서, 자동 미분, 경사 하강법을 다룬다. 그 도구를 하나도 쓰지 않고 무엇을 할 수 있는지 먼저 보자.
...
둘째, 82.03%라는 값이 이 책 전체의 출발점을 정해 준다. 같은 데이터에서 로지스틱 회귀는 92% 언저리, 작은 합성곱 신경망은 99%를 넘긴다. 그 사이의 간격이 곧 "학습한다"는 일이 벌어 주는 몫이며, 다음 절부터 그 간격을 메우는 도구를 하나씩 익힌다.
...
이 장의 나머지와 다음 장들이 이 세 가지를 차례로 걷어낸다. 2.4 자동 미분과 2.5 경사 하강법은 가중치를 고정하는 대신 데이터에 맞추어 움직이는 법을 알려 주고, 합성곱 신경망은 평행 이동에 강한 표현을 만든다.
...
??? success "연습문제 4 풀이"
    ...
    곧 이 방법을 막고 있는 것은 **데이터의 양이 아니다.** 아무리 부어도 82% 언저리에서 멈춘다. 막고 있는 것은 픽셀 공간에서 거리를 잰다는 사실이며, 그래서 다음 절부터 배우는 도구가 필요해진다.
...
이 예제에는 이 장에서 배울 도구가 대부분 등장한다. 텐서 생성, 인덱싱, 브로드캐스팅, 축약 연산, `DataLoader`가 그것이다. 아직 쓰지 않은 것은 자동 미분(2.4)과 경사 하강법(2.5)뿐이며, 그 둘이 바로 위의 간격을 메우는 도구이다. 앞의 연습문제 4개로 직접 확인할 수 있다.
