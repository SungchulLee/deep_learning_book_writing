...
이 단원은 요즘 만들어 내는 모델의 핵심 부품인 점수 확률 미분 방정식 틀을 짠다. 여기서 보이는 개념과 재주를 알면 확산 모델과 점수 바탕 만들어 내는 방법을 다루는 데 꼭 필요한 앎을 얻는다. 이 짜기는 또렷함과 실제 쓸모의 균형을 맞추어 배우기에도 실험하기에도 알맞다.
...
class VESDE:
    ...
    def reverse_sde(self, score_fn, x, t):
        ...
        # 고침: 퍼뜨리기에 맞게 확산의 꼴을 바꾼다
        # 확산의 꼴은 (배치,)이지만 점수의 꼴은 (배치, 차원)이다
        # 퍼뜨리기를 위해 확산의 꼴을 (배치, 1)으로 만들어야 한다
...
```
MODULE 08: Score-Based SDE Framework
================================================================================
...
✓ Module 08 complete!
Next: Apply to real images (Module 09)!
```
...
점수 확률 미분 방정식 틀의 짜기는 이 마당에 자리 잡은 방식을 따른다.

고갱이 갈래는 `VESDE`, `VPSDE`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
