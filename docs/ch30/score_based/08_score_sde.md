...
이 단원은 요즘 만들어 내는 모델의 핵심 부품인 점수 확률 미분 방정식을 짠다. 여기서 보이는 개념과 재주를 알면 확산 모델과 점수 바탕 만들어 내는 방법을 다루는 데 꼭 필요한 앎을 얻는다. 이 짜기는 또렷함과 실제 쓸모의 균형을 맞추어 배우기에도 실험하기에도 알맞다.
...
수학 바탕:
    점수 바탕 확률 미분 방정식은 확산을 이어진 때로 적어 준다.
...
점수 확률 미분 방정식의 짜기는 이 마당에 자리 잡은 방식을 따른다.

고갱이 갈래는 `ScoreSDE`, `VESDE`, `VPSDE`, `SimpleScoreModel`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
