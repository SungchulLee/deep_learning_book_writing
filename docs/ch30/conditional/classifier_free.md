# 분류기 없는 이끌기
**분류기 없는 이끌기(CFG)**는 따로 분류기를 두지 않고 확산 모델의 조건 만들어 내기 품질을 높이는 재주이다.
...
조건 확산 모델 $p_\theta(x|c)$은 흔히 다음과 같은 표본을 낸다.
...
CFG는 **다양함**(내놓기의 여러 가지)을 대가로 **충실함**(표본이 조건에 얼마나 맞는지)을 높인다.
...
CFG는 잡음 없애기 걸음마다 **앞먹임 두 번**(조건 있음 + 조건 없음)이 필요하다.

### 가장 좋게 하기

1. **묶어 하기**: 배치 크기 2으로 앞먹임 한 번에 둘 다 셈한다
...
Stable Diffusion은 $w \approx 7.5$으로 CFG를 쓴다.

- CFG 없이: 흐릿하고 조건이 여리다
- CFG와 함께: 또렷하고 글에 세게 조건 지어진다

### 갈래 조건 만들어 내기

ImageNet 모델은 $w \approx 2-4$으로 CFG를 쓴다.
...
조건 확산 모델에서 분류기 이끌기와 분류기 없는 이끌기의 차이를 설명하라.

</div>

??? success "연습문제 1 풀이"
    **분류기 이끌기**는 잡음을 아는 분류기 $p(y|x_t)$을 따로 익혀 점수를 $\tilde{s}(x_t) = s(x_t) + w \nabla_{x_t} \log p(y|x_t)$으로 고쳐 표본을 갈래 $y$ 쪽으로 이끈다. **분류기 없는 이끌기**는 조건 있는 모드와 없는 모드를 지닌 확산 모델 하나를 익힌 뒤 $\tilde{\epsilon} = (1+w)\epsilon_\theta(x_t, y) - w \epsilon_\theta(x_t, \emptyset)$으로 아우른다. 분류기 없는 이끌기를 더 낫게 여기는 까닭은 (1) 따로 분류기가 필요 없고, (2) 분류기가 잡음 섞인 들임을 다루지 않아도 되며, (3) 어떤 조건 신호(글, 갈래, 그림)에도 통하고, (4) 실제로 품질 높은 표본을 내기 때문이다.
