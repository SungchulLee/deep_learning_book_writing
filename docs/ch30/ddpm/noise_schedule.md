# 잡음 일정
**잡음 일정** $\{\beta_t\}_{t=1}^T$은 앞 확산 과정에서 잡음을 얼마나 빨리 더할지 다스린다. 사소해 보이는 이 고름이 모형의 성능과 만들어 내는 좋음에 깊이 영향을 준다.
...
잡음 차례표는 앞 확산의 움직임을 다스리는 결정적인 웃매개변수이다. 선형 차례표는 단순하고 역사에서 중요하지만, 코사인 차례표가 때 걸음에 걸쳐 앎의 잃음을 더 고르게 나누어 흔히 더 좋은 결과를 준다. 앞선 방법은 배운 차례표와 이어진 때 차례표로 더 나아지려 한다.
