...
이 단원은 요즘 만들어 내는 모델의 핵심 부품인 조건 ddpm을 짠다. 여기서 보이는 개념과 재주를 알면 확산 모델과 점수 바탕 만들어 내는 방법을 다루는 데 꼭 필요한 앎을 얻는다. 이 짜기는 또렷함과 실제 쓸모의 균형을 맞추어 배우기에도 실험하기에도 알맞다.
...
ATTN_RES = {16}          # 이 공간 크기에서 어텐션을 쓴다. 끄려면 set()
...
조건 ddpm의 짜기는 이 마당에 자리 잡은 방식을 따른다.

고갱이 갈래는 `SinusoidalPosEmb`, `ResidualBlock`, `SelfAttention2d`, `Downsample`이며 앞의 연습문제 3개로 스스로 따져 볼 수 있다.
