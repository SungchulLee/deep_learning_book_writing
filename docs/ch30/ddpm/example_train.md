...
이 단원은 요즘 만들어 내는 모델의 핵심 부품인 익히기 대본 보기를 짠다. 여기서 보이는 개념과 재주를 알면 퍼짐 모델과 점수 바탕 만들어 내는 방법을 다루는 데 꼭 필요한 앎을 얻는다. 이 짜기는 또렷함과 실제 쓸모의 균형을 맞추어 배우기에도 실험하기에도 알맞다.
...
기본 자리매김으로 MNIST 퍼짐 모델을 익히는 단순한 대본.
실험에 맞게 웃매개변수를 조정하라.
"""
