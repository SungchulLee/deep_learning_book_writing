...
- 분류기 이끌기 -- 바깥 분류기 기울기로 퍼짐 뽑기를 바라는 갈래 쪽으로 이끈다.
- [분류기 없는 이끌기](conditional/classifier_free.md) -- 조건 있는 모델과 없는 모델을 함께 익혀 따로 분류기 없이 조건 만들어 내기 품질을 높인다.
- 글에서 그림으로 만들어 내기 -- 퍼짐 모델과 CLIP 글 인코더를 아울러 자연어 설명에서 그림을 만든다.

---

## 7. 얼개

- U-Net 잡음 없애개 -- 건너뛰기 이음과 때 걸음 조건 주기를 갖춘 퍼짐 모델의 여느 인코더-디코더 얼개.
