...
!!! info "배움 목표"
    ...
    - 가능도 잣대의 수학 바탕을 이해한다
    - 음의 로그 가능도, 차원마다 비트, 헷갈림도 셈하기를 PyTorch로 짠다
...
class PerplexityCalculator:
    ...
    def compute_perplexity(log_probs: torch.Tensor,
        ...
        인수:
            ...
            lengths: 길이가 다른 배치를 위한 차례 길이(있으면)
    ...
    def evaluate_language_model(model,
        ...
        with torch.no_grad():
            for i in range(0, len(input_ids), batch_size):
                ...
                # 순전파
                logits = model(batch_ids)  # [배치, 차례 길이, 낱말 수]
...
|------------|------------------------|-------------------|
| 변분 오토인코더 | 증거 하한(아래 한계) | 증거 하한, 되짓기 음의 로그 가능도 |
| 고르게 하는 흐름 | 정확함 | 음의 로그 가능도, 차원마다 비트 |
| 자기 되돌이 | 정확함 | 음의 로그 가능도, 차원마다 비트, 헷갈림도 |
| 확산 | 어림(증거 하한으로) | 차원마다 비트, FID |
| 맞겨루기 생성기 | **아니다** | FID와 인셉션 점수만 |
| 에너지 바탕 | 다룰 수 없다 | FID와 그 밖의 잣대 |
...
생성기 모형을 따지는 자로서 FID, 인셉션 점수, 로그 가능도를 견주어라.
...
??? success "연습문제 2 풀이"
    ...
    | **로그 가능도** | 밀도의 정확함 | 그렇다(시험 배치) | 그렇다 | 여림 |

    FID는 사람의 판단과 잘 이어지고 품질과 다양함을 모두 담아 가장 널리 쓰인다. 인셉션 점수는 만든 표본만 따진다. 로그 가능도는 이론으로 원칙이 있지만 느낌의 품질과 어긋날 수 있다. 가장 좋은 방식은 셋 다 알리는 것이다.
...
차원마다 비트(BPD) 잣대란 무엇인가? 확산 모델에서 어떻게 셈하는가?

</div>

??? success "연습문제 3 풀이"
    차원마다 비트는 음의 로그 가능도를 자료 차원으로 고르게 맞추고 비트로 바꾼다. 곧 $\text{BPD} = -\frac{\log_2 p(x)}{d}$이며 $d$은 차원의 수이다(예컨대 CIFAR-10은 $3 \times 32 \times 32 = 3072$). 확산 모델에서 로그 가능도는 증거 하한으로 가둬진다. 곧 $\log p(x) \geq \text{ELBO} = -\sum_t L_t$이며 $L_t$은 쿨백-라이블러 벌어짐 항이다. 정확한 셈은 확률 흐름 상미분 방정식과 순간 변수 바꿈 공식을 쓴다. 차원마다 비트가 낮을수록 좋은 모델이다. 최고 수준의 확산 모델은 CIFAR-10에서 약 2.5을 이룬다.
