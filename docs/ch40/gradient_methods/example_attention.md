...
쓰는 보기: 변환기의 어텐션 그림 그리기. 이 글은 변환기 모형의 어텐션 결을 그리는 법을 보인다.
...
```python
"""
쓰는 보기: 변환기의 어텐션 그림 그리기

이 글은 변환기 모형의 어텐션 결을 그리는 법을 보인다.
"""
...
def example_bert_attention():
    ...
    print(f"\n켜의 수: {len(attentions)}")
    print(f"켜마다의 어텐션 꼴: {attentions[0].shape}")
...
def example_gpt2_attention():
    ...
    # 인과 어텐션 결을 그린다(GPT-2은 인과 가리개를 쓴다)
    print("\n인과 어텐션 결을 그리는 중...")
    fig = visualize_token_attention(attentions, tokens, layer_idx=-1)
...
def example_attention_patterns():
    ...
    for idx, (name, pattern) in enumerate(patterns.items()):
        ...
        axes[idx].set_title(f'{name} 어텐션 결')
        axes[idx].set_xlabel('열쇠 자리')
        axes[idx].set_ylabel('물음 자리')
...
============================================================
그림을 만드는 중...
'attention_simple.png'에 갈무리했다
'attention_all_heads.png'에 갈무리했다

============================================================
보기 4: 흔한 어텐션 결
============================================================
'attention_patterns.png'에 갈무리했다
...
어텐션 셈속을 셈하는 중...
'attention_statistics.png'에 갈무리했다

켜마다의 셈속:
  켜 0: 엔트로피=2.718, 거리=4.223
  켜 1: 엔트로피=2.702, 거리=4.142
  켜 2: 엔트로피=2.711, 거리=3.918
  켜 3: 엔트로피=2.702, 거리=4.012
  켜 4: 엔트로피=2.710, 거리=3.945
  켜 5: 엔트로피=2.702, 거리=4.015
  켜 6: 엔트로피=2.696, 거리=3.925
  켜 7: 엔트로피=2.701, 거리=4.016
  켜 8: 엔트로피=2.705, 거리=3.889
  켜 9: 엔트로피=2.708, 거리=4.198
  켜 10: 엔트로피=2.708, 거리=4.069

... (5 lines omitted)

transformers 곳집이 없다. 이렇게 깐다: pip install transformers

============================================================
보기 3: GPT-2 어텐션 그림 그리기
============================================================
transformers 곳집이 없다. 이렇게 깐다: pip install transformers
