...
잡음 지우개는 (맞섬, 맑음) 짝으로 익힌다. 그다음 분류기는 잡음을 지운 들임으로 움직인다.

### 확산에 기댄 맑히기

요즘 일은 **확산 모형**으로 잡음을 더했다가 지워 맞서는 보기를 맑힌다.

1. 가우스 잡음을 더한다: $\mathbf{x}_t = \sqrt{\bar{\alpha}_t}\mathbf{x}_{\text{adv}} + \sqrt{1-\bar{\alpha}_t}\boldsymbol{\epsilon}$
2. 확산 모형으로 잡음을 지운다: $\hat{\mathbf{x}}_0 = \text{denoise}(\mathbf{x}_t, t)$
3. 가른다: $\hat{y} = f(\hat{\mathbf{x}}_0)$

잡음을 섞으면 맞서는 얼개가 무너지고, 확산 모형이 맑은 것에 가깝게 되살린다.
