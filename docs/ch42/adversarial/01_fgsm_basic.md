...
62.1 배치: 빠른 기울기 부호 방법(FGSM) - 첫걸음. 이 배치는 가장 단순한 치기 가운데 하나인 빠른 기울기 부호 방법(FGSM)을 짜 놓았다
...
```python
"""
62.1 배치: 빠른 기울기 부호 방법(FGSM) - 첫걸음

이 배치는 가장 단순하고 널리 힘을 미친 맞서는 치기 방법 가운데 하나인
...
class FGSM:
    ...
    def __init__(
        ...
        # 모형을 따짐 모드로 둔다(배치 잣대 잡기와 드롭아웃에 종요롭다)
        self.model.eval()
...
고갱이 갈래는 `FGSM`이며 앞의 연습문제 4개로 스스로 따져 볼 수 있다.
