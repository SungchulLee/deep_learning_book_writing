...
여기서 $S$은 고른 낱말 자리의 배치가고, 갈음 $w_i'$은 (낱말 임베딩의 거리나 비슷한 말 모임으로) $w_i$과 뜻이 가까우면서 모형의 날임을 가장 크게 바꾸도록 고른다.
...
**2도막 — 낱말 갈음하기**: 중요도 차례로 낱말마다, 맞맞춘 임베딩 밭(비슷한 말은 가깝고 반대말은 먼 밭)에서 갈음 후보를 찾는다. 후보는 다음을 채우도록 거른다.

1. **뜻이 비슷함**: 갈음 $w_i'$이 임베딩 밭에서 $w_i$과 가까워야 한다
2. **품사가 한결같음**: $w_i'$이 $w_i$과 같은 품사여야 한다
3. **월이 비슷함**: 월 전체의 임베딩이 비슷하게 남아야 한다(두루 쓰는 월 인코더로 잰다)
...
이 마당은 글월 맞서는 치기의 갈래, TextFooler: 낱말 켜의 검은 상자 치기, 글월 모형의 막이, 계량 금융에 걸리는 바를 차례로 짚었다.
