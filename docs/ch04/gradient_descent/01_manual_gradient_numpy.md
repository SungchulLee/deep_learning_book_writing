...
"""
================================================================================
1단계 - 보기 1: 넘파이로 직접 하는 경사 하강법
================================================================================

학습 목표:
- 경사 하강법을 첫 원리부터 이해한다
- 미적분으로 기울기를 직접 계산한다
- 기계 학습 꾸러미 없이 경사 하강법을 짠다
...
print("="*80)
print("""
1. 경사 하강법은 루프로 가장 좋게 하는 알고리즘이다
   - 마구잡이 매개변수에서 비롯한다
   - 기울기를 계산한다(매개변수에 대한 손실의 도함수)
...
4. 기울기는 가장 가파르게 오르는 쪽을 가리킨다
   - 손실을 줄이려고 반대쪽(-기울기)으로 간다

5. 기울기가 아주 작아지면 모여든 것이다
   - 손실이 뚜렷하게 줄지 않는다
   - 매개변수가 가장 좋은 값 언저리에서 초기화한다
""")
...
print("""
코드를 고쳐 가며 살펴보아라.
1. 여러 학습률(0.001, 0.1, 0.5) — 어떻게 되는가?
2. 여러 첫 가중치(5.0, -3.0) — 그래도 모여드는가?
3. 학습 데이터 점을 더 늘리면 — 정확도이 나아지는가?
4. 잡음 섞인 데이터(y에 마구잡이 잡음을 더한다) — 경사 하강법은 얼마나 든든한가?
""")
print("="*80)
...
```
================================================================================
GRADIENT DESCENT: LEARNING FROM SCRATCH
================================================================================
...
Training Data:
X (inputs):  [1. 2. 3. 4. 5.]
y (targets): [ 2.  4.  6.  8. 10.]

True relationship: y = 2*x (so optimal w = 2.0)

Initial weight w = 0.000
Initial prediction for x=5: y = 0.000 (should be 10.0)
...
Epoch | Weight (w) | Loss      | Gradient
--------------------------------------------------
   1  |   0.440000 | 44.000000 | -44.000000
   3  |   1.050896 | 16.286625 | -26.769598
   5  |   1.422565 |  6.028503 | -16.286625
   7  |   1.648689 |  2.231453 | -9.908781
   9  |   1.786262 |  0.825974 | -6.028504
  11  |   1.869962 |  0.305734 | -3.667740
  13  |   1.920885 |  0.113168 | -2.231454
  15  |   1.951866 |  0.041889 | -1.357618
  17  |   1.970715 |  0.015505 | -0.825974
