...
```
MNIST 1 템플릿  82.03%
MNIST 2 선형  92.35%  매개변수 7,850
MNIST 3 MLP  97.52%  매개변수 101,770
MNIST 4 CNN  99.02%  매개변수 421,642
CIFAR10 1 템플릿  27.69%
CIFAR10 2 선형  37.15%  매개변수 30,730
CIFAR10 3 MLP  51.00%  매개변수 394,634
CIFAR10 4 CNN  71.78%  매개변수 545,098
```

여덟 줄이 이 절의 표 전부다. 1걸음의 두 값만 소수점까지 되풀이되고, 나머지 여섯은 씨앗을 바꾸면 흔들린다. 얼마나 흔들리는지가 아래 "이 표를 믿어도 되는가"의 주제다.
...
??? success "연습문제 5 풀이"
    ...
    **실제로 재어 보면 84.93%다**([4.3절](03_vgg16_transfer.md)). 외삽이 7~9%포인트 높게 빗나갔다. 지운 몫도 70~80%가 아니라 47%였다. 세 점으로 그린 추세를 밀지 말라는 경고가 맞았던 셈이며, 이런 외삽은 **다음 걸음을 판단할 눈금으로 쓸 것이지 예측으로 믿을 것이 아니다.**
