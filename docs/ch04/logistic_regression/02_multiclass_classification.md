...
`MultiClassLogisticRegression` 클래스는 PyTorch의 `nn.Module` 인터페이스를 사용하여 모델 구조를 감싼다.

핵심 클래스는 `MultiClassLogisticRegression`이며 앞의 연습문제 4개로 직접 확인할 수 있다.
