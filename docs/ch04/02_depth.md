...
!!! note "71.33%와 4.1절의 71.78%는 같은 모델이다"
    이 절 첫머리가 4.1절의 값을 71.78%라 적었는데 표에는 71.33%가 있다. **4.1절은 씨 하나로 한 번 돌린 값이고, 이 표는 같은 모델을 씨 다섯으로 돌린 평균이다.**

    71.78%는 다섯 씨의 범위 70.68\~71.79의 **거의 꼭대기**에 있다. 벗어나지는 않았으니 4.1절의 수가 틀린 것은 아니지만, 그 수가 다섯 번 가운데 가장 좋은 쪽에 가까웠다는 뜻이다.

    이 절부터 표에 **퍼짐 열이 붙는 까닭**이 그것이다. 4.1절은 사다리의 칸을 보이는 자리라 한 번씩 재어도 됐지만, 여기서부터는 0.2%포인트짜리 차이를 다투므로 한 번 잰 수로는 판정할 수 없다.
