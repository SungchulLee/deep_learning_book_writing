...
이진 분류 예제(사기 탐지)는 불균형한 데이터에서 지표 선택이 왜 중요한지 보여 준다.

핵심 클래스는 `Example1_BinaryClassification`, `Example2_MulticlassClassification`, `Example3_Regression`이며 앞의 연습문제 3개로 직접 확인할 수 있다.
