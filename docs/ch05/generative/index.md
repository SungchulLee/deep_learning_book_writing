...
**VAE가 오토인코더를 뚜렷이 이긴 것은 고름뿐이다.** 클래스 안 다양함은 0.640과 0.628로 사실상 같다.

아래 5절이 씨 다섯 벌로 다시 재는데, 이 읽기가 거기서 그대로 선다. 고름은 0.567\~0.637과 0.884\~0.899로 **0.247만큼** 갈리는 반면, 클래스 안 다양함은 0.635\~0.648과 0.621\~0.630으로 겨우 **0.005** 떨어져 있다. 범위가 갈리기는 하나 그 폭이 퍼짐과 같은 자리수이므로, **둘 다 진짜의 63%에 그친다**는 읽기를 바꾸지 못한다.

곧 KL 항이 한 일은 **어느 숫자를 만들지 고르게 한 것**이지 **각 숫자를 다양하게 만든 것**이 아니다. 만들어 낸 3은 진짜 3보다 밋밋하다.
...
!!! note "5.2절의 표와 조금 다른 까닭"
    ...
    VAE-64도 함께 움직인다(확신도 0.769 → 0.758, 고름 0.877 → 0.887). 두 수가 **서로 반대쪽으로** 갔다는 데 눈길을 둘 만하다. 자르기는 모든 줄을 같은 쪽으로 미는 보정이 아니어서, "몇 점 보태면 맞겠지" 하고 손으로 맞춰 볼 수 있는 것이 아니다.
...
!!! note "열 장으로 판단하지 말 것"
    이 그림만 보면 오토인코더의 표본이 VAE보다 못해 보인다. 그런데 5,000장을 심판에 넣어 재면 확신도가 0.777 대 0.769로 **오토인코더 쪽이 오히려 조금 높다**([VAE 쪽](../latent_generative/01_vae.md)).
...
다만 이 0.953은 씨 하나의 값이고 조금 높게 뽑힌 값이다. 다섯 벌로 재면 0.878이며, 그래도 **뛰는 것은 그대로다**(0.627에서 0.878로). 아래 5절이 그 이야기다.

---

## 5. 이 수들을 얼마나 믿을 수 있는가

위의 표는 모두 씨 42 한 번이다. [4.1절](../../ch04/01_two_ladders.md)의 규율대로라면 그것만으로는 칸 사이의 차이가 뜻이 있는지 말할 수 없다. 그래서 네 모델을 **씨 다섯 벌씩 새로 학습해** 값이 어디까지 움직이는지를 쟀다.

| 부호 64 | 확신도 | 고름 | 최대 클래스 | 클래스 안 다양함 |
|---|---|---|---|---|
| [AE_MLP-64](../dimreduction/03_ae_mlp.md) | 0.720\~0.749 | 0.567\~0.637 | 0.564\~0.632 | 0.635\~0.648 |
| [VAE-64](../latent_generative/01_vae.md) | 0.752\~0.762 | 0.884\~0.899 | 0.324\~0.349 | 0.621\~0.630 |
| [GAN (MLP)](01_gan.md) | 0.903\~0.911 | 0.913\~0.937 | 0.194\~0.210 | 0.831\~0.905 |
| [DCGAN](02_dcgan.md) | 0.899\~0.906 | 0.966\~0.978 | 0.147\~0.163 | 1.055\~1.121 |

### 이 절의 두 걸음은 모두 참이다

앞의 "두 걸음, 두 가지 바뀐 것"이 적은 두 걸음을 이 표로 다시 읽으면 이렇다.

| 걸음 | 주장 | 범위로 보면 |
|---|---|---|
| VAE → GAN | 클래스 안 다양함이 뛴다 | **참** — 0.621\~0.630과 0.831\~0.905, 0.201만큼 떨어진다 |
| GAN → DCGAN | 고름이 오른다 | **참** — 0.913\~0.937과 0.966\~0.978, 0.029만큼 |
| GAN → DCGAN | 클래스 안 다양함이 오른다 | **참** — 0.150만큼 |

그리고 표에 적지 않았던 것이 하나 더 드러난다. **최대 클래스도 DCGAN이 낫다**(0.147\~0.163 대 0.194\~0.210). 곧 DCGAN은 GAN을 세 자에서 이기고 확신도에서만 비긴다(0.899\~0.906과 0.903\~0.911이 겹친다).

### 그런데 GAN 줄의 수는 조심해서 읽어야 한다

씨 42의 값이 제 범위 안에 드는지를 모델마다 세어 보면 한 줄만 유난하다.

| 모델 | 범위 밖으로 나간 칸 |
|---|---|
| AE_MLP-64 | 없음 (4칸 모두 안) |
| VAE-64 | 없음 (4칸 모두 안) |
| **GAN (MLP)** | **네 칸 가운데 셋** — 고름, 최대 클래스, 클래스 안 다양함 |
| DCGAN | 하나 (확신도 0.909, 범위는 0.899\~0.906) |

특히 **클래스 안 다양함 0.953은 범위가 0.831\~0.905이므로 한참 위다.** 다섯 벌의 평균은 0.878이다. 이 절이 "0.953"이라 적은 자리에서 실제로 기대할 값은 0.88 언저리다.

퍼짐 자체도 GAN이 가장 크다. 클래스 안 다양함의 퍼짐이 **0.074**로 VAE의 0.009보다 여덟 배이며, 이것은 적대적 학습이 씨에 따라 다른 곳에 내려앉는다는 뜻이다. [GAN 쪽](01_gan.md)이 손실이 오르내리는 것을 두고 "정상"이라 적었는데, **그 오르내림은 끝난 뒤의 값에도 남는다.**

### 1에 얼마나 가까운가로 줄을 세우면

클래스 안 다양함은 1.00이 기준이므로, 크고 작음이 아니라 **1에서 얼마나 떨어졌는가**로 보아야 한다. 씨마다 $|1 - x|$를 구해 평균하면 네 모델이 고르게 줄을 선다.

| | 다섯 씨 평균 | 1과의 거리 |
|---|---|---|
| [VAE-64](../latent_generative/01_vae.md) | 0.627 | 0.373 |
| [GAN (MLP)](01_gan.md) | 0.878 | 0.122 |
| [DCGAN](02_dcgan.md) | 1.084 | 0.084 |
| ([DDPM](../diffusion/01_ddpm.md), [5.4절](../diffusion/index.md)) | 0.986 | **0.048** |

**걸음마다 1에 가까워진다.** 씨 하나로 보면 GAN의 0.953이 DCGAN의 1.085보다 1에 가까워 보여 순서가 뒤집히지만, 다섯 벌로 보면 뒤집히지 않는다. 0.953이 높게 뽑힌 한 벌이었을 뿐이다.

!!! warning "평균을 낸 뒤에 1을 빼면 안 된다"
    위의 "1과의 거리"는 씨마다 $|1 - x|$를 구해 평균한 것이다. 평균을 먼저 내고 1을 빼면 1을 넘은 씨와 못 미친 씨가 서로 지워 실제보다 좋아 보인다. DCGAN처럼 모든 씨가 1을 넘는 모델에서는 차이가 없지만, [DDPM](../diffusion/01_ddpm.md)처럼 1을 넘나드는 모델에서는 세 배 넘게 부풀려진다.
