...
`fake.detach()`가 중요하다. 판별기를 학습할 때는 생성기로 기울기가 흘러가면 안 된다. 떼어 놓지 않으면 판별기의 손실을 줄이려고 생성기가 **더 나쁜 그림을 그리도록** 학습된다.

이 쪽의 생성기와 판별기는 모두 **촘촘한 층**이다. [VAE](../latent_generative/01_vae.md)와 구조가 같으므로, 여기서 얻는 것은 전부 **손실을 바꾼 몫**이다.
...
VAE 줄의 0.758과 0.887은 [5.2절](../latent_generative/01_vae.md)이 적은 0.769, 0.877과 다르다. 이 절이 모든 줄을 $[0, 1]$로 자른 뒤 재기 때문이며, 까닭은 [절 첫머리](index.md)에 적어 두었다. **이 표 안에서만 견주어야 한다.**
...
!!! warning "지은이가 여기서 틀렸다"
    ...
    GAN에서는 그 경고가 더 세게 적용된다. 두 손실이 **서로 물려 있기** 때문이다. 판별기가 앞서면 생성기가 받는 기울기가 세지고, 그러면 생성기가 따라붙는다. 오르내리는 것이 **정상**이다.
...
??? success "연습문제 1 풀이"
    **생성기가 반대 방향으로 학습된다.**
    ...
    떼어 놓지 않으면 `lossD`를 줄이는 역전파가 생성기의 가중치까지 흘러간다. `lossD`는 가짜를 가짜라고 맞히면 작아지므로, 그 기울기를 따라 생성기를 갱신하면 **더 알아보기 쉬운 가짜를 만들도록** 움직인다. 생성기가 제 목적과 정반대로 학습되는 셈이다.

    `optimizer` 두 개를 따로 두었으니 괜찮다고 생각하기 쉽지만 그렇지 않다. `oD.step()`은 판별기 매개변수만 갱신하더라도, 기울기는 이미 생성기에 **쌓여** 있다. 다음 `oG.step()`이 그 오염된 기울기를 함께 쓴다. `oG.zero_grad()`를 부르는 자리에 따라 증상이 달라져 찾기 어려운 버그가 된다.
...
??? success "연습문제 3 풀이"
    **판단할 수 없다.** [DCGAN](02_dcgan.md)은 판별기가 줄곧 앞섰고 격차가 벌어졌는데도 표본은 더 좋았다.
    ...
    손실에서 읽을 수 있는 것은 **극단적인 실패**뿐이다. $D$가 0에 붙으면 판별기가 완벽히 이겨 생성기에 기울기가 가지 않고, $G$가 0에 붙으면 판별기가 무너진 것이다. 그 사이의 값들은 좋고 나쁨을 말해 주지 않는다.
