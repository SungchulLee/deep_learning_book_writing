...
**클래스 안 다양함은 1.047로 DCGAN의 1.085보다 1에 가깝다.** 5.3절이 "1을 넘으면 이 자가 다양함만 재고 있지 않다는 신호"라고 적었으므로, 1에 가깝다는 것은 **그 경고에 덜 걸린다**는 뜻이다.

여기까지가 **씨 하나**로 본 것이다. 이제 씨를 다섯으로 늘려 본다.

---

## 5. 씨를 다섯으로 늘리면

위의 표는 씨 42 한 번이다. 같은 규약으로 씨 0\~4를 새로 학습해 다섯 벌씩 만들고, 값이 어디까지 움직이는지를 쟀다.

| | 확신도 | 고름 | 최대 클래스 | 클래스 안 다양함 |
|---|---|---|---|---|
| [DCGAN](../generative/02_dcgan.md) | 0.899\~0.906 | 0.966\~0.978 | 0.147\~0.163 | 1.055\~1.121 |
| **DDPM** | **0.935\~0.948** | 0.971\~0.993 | 0.123\~0.172 | 0.879\~1.043 |

**네 칸 가운데 둘만 갈라진다.**

**확신도는 갈라진다.** DDPM의 가장 낮은 씨(0.935)가 DCGAN의 가장 높은 씨(0.906)보다 위에 있다. 두 범위가 0.029만큼 떨어져 있으므로 씨를 바꾸어 뒤집히지 않는다. 위 문단의 결론은 그대로 선다.

**고름은 갈라지지 않는다.** 0.971\~0.993과 0.966\~0.978이 겹친다. 씨 42에서 본 0.986 대 0.970은 **두 모델의 차이가 아니라 씨의 차이였다.**

**최대 클래스도 갈라지지 않는다.** 0.123\~0.172가 0.147\~0.163을 통째로 품는다.

### 하마터면 적을 뻔한 것

이 자리에는 본디 "두 고름 자가 엇갈린다"는 꼭지가 있었다. 씨 42에서 DDPM은 고름이 더 높은데(0.986 대 0.970) 최대 몫도 더 컸다(0.175 대 0.162). 같은 "고르다"를 재는 자 둘이 서로 다른 답을 냈으니 까닭을 밝혀야 했고, 이렇게 적었다.

> 최대 몫은 **가장 높은 봉우리 하나**만 보고, 고름은 **열 칸 전체**를 본다. DCGAN은 봉우리가 낮은 대신 바닥에 아주 드문 클래스가 있어 고름이 깎였고, DDPM은 봉우리가 조금 높아도 나머지가 고르다.

**이 설명은 틀리지 않았다.** 엔트로피와 최대 몫이 서로 다른 곳을 본다는 것은 참이고, 실제로 엇갈릴 수 있다. 다만 **위의 두 수가 그 보기가 아니었을 뿐이다.** 두 차이 모두 퍼짐 안에 있었으니 엇갈린 것은 자가 아니라 씨였고, 설명해야 할 일이 애초에 없었다.

무엇이 이 잘못을 잡아낼 수 있었는지를 보라. 설명이 그럴듯하지 않아서 걸린 것이 아니다. 오히려 **너무 그럴듯해서** 걸리지 않았다. 기제가 옳고 수가 맞아떨어지면 더 볼 것이 없어 보인다. 씨를 넷 더 돌리는 것 말고는 이것을 잡을 방법이 없었다.

**설명이 그럴듯한지는 그 설명이 필요한지를 말해 주지 않는다.**

### 클래스 안 다양함은 평균의 이야기다

이 칸은 갈라지되, 갈라지는 폭이 좁다(0.012). 1.00에서 얼마나 떨어져 있는지를 씨마다 재어 평균하면 이렇다.

| | 다섯 씨 평균 | 1과의 거리 평균 | 가장 좋은 씨 | 가장 나쁜 씨 |
|---|---|---|---|---|
| DDPM | 0.986 | **0.048** | **0.008** | 0.121 |
| DCGAN | 1.084 | 0.084 | 0.055 | 0.121 |

**평균으로는 DDPM이 뚜렷이 가깝고, 최악으로는 둘이 같다.** 0.121로 똑같이 어긋난다.

곧 DDPM이 이 자에서 나은 것은 맞지만, 그 나음은 **평균의 나음**이지 보장의 나음이 아니다. 어느 한 벌을 학습해 쓴다면 DCGAN만큼 어긋난 것을 받을 수 있다.

!!! warning "1과의 거리는 평균한 뒤에 재지 않는다"
    위 표의 셋째 열은 씨마다 $|1 - x|$를 구해 평균한 것이지, 평균을 구한 뒤 1을 뺀 것이 아니다. 뒤엣것으로 재면 1을 넘은 씨와 못 미친 씨가 서로 지워 DDPM이 0.014로 나온다. 실제보다 세 배 넘게 좋아 보인다.

!!! note "씨 42는 뽑기 운이 좋았는가"
    위 표의 값들이 저마다 제 범위의 어디에 있는지를 보면 한쪽으로 쏠려 있지 않다. DCGAN의 확신도 0.909는 범위(0.899\~0.906) **위로** 벗어났지만, 나머지 셋은 범위 안에 있다. DDPM 쪽은 확신도 0.940과 고름 0.986이 안에 있고 최대 몫 0.175와 클래스 안 다양함 1.047이 위로 벗어났는데, 최대 몫은 **작을수록 좋은** 자이므로 그 벗어남은 불리한 쪽이다.

    곧 씨 42가 골라 뽑은 씨는 아니다. **한 씨는 어떤 자에서는 유리하게, 어떤 자에서는 불리하게 떨어진다.** 이것이 씨 하나를 적을 때 벌어지는 일이다.
