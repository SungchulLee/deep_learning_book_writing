...
이 "맞먹는다"는 [DDPM 쪽](01_ddpm.md)이 씨 다섯 벌로 잰 뒤 오히려 단단해졌다. DCGAN의 확신도는 씨에 따라 0.899\~0.906을 오가는데, **DDIM 10걸음의 0.904가 그 안에 든다.** 곧 두 값이 가까운 정도가 아니라, DCGAN을 다시 학습할 때 나올 법한 값 가운데 하나다.

!!! warning "다만 DDIM 쪽은 씨 하나다"
    위 표의 DDIM 값들은 모두 씨 42로 학습한 **하나의 DDPM 가중치**에서 나왔다. 걸음 수만 바꾸었을 뿐 다시 학습하지 않았으므로, 이 표의 세로 방향(걸음 수에 따른 변화)은 믿을 만하지만 **가로 방향으로 다른 모델과 견줄 때는 DDIM 쪽에만 오차 막대가 없다.**
...
??? success "연습문제 2 풀이"
    **나아질 수 있고, 실제로 그렇게 한다.**
    ...
    실제로 쓰이는 방법들이 이 자리를 다룬다. 이차 간격(quadratic spacing)이 흔하고, [31장](../../ch30/index.md)이 다루는 DPM-Solver 같은 것들은 한 걸음 안에서 더 높은 차수의 어림을 써서 같은 걸음 수로 더 멀리 간다.
