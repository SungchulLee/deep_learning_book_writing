...
여기가 [4.4절](../../ch04/04_dimensionality_reduction.md)과 갈리는 자리다. 그 절에서 같은 수법을 CIFAR-10에 걸었을 때는 **아무것도 벌지 못했다**(0.1341 대 PCA의 0.1337).
...
??? success "연습문제 1 풀이"
    **두 자가 서로 다른 것을 재기 때문이다.**
    ...
    곧 **MSE가 크게 치는 오차와 정체를 바꾸는 오차가 서로 다르다.** 비선형 부호기가 잘한 일은 후자 쪽, 곧 숫자를 가르는 데 중요한 것을 2차원 안에 밀어 넣은 것이다.
