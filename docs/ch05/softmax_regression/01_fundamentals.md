...
그 사슬은 [2.2 선형 모델과 소프트맥스](../../ch03/mnist/02_linear_softmax.md)가 MNIST를 예로 처음부터 끝까지 다루고, 가능도라는 개념 자체는 [표지-재포획 MLE](../mle/capture_recapture_mle.md)가 매개변수 하나짜리 예제로 보인다. 이 절은 모델의 정의에 집중한다.
