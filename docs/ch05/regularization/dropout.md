...
!!! info "함께 볼 것: 불확실성 추정을 위한 몬테카를로 드롭아웃"
    드롭아웃의 베이즈적 해석은 불확실성 정량화의 강력한 기법인 **몬테카를로 드롭아웃**을 가능하게 한다. 추론 중에도 드롭아웃을 켠 채 순전파를 여러 번 돌리면 예측의 불확실성을 추정할 수 있다. 다음 내용은 **40.2절 몬테카를로 드롭아웃**을 보라.
    
    - 변분 추론의 전체 유도 ([이론](../../ch40/mc_dropout/theory.md))
    - 실전 구현 방식 (구현)
    - 표본 수렴 분석 ([수렴](../../ch40/mc_dropout/convergence.md))
    - 보정을 위한 드롭아웃 비율 선택 ([드롭아웃 비율](../../ch40/mc_dropout/dropout_rate.md))
