...
구조와 규약은 [5.1절의 AE_MLP](../dimreduction/03_ae_mlp.md)와 같다. 부호기 $784 \to 256 \to k$, Adam $10^{-3}$, 배치 100, 100 에포크다. **달라지는 것은 손실뿐이다.** 이 쪽은 $\beta = 1$로 두고, 손잡이를 돌리는 이야기는 [다음 쪽](02_beta_vae.md)이 한다.
...
**첫째, 이 자는 쓸 만하다.** 심판은 진짜 숫자에 0.992를 주고 순수한 잡음에는 0.32~0.49를 준다. 잡음 가운데 0.9를 넘는 것은 0.5%도 되지 않는다. 분류기가 처음 보는 입력에 함부로 확신하는 일이 흔하다는 점을 생각하면(그 이야기는 [42장](../../ch42/index.md)이 다룬다) 이 심판은 뜻밖에 잘 가려낸다.
...
그러므로 자기 부호기의 표본이 못 쓸 것은 아니다. 하나하나는 VAE의 것만큼 그럴듯하다. 문제는 **그 가운데 셋 중 둘이 3이라는 것**이다.
...
부호 2에서는 VAE가 자기 부호기를 이기지 못하고 부호 64에서는 크게 이긴다. 이 사실만으로 "부호는 크게 두는 편이 낫다"고 말할 수 있는가?
...
??? success "연습문제 2 풀이"
    **말할 수 없다.** 부호를 키우면 VAE의 **상대적** 이득이 커질 뿐, 절대적으로 좋아진다는 뜻이 아니다.
    ...
    실제로 두 가지가 함께 일어난다. 부호가 커지면 되살리기가 좋아지고(0.4274 → 0.0750), 동시에 **뽑기가 어려워진다.** 자기 부호기의 고름이 0.973에서 0.541로 무너지는 것이 그 증거다. VAE는 그 무너짐을 되돌리는 것이지 없는 이득을 만드는 것이 아니다.
