...
for k in (2, 64):
    ...
    # 정체와 고름도 함께 찍는다. 3절이 이 두 수를 읽지 말라고 말하기 위해서다
    ident = 100.0 * (judge_probs(rec).argmax(1) == yte).float().mean()
    _, bal, _ = sample_stats(s)
    tag = f"cvae{k}"
    print(f"  {tag:7s} 복원 {((rec - Xte) ** 2).mean():.5f}  "
          f"조건 정확도 {cond_acc:.2f}%  정체 {ident:.2f}%  고름 {bal:.3f}")
```

**출력:**

```
  cvae2   복원 0.34787  조건 정확도 99.66%  정체 99.96%  고름 1.000
  cvae64  복원 0.07618  조건 정확도 73.64%  정체 98.79%  고름 0.964
```
...
오른쪽 두 칸은 눈이 먼저 가는 자리인데 **읽으면 안 되는 수**다. 까닭은 3절에 적는다. 여기서 볼 것은 왼쪽 두 칸뿐이다.
